		...
		if (unlikely(fdb->is_local)) {
            ...
            if(loopdetect_start == 1 && loop_intf == 0xff)
            {
                    if(memcmp(source->dev->name , "eth0.", 5) == 0 && strlen(source->dev->name) == 6)
                    {
                            ...
                            loopdetect_disable_intf(loop_intf);
                    }
			...
			if(loopdetect_start == 2 && loop_intf == 0x0)
			{
				 #ifdef CONFIG_RTL_MULTI_LAN_DEV
				 if(memcmp(source->dev->name, "eth0.", 5) == 0 && strlen(source->dev->name) == 6)
                 {
                     loop_intf |= 1<<(source->dev->name[5] - '0' - 2);
					 /* siyuan 2106-08-04: eponoamd may have no chance to disable interface when cpu is busy,
						so disable interface in kernel */
					 rtk_loopback_disable_intf((source->dev->name[5] - '0' - 2));
                 }
				 #else
