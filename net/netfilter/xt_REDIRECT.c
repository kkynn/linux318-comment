...
#ifdef CONFIG_CTC_AWIFI_KERNEL_JITUAN
		struct iphdr *iph;  
		iph = ip_hdr(skb);
#endif
		newdst = 0;
...
/*star:20170301 add to get the right local address, such as secondary ip on br0*/
#ifdef CONFIG_CTC_AWIFI_KERNEL_JITUAN	
		for (ifa=indev->ifa_list; ifa != NULL; ifa=ifa->ifa_next) {
			if((ifa->ifa_local&ifa->ifa_mask) == (iph->saddr&ifa->ifa_mask)){
				newdst = ifa->ifa_local;
				//printk("find right local address %x\n",newdst);
				break;
			}
