...
//		ct->m_1p=skb->mark;		
		...
		{	
			...
			if(!test_bit(IPS_8676HW_NAPT_DONE_BIT, &ct->status) && skb_dst(skb) && skb_dst(skb)->neighbour)
			{	
				...
				if (skb_mac_header_was_set(skb) && (SUCCESS == rtl865x_Lookup_L2_by_MAC(eth_hdr(skb)->h_source))) 
				{
					...
					read_lock_bh(&skb_dst(skb)->neighbour->lock);			
					upstream_nexthop_ip = *(u32*)skb_dst(skb)->neighbour->primary_key;
					read_unlock_bh(&skb_dst(skb)->neighbour->lock);					
					#ifdef CONFIG_RTL_HW_QOS_SUPPORT
					{
							...
							if(dev_info->m_1p!=0)
							{
								...
								{
								ret = rtl8676_add_L34Unicast_hwacc_ct(ct,upstream_nexthop_ip,skb_dst(skb)->dev->name,1,
									0,ct->m_1p&0x7,0);								
								}	
								else
								{	
							...
							ret = rtl8676_add_L34Unicast_hwacc_ct(ct,upstream_nexthop_ip,skb_dst(skb)->dev->name,0,
									0,0,0);
					}
					#else
					ret = rtl8676_add_L34Unicast_hwacc_ct(ct,upstream_nexthop_ip,skb_dst(skb)->dev->name);
					#endif
					if(ret==SUCCESS)
