			...
			if (1) {
				...
				CTAILQ_FOREACH(entry_path, &table_path->list[hash], path_link) {
					if ((entry_path->protocol == &ep->protocol) && (entry_path->course == 1)) {
						...
						if(entry_path->neigh)
						{
							neigh_release(entry_path->neigh);
						}
...
#if defined(CONFIG_CTC_AWIFI_KERNEL_JITUAN)
					entry_path->touch_cnt++;
					if (entry_path->touch_cnt < FP_TOUCH_THRESHOLD) {
						return 0;
					}
#endif
...
//						#endif//end of CONFIG_NET_IPIP
					...
					#endif
					{
						...
						if (!pppoe_proxy && entry_path->neigh == NULL)
						{
...
int fastpath_read_proc(struct seq_file *seq, void *v)
{
#ifdef CONFIG_DSL_CODESWAP
	seq_printf(seq, "admin:%d op:%d\n", fp_admin, fp_on);
#else
	if(fp_on==2)
		seq_printf(seq, "%s fastpath ON!(both up and downstream)\n",REALTEK_FASTPATH_VERSION);
	if(fp_on==1)
		seq_printf(seq, "%s fastpath ON!(only downstream)\n",REALTEK_FASTPATH_VERSION);
	if(fp_on==0)
		seq_printf(seq, "%s fastpath OFF!\n",REALTEK_FASTPATH_VERSION);
#endif
	  
      return 0;
}

static int proc_FastPath_open(struct inode *inode, struct file *file)
{
	return single_open(file, fastpath_read_proc, inode->i_private);
}

int FastPath_write_proc(struct file *file, const char __user *buffer, size_t len, loff_t *ppos)
{	
	...
	{
		flag=simple_strtoul(tmpBuf, NULL, 16);
		switch(flag) 
		{
#ifdef CONFIG_DSL_CODESWAP
			case 0:
			...
			*/
			...
			case 0:
				fp_on = 0;
				printk("rtk fastpath is disabled\n");
				break;
			case 1:
				fp_on = 1;
				printk("rtk fastpath downstream is enabled\n");
				break;
			case 2:
				fp_on = 2;
				printk("rtk fastpath both up & down stream is enabled\n");
				break;
#endif				
			default:
...
static const struct file_operations fastpath_proc_fops = {
        .owner          = THIS_MODULE,
        .open           = proc_FastPath_open,
        .read           = seq_read,
        .write          = FastPath_write_proc,
        .llseek         = seq_lseek,
        .release        = single_release,
};
...
static int __init fastpath_init(void)
{
	...
	FP_Proc_File->read_proc  = fp_proc_read;
	FP_Proc_File->write_proc  = fp_proc_write;
...
#else
	FP_Proc_File = proc_create_data(PROCFS_NAME, 0644, NULL, &fastpath_proc_fops,NULL);
	if (FP_Proc_File == NULL)
	{
		remove_proc_entry(PROCFS_NAME, NULL);
		printk(KERN_ALERT "Error: Could not initialize /proc/%s\n",
...
//#ifdef CONFIG_NET_IPIP
		case IPPROTO_IPIP:
		{
			...
			CTAILQ_FOREACH(entry_path, &table_path->list[hash], path_link) {
				if ((*entry_path->in_sIp == sIp) &&
					(*entry_path->in_dIp == dIp) &&
					(*entry_path->protocol == IPPROTO_IPIP)) {
						skb_awifi_set(skb, 1);
						return 1;
					}
			}
