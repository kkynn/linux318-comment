...
void tr068_internet_led_start(void) {
	...
	//cxy : for only internet data(wan connection type is INTERNET) flash internet led.
	led_flash_start(&led_internet_act, LED_INTERNET_GREEN, HZ / 20);
#else
...
static int led_read_proc(struct seq_file *seq, void *v)
{
	return 0;
}

static int led_write_proc(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    char led_control[2];
...
#ifdef CONFIG_RTK_VOIP
static int led_fxs_open(struct inode *inode, struct file *file)
{
        return single_open(file, led_read_proc, inode->i_private);
}
static const struct file_operations ledfxs_fops = {
        .owner          = THIS_MODULE,
        .open           = led_fxs_open,
        .read           = seq_read,
        .write          = led_write_proc,
        .llseek         = seq_lseek,
...
#ifdef CONFIG_RTK_VOIP
	proc_create_data("led_fxs", 0644, NULL,&ledfxs_fops, NULL);
#endif
...
void internet_dsl_led_stop(void){
	led_flash_stop(&led_flash_dsl); 
	tr068_internet_led_stop();
	led_off(LED_INTERNET_GREEN);
	led_off(LED_INTERNET_RED);
