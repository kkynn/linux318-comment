 ...
 */
  ...
  #define CACHE_DCACHE_INVALID	0x11
...
#if defined(CONFIG_CPU_HAS_WBC) || defined(CONFIG_CPU_HAS_L2C)
  #define CCTL_DCACHE_INVALID		0x001
  #define CCTL_DCACHE_WBACK		0x100
  #define CCTL_DCACHE_FLUSH		0x200
#else
  #define CCTL_DCACHE_INVALID		0x1
...
static inline void rlx_invalid_dcache_fast(unsigned long start, unsigned long end)
{
	...
	for (p = start; p <= end-0x080; p += 0x080) {
		DCACHE_UNROLL(CACHE_DCACHE_INVALID, p);
	}

	p = p & ~(cpu_dcache_line -1);
	while (p < end) {
		CACHE_OP(CACHE_DCACHE_INVALID, p);
		p += cpu_dcache_line;
	}
}
#endif
...
static inline void rlx_invalid_dcache_range(unsigned long start, unsigned long end)
{
#ifdef CONFIG_CPU_HAS_DCACHE_OP
	if (end - start > cpu_dcache_size) {
		CCTL_OP(CCTL_DCACHE_FLUSH);
...
void rlx_dma_cache_inv(unsigned long start, unsigned long size)
{
	...
	iob();
	rlx_invalid_dcache_range(start, start + size);
}
#endif
