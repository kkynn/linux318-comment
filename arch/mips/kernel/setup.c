...
void __init prom_init_cmdline(void)
{
	char *cp;
	int actr;
#if defined(CONFIG_MTD_NOR_SPIF_G3)
	char old_map_dev_name[] = "mtdparts=rtk_spi_nor_mtd";
	char new_map_dev_name[] = "  mtdparts=physmap-flash";
	char *map_dev_name_pos;
#endif
...
#if defined(CONFIG_MTD_NOR_SPIF_G3)
	map_dev_name_pos = strstr(arcs_cmdline, old_map_dev_name);
	if (map_dev_name_pos) {
		printk(KERN_INFO "II: Kernel command line modified: %s -> %s\n",
		       old_map_dev_name, new_map_dev_name);
		strncpy(map_dev_name_pos, new_map_dev_name, sizeof(new_map_dev_name) - 1);
	}
#endif
}
...
int check_argv(void)
{
        ...
        for(i=1;i<prom_argc;i++) {
                if((char*)*(_prom_argv+i)==NULL) {
                        prom_printf("Null pointer ==> argv[%d] !!!\n",i);
                        return 0;
                }
        }
        return 1;
...
void __init uboot_bootargs_init(void)
{
	prom_argc = fw_arg0;
	...
	/* Only retrieve bootargs when it exists */
	 if(prom_argc > 1) {
                if(check_argv()) {
                        ...
                        prom_printf("Null bootargs. Skip parsing bootargs !!!\n");
                }
        }
...
extern void __weak reset_sram(void);
...
#endif
	if(reset_sram) reset_sram(); //reset SRAM after preloader_parameters_init()
