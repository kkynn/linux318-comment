...
void __noreturn die(const char *str, struct pt_regs *regs)
{
	...
	extern int panic_log_on;
	extern int panic_lock;
	extern int panic_write(void);
	extern int fault_panic_write_file;
	int panic_write_file = 0;
	if(panic_lock == 0){
		panic_lock = 1;
		panic_log_on = 1;
		panic_write_file = 1;
...
#ifdef CONFIG_RTK_KERNEL_OMD
	if(panic_write_file || fault_panic_write_file){
		panic_write();
	}
#endif
	oops_exit();
 ...
 */
#if 0//def CONFIG_HARDWARE_WATCHPOINTS
...
{	
	...
	extern void watch_kernel(void);
	
	addr = read_lxc0_wmpvaddr();
	entry = (read_lxc0_wmpstatus() & ENTRY_MATCH) >> 16;
	attr = (read_lxc0_wmpstatus() & 0x7);
	printk("WMPU exception (%s): addr:0x%x entry:0x%x attr:0x%x\n", __FUNCTION__, addr, entry, attr);
