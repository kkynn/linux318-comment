...
#define MP_GPIO_SET(w, op)  do { \
	...
	if (LED_ON==op) {\
		if(ledpwr_toggle_flag==1)gpioClear(w);}\
	else gpioSet(w); \
} while (0);
#else
#define MP_GPIO_SET(w, op)  do { \
...
#define MP_GPIO_SET(w, op)  do { \
	gpioConfig(w, GPIO_FUNC_OUTPUT); \
	if (LED_ON==op) {\
		if(ledpwr_toggle_flag==1)gpioSet(w);}\
	else gpioClear(w); \
} while (0);
#else
#define MP_GPIO_SET(w, op)  do { \
...
// gpio state 0~31, 0xbb000038
unsigned int GPIO_EN_0,DIR_0,DAT_0;
// gpio state 32~63, 0xbb00003C
unsigned int GPIO_EN_1,DIR_1,DAT_1;
// gpio state 64~96, 0xbb000040
unsigned int GPIO_EN_2,DIR_2,DAT_2;
//#define SOC_GPIO_PABCD_DIR (0xb8003308)
//#define SOC_GPIO_PABCD_DAT (0xb800330C)
//#define SOC_GPIO_PABCD_DIR (0xb8003324)
//#define SOC_GPIO_PABCD_DAT (0xb8003328)
...
RTL9602[]={{LED_LAN0,26,0},{LED_LAN1,39,0},{LED_NET_G,31,0},{LED_PWR,36,0}, 
			{LED_PON,38,0},{LED_LOS,28,0},{LED_USB0,21,0},{LED_FX0,29,0}};
#elif defined(CONFIG_PON_LED_PROFILE_DEMO_RTL9607C)
RTL9607CP[]={{LED_NET_G,57,1},{LED_PWR,39,0},{LED_USB0,15,0}};
#elif defined(CONFIG_PON_LED_PROFILE_DEMO_RTL9607C_TBL_B)
//review this for RTL9603C IAD V00A
RTL9607CP[]={{LED_PWR,6,0},{LED_USB0,20,0},{LED_FX0,38,0},{LED_FX1,33,0}, 
             {LED_NET_G,57,1},{LED_NET_R,7,0},{LED_PON,10,0},{LED_LOS,21,0},   
             {LED_LAN0,8,0},{LED_LAN1,9,0},{LED_LAN2,35,0},{LED_LAN3,12,0},
             {LED_WPSR,56,1},
};
...
#ifndef CONFIG_YUEME
#ifdef CONFIG_LEDPWR_PBC
	if(ledpwr_toggle_flag==0)//led power off
		return;
#endif
...
static int mptest_level_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (buffer && !copy_from_user(&flag, buffer, 1)) 
	{
		...
		if (flag == '0')
		{
			...
			wps_8192_led_off();
	#endif		
#elif defined(CONFIG_PON_LED_PROFILE_DEMO_RTL9607C) || defined(CONFIG_PON_LED_PROFILE_DEMO_RTL9607C_TBL_B)
			print_string("RTL9607C LED OFF\n");
			mptest_off(RTL9607CP,sizeof(RTL9607CP)/sizeof(RTL9607CP[0]));
#endif
		}
		else if (flag == '1')
		{
	...
	#endif	
#elif defined(CONFIG_PON_LED_PROFILE_DEMO_RTL9607C)	|| defined(CONFIG_PON_LED_PROFILE_DEMO_RTL9607C_TBL_B)
			PRINT("RTL9607CP LED ON\n");
			mptest_on(RTL9607CP,sizeof(RTL9607CP)/sizeof(RTL9607CP[0]));
			regValue = REG32(0xBB023014); //record current status
#if defined(CONFIG_RTK_VOIP_DRIVERS_SLIC_ZSI)
			REG32(0xBB023014) = ((1<<5)|(1<<4)|(1<<6)|(1<<16)|(1<<13)|(1<<14));// 4:5 UART, 6 SPI, 16 ZSI
#elif defined(CONFIG_RTK_VOIP_DRIVERS_SLIC_ISI)
			REG32(0xBB023014) = ((1<<5)|(1<<4)|(1<<6)|(1<<15)|(1<<13)|(1<<14));// 4:5 UART, 6 SPI, 15 ISI
#else
			REG32(0xBB023014) = ((1<<5)|(1<<4)|(1<<6)|(1<<17)|(1<<13)|(1<<14));// 4:5 UART, 6 SPI, 17 SPI/PCM
#endif
			mptest_on(RTL9607CP,sizeof(RTL9607CP)/sizeof(RTL9607CP[0]));
#endif
		}else if (flag == '2'){
			//restore status
			PRINT("restore led status led_mp_test_flag=%d gMPTEST_STATE=%d regValue=0x%08X\n",led_mp_test_flag,gMPTEST_STATE, regValue);
#ifdef CONFIG_LEDPWR_PBC
			if(ledpwr_toggle_flag==0)//led power off
			{
				printk("led power off don't restore led status\n");
				return;
			}
#endif
			...
			if(0 != regValue)
			{
				REG32(IO_LED_EN_REG) = regValue;
				PRINT("restore led IO_LED_EN_REG=0x%08X\n",REG32(IO_LED_EN_REG));
				regValue = 0;
			}
#if defined(CONFIG_PON_LED_PROFILE_DEMO_RTL9602C)
			...
			#elif defined(CONFIG_PON_LED_PROFILE_DEMO_RTL9607C_TBL_B)
			restore_led_state(RTL9607CP,sizeof(RTL9607CP)/sizeof(RTL9607CP[0]));
			#endif
			if(wps_led_restore_state==1)
			{
#if (defined(CONFIG_SLOT_0_8812) || defined(CONFIG_SLOT_0_8812AR_VN))  && defined(CONFIG_WLAN0_5G_WLAN1_2G)
				wps_8812_led_on();
#elif (defined(CONFIG_SLOT_0_8192EE) || defined(CONFIG_SLOT_1_8192EE) )
...
{	  
	...
	if (ethtest) {
        ...
        /* 
          BEGIN: Modified by huangyl, 2018/12/11,
          修改 led test ethstatus 指令，通过 rtk_port_link_get 接口获取LAN口状态
        */
            rtk_port_linkStatus_t portLinkStatus;       
            rtk_port_link_get(portnum, &portLinkStatus);
			seq_printf(seq, "Test Switch LAN PORT %d %s\n",
					portnum+1, portLinkStatus?"UP":"DOWN");         
#if 0            
			LinkStatus = ((*(volatile unsigned int *)(P_ABLTY + (portnum<<2))) >> 4)&0x1;
			seq_printf(seq, "Test Switch LAN PORT %d %s\n",
					portnum+1, LinkStatus?"UP":"DOWN");
#endif    
        /* END:   Modified by huangyl, 2018/12/11 */
		}
		...
		if (pb_is_pushed(PB_WIFISW) == 1)
			seq_printf(seq, "WLAN success!\n");
#endif
#ifdef CONFIG_LEDPWR_PBC
		if(ledpwr_toggle_flag == 2)
			seq_printf(seq,"LED POWER success!\n");
#endif
	}
