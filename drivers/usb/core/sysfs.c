...
#endif 
extern int rtk_usb_test_mode_set(struct usb_hcd *hcd, u8 mode);
extern int rtk_usb_test_mode_get(struct usb_hcd *hcd, u8 *mode);
...
static ssize_t  show_bPortTestMode (struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (!is_root_hub(udev)) {
		printk("Must be root-hub\n");
		return -EINVAL;
	}

	ret = rtk_usb_test_mode_get(hcd, &mode);
...
set_bPortTestMode (struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = rtk_usb_test_mode_set(hcd, config);
