 ...
 * in ppp_channel_push.
 *
 * ref. https://patchwork.ozlabs.org/patch/659678/
 *
 */
struct channel_lock {
	spinlock_t lock;
...
#if 0 //#if defined(CONFIG_PPTP) || defined(CONFIG_PPPOL2TP)
...
ppp_decompress_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if (proto == PPP_COMP) {
		...
		* TODO
		*
		* why does this add one???? for MPPE
...
void clear_ppp_stats(struct net_device *dev)
{
	struct ppp *ppp = netdev_priv(dev);

	memset(&ppp->stats64, 0, sizeof(struct ppp_link_stats));
}
