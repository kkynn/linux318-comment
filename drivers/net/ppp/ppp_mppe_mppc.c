		...
		unsigned char *obuf, int osize)
{
    ...
    if (state->stateless) {
		...
		/* 
		 * RFC 3078 8.1. Stateless Synchronization,
		 * (the received packet (C1), last packet previously received (C2)),
		 * RFC 3078 did not mension about the situation when C1(seq) < C2(lcount).
		 * We should not change key when C1(seq) < C2(lcount),
		 * otherwise session_key will out of sequence after we change key (MPPE_MAX_CCOUNT-1) times.
		*/
		if(seq > state->lcount) {
			/* RFC 3078, sec 8.1. */
			mppe_increase_ccount(state);
