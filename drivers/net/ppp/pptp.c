	...
	*(unsigned int *)(iph+16)	= opt->dst_addr.sin_addr.s_addr;
	...
	if (FastPath_Enter(skb) != 1) {
...

		{
			...
			fl4.flowi4_oif = 0;
		#ifdef CONFIG_IPV6_VPN
			fl4.daddr = opt->dst_addr.sin_addr.in.s_addr,
			fl4.saddr = opt->src_addr.sin_addr.in.s_addr,
		#else
			fl4.daddr = opt->dst_addr.sin_addr.s_addr,
			fl4.saddr = opt->src_addr.sin_addr.s_addr,
		#endif
...
static int pptp_rcv_core(struct sock *sk, struct sk_buff *skb)
{
	...
	if (seq < opt->seq_recv + 1 || WRAPPED(opt->seq_recv, seq)) {
#ifdef CONFIG_YUEME
		/* We find www.openwrtdl.com PPTP server sequence number may out-of-order */
		/* Remove these code if you can find a way to re-order packets. */
		if( PPP_PROTOCOL(payload) == PPP_CHAP)
			goto allow_packet;
#endif
		if ((payload[0] == PPP_ALLSTATIONS) && (payload[1] == PPP_UI) &&
