    ...
    cparser_context_t *context)
{
	...
	for(i = 0 ; i < RTK_RG_METER_MAX ; i++)
	{
		...
		if(meterConf.type == RTK_RG_METER_ACL)
			diag_util_printf("---------- funcbasedMeter type: %s ----------\n", "ACL");
		...
		for(j = 0 ; j < MAX_FUNCBASEDMETER_SIZE ; j++)
		{
			...
			if(ret == RT_ERR_RG_OK)
			{
    			diag_util_printf("funcbasedMeter[%2d]: %s", meterConf.idx, meterConf.state?"Enable":"Disable");
				if(meterConf.state)
					diag_util_printf(" (rate=%d, ifgInclude=%s)", meterConf.rate, meterConf.ifgInclude?"Include":"Exclude");
			}
			else
			{
				diag_util_printf("Get funcbasedMeter[%d] failed. return value = %x", meterConf.idx, ret);
				return CPARSER_NOT_OK;
			}
			diag_util_printf("\n");
		}
    ...
    uint32_t  *type_ptr)
{
	...
	{
		diag_util_printf("Invalid funcbasedMeter type.\n");
    ...
    uint32_t  *funcbasedMeter_index_ptr)
{
	...
	{
		diag_util_printf("Invalid funcbasedMeter index.\n");
		return CPARSER_ERR_INVALID_PARAMS;
	}
	
	if(rtk_rg_funcbasedMeter_get(&meterConf) == RT_ERR_RG_OK)
	{
    	diag_util_printf("funcbasedMeter[%2d]: %s", meterConf.idx, meterConf.state?"Enable":"Disable");
		if(meterConf.state)
			diag_util_printf(" (rate=%d, ifgInclude=%s)", meterConf.rate, meterConf.ifgInclude?"Include":"Exclude");
