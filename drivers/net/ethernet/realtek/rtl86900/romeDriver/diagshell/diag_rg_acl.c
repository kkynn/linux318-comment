...
{		
	...
	if(acl_parameter.filter_fields & INGRESS_L4_TCP_BIT){
		...
		diag_util_printf("l4-protocal: none-tcp and none-udp \n");
	}
	else{
	...
	diag_util_printf("ingress_ipv6_flow_label: %d  ", acl_parameter.ingress_ipv6_flow_label);
#endif

	diag_util_printf("ingress_wlanDevMask:  %d \n", acl_parameter.ingress_wlanDevMask);
#if defined(CONFIG_RG_FLOW_BASED_PLATFORM)
	diag_util_printf("egress_wlanDevMask:  %d \n", acl_parameter.egress_wlanDevMask);
#endif
	...
	switch(acl_parameter.action_type){
		...
		case ACL_ACTION_TYPE_SW_MIRROR_WITH_UDP_ENCAP:
			diag_util_printf("action type: ACL_ACTION_TYPE_SW_MIRROR_WITH_UDP_ENCAP \n");
			diag_util_printf(" encap_smac: %02X:%02X:%02X:%02X:%02X:%02X\n",
			...
			diag_util_printf(" encap_dmac: %02X:%02X:%02X:%02X:%02X:%02X\n", 			
			...
			diag_util_printf(" encap_sip: %d.%d.%d.%d\n",
			...
			diag_util_printf(" encap_dip: %d.%d.%d.%d\n",				
			...
			diag_util_printf(" encap_dport:%d\n",acl_parameter.action_encap_udp.encap_Dport);
			break;					
		case ACL_ACTION_TYPE_FLOW_MIB:
			diag_util_printf("action type: ACL_ACTION_TYPE_FLOW_MIB \n");
			break;
		case ACL_ACTION_TYPE_TRAP_WITH_PRIORITY:
			diag_util_printf("action type: ACL_ACTION_TYPE_TRAP_WITH_PRIORITY \n");
...
/*
 * rg set acl-filter action udp_encap assign_sip <IPV4ADDR:assign_sip> assign_dip <IPV4ADDR:assign_dip> assign_dport <UINT:assign_dport>
 */
cparser_result_t
cparser_cmd_rg_set_acl_filter_action_udp_encap_assign_sip_assign_sip_assign_dip_assign_dip_assign_dport_assign_dport(
    cparser_context_t *context,
    uint32_t  *assign_sip_ptr,
    uint32_t  *assign_dip_ptr,
    uint32_t  *assign_dport_ptr)
{
    ...
    return CPARSER_OK;
}    /* end of cparser_cmd_rg_set_acl_filter_action_udp_encap_assign_sip_assign_sip_assign_dip_assign_dip_assign_dport_assign_dport */
    ...
    uint32_t  *flow_counter_index_ptr)
{
	...
	else
	{
		...
		if(*flow_counter_index_ptr >= G3_FLOW_POLICER_FLOWMIB_SIZE)
			diag_util_printf("[ERROR!]mib index must less than %d! \n", G3_FLOW_POLICER_FLOWMIB_SIZE);
		DIAG_UTIL_PARAM_RANGE_CHK((*flow_counter_index_ptr >= G3_FLOW_POLICER_FLOWMIB_SIZE), CPARSER_ERR_INVALID_PARAMS);
#else	
		if(*flow_counter_index_ptr >= RTL9607C_TABLESIZE_FLOWMIB)
			diag_util_printf("[ERROR!]mib index must less than %d! \n", RTL9607C_TABLESIZE_FLOWMIB);
		DIAG_UTIL_PARAM_RANGE_CHK((*flow_counter_index_ptr >= RTL9607C_TABLESIZE_FLOWMIB), CPARSER_ERR_INVALID_PARAMS);
#endif	
    ...
    uint32_t  *share_meter_ptr)
{
	...
	{
#if defined(CONFIG_RG_FLOW_BASED_PLATFORM)
		//return error during _rtk_rg_aclSWEntry_not_support(), otherwise, it will add success with qos action 0x0
