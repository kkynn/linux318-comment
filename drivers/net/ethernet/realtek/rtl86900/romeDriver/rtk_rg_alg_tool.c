...
int _rtk_rg_extPortPair_get(int isTcp, uint16 port)
{
	...
	while(1)
	{
		...
		if(_rtk_rg_naptExtPortInUsedCheck(FALSE,isTcp,tmpPort,FALSE,TRUE)==0
			&& _rtk_rg_naptExtPortInUsedCheck(FALSE,isTcp,tmpPort+1,FALSE,TRUE)==0) // free
			return tmpPort;
...
int _rtk_rg_update_tcp_seqNdelta(int direct, rtk_rg_pktHdr_t * pPktHdr, rtk_rg_alg_connection_t * pConn)
{
	...
	nowIpid = ntohs(*pPktHdr->pIpv4Identification);
