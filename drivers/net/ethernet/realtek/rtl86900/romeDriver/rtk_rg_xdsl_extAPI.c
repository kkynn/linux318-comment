...
int32 _rtk_syncLutTb4way_FormHw2Sw(unsigned char *mac)
{
	...
	oriHashL2idx = _rtk_rg_hash_mac_fid_efid(mac,LAN_FID,0) << MAX_LUT_HASH_WAY_SHIFT;
...
//find first invalid empty(=first_invalid) and check mac exit(=search_index).
	l2Idx = _rtk_rg_hash_mac_fid_efid(pL2Addr->mac.octet, pL2Addr->fid,0) << MAX_LUT_HASH_WAY_SHIFT;
...
//		DEBUG("search_idx is %d",search_index);
	...
	if(mac_exist)
	{
		...
		ret = convert_setAsicL2Table(search_index>>MAX_LUT_HASH_WAY_SHIFT,search_index&0x3,(ether_addr_t *)(&(pL2Addr->mac)),cpu,
	...
	else
	{
		...
		ret = convert_setAsicL2Table(first_invalid>>MAX_LUT_HASH_WAY_SHIFT,first_invalid&0x3,(ether_addr_t *)(&(pL2Addr->mac)),cpu,
									SrcBlk,mbr,aging,isStatic,nhflag,fid,pL2Addr->auth);
...
int32 rtk_init_without_pon(void)
{
	...
	//set  l4 table outbound 4-way/enhanceHash1/PerFlow 
	_set4WayHash(1);
...
int32 rtk_l34_ipv6NeighborTable_set(uint32 idx,rtk_ipv6Neighbor_entry_t *ipv6NeighborEntry)
{
	...
	arpv6.nextHopColumn = (ipv6NeighborEntry->l2Idx)%MAX_LUT_HASH_WAY_SIZE;
	arpv6.nextHopRow = (ipv6NeighborEntry->l2Idx)>>MAX_LUT_HASH_WAY_SHIFT ;
