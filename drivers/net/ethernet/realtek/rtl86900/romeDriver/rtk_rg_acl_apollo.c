...
PORTMASK_NEED_DOUBLE_RULE:
	...
	{	
		...
		{
			if(rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_L2TP_CONTROL_LCP_PACKET_TRAP_AND_ASSIGN_PRIORITY]==DISABLED) //FS[3] is out of template, due to such template field is using for pktLen 
			{
...
static int _rtk_rg_aclSWEntry_and_asic_reAdd(rtk_rg_aclFilterAndQos_t *acl_filter, int *acl_filter_idx)
{
	...
	else if(acl_filter->action_type==ACL_ACTION_TYPE_SW_PERMIT)
	{
		//skip add to HWNAT
	}
	else{
...
int _rtk_rg_aclSWEntry_and_asic_rearrange(void)
{
	...
	for(i=0,j=0; i<MAX_ACL_SW_ENTRY_SIZE; i++)
	{
		...
		else if(rg_db.systemGlobal.acl_filter_temp[ruleIdx].fwding_type_and_direction==ACL_FWD_TYPE_DIR_INGRESS_OR_EGRESS_L34_DOWN_CVLAN_SVLAN)
		{	//ingress_vid also support forward type = 4 in apollo, this action not use CF, so it also allow to add in this array
			if( (rg_db.systemGlobal.acl_filter_temp[ruleIdx].action_type==ACL_ACTION_TYPE_QOS) &&
...
int32 _rtk_rg_apollo_aclFilterAndQos_add(rtk_rg_aclFilterAndQos_t *acl_filter, int *acl_filter_idx)
{
	...
	{
		DEBUG("PACL_ACTION_TYPE_SW_PERMIT! do not add to ASIC!!");
	}
	else
	{
...
int32 _rtk_rg_apollo_classifyEntry_add(rtk_rg_classifyEntry_t *classifyFilter){
	...
	if(classifyFilter->filter_fields & EGRESS_GEMIDX_BIT){
		...
		rg_db.systemGlobal.classifyField_1[CF_PATTERN_GEMIDX_OR_LLID].classify_pattern.fieldData.value=(classifyFilter->gemidx & 0xff);
		if(classifyFilter->gemidx_mask==0x0){
			//for compitible of non-mask version (mask not set means all care)
			rg_db.systemGlobal.classifyField_1[CF_PATTERN_GEMIDX_OR_LLID].classify_pattern.fieldData.mask=0xff;	
		}else{
			rg_db.systemGlobal.classifyField_1[CF_PATTERN_GEMIDX_OR_LLID].classify_pattern.fieldData.mask=(classifyFilter->gemidx_mask&0xff);	
		}
	}

	if(classifyFilter->filter_fields & EGRESS_LLID_BIT){
		rg_db.systemGlobal.classifyField_1[CF_PATTERN_GEMIDX_OR_LLID].fieldType=CLASSIFY_FIELD_TOS_DSIDX;
		rg_db.systemGlobal.classifyField_1[CF_PATTERN_GEMIDX_OR_LLID].classify_pattern.fieldData.value=(classifyFilter->llid & 0xf); //llid 4 bits only
		rg_db.systemGlobal.classifyField_1[CF_PATTERN_GEMIDX_OR_LLID].classify_pattern.fieldData.mask=0xf;	
	}
...
int _rtk_rg_aclAndCfReservedRuleHeadReflash(void)
{
	...
	//reflash th rules
	for(i=2;i<rg_db.systemGlobal.aclAndCfReservedRule.aclLowerBoundary;i++){  //index 0 reserved for temporary mulitcast permit rule.  index 1 reserved for gpon_status
				...
				case RTK_RG_ACLANDCF_RESERVED_L2TP_CONTROL_LCP_PACKET_TRAP_AND_ASSIGN_PRIORITY:
					{
						...
						aclField.fieldUnion.data.value = 0x1; //using pktLen[0]
						aclField.fieldUnion.data.mask = 0xffff;
						if(rtk_acl_igrRuleField_add(&aclRule, &aclField)){
							ACL_RSV("add reserved ACL&CF RTK_RG_ACLANDCF_RESERVED_L2TP_CONTROL_LCP_PACKET_TRAP_AND_ASSIGN_PRIORITY failed!!!");
							addRuleFailedFlag=1;
...
pppoeMulticastIntfPermit:
				...
				case RTK_RG_ACLANDCF_RESERVED_BRIDGE_UPSTREAM_HTTP_TRAP:
					...
					aclRule.index=aclIdx;
					aclRule.activePorts.bits[0]=RTK_RG_ALL_LAN_PORTMASK;
					aclRule.templateIdx=1; //dport=80
					
					//dport
					aclField.fieldType = ACL_FIELD_PATTERN_MATCH;
					aclField.fieldUnion.pattern.fieldIdx = 0;//Dport
					aclField.fieldUnion.data.value= 80; //http
					aclField.fieldUnion.data.mask=0xffff;
					if(rtk_acl_igrRuleField_add(&aclRule, &aclField)){
						ACL_RSV("add reserved ACL&CF RTK_RG_ACLANDCF_RESERVED_BRIDGE_UPSTREAM_HTTP_TRAP failed!!!");
						addRuleFailedFlag=1;
...
int _rtk_rg_aclAndCfReservedRuleAdd(rtk_rg_aclAndCf_reserved_type_t rsvType, void *parameter)
{
	...
	switch(rsvType){
		...
		case RTK_RG_ACLANDCF_RESERVED_SLAVE_WIFI_BC_MC_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_SLAVE_WIFI_BC_MC_TRAP]=ENABLED; 
			break;
		...
		case RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE0_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(2,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.mc_pppoe_trap[0],(rtk_rg_aclAndCf_multicast_pppoe_trap_t*)parameter,sizeof(rtk_rg_aclAndCf_multicast_pppoe_trap_t));				
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE0_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE1_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(2,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.mc_pppoe_trap[1],(rtk_rg_aclAndCf_multicast_pppoe_trap_t*)parameter,sizeof(rtk_rg_aclAndCf_multicast_pppoe_trap_t));				
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE1_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE2_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(2,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.mc_pppoe_trap[2],(rtk_rg_aclAndCf_multicast_pppoe_trap_t*)parameter,sizeof(rtk_rg_aclAndCf_multicast_pppoe_trap_t));				
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE2_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE3_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(2,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.mc_pppoe_trap[3],(rtk_rg_aclAndCf_multicast_pppoe_trap_t*)parameter,sizeof(rtk_rg_aclAndCf_multicast_pppoe_trap_t));				
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE3_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(2,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.mc_pppoe_trap[4],(rtk_rg_aclAndCf_multicast_pppoe_trap_t*)parameter,sizeof(rtk_rg_aclAndCf_multicast_pppoe_trap_t));				
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE5_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(2,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.mc_pppoe_trap[5],(rtk_rg_aclAndCf_multicast_pppoe_trap_t*)parameter,sizeof(rtk_rg_aclAndCf_multicast_pppoe_trap_t));				
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE5_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(2,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.mc_pppoe_trap[6],(rtk_rg_aclAndCf_multicast_pppoe_trap_t*)parameter,sizeof(rtk_rg_aclAndCf_multicast_pppoe_trap_t));				
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE7_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(2,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.mc_pppoe_trap[7],(rtk_rg_aclAndCf_multicast_pppoe_trap_t*)parameter,sizeof(rtk_rg_aclAndCf_multicast_pppoe_trap_t));				
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_MULTICAST_PPPOE_RULE7_TRAP]=ENABLED; 
			break;
		...
		case RTK_RG_ACLANDCF_RESERVED_BRIDGE_UPSTREAM_HTTP_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(2,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_BRIDGE_UPSTREAM_HTTP_TRAP]=ENABLED; 
			break;

		case RTK_RG_ACLANDCF_RESERVED_NETIF0_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF0_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF1_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF1_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF2_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF2_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF3_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF3_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF4_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF4_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF5_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF5_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF6_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF6_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF7_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF7_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF8_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF8_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF9_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF9_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF10_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF10_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF11_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF11_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF12_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF12_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF13_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF13_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF14_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF14_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF15_GATEWAY_IPv4_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF15_GATEWAY_IPv4_TRAP]=ENABLED; 
			break;
			
		case RTK_RG_ACLANDCF_RESERVED_NETIF0_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF0_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF1_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF1_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF2_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF2_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF3_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF3_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF4_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF4_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF5_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF5_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF6_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF6_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF7_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF7_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF8_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF8_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF9_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF9_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF10_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF10_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF11_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF11_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF12_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF12_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF13_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF13_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF14_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF14_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_NETIF15_GATEWAY_IPv6_TRAP:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_NETIF15_GATEWAY_IPv6_TRAP]=ENABLED; 
			break;
		...
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF0_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[0],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF0_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF1_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[1],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF1_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF2_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[2],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF2_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF3_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[3],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF3_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF4_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[4],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF4_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF5_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[5],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF5_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF6_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[6],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF6_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF7_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[7],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF7_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF8_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[8],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF8_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF9_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[9],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF9_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF10_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[10],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF10_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF11_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[11],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF11_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF12_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[12],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF12_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF13_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[13],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF13_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF14_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[14],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF14_REMOTE_MAC]=ENABLED; 
			break;
		case RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF15_REMOTE_MAC:
			assert_ok(_rtk_rg_aclAndCfReservedRuleAddCheck(1,0));
			memcpy(&rg_db.systemGlobal.aclAndCfReservedRule.gpon_smallbw_bridge_port_redirect[15],(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t*)parameter,sizeof(rtk_rg_aclAndCf_reserved_gpon_smallbw_bridge_port_redirect_t));	
			rg_db.systemGlobal.aclAndCfReservedRule.reservedMask[RTK_RG_ACLANDCF_RESERVED_GPON_PPPoE_SMALL_BANDWIDTH_BRIDGE_DMAC_SAME_AS_INTF15_REMOTE_MAC]=ENABLED; 
			break;
...
#endif //end of  #if defined(CONFIG_APOLLO_MP)
...
#endif	//#if defined(CONFIG_APOLLO_MP)
...
int _dump_rg_acl_and_cf_diagshell(struct seq_file *s)
{
	...
	PROC_PRINTF("rg set acl-filter action udp_encap assign_smac %02X:%02X:%02X:%02X:%02X:%02X assign_dmac %02X:%02X:%02X:%02X:%02X:%02X assign_sip %d.%d.%d.%d assign_dip %d.%d.%d.%d assign_dport %d \n",
