...
dal_ca8279_sec_portAttackPreventState_set(rtk_port_t port, rtk_enable_t enable)
{
    ...
    if((atkPrvt[port].enable == DISABLED)&&(enable == ENABLED)) //DISABLE -> ENABLE
    {
        ...
        {
            if((ret = _dal_ca8279_icmp_flood_classification_set(port,ICMP_FLOOD_ACTION,ICMP_FLOOD_TH,&atkPrvt[port].icmp_flood_index,0)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                return ret;
            }

            if((ret = _dal_ca8279_icmp_flood_classification_set(port,ICMP_FLOOD_ACTION,ICMP_FLOOD_TH,&atkPrvt[port].icmp_flood_index,1)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                return ret;
            }
        ...
        {
            if((ret = _dal_ca8279_syn_flood_classification_set(port,SYN_FLOOD_ACTION,SYN_FLOOD_TH,&atkPrvt[port].syn_flood_index,0)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                return ret;
            }

            if((ret = _dal_ca8279_syn_flood_classification_set(port,SYN_FLOOD_ACTION,SYN_FLOOD_TH,&atkPrvt[port].syn_flood_index,1)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                return ret;
            }
        ...
        {
            if((ret = _dal_ca8279_fin_flood_classification_set(port,FIN_FLOOD_ACTION,FIN_FLOOD_TH,&atkPrvt[port].fin_flood_index,0)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                return ret;
            }

            if((ret = _dal_ca8279_fin_flood_classification_set(port,FIN_FLOOD_ACTION,FIN_FLOOD_TH,&atkPrvt[port].fin_flood_index,1)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                return ret;
            }
        ...
        case ICMPFLOOD_DENY:
            ...
            if((ICMP_FLOOD_ACTION == ACTION_FORWARD) && (action != ACTION_FORWARD))
            {
                HAL_SCAN_ALL_PORT(port)
                {
                    if(atkPrvt[port].enable == ENABLED)
                    {
                        if((ret = _dal_ca8279_icmp_flood_classification_set(port,action,ICMP_FLOOD_TH,&atkPrvt[port].icmp_flood_index,0)) != RT_ERR_OK)
                        {
                            RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                            return ret;
                        }
        ...
        case SYNFLOOD_DENY:
            ...
            if((SYN_FLOOD_ACTION == ACTION_FORWARD) && (action != ACTION_FORWARD))
            {
                HAL_SCAN_ALL_PORT(port)
                {
                    if(atkPrvt[port].enable == ENABLED)
                    {
                        if((ret = _dal_ca8279_syn_flood_classification_set(port,action,SYN_FLOOD_TH,&atkPrvt[port].syn_flood_index,0)) != RT_ERR_OK)
                        {
                            RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                            return ret;
                        }

                        if((ret = _dal_ca8279_syn_flood_classification_set(port,action,SYN_FLOOD_TH,&atkPrvt[port].syn_flood_index,1)) != RT_ERR_OK)
                        {
                            RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                            return ret;
                        }
        ...
        case FINFLOOD_DENY:
            ...
            if((FIN_FLOOD_ACTION == ACTION_FORWARD) && (action != ACTION_FORWARD))
            {
                HAL_SCAN_ALL_PORT(port)
                {
                    if(atkPrvt[port].enable == ENABLED)
                    {
                        if((ret = _dal_ca8279_fin_flood_classification_set(port,action,FIN_FLOOD_TH,&atkPrvt[port].fin_flood_index,0)) != RT_ERR_OK)
                        {
                            RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                            return ret;
                        }
            
                        if((ret = _dal_ca8279_fin_flood_classification_set(port,action,FIN_FLOOD_TH,&atkPrvt[port].fin_flood_index,1)) != RT_ERR_OK)
                        {
                            RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                            return ret;
                        }
            ...
            if((ICMP_FLOOD_ACTION != ACTION_FORWARD) && (ICMP_FLOOD_TH != floodThresh))
            {
                HAL_SCAN_ALL_PORT(port)
                {
                    if(atkPrvt[port].enable == ENABLED)
                    {
                        ...
                        if((ret = _dal_ca8279_icmp_flood_classification_set(port,ICMP_FLOOD_ACTION,floodThresh,&atkPrvt[port].icmp_flood_index,1)) != RT_ERR_OK)
                        {
                            RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                            return ret;
                        }
            ...
            if((SYN_FLOOD_ACTION != ACTION_FORWARD) && (SYN_FLOOD_TH != floodThresh))
            {
                HAL_SCAN_ALL_PORT(port)
                {
                    if(atkPrvt[port].enable == ENABLED)
                    {
                        ...
                        if((ret = _dal_ca8279_syn_flood_classification_set(port,SYN_FLOOD_ACTION,floodThresh,&atkPrvt[port].syn_flood_index,1)) != RT_ERR_OK)
                        {
                            RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                            return ret;
                        }
            ...
            if((FIN_FLOOD_ACTION != ACTION_FORWARD) && (FIN_FLOOD_TH != floodThresh))
            {
                HAL_SCAN_ALL_PORT(port)
                {
                    if(atkPrvt[port].enable == ENABLED)
                    {
                        ...
                        if((ret = _dal_ca8279_fin_flood_classification_set(port,FIN_FLOOD_ACTION,floodThresh,&atkPrvt[port].fin_flood_index,1)) != RT_ERR_OK)
                        {
                            RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
                            return ret;
                        }
