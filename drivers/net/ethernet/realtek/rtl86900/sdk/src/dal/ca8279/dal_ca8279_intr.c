...
ca_uint32_t ca_rtk_onu_state_change_intr(ca_device_id_t device_id,ca_uint32_t event_id,ca_void *event,ca_uint32_t len,ca_void *cookie)
{
#ifdef CONFIG_SDK_MODULES
    ca_event_omcc_link_t* omcc_link_ptr = (ca_event_omcc_link_t *)event;
    int32 portId;
    uint32 intrSubType;
    ...
    if(omcc_link_ptr->status == CA_OMCC_LINK_UP)
    {
        intrSubType = 5; //O5
    }
    if(omcc_link_ptr->status == CA_OMCC_LINK_DOWN)
    {
        intrSubType = 1; //O1
    }
...
dal_ca8279_intr_init(void)
{
    ...
    if((ret = dal_ca8279_intr_imr_set(INTR_TYPE_LINK_CHANGE,ENABLED)) != RT_ERR_OK)
    {
    ...
    if((ret = dal_ca8279_intr_imr_set(INTR_TYPE_GPON,ENABLED)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_INTR | MOD_DAL), "");
        intr_init = INIT_NOT_COMPLETED;
        return ret;
...
dal_ca8279_intr_imr_set(rtk_intr_type_t intr, rtk_enable_t enable)
{
    ...
    switch(intr)
    {
        ...
        case INTR_TYPE_GPON:
            if(LUNA_G3_INTR_REG[intr] == DISABLED && enable == ENABLED)
            {
                if((ret = ca_event_register(0,CA_EVENT_OMCC_LINK,ca_rtk_onu_state_change_intr,NULL))!=CA_E_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_INTR), "");
                    return RT_ERR_FAILED;
                }
            ...
            if(LUNA_G3_INTR_REG[intr] == ENABLED && enable == DISABLED)
            {
                if((ret = ca_event_deregister(0,CA_EVENT_OMCC_LINK, ca_rtk_onu_state_change_intr))!=CA_E_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_INTR), "");
                    return RT_ERR_FAILED;
                }
    ...
    return RT_ERR_OK;
} /* end of dal_ca8279_intr_imr_set */
