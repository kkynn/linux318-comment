...
#include <aal/include/aal_l2_vlan.h>
...
#if 1 /*For RG*/
static ca_uint32_t g_vlan_flooding_domain_1[CFG_ID_FLOODING_DOMAIN_LEN];
#endif
...
#if 1 /*For RG*/
    if((ret = scfg_read(0, CFG_ID_FLOODING_DOMAIN_1, CFG_ID_FLOODING_DOMAIN_LEN * sizeof(ca_uint32_t), (ca_uint8_t *) g_vlan_flooding_domain_1))!=CA_E_OK)
    {
...
#if 1 /*For RG*/
    int i;
    for (port = 0; port < CFG_ID_FLOODING_DOMAIN_LEN; port++)
    {
        if (g_vlan_flooding_domain_1[port] == 0xFF)
            break;

        port_id = RTK2CA_PORT_ID(g_vlan_flooding_domain_1[port]);
#else
