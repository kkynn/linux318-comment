...
#include <dal/rtl9603d/dal_rtl9603d_mapper.h>
#endif
...
dal_mgmt_find_mapper(void)
{
#if defined(CONFIG_SDK_LUNA_G3)
#if defined(CONFIG_ARCH_RTL8198F)
	return dal_rtl8198f_mapper_get();
#else
    return dal_luna_g3_mapper_get(); 
#endif
#elif defined(CONFIG_SDK_CA8279)
    return dal_ca8279_mapper_get(); 
#else
    ...
    for (mapper_index = 0; mapper_index < mapper_size; mapper_index++)
    {
        ...
        #endif
		#if defined(CONFIG_SDK_RTL9603D)
		if(RTL9603D_CHIP_ID == dal_mapper_database[mapper_index].chip_id)
            dal_mapper_database[mapper_index].pMapper=dal_rtl9603d_mapper_get();
        #endif
    }
