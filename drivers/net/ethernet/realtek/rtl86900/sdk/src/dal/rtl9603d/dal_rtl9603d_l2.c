...
#include <dal/rtl9603d/dal_rtl9603d_l2.h>
 ...
 *      None
 */
int32 rtl9603d_raw_l2_lookUpTb_set(rtl9603d_lut_table_t *pL2Table)
{
   ...
   uint32 hashType;
    ...
    RT_PARAM_CHK(pL2Table->table_type > RTL9603D_RAW_LUT_ENTRY_TYPE_END, RT_ERR_OUT_OF_RANGE);

    if ((ret = reg_field_read(RTL9603D_LUT_CFGr, RTL9603D_BCAM_DISf, &state)) != RT_ERR_OK)
    {
    ...
    if(0 == state)
        RT_PARAM_CHK(pL2Table->address > (RTL9603D_LUT_4WAY_NO + RTL9603D_LUT_CAM_NO), RT_ERR_INPUT);
    else
        RT_PARAM_CHK(pL2Table->address > RTL9603D_LUT_4WAY_NO, RT_ERR_INPUT);

#ifndef CONFIG_SDK_ASICDRV_TEST
    RT_PARAM_CHK(RTL9603D_RAW_LUT_READ_METHOD_ADDRESS == pL2Table->method, RT_ERR_INPUT);
#endif
    /*commna part check*/
    ...
    RT_PARAM_CHK(pL2Table->age > RTL9603D_RAW_LUT_AGEMAX, RT_ERR_OUT_OF_RANGE);
    ...
    RT_PARAM_CHK(pL2Table->ext_spa > RTL9603D_RAW_LUT_EXT_SPAMAX, RT_ERR_OUT_OF_RANGE);

    /* ---L3 MC---*/
    if ((ret = reg_field_read(RTL9603D_LUT_CFGr, RTL9603D_LUT_IPMC_HASHf, &hashType)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    if(hashType != RTL9603D_RAW_IPMC_TYPE_GIP_AND_SIP)
    {
        if(pL2Table->ivl_svl == RTL9603D_RAW_L2_HASH_IVL)
        {
            RT_PARAM_CHK(pL2Table->sip_cvid_fid > RTK_VLAN_ID_MAX, RT_ERR_OUT_OF_RANGE);
        }
        else
        {
            RT_PARAM_CHK(pL2Table->sip_cvid_fid > RTL9603D_RAW_LUT_FIDMAX, RT_ERR_OUT_OF_RANGE);
        }
    }

    /*(L2 MC)(L3 MC)*/
    RT_PARAM_CHK(pL2Table->mbr > RTL9603D_PORTMASK, RT_ERR_OUT_OF_RANGE);
    RT_PARAM_CHK(pL2Table->ext_mbr_idx > RTL9603D_RAW_LUT_EXTMBRIDX, RT_ERR_OUT_OF_RANGE);

    /*static entry will assign age to none-ZERO*/
    if(pL2Table->nosalearn && pL2Table->table_type == RTL9603D_RAW_LUT_ENTRY_TYPE_L2UC)
        pL2Table->age = 1;

    /*write to l2 table*/
    if ((ret = table_write(RTL9603D_L2_UCt, pL2Table->address, (uint32 *)pL2Table)) != RT_ERR_OK)
    {
 ...
 *      None
 */
int32 rtl9603d_raw_l2_lookUpTb_get(rtl9603d_lut_table_t *pL2Table)
{
    int32 ret = RT_ERR_FAILED;
    uint32 state;

    RT_PARAM_CHK(pL2Table == NULL, RT_ERR_NULL_POINTER);
    RT_PARAM_CHK(pL2Table->method > RTL9603D_RAW_LUT_READ_METHOD_END, RT_ERR_INPUT);
    ...
    if ((ret = table_read(RTL9603D_L2_UCt, pL2Table->address, (uint32 *)pL2Table)) != RT_ERR_OK)
    {
        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "return failed ret value = %x",ret);
        return RT_ERR_FAILED;
    }
 ...
 *      None
 */
int32 rtl9603d_raw_l2_flushEn_set(rtk_port_t port, rtk_enable_t enabled)
{
    int32 ret;
    rtl9603d_raw_l2_flushStatus_t status;
    uint32 cnt=0;

    RT_PARAM_CHK(!HAL_IS_PORT_EXIST(port), RT_ERR_PORT_ID);
	RT_PARAM_CHK((RTK_ENABLE_END <= enabled), RT_ERR_INPUT);

 	if ((ret = reg_array_field_write(RTL9603D_L2_TBL_FLUSH_ENr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &enabled)) != RT_ERR_OK )
    {
    ...
    if ((ret = reg_field_read(RTL9603D_L2_TBL_FLUSH_CTRLr, RTL9603D_FLUSH_STATUSf ,&status)) != RT_ERR_OK )
    {
            RT_ERR(ret, (MOD_L2|MOD_DAL), "");
            return ret;
    }
    while (status != RTL9603D_RAW_FLUSH_STATUS_NONBUSY)
    {
        if(cnt++ > 0xFFFF)
 ...
 *      None
 */
int32 rtl9603d_raw_l2_flushEn_get(rtk_port_t port, rtk_enable_t *pEnabled)
{
    int32 ret;
	...
	if ((ret = reg_array_field_read(RTL9603D_L2_TBL_FLUSH_ENr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, pEnabled)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
 ...
 *      None
 */
int32 rtl9603d_raw_l2_flushCtrl_set(rtl9603d_raw_flush_ctrl_t *pCtrl)
{
    int32 ret = RT_ERR_FAILED;
    uint32 staticFlag;
    uint32 dynamicFlag;
    ...
    RT_PARAM_CHK(pCtrl->flushMode >= RTL9603D_RAW_FLUSH_MODE_END, RT_ERR_OUT_OF_RANGE);
    RT_PARAM_CHK(pCtrl->flushType >= RTL9603D_RAW_FLUSH_TYPE_END, RT_ERR_OUT_OF_RANGE);
    RT_PARAM_CHK(pCtrl->vid > RTK_VLAN_ID_MAX , RT_ERR_VLAN_VID);
    RT_PARAM_CHK(pCtrl->fid > HAL_VLAN_FID_MAX() , RT_ERR_L2_FID);

    if ((ret = reg_field_write(RTL9603D_L2_TBL_FLUSH_CTRLr, RTL9603D_LUT_FLUSH_MODEf, &pCtrl->flushMode)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    {
        case RTL9603D_RAW_FLUSH_TYPE_STATIC:
            staticFlag = 1;
            dynamicFlag = 0;
            break;
        case RTL9603D_RAW_FLUSH_TYPE_DYNAMIC:
            staticFlag = 0;
            dynamicFlag = 1;
            break;
        case RTL9603D_RAW_FLUSH_TYPE_BOTH:
            staticFlag = 1;
            dynamicFlag = 1;
            break;
        default:
    ...
    if ((ret = reg_field_write(RTL9603D_L2_TBL_FLUSH_CTRLr, RTL9603D_LUT_FLUSH_STATICf, &staticFlag)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }

    if ((ret = reg_field_write(RTL9603D_L2_TBL_FLUSH_CTRLr, RTL9603D_LUT_FLUSH_DYNAMICf, &dynamicFlag)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }

    if ((ret = reg_field_write(RTL9603D_L2_TBL_FLUSH_CTRLr, RTL9603D_LUT_FLUSH_VIDf, &pCtrl->vid)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    if ((ret = reg_field_write(RTL9603D_L2_TBL_FLUSH_CTRLr, RTL9603D_LUT_FLUSH_FIDf, &pCtrl->fid)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
 ...
 *      None
 */
int32 rtl9603d_raw_l2_flushCtrl_get(rtl9603d_raw_flush_ctrl_t *pCtrl)

{
    ...
    if ((ret = reg_field_read(RTL9603D_L2_TBL_FLUSH_CTRLr, RTL9603D_LUT_FLUSH_MODEf, &pCtrl->flushMode)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }

    if ((ret = reg_field_read(RTL9603D_L2_TBL_FLUSH_CTRLr, RTL9603D_LUT_FLUSH_STATICf, &staticFlag)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    if ((ret = reg_field_read(RTL9603D_L2_TBL_FLUSH_CTRLr, RTL9603D_LUT_FLUSH_DYNAMICf, &dynamicFlag)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
	...
	if (staticFlag && dynamicFlag)
		pCtrl->flushType = RTL9603D_RAW_FLUSH_TYPE_BOTH;
	else if (staticFlag)
		pCtrl->flushType = RTL9603D_RAW_FLUSH_TYPE_STATIC;
	else if (dynamicFlag)
		pCtrl->flushType = RTL9603D_RAW_FLUSH_TYPE_DYNAMIC;
    else
        pCtrl->flushType = RTL9603D_RAW_FLUSH_TYPE_END;

    if ((ret = reg_field_read(RTL9603D_L2_TBL_FLUSH_CTRLr, RTL9603D_LUT_FLUSH_VIDf, &pCtrl->vid)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }

    if ((ret = reg_field_read(RTL9603D_L2_TBL_FLUSH_CTRLr, RTL9603D_LUT_FLUSH_FIDf, &pCtrl->fid)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }

    if ((ret = reg_field_read(RTL9603D_L2_TBL_FLUSH_CTRLr, RTL9603D_FLUSH_STATUSf ,&pCtrl->flushStatus)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
...
static int32
_dal_rtl9603d_rawtoL2uc(rtk_l2_ucastAddr_t *pL2Data, rtl9603d_lut_table_t *pLut_entry)
{
...
static int32
_dal_rtl9603d_l2uctoRaw(rtl9603d_lut_table_t *pLut_entry, rtk_l2_ucastAddr_t *pL2Data)
{
    ...
    if (pL2Data->flags & RTK_L2_UCAST_FLAG_IVL)
        pLut_entry->ivl_svl = RTL9603D_RAW_L2_HASH_IVL;
...
static int32
_dal_rtl9603d_rawtoL2mc(rtk_l2_mcastAddr_t *pL2Data, rtl9603d_lut_table_t *pLut_entry)
{
    pL2Data->mac.octet[0]   = pLut_entry->mac.octet[0];
    pL2Data->mac.octet[1]   = pLut_entry->mac.octet[1];
    pL2Data->mac.octet[2]   = pLut_entry->mac.octet[2];
    ...
    if(RTL9603D_RAW_L2_HASH_IVL == pLut_entry->ivl_svl)
    {
...
static int32
_dal_rtl9603d_l2mctoRaw(rtl9603d_lut_table_t *pLut_entry, rtk_l2_mcastAddr_t *pL2Data)
{
    pLut_entry->mac.octet[0] = pL2Data->mac.octet[0];
    pLut_entry->mac.octet[1] = pL2Data->mac.octet[1];
    pLut_entry->mac.octet[2] = pL2Data->mac.octet[2];
...
static int32
_dal_rtl9603d_rawtoIpmc(rtk_l2_ipMcastAddr_t *pL2Data, rtl9603d_lut_table_t *pLut_entry)
{
    int ret;
    rtl9603d_raw_ipmc_type_t hashType;
...
static int32
_dal_rtl9603d_IpmctoRaw(rtl9603d_lut_table_t *pLut_entry, rtk_l2_ipMcastAddr_t *pL2Data)
{
    int ret;
    rtl9603d_raw_ipmc_type_t hashType;
    ...
    if(hashType != RTL9603D_RAW_IPMC_TYPE_GIP_AND_SIP)
    {
        ...
        else
        {
            ...
            pLut_entry->ivl_svl  = RTL9603D_RAW_L2_HASH_SVL;
        }
    }
    else
    {
...
static int32
_dal_rtl9603d_lutReadMethodtoRaw(rtl9603d_raw_l2_readMethod_t *raw, rtk_l2_readMethod_t cfg)
{
   	switch(cfg){
	case LUT_READ_METHOD_MAC:
		*raw = RTL9603D_RAW_LUT_READ_METHOD_MAC;
	break;
	case LUT_READ_METHOD_ADDRESS:
		*raw = RTL9603D_RAW_LUT_READ_METHOD_ADDRESS;
	break;
	case LUT_READ_METHOD_NEXT_ADDRESS:
		*raw = RTL9603D_RAW_LUT_READ_METHOD_NEXT_ADDRESS;
	break;
	case LUT_READ_METHOD_NEXT_L2UC:
		*raw = RTL9603D_RAW_LUT_READ_METHOD_NEXT_L2UC;
	break;
	case LUT_READ_METHOD_NEXT_L2MC:
		*raw = RTL9603D_RAW_LUT_READ_METHOD_NEXT_L2MC;
	break;
	case LUT_READ_METHOD_NEXT_L3MC:
		*raw = RTL9603D_RAW_LUT_READ_METHOD_NEXT_L3MC;
	break;
	case LUT_READ_METHOD_NEXT_L2L3MC:
		*raw = RTL9603D_RAW_LUT_READ_METHOD_NEXT_L2L3MC;
	break;
	case LUT_READ_METHOD_NEXT_L2UCSPA:
		*raw = RTL9603D_RAW_LUT_READ_METHOD_NEXT_L2UCSPA;
	break;
	default:
	break;
	}
...
static int _dal_rtl9603d_l2MacSvlHash(rtk_mac_t mac, uint32 fid, uint32 *pHashValue)
{
...
static int _dal_rtl9603d_l2MacIvlHash(rtk_mac_t mac, uint32 vid, uint32 *pHashValue)
{
    unsigned char index[9];
...
static int _dal_rtl9603d_l2MacHash(rtk_mac_t mac, uint32 *pHashValue)
{
    unsigned char index[9];
...
static int _dal_rtl9603d_l3GipSipHash(rtk_ip_addr_t gip, rtk_ip_addr_t sip, uint32 *pHashValue)
{
    unsigned char index[9];
...
static int _dal_rtl9603d_l3GipFidHash(rtk_ip_addr_t gip, uint32 fid, uint32 *pHashValue)
{
    unsigned char index[9];
    unsigned char gip_addr[4] = { ((unsigned char *)(&gip))[3], ((unsigned char *)(&gip))[2], ((unsigned char *)(&gip))[1], ((unsigned char *)(&gip))[0] };
...
static int _dal_rtl9603d_l3GipVidHash(rtk_ip_addr_t gip, uint32 vid, uint32 *pHashValue)
{
    unsigned char index[9];
    unsigned char gip_addr[4] = { ((unsigned char *)(&gip))[3], ((unsigned char *)(&gip))[2], ((unsigned char *)(&gip))[1], ((unsigned char *)(&gip))[0] };
...
static int _dal_rtl9603d_l3GipHash(rtk_ip_addr_t gip, uint32 *pHashValue)
{
    unsigned char index[9];
    unsigned char gip_addr[4] = { ((unsigned char *)(&gip))[3], ((unsigned char *)(&gip))[2], ((unsigned char *)(&gip))[1], ((unsigned char *)(&gip))[0] };
...
/* Function Name:
 *		rtl9603d_l2_unknReservedIpv4McAction_set
 * Description:
 *		Set unknown Reserved IPv4 multicast address packet action
 * Input:
 *		act	- rtl9603d_l2_resAct_t type data which has following configurations
 *          RTL9603D_L2_RESACT_NORMAL: treat as normal unknown multicast packet
 *          RTL9603D_L2_RESACT_FLOODING: always flooding
 *          RTL9603D_L2_RESACT_TRAP: trap to CPU
 * Output:
 *		Nnoe.
 * Return:
 *		RT_ERR_OK
 ...
 *		None
 */
int32 rtl9603d_l2_unknReservedIpv4McAction_set(rtl9603d_l2_resAct_t act)
{
	int32 ret;

	RT_PARAM_CHK((RTL9603D_L2_RESACT_END <= act), RT_ERR_INPUT);

	if ((ret = reg_field_write(RTL9603D_UNKN_MC_CFGr, RTL9603D_UNKN_RSV_IP4_ACTf, &act)) != RT_ERR_OK)
	{
		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
		return ret;
	}
...
/* Function Name:
 *		rtl9603d_l2_unknReservedIpv4McAction_get
 * Description:
 *		get unknown Reserved IPv4 multicast address packet action
 * Input:
 *		None.
 * Output:
 *		pAct - rtl9603d_l2_resAct_t type data which has following configurations
 *          RTL9603D_L2_RESACT_NORMAL: treat as normal unknown multicast packet
 *          RTL9603D_L2_RESACT_FLOODING: always flooding
 *          RTL9603D_L2_RESACT_TRAP: trap to CPU
 * Return:
 ...
 *		None
 */
int32 rtl9603d_l2_unknReservedIpv4McAction_get(rtl9603d_l2_resAct_t *pAct)
{
	int32 ret;

	RT_PARAM_CHK((NULL == pAct), RT_ERR_NULL_POINTER);

	if ((ret = reg_field_read(RTL9603D_UNKN_MC_CFGr, RTL9603D_UNKN_RSV_IP4_ACTf, pAct)) != RT_ERR_OK)
	{
		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
		return ret;
	}
...
/* Function Name:
 *		rtl9603d_l2_unknReservedIpv6McAction_set
 * Description:
 ...
 *		None
 */
int32 rtl9603d_l2_unknReservedIpv6McAction_set(rtl9603d_l2_resAct_t act)
{
	int32 ret;

	RT_PARAM_CHK((RTL9603D_L2_RESACT_END <= act), RT_ERR_INPUT);

	if ((ret = reg_field_write(RTL9603D_UNKN_MC_CFGr, RTL9603D_UNKN_RSV_IP6_ACTf, &act)) != RT_ERR_OK)
	{
		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
		return ret;
	}
...
/* Function Name:
 *		rtl9603d_l2_unknReservedIpv6McAction_get
 * Description:
 ...
 *		None
 */
int32 rtl9603d_l2_unknReservedIpv6McAction_get(rtl9603d_l2_resAct_t *pAct)
{
	int32 ret;

	RT_PARAM_CHK((NULL == pAct), RT_ERR_NULL_POINTER);

	if ((ret = reg_field_read(RTL9603D_UNKN_MC_CFGr, RTL9603D_UNKN_RSV_IP6_ACTf, pAct)) != RT_ERR_OK)
	{
		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
		return ret;
	}
 ...
 *		None
 */
int32 rtl9603d_l2_unknMcIcmpv6_set(rtk_enable_t enable)
{
	int32 ret;

	RT_PARAM_CHK((RTK_ENABLE_END <= enable), RT_ERR_INPUT);

	if ((ret = reg_field_write(RTL9603D_UNKN_MC_CFGr, RTL9603D_UNKN_MC_ICMP6_TRAPf, &enable)) != RT_ERR_OK)
	{
		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
		return ret;
	}
 ...
 *		None
 */
int32 rtl9603d_l2_unknMcIcmpv6_get(rtk_enable_t *pEnable)
{
	int32 ret;

	RT_PARAM_CHK((NULL == pEnable), RT_ERR_NULL_POINTER);

	if ((ret = reg_field_read(RTL9603D_UNKN_MC_CFGr, RTL9603D_UNKN_MC_ICMP6_TRAPf, pEnable)) != RT_ERR_OK)
	{
		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
		return ret;
	}
 ...
 *		None
 */
int32 rtl9603d_l2_unknMcDhcpv6_set(rtk_enable_t enable)
{
	int32 ret;

	RT_PARAM_CHK((RTK_ENABLE_END <= enable), RT_ERR_INPUT);

	if ((ret = reg_field_write(RTL9603D_UNKN_MC_CFGr, RTL9603D_UNKN_MC_DHCP6_TRAPf, &enable)) != RT_ERR_OK)
	{
		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
		return ret;
	}
 ...
 *		None
 */
int32 rtl9603d_l2_unknMcDhcpv6_get(rtk_enable_t *pEnable)
{
	int32 ret;

	RT_PARAM_CHK((NULL == pEnable), RT_ERR_NULL_POINTER);

	if ((ret = reg_field_read(RTL9603D_UNKN_MC_CFGr, RTL9603D_UNKN_MC_DHCP6_TRAPf, pEnable)) != RT_ERR_OK)
	{
		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
		return ret;
	}
...
dal_rtl9603d_l2_init(void)
{
    ...
    uint32 value;
    rtl9603d_raw_flush_ctrl_t rtl9603d_cfg;

    RT_DBG(LOG_DEBUG, (MOD_DAL|MOD_L2),"%s",__FUNCTION__);

    l2_init = INIT_COMPLETED;

    if((ret = dal_rtl9603d_l2_flushLinkDownPortAddrEnable_set(ENABLED)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
    ...
    if((ret = dal_rtl9603d_l2_table_clear()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    {
        if((ret = dal_rtl9603d_l2_portLimitLearningCnt_set(port, HAL_L2_LEARN_LIMIT_CNT_MAX())) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
            l2_init = INIT_NOT_COMPLETED;
            return ret;
        ...
        if((ret = dal_rtl9603d_l2_portLimitLearningCntAction_set(port, LIMIT_LEARN_CNT_ACTION_FORWARD)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
            l2_init = INIT_NOT_COMPLETED;
            return ret;
        ...
        if((ret = dal_rtl9603d_l2_portAgingEnable_set(port, ENABLED)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
            l2_init = INIT_NOT_COMPLETED;
            return ret;
        ...
        if((ret = dal_rtl9603d_l2_newMacOp_set(port, HARDWARE_LEARNING, ACTION_FORWARD)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
            l2_init = INIT_NOT_COMPLETED;
            return ret;
        ...
        if((ret = dal_rtl9603d_l2_illegalPortMoveAction_set(port, ACTION_FORWARD)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
            l2_init = INIT_NOT_COMPLETED;
            return ret;
        ...
        if((ret = dal_rtl9603d_l2_portLookupMissAction_set(port, DLF_TYPE_IPMC, ACTION_FORWARD)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
            l2_init = INIT_NOT_COMPLETED;
            return ret;
        ...
        if((ret = dal_rtl9603d_l2_portLookupMissAction_set(port, DLF_TYPE_MCAST, ACTION_FORWARD)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
            l2_init = INIT_NOT_COMPLETED;
            return ret;
        ...
        if((ret = dal_rtl9603d_l2_portLookupMissAction_set(port, DLF_TYPE_UCAST, ACTION_FORWARD)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
            l2_init = INIT_NOT_COMPLETED;
            return ret;
    ...
    if((ret = dal_rtl9603d_l2_limitLearningCnt_set(HAL_L2_LEARN_LIMIT_CNT_MAX())) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    if((ret = dal_rtl9603d_l2_aging_set(RTK_L2_DEFAULT_AGING_TIME)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    if((ret = dal_rtl9603d_l2_lookupMissFloodPortMask_set(DLF_TYPE_IPMC, &all_portmask)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    if((ret = dal_rtl9603d_l2_lookupMissFloodPortMask_set(DLF_TYPE_BCAST, &all_portmask)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    if((ret = dal_rtl9603d_l2_lookupMissFloodPortMask_set(DLF_TYPE_UCAST, &all_portmask)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    if((ret = dal_rtl9603d_l2_ipmcMode_set(LOOKUP_ON_MAC_AND_VID_FID)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    value = 0;
    if ((ret = reg_field_write(RTL9603D_UNKN_MC_CFGr, RTL9603D_UNKN_RSV_IP4_ACTf, &value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    if((ret = dal_rtl9603d_l2_srcPortEgrFilterMask_set(&all_portmask)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    if((ret = dal_rtl9603d_l2_extPortEgrFilterMask_set(&all_extPortmask)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    if((ret = rtl9603d_raw_l2_flushCtrl_get(&rtl9603d_cfg)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    rtl9603d_cfg.flushType = RTL9603D_RAW_FLUSH_TYPE_DYNAMIC;

    if((ret = rtl9603d_raw_l2_flushCtrl_set(&rtl9603d_cfg)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    HAL_GET_ALL_PORTMASK(all_portmask);
    if((ret = dal_rtl9603d_l2_limitLearningPortMask_set(all_portmask)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        l2_init = INIT_NOT_COMPLETED;
        return ret;
...
dal_rtl9603d_l2_flushLinkDownPortAddrEnable_get(rtk_enable_t *pEnable)
{
    ...
    if ((ret = reg_field_read(RTL9603D_LUT_CFGr, RTL9603D_LINKDOWN_AGEOUTf, pEnable)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_flushLinkDownPortAddrEnable_get */
...
dal_rtl9603d_l2_flushLinkDownPortAddrEnable_set(rtk_enable_t enable)
{
    ...
    if ((ret = reg_field_write(RTL9603D_LUT_CFGr, RTL9603D_LINKDOWN_AGEOUTf, &enable)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_flushLinkDownPortAddrEnable_set */
...
dal_rtl9603d_l2_ucastAddr_flush(rtk_l2_flushCfg_t *pConfig)
{
    int32   ret;
    int32   back_ret;
    rtl9603d_raw_flush_ctrl_t flush_ctrl;
    rtk_port_t port;
    ...
    /* Set Configuration */
    osal_memset(&flush_ctrl, 0x00, sizeof(rtl9603d_raw_flush_ctrl_t));
    if(ENABLED == pConfig->flushByVid)
    {
        flush_ctrl.flushMode = RTL9603D_RAW_FLUSH_MODE_VID;
        flush_ctrl.vid = pConfig->vid;
    }
    else if(ENABLED == pConfig->flushByFid)
    {
        flush_ctrl.flushMode = RTL9603D_RAW_FLUSH_MODE_FID;
        flush_ctrl.fid = pConfig->fid;
    }
    else if(ENABLED == pConfig->flushByPort)
    {
        flush_ctrl.flushMode = RTL9603D_RAW_FLUSH_MODE_PORT;
    }

    if( (ENABLED == pConfig->flushStaticAddr) && (ENABLED == pConfig->flushDynamicAddr) )
        flush_ctrl.flushType = RTL9603D_RAW_FLUSH_TYPE_BOTH;
    else if(ENABLED == pConfig->flushStaticAddr)
        flush_ctrl.flushType = RTL9603D_RAW_FLUSH_TYPE_STATIC;
    else if(ENABLED == pConfig->flushDynamicAddr)
        flush_ctrl.flushType = RTL9603D_RAW_FLUSH_TYPE_DYNAMIC;
    else
        return RT_ERR_INPUT;

    if((ret = rtl9603d_raw_l2_flushCtrl_set(&flush_ctrl)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    {
        if((ret = rtl9603d_raw_l2_flushEn_set(pConfig->port, ENABLED)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        }
    }
    else if(ENABLED == pConfig->flushAddrOnAllPorts)
    {
        HAL_SCAN_ALL_PORT(port)
        {
            if((ret = rtl9603d_raw_l2_flushEn_set(port, ENABLED)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
                break;
            }
    ...
    flush_ctrl.flushType = RTL9603D_RAW_FLUSH_TYPE_DYNAMIC;
    if((back_ret = rtl9603d_raw_l2_flushCtrl_set(&flush_ctrl)) != RT_ERR_OK)
    {
    ...
    return ret;
} /* end of dal_rtl9603d_l2_ucastAddr_flush */
...
dal_rtl9603d_l2_table_clear(void)
{
    ...
    if((ret = table_clear(RTL9603D_L2_UCt, 0, RTL9603D_LUT_TBL_MAX)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
}   /* end of dal_rtl9603d_l2_table_clear */
...
dal_rtl9603d_l2_limitLearningOverStatus_get(uint32 *pStatus)
{
    ...
    if((ret = reg_field_read(RTL9603D_L2_SYS_LRN_OVER_STSr, RTL9603D_LRN_OVER_INDf, pStatus)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
...
dal_rtl9603d_l2_limitLearningOverStatus_clear(void)
{
    ...
    if((ret = reg_field_write(RTL9603D_L2_SYS_LRN_OVER_STSr, RTL9603D_LRN_OVER_INDf, &regData)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
...
dal_rtl9603d_l2_learningCnt_get(uint32 *pMacCnt)
{
    ...
    /* get value from CHIP*/
    if((ret = reg_field_read(RTL9603D_L2_SYS_LRN_CNTr, RTL9603D_SYS_L2_LRN_CNTf, pMacCnt)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_learningCnt_get */
...
dal_rtl9603d_l2_limitLearningCnt_get(uint32 *pMacCnt)
{
    ...
    /* get value from CHIP*/
    if((ret = reg_field_read(RTL9603D_LUT_SYS_LRN_LIMITNOr, RTL9603D_SYS_LRN_LIMITNOf, pMacCnt)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_limitLearningCnt_get */
...
dal_rtl9603d_l2_limitLearningCnt_set(uint32 macCnt)
{
    ...
    if((ret = reg_field_write(RTL9603D_LUT_SYS_LRN_LIMITNOr, RTL9603D_SYS_LRN_LIMITNOf, &macCnt)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_limitLearningCnt_set */
...
dal_rtl9603d_l2_limitLearningCntAction_get(rtk_l2_limitLearnCntAction_t *pLearningAction)
{
    ...
    /* get value from CHIP*/
    if((ret = reg_field_read(RTL9603D_LUT_SYS_LRN_OVER_CTRLr, RTL9603D_ACTf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_limitLearningCntAction_get */
...
dal_rtl9603d_l2_limitLearningCntAction_set(rtk_l2_limitLearnCntAction_t learningAction)
{
    ...
    if((ret = reg_field_write(RTL9603D_LUT_SYS_LRN_OVER_CTRLr, RTL9603D_ACTf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_limitLearningCntAction_set */
...
dal_rtl9603d_l2_limitLearningEntryAction_get(rtk_l2_limitLearnEntryAction_t *pLearningAction)
{
    ...
    /* get value from CHIP*/
    if((ret = reg_field_read(RTL9603D_LUT_CFGr, RTL9603D_LUT_ENTRY_FULL_ACTf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_limitLearningEntryAction_get */
...
dal_rtl9603d_l2_limitLearningEntryAction_set(rtk_l2_limitLearnEntryAction_t learningAction)
{
    ...
    /* programming value to CHIP*/
    if((ret = reg_field_write(RTL9603D_LUT_CFGr, RTL9603D_LUT_ENTRY_FULL_ACTf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_limitLearningEntryAction_set */
...
dal_rtl9603d_l2_limitLearningPortMask_get(rtk_portmask_t *pPortmask)
{
    ...
    HAL_SCAN_ALL_PORT(port)
    {
        if((ret = reg_array_field_read(RTL9603D_LUT_SYS_LRN_LIMITr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &value)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
            return ret;
        }
...
dal_rtl9603d_l2_limitLearningPortMask_set(rtk_portmask_t portmask)
{
    ...
    HAL_SCAN_ALL_PORT(port)
    {
        ...
        if((ret = reg_array_field_write(RTL9603D_LUT_SYS_LRN_LIMITr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &value)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
            return ret;
        }
...
dal_rtl9603d_l2_portLimitLearningOverStatus_get(rtk_port_t port, uint32 *pStatus)
{
    ...
    if((ret = reg_array_field_read(RTL9603D_L2_LRN_OVER_STSr, (uint32)port, REG_ARRAY_INDEX_NONE, RTL9603D_LRN_OVER_INDf, pStatus)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
...
dal_rtl9603d_l2_portLimitLearningOverStatus_clear(rtk_port_t port)
{
    ...
    if((ret = reg_array_field_write(RTL9603D_L2_LRN_OVER_STSr, (uint32)port, REG_ARRAY_INDEX_NONE, RTL9603D_LRN_OVER_INDf, &regData)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
...
dal_rtl9603d_l2_portLearningCnt_get(rtk_port_t port, uint32 *pMacCnt)
{
	...
	if((ret = reg_array_field_read(RTL9603D_L2_LRN_CNTr, port, REG_ARRAY_INDEX_NONE, RTL9603D_L2_LRN_CNTf, pMacCnt)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_portLearningCnt_get */
...
dal_rtl9603d_l2_portLimitLearningCnt_get(rtk_port_t port, uint32 *pMacCnt)
{
	...
	if (( ret = reg_array_field_read(RTL9603D_LUT_LRN_LIMITNOr, port, REG_ARRAY_INDEX_NONE, RTL9603D_NUMf, pMacCnt)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_portLimitLearningCnt_get */
...
dal_rtl9603d_l2_portLimitLearningCnt_set(rtk_port_t port, uint32 macCnt)
{
    ...
    if ((ret = reg_array_field_write(RTL9603D_LUT_LRN_LIMITNOr, port,REG_ARRAY_INDEX_NONE, RTL9603D_NUMf, &macCnt)) != RT_ERR_OK )
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_portLimitLearningCnt_set */
...
dal_rtl9603d_l2_portLimitLearningCntAction_get(rtk_port_t port, rtk_l2_limitLearnCntAction_t *pLearningAction)
{
    ...
    if ((ret = reg_array_field_read(RTL9603D_LUT_LEARN_OVER_CTRLr, port,REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_portLimitLearningCntAction_get */
...
dal_rtl9603d_l2_portLimitLearningCntAction_set(rtk_port_t port, rtk_l2_limitLearnCntAction_t learningAction)
{
    ...
    if ((ret = reg_array_field_write(RTL9603D_LUT_LEARN_OVER_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act))!=RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_portLimitLearningCntAction_set */
...
dal_rtl9603d_l2_aging_get(uint32 *pAgingTime)
{
    ...
    if ((ret = reg_field_read(RTL9603D_LUT_CFGr, RTL9603D_AGE_SPDf, pAgingTime)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_aging_get */
...
dal_rtl9603d_l2_aging_set(uint32 agingTime)
{
    ...
    if ((ret = reg_field_write(RTL9603D_LUT_CFGr, RTL9603D_AGE_SPDf, &newAgingTime)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_aging_set */
...
dal_rtl9603d_l2_portAgingEnable_get(rtk_port_t port, rtk_enable_t *pEnable)
{
    ...
    if ((ret = reg_array_field_read(RTL9603D_LUT_AGEOUT_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_AGEOUT_OUTf, &value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_portAgingEnable_get */
...
dal_rtl9603d_l2_portAgingEnable_set(rtk_port_t port, rtk_enable_t enable)
{
    ...
    if ((ret = reg_array_field_write(RTL9603D_LUT_AGEOUT_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_AGEOUT_OUTf, &value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_portAgingEnable_set */
...
dal_rtl9603d_l2_lookupMissAction_get(rtk_l2_lookupMissType_t type, rtk_action_t *pAction)
{
    ...
    switch(type)
    {
		...
		case DLF_TYPE_MCAST:
		case DLF_TYPE_BCAST:
			if((ret = dal_rtl9603d_l2_portLookupMissAction_get(0, type, pAction)) != RT_ERR_OK)
			{
				RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
				return ret;
			}
		...
		case DLF_TYPE_RSVIPMC:
			if ((ret = rtl9603d_l2_unknReservedIpv4McAction_get(&resAct)) != RT_ERR_OK)
			{
				RT_ERR(ret, (MOD_L2|MOD_DAL), "");
				return ret;
			}
			...
			switch(resAct)
			{
				case RTL9603D_L2_RESACT_NORMAL_UKNOWN_MC:
					*pAction = ACTION_FORWARD;
					break;			
				case RTL9603D_L2_RESACT_TRAP:
					*pAction = ACTION_TRAP2CPU;
					break;
				case RTL9603D_L2_RESACT_FLOODING:
		...
		case DLF_TYPE_RSVIP6MC:
			if ((ret = rtl9603d_l2_unknReservedIpv6McAction_get(&resAct)) != RT_ERR_OK)
			{
				RT_ERR(ret, (MOD_L2|MOD_DAL), "");
				return ret;
			}
		...
		case DLF_TYPE_ICMP6MC:
			if ((ret = rtl9603d_l2_unknMcIcmpv6_get(&enable)) != RT_ERR_OK)
			{
				RT_ERR(ret, (MOD_L2|MOD_DAL), "");
				return ret;
			}
		...
		case DLF_TYPE_DHCP6MC:
			if ((ret = rtl9603d_l2_unknMcDhcpv6_get(&enable)) != RT_ERR_OK)
			{
				RT_ERR(ret, (MOD_L2|MOD_DAL), "");
				return ret;
			}
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_lookupMissAction_get */
...
dal_rtl9603d_l2_lookupMissAction_set(rtk_l2_lookupMissType_t type, rtk_action_t action)
{
    ...
    switch(type)
    {
		...
		case DLF_TYPE_BCAST:
			HAL_SCAN_ALL_PORT(port)
			{
				if ((ret = dal_rtl9603d_l2_portLookupMissAction_set(port, type, action)) != RT_ERR_OK)
				{
					RT_ERR(ret, (MOD_L2|MOD_DAL), "");
					return ret;
				}
		...
		case DLF_TYPE_RSVIPMC:
			...
			{
				case ACTION_FORWARD:
					resAct = RTL9603D_L2_RESACT_NORMAL_UKNOWN_MC;
					break;			
				case ACTION_TRAP2CPU:
					resAct = RTL9603D_L2_RESACT_TRAP;
					break;
				case ACTION_FLOOD_IN_VLAN:
					resAct = RTL9603D_L2_RESACT_FLOODING;
					break;
				default:
					return RT_ERR_CHIP_NOT_SUPPORTED;
			...
			if ((ret = rtl9603d_l2_unknReservedIpv4McAction_set(resAct)) != RT_ERR_OK)
			{
				RT_ERR(ret, (MOD_L2|MOD_DAL), "");
				return ret;
			}
		...
		case DLF_TYPE_RSVIP6MC:
			...
			if ((ret = rtl9603d_l2_unknReservedIpv6McAction_set(resAct)) != RT_ERR_OK)
			{
				RT_ERR(ret, (MOD_L2|MOD_DAL), "");
				return ret;
			}
		...
		case DLF_TYPE_ICMP6MC:
			...
			if ((ret = rtl9603d_l2_unknMcIcmpv6_set(enable)) != RT_ERR_OK)
			{
				RT_ERR(ret, (MOD_L2|MOD_DAL), "");
				return ret;
			}
		...
		case DLF_TYPE_DHCP6MC:
			...
			if ((ret = rtl9603d_l2_unknMcDhcpv6_set(enable)) != RT_ERR_OK)
			{
				RT_ERR(ret, (MOD_L2|MOD_DAL), "");
				return ret;
			}
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_lookupMissAction_set */
...
dal_rtl9603d_l2_portLookupMissAction_get(rtk_port_t port, rtk_l2_lookupMissType_t type, rtk_action_t *pAction)
{
    ...
    {
        case DLF_TYPE_IPMC:
            if ((ret = reg_array_field_read(RTL9603D_UNKN_IP4_MCr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
            ...
            break;
        case DLF_TYPE_IP6MC:
            if (( ret = reg_array_field_read(RTL9603D_UNKN_IP6_MCr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
            ...
            break;
        case DLF_TYPE_UCAST:
            if ((ret = reg_array_field_read(RTL9603D_LUT_UNKN_UC_DA_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
            ...
            break;
        case DLF_TYPE_MCAST:
            if ((ret = reg_array_field_read(RTL9603D_UNKN_L2_MCr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
            ...
            break;
        case DLF_TYPE_BCAST:
            if ((ret = reg_array_field_read(RTL9603D_LUT_BC_BEHAVEr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTIONf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
...
} /* end of dal_rtl9603d_l2_portLookupMissAction_get */
...
dal_rtl9603d_l2_portLookupMissAction_set(rtk_port_t port, rtk_l2_lookupMissType_t type, rtk_action_t action)
{
    ...
    switch(type)
    {
        case DLF_TYPE_IPMC:
            ...
            if ((ret = reg_array_field_write(RTL9603D_UNKN_IP4_MCr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
        ...
        case DLF_TYPE_IP6MC:
            ...
            if ((ret=reg_array_field_write(RTL9603D_UNKN_IP6_MCr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
        ...
        case DLF_TYPE_UCAST:
            ...
            if ((ret = reg_array_field_write(RTL9603D_LUT_UNKN_UC_DA_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
        ...
        case DLF_TYPE_MCAST:
            ...
            if ((ret = reg_array_field_write(RTL9603D_UNKN_L2_MCr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
        ...
        case DLF_TYPE_BCAST:
            ...
            if ((ret = reg_array_field_write(RTL9603D_LUT_BC_BEHAVEr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTIONf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
...
} /* end of dal_rtl9603d_l2_portLookupMissAction_set */
...
dal_rtl9603d_l2_lookupMissFloodPortMask_get(rtk_l2_lookupMissType_t type, rtk_portmask_t *pFlood_portmask)
{
    ...
    switch(type)
    {
        ...
        case DLF_TYPE_MCAST:
            HAL_SCAN_ALL_PORT(port)
            {
                if ((ret = reg_array_field_read(RTL9603D_LUT_UNKN_MC_FLOODr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &act)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
        ...
        case DLF_TYPE_UCAST:
            HAL_SCAN_ALL_PORT(port)
            {
                if ((ret = reg_array_field_read(RTL9603D_LUT_UNKN_UC_FLOODr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &act)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
        ...
        case DLF_TYPE_BCAST:
            HAL_SCAN_ALL_PORT(port)
            {
                if (( ret = reg_array_field_read(RTL9603D_LUT_BC_FLOODr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &act)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_lookupMissFloodPortMask_get */
...
dal_rtl9603d_l2_lookupMissFloodPortMask_set(rtk_l2_lookupMissType_t type, rtk_portmask_t *pFlood_portmask)
{
    ...
    switch(type)
    {
        ...
        case DLF_TYPE_MCAST:
            HAL_SCAN_ALL_PORT(port)
            {
                ...
                if ((ret = reg_array_field_write(RTL9603D_LUT_UNKN_MC_FLOODr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &act)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
        ...
        case DLF_TYPE_UCAST:
            HAL_SCAN_ALL_PORT(port)
            {
                ...
                if (( ret = reg_array_field_write(RTL9603D_LUT_UNKN_UC_FLOODr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &act)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
        ...
        case DLF_TYPE_BCAST:
            HAL_SCAN_ALL_PORT(port)
            {
                ...
                if ((ret = reg_array_field_write(RTL9603D_LUT_BC_FLOODr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &act)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_lookupMissFloodPortMask_set */
...
dal_rtl9603d_l2_lookupMissFloodPortMask_add(rtk_l2_lookupMissType_t type, rtk_port_t flood_port)
{
    ...
    switch (type)
    {
        ...
        case DLF_TYPE_MCAST:
            act = 1;
            if ((ret = reg_array_field_write(RTL9603D_LUT_UNKN_MC_FLOODr, flood_port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
        ...
        case DLF_TYPE_UCAST:
            act = 1;
            if (( ret = reg_array_field_write(RTL9603D_LUT_UNKN_UC_FLOODr, flood_port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
        ...
        case DLF_TYPE_BCAST:
            act = 1;
            if ((ret = reg_array_field_write(RTL9603D_LUT_BC_FLOODr, flood_port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &act)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_lookupMissFloodPortMask_add */
...
dal_rtl9603d_l2_lookupMissFloodPortMask_del(rtk_l2_lookupMissType_t type, rtk_port_t flood_port)
{
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_lookupMissFloodPortMask_del */
    ...
    rtk_action_t            *pFwdAction)
{
    ...
    if ((ret = reg_array_field_read(RTL9603D_LUT_UNKN_SA_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    *pLrnMode = HARDWARE_LEARNING;
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_newMacOp_get */
    ...
    rtk_action_t            fwdAction)
{
    ...
    if ((ret = reg_array_field_write(RTL9603D_LUT_UNKN_SA_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_newMacOp_set */
...
{
    int32   ret;
    rtl9603d_lut_table_t lut_entry;
    ...
    osal_memset(&lut_entry, 0x0, sizeof(rtl9603d_lut_table_t));
    lut_entry.method = RTL9603D_RAW_LUT_READ_METHOD_NEXT_L2UC;
    lut_entry.address = *pScanIdx;
    if ((ret = rtl9603d_raw_l2_lookUpTb_get(&lut_entry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    if ((ret = _dal_rtl9603d_rawtoL2uc(pL2UcastData, &lut_entry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    *pScanIdx = lut_entry.address;
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_nextValidAddr_get */
    ...
    rtk_l2_ucastAddr_t  *pL2UcastData)
{
    ...
    osal_memset(&lut_entry, 0x0, sizeof(rtl9603d_lut_table_t));
    lut_entry.method = RTL9603D_RAW_LUT_READ_METHOD_NEXT_L2UCSPA;
    lut_entry.address = *pScanIdx;
    ...
    *pScanIdx = lut_entry.address;
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_nextValidAddrOnPort_get */
    ...
    rtk_l2_mcastAddr_t  *pL2McastData)
{
    ...
    osal_memset(&lut_entry, 0x0, sizeof(rtl9603d_lut_table_t));
    lut_entry.method = RTL9603D_RAW_LUT_READ_METHOD_NEXT_L2MC;
    lut_entry.address = *pScanIdx;
    if ((ret = rtl9603d_raw_l2_lookUpTb_get(&lut_entry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
    ...
    if ((ret = _dal_rtl9603d_rawtoL2mc(pL2McastData, &lut_entry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    *pScanIdx = lut_entry.address;
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_nextValidMcastAddr_get */
    ...
    rtk_l2_ipMcastAddr_t    *pIpMcastData)
{
    ...
    osal_memset(&lut_entry, 0x0, sizeof(rtl9603d_lut_table_t));
    lut_entry.method = RTL9603D_RAW_LUT_READ_METHOD_NEXT_L3MC;
    lut_entry.address = *pScanIdx;
    if ((ret = rtl9603d_raw_l2_lookUpTb_get(&lut_entry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
    ...
    if ((ret = _dal_rtl9603d_rawtoIpmc(pIpMcastData, &lut_entry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    *pScanIdx = lut_entry.address;
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_nextValidIpMcastAddr_get */
        ...
        rtk_l2_addr_table_t     *pL2Entry)
{
     ...
     * To use LUT_READ_METHOD_MAC, it should use dal_rtl9603d_l2_addr_get
     */
    if(pL2Entry->method != LUT_READ_METHOD_MAC)
    {
        _dal_rtl9603d_lutReadMethodtoRaw(&lut_entry.method, pL2Entry->method);
    }
    else
    {
        lut_entry.method = RTL9603D_RAW_LUT_READ_METHOD_NEXT_ADDRESS;
    }
#else
    lut_entry.method = RTL9603D_RAW_LUT_READ_METHOD_NEXT_ADDRESS;
#endif
    ...
    if(lut_entry.l3lookup == 0)
    {
        if((lut_entry.mac.octet[0] & 0x01) == 0)
        {
            ...
            if ((ret = _dal_rtl9603d_rawtoL2uc(&(pL2Entry->entry.l2UcEntry), &lut_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
                return ret;
            }
        ...
        else
        {
            ...
            if ((ret = _dal_rtl9603d_rawtoL2mc(&(pL2Entry->entry.l2McEntry), &lut_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
                return ret;
            }
    ...
    else
    {
        ...
        if ((ret = _dal_rtl9603d_rawtoIpmc(&(pL2Entry->entry.ipmcEntry), &lut_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
            return ret;
        }
    ...
    *pScanIdx = lut_entry.address;
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_nextValidEntry_get */
...
dal_rtl9603d_l2_addr_add(rtk_l2_ucastAddr_t *pL2Addr)
{
    ...
    RT_PARAM_CHK(pL2Addr->ext_port > RTL9603D_RAW_LUT_EXT_SPAMAX, RT_ERR_OUT_OF_RANGE);
    ...
    osal_memset(&lut_entry, 0x00, sizeof(rtl9603d_lut_table_t));

    if ((ret = _dal_rtl9603d_l2uctoRaw(&lut_entry, pL2Addr)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    lut_entry.table_type = RTL9603D_RAW_LUT_ENTRY_TYPE_L2UC;
    if ((ret = rtl9603d_raw_l2_lookUpTb_set(&lut_entry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_addr_add */
...
dal_rtl9603d_l2_addr_del(rtk_l2_ucastAddr_t *pL2Addr)
{
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_addr_del */
...
dal_rtl9603d_l2_addr_get(rtk_l2_ucastAddr_t *pL2Addr)
{
    ...
    lut_entry.method = RTL9603D_RAW_LUT_READ_METHOD_MAC;
    lut_entry.table_type = RTL9603D_RAW_LUT_ENTRY_TYPE_L2UC;
    if ((ret = rtl9603d_raw_l2_lookUpTb_get(&lut_entry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
    ...
    if ((ret = _dal_rtl9603d_rawtoL2uc(pL2Addr, &lut_entry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_addr_get */
...
{
    int32   ret;
    rtl9603d_raw_flush_ctrl_t flushCtrl, originalCtrl;
    rtk_port_t port;
    ...
    if((ret = rtl9603d_raw_l2_flushCtrl_get(&originalCtrl)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    osal_memset(&flushCtrl, 0x00, sizeof(rtl9603d_raw_flush_ctrl_t));
    if (ENABLED == includeStatic)
        flushCtrl.flushType = RTL9603D_RAW_FLUSH_TYPE_BOTH;
    else
        flushCtrl.flushType = RTL9603D_RAW_FLUSH_TYPE_DYNAMIC;

    flushCtrl.flushMode = RTL9603D_RAW_FLUSH_MODE_PORT;
    if((ret = rtl9603d_raw_l2_flushCtrl_set(&flushCtrl)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    if((ret = rtl9603d_raw_l2_flushCtrl_set(&originalCtrl)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_addr_delAll */
...
dal_rtl9603d_l2_mcastAddr_add(rtk_l2_mcastAddr_t *pMcastAddr)
{
    ...
    RT_PARAM_CHK(pMcastAddr->ext_portmask_idx > RTL9603D_RAW_LUT_EXTMBRIDX, RT_ERR_ENTRY_INDEX);
    RT_PARAM_CHK((pMcastAddr->flags > RTK_L2_MCAST_FLAG_ALL), RT_ERR_INPUT);

    osal_memset(&lut_entry, 0x0, sizeof(rtl9603d_lut_table_t));
    if((ret = _dal_rtl9603d_l2mctoRaw(&lut_entry, pMcastAddr)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }

    lut_entry.table_type = RTL9603D_RAW_LUT_ENTRY_TYPE_L2MC;
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_mcastAddr_add */
...
dal_rtl9603d_l2_mcastAddr_del(rtk_l2_mcastAddr_t *pMcastAddr)
{
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_mcastAddr_del */
...
dal_rtl9603d_l2_mcastAddr_get(rtk_l2_mcastAddr_t *pMcastAddr)
{
    ...
    if ((ret = _dal_rtl9603d_rawtoL2mc(pMcastAddr, &lut_entry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_mcastAddr_get */
...
dal_rtl9603d_l2_extMemberConfig_get(uint32 cfgIndex, rtk_portmask_t *pPortmask)
{
    ...
    /* parameter check */
    RT_PARAM_CHK((cfgIndex > RTL9603D_RAW_LUT_EXTMBRIDX), RT_ERR_INPUT);
    RT_PARAM_CHK((NULL == pPortmask), RT_ERR_NULL_POINTER);

    RTK_PORTMASK_RESET(mask);
    if((ret = reg_array_field_read(RTL9603D_LUT_EXT_MBRr, REG_ARRAY_INDEX_NONE, cfgIndex, RTL9603D_MBRf, &value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
...
dal_rtl9603d_l2_extMemberConfig_set(uint32 cfgIndex, rtk_portmask_t portmask)
{
    ...
    if((ret = reg_array_field_write(RTL9603D_LUT_EXT_MBRr, REG_ARRAY_INDEX_NONE, cfgIndex, RTL9603D_MBRf, &value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    rtk_action_t        *pFwdAction)
{
    ...
    if ((ret = reg_array_field_read(RTL9603D_LUT_UNMATCHED_VLAN_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_vidUnmatchAction_get */
    ...
    rtk_action_t        fwdAction)
{
    ...
    if ((ret = reg_array_field_write(RTL9603D_LUT_UNMATCHED_VLAN_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_vidUnmatchAction_set */
    ...
    rtk_action_t        *pFwdAction)
{
    ...
    if ((ret = reg_array_field_read(RTL9603D_LUT_UNMATCHED_SA_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_illegalPortMoveAction_get */
    ...
    rtk_action_t        fwdAction)
{
    ...
    if ((ret = reg_array_field_write(RTL9603D_LUT_UNMATCHED_SA_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ACTf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_illegalPortMoveAction_set */
...
dal_rtl9603d_l2_ipmcMode_get(rtk_l2_ipmcMode_t *pMode)
{
    ...
    if ((ret = reg_field_read(RTL9603D_LUT_CFGr, RTL9603D_LUT_IPMC_HASHf, &type)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    switch(type)
    {
        case RTL9603D_RAW_IPMC_TYPE_MAC_AND_FID_VID:
            *pMode = LOOKUP_ON_MAC_AND_VID_FID;
            break;

        case RTL9603D_RAW_IPMC_TYPE_GIP_AND_FID_VID:
            *pMode = LOOKUP_ON_DIP_AND_VID_FID;
            break;

        case RTL9603D_RAW_IPMC_TYPE_GIP_AND_SIP:
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_ipmcMode_get */
...
dal_rtl9603d_l2_ipmcMode_set(rtk_l2_ipmcMode_t mode)
{
    ...
    switch(mode)
    {
        case LOOKUP_ON_MAC_AND_VID_FID:
            type = RTL9603D_RAW_IPMC_TYPE_MAC_AND_FID_VID;
            break;

        case LOOKUP_ON_DIP_AND_VID_FID:
            type = RTL9603D_RAW_IPMC_TYPE_GIP_AND_FID_VID;
            break;

        case LOOKUP_ON_DIP_AND_SIP:
            type = RTL9603D_RAW_IPMC_TYPE_GIP_AND_SIP;
            break;
    ...
    if ((ret = reg_field_write(RTL9603D_LUT_CFGr, RTL9603D_LUT_IPMC_HASHf, &type)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_ipmcMode_set */
...
dal_rtl9603d_l2_ipmcVlanMode_get(rtk_l2_ipmcVlanMode_t *pMode)
{
    ...
    if ((ret = reg_field_read(RTL9603D_LUT_CFGr, RTL9603D_LUT_IPMC_VLAN_MODEf, &type)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    switch(type)
    {
        case RTL9603D_RAW_IPMC_VLAN_MODE_IVL_SVL:
            *pMode = LOOKUP_VLAN_BY_IVL_SVL;
            break;

        case RTL9603D_RAW_IPMC_VLAN_MODE_FORCE_NO_VLAN:
            *pMode = LOOKUP_VLAN_FORCE_NO_VLAN;
            break;

        case RTL9603D_RAW_IPMC_VLAN_MODE_FORCE_VLAN:
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_ipmcVlanMode_get */
...
dal_rtl9603d_l2_ipmcVlanMode_set(rtk_l2_ipmcVlanMode_t mode)
{
    ...
    switch(mode)
    {
        case LOOKUP_VLAN_BY_IVL_SVL:
            type = RTL9603D_RAW_IPMC_VLAN_MODE_IVL_SVL;
            break;

        case LOOKUP_VLAN_FORCE_NO_VLAN:
            type = RTL9603D_RAW_IPMC_VLAN_MODE_FORCE_NO_VLAN;
            break;

        case LOOKUP_VLAN_FORCE_VLAN:
            type = RTL9603D_RAW_IPMC_VLAN_MODE_FORCE_VLAN;
            break;
    ...
    if ((ret = reg_field_write(RTL9603D_LUT_CFGr, RTL9603D_LUT_IPMC_VLAN_MODEf, &type)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_ipmcVlanMode_set */
...
dal_rtl9603d_l2_ipmcGroup_add(ipaddr_t gip, rtk_portmask_t *pPortmask)
{
    ...
    for(idx = 0 ; idx <= RTL9603D_IPMC_TABLE_IDX_MAX ; idx++)
    {
        if ((ret = reg_array_field_read(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, idx, RTL9603D_VALIDf, &valid)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2|MOD_DAL), "");
            return ret;
        }
        ...
        if(valid)
        {
            if ((ret = reg_array_field_read(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, idx, RTL9603D_GIPf, &raw_dip)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                return ret;
            }
            ...
            if (raw_dip == gip)
            {
                if ((ret = reg_array_field_write(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, idx, RTL9603D_PMSKf, &RTK_PORTMASK_TO_UINT_PORTMASK(pPortmask))) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
    ...
    if(empty_idx != 0xFFFFFFFF)
    {
        ...
        if ((ret = reg_array_field_write(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, empty_idx, RTL9603D_GIPf, &write_dip)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2|MOD_DAL), "");
            return ret;
        }
        if ((ret = reg_array_field_write(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, empty_idx, RTL9603D_PMSKf, &RTK_PORTMASK_TO_UINT_PORTMASK(pPortmask))) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2|MOD_DAL), "");
            return ret;
        }
        valid = 1;
        if ((ret = reg_array_field_write(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, empty_idx, RTL9603D_VALIDf, &valid)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2|MOD_DAL), "");
            return ret;
        }
...
} /* end of dal_rtl9603d_l2_ipmcGroup_add */
...
dal_rtl9603d_l2_ipmcGroupExtMemberIdx_add(ipaddr_t gip, uint32 index)
{
    ...
    /* parameter check */
    RT_PARAM_CHK(( (gip > 0xEFFFFFFF) || (gip <= 0xE0000000) ), RT_ERR_INPUT);
    RT_PARAM_CHK(index > RTL9603D_RAW_LUT_EXTMBRIDX, RT_ERR_INPUT);
    ...
    for(idx = 0 ; idx <= RTL9603D_IPMC_TABLE_IDX_MAX ; idx++)
    {
        ...
        if(valid)
        {
            ...
            if (raw_dip == gip)
            {
                if ((ret = reg_array_field_write(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, idx, RTL9603D_EXTMBRIDXf, &index)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
    ...
    if(empty_idx != 0xFFFFFFFF)
    {
        ...
        if ((ret = reg_array_field_write(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, empty_idx, RTL9603D_EXTMBRIDXf, &index)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2|MOD_DAL), "");
            return ret;
        }
    ...
    for(idx = 0; idx <= RTL9603D_IPMC_TABLE_IDX_MAX; idx++)
    {
        ...
        if(valid)
        {
            ...
            if (gip == raw_dip)
            {
                write_dip = 0;
                raw_portmask = 0;
                if ((ret = reg_array_field_write(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, idx, RTL9603D_GIPf, &write_dip)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
                if ((ret = reg_array_field_write(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, idx, RTL9603D_PMSKf, &raw_portmask)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
                if ((ret = reg_array_field_write(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, idx, RTL9603D_EXTMBRIDXf, &raw_portmask)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
                valid = 0;
                if ((ret = reg_array_field_write(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, idx, RTL9603D_VALIDf, &valid)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
...
} /* end of dal_rtl9603d_l2_ipmcGroup_del */
...
dal_rtl9603d_l2_ipmcGroup_get(ipaddr_t gip, rtk_portmask_t *pPortmask)
{
    ...
    for(idx = 0; idx <= RTL9603D_IPMC_TABLE_IDX_MAX; idx++)
    {
        ...
        if(valid)
        {
            ...
            if (raw_dip == gip)
            {
                if ((ret = reg_array_field_read(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, idx, RTL9603D_PMSKf, &raw_portmask)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
    ...
    return RT_ERR_ENTRY_NOTFOUND;
} /* end of dal_rtl9603d_l2_ipmcGroup_get */
...
dal_rtl9603d_l2_ipmcGroupExtMemberIdx_get(ipaddr_t gip, uint32 *pIndex)
{
    ...
    for(idx = 0; idx <= RTL9603D_IPMC_TABLE_IDX_MAX; idx++)
    {
        ...
        if(valid)
        {
            ...
            if (raw_dip == gip)
            {
                if ((ret = reg_array_field_read(RTL9603D_IGMP_MC_GROUPr, REG_ARRAY_INDEX_NONE, idx, RTL9603D_EXTMBRIDXf, &extMbrIdx)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_L2|MOD_DAL), "");
                    return ret;
                }
...
dal_rtl9603d_l2_portIpmcAction_get(rtk_port_t port, rtk_action_t *pAction)
{
    ...
    if ((ret = reg_array_field_read(RTL9603D_IGMP_P_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ALLOW_MC_DATAf, (uint32 *)&act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
}   /* end of dal_rtl9603d_l2_portIpmcAction_get */
...
dal_rtl9603d_l2_portIpmcAction_set(rtk_port_t port, rtk_action_t action)
{
    ...
    if ((ret = reg_array_field_write(RTL9603D_IGMP_P_CTRLr, port, REG_ARRAY_INDEX_NONE, RTL9603D_ALLOW_MC_DATAf, (uint32 *)&act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
}   /* end of dal_rtl9603d_l2_portIpmcAction_set */
...
dal_rtl9603d_l2_ipMcastAddr_add(rtk_l2_ipMcastAddr_t *pIpmcastAddr)
{
    ...
    RT_PARAM_CHK(pIpmcastAddr->ext_portmask_idx > RTL9603D_RAW_LUT_EXTMBRIDX, RT_ERR_ENTRY_INDEX);
    ...
    if(hashType != RTL9603D_RAW_IPMC_TYPE_GIP_AND_SIP)
    {
        ...
        {
            RT_PARAM_CHK((pIpmcastAddr->fid > RTL9603D_RAW_LUT_FIDMAX), RT_ERR_L2_FID);
        }
    }

    osal_memset(&lut_entry, 0x0, sizeof(rtl9603d_lut_table_t));
    if ((ret = _dal_rtl9603d_IpmctoRaw(&lut_entry, pIpmcastAddr)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }

    lut_entry.table_type = RTL9603D_RAW_LUT_ENTRY_TYPE_L3MC;
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_ipMcastAddr_add */
...
dal_rtl9603d_l2_ipMcastAddr_del(rtk_l2_ipMcastAddr_t *pIpmcastAddr)
{
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_ipMcastAddr_del */
...
dal_rtl9603d_l2_ipMcastAddr_get(rtk_l2_ipMcastAddr_t *pIpmcastAddr)
{
    ...
    if ((ret = _dal_rtl9603d_rawtoIpmc(pIpmcastAddr, &lut_entry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_L2 | MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_ipMcastAddr_get */
...
dal_rtl9603d_l2_srcPortEgrFilterMask_get(rtk_portmask_t *pFilter_portmask)
{
    ...
    HAL_SCAN_ALL_PORT(port)
    {
        if ((ret = reg_array_field_read(RTL9603D_L2_SRC_PORT_PERMITr, (uint32)port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &permit)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2|MOD_DAL), "");
            return ret;
        }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_srcPortEgrFilterMask_get */
...
dal_rtl9603d_l2_srcPortEgrFilterMask_set(rtk_portmask_t *pFilter_portmask)
{
    ...
    HAL_SCAN_ALL_PORT(port)
    {
        ...
        if ((ret = reg_array_field_write(RTL9603D_L2_SRC_PORT_PERMITr, (uint32)port, REG_ARRAY_INDEX_NONE, RTL9603D_ENf, &permit)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2|MOD_DAL), "");
            return ret;
        }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_srcPortEgrFilterMask_set */
...
dal_rtl9603d_l2_extPortEgrFilterMask_get(rtk_portmask_t *pExt_portmask)
{
    ...
    {
        if ((ret = reg_array_field_read(RTL9603D_L2_SRC_EXT_PERMITr, REG_ARRAY_INDEX_NONE, port, RTL9603D_ENf, &permit)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2|MOD_DAL), "");
            return ret;
        }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_extPortEgrFilterMask_get */
...
dal_rtl9603d_l2_extPortEgrFilterMask_set(rtk_portmask_t *pExt_portmask)
{
    ...
    HAL_SCAN_ALL_EXT_PORT(port)
    {
        ...
        if ((ret = reg_array_field_write(RTL9603D_L2_SRC_EXT_PERMITr, REG_ARRAY_INDEX_NONE, (uint32)port, RTL9603D_ENf, &permit)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_L2|MOD_DAL), "");
            return ret;
        }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_extPortEgrFilterMask_set */
...
dal_rtl9603d_l2_camState_set(rtk_enable_t camState)
{
    ...
    if ((ret = reg_field_write(RTL9603D_LUT_CFGr, RTL9603D_BCAM_DISf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_camState_set */
...
dal_rtl9603d_l2_camState_get(rtk_enable_t *pCamState)
{
    ...
    if ((ret = reg_field_read(RTL9603D_LUT_CFGr, RTL9603D_BCAM_DISf, &act)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_camState_get */
...
dal_rtl9603d_l2_ip6mcReservedAddrEnable_set(rtk_l2_ip6McReservedAddr_t type, rtk_enable_t enable)
{
	...
	if ((ret = reg_field_read(RTL9603D_UNKN_MC_CFGr, RTL9603D_UNKN_MC_IP6_RSV_ADDRf, &regData)) != RT_ERR_OK)
	{
		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
		return ret;
	}
    ...
    if ((ret = reg_field_write(RTL9603D_UNKN_MC_CFGr, RTL9603D_UNKN_MC_IP6_RSV_ADDRf, &regData)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_ip6mcReservedAddrEnable_set */
...
dal_rtl9603d_l2_ip6mcReservedAddrEnable_get(rtk_l2_ip6McReservedAddr_t type, rtk_enable_t *pEnable)
{
	...
	return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_ip6mcReservedAddrEnable_get */
...
dal_rtl9603d_l2_hashValue_get(rtk_l2_hashType_t type, rtk_l2_hashData_t hashData, uint32 *pHashValue)
{
	...
	switch(type)
	{
        ...
        case LUT_HASHTYPE_L2_MCSVL:
            RT_PARAM_CHK(!RTL9603D_HASH_FLAG_CHECK(hashData.dataFlags, LUT_HASHFLAG_MAC | LUT_HASHFLAG_FID), RT_ERR_INPUT);
            if((ret = _dal_rtl9603d_l2MacSvlHash(hashData.mac, hashData.fid, pHashValue)) != RT_ERR_OK)
            {
        		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
        ...
        case LUT_HASHTYPE_L2_MCIVL:
            RT_PARAM_CHK(!RTL9603D_HASH_FLAG_CHECK(hashData.dataFlags, LUT_HASHFLAG_MAC | LUT_HASHFLAG_VID), RT_ERR_INPUT);
            if((ret = _dal_rtl9603d_l2MacIvlHash(hashData.mac, hashData.vid, pHashValue)) != RT_ERR_OK)
            {
        		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
        ...
        case LUT_HASHTYPE_L2_MC:
            RT_PARAM_CHK(!RTL9603D_HASH_FLAG_CHECK(hashData.dataFlags, LUT_HASHFLAG_MAC), RT_ERR_INPUT);
            if((ret = _dal_rtl9603d_l2MacHash(hashData.mac, pHashValue)) != RT_ERR_OK)
            {
        		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
        ...
        case LUT_HASHTYPE_L3_SIPGIP:
            RT_PARAM_CHK(!RTL9603D_HASH_FLAG_CHECK(hashData.dataFlags, LUT_HASHFLAG_SIP | LUT_HASHFLAG_GIP), RT_ERR_INPUT);
            if((ret = _dal_rtl9603d_l3GipSipHash(hashData.gip, hashData.sip, pHashValue)) != RT_ERR_OK)
            {
        		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
        ...
        case LUT_HASHTYPE_L3_GIPFID:
            RT_PARAM_CHK(!RTL9603D_HASH_FLAG_CHECK(hashData.dataFlags, LUT_HASHFLAG_GIP | LUT_HASHFLAG_FID), RT_ERR_INPUT);
            if((ret = _dal_rtl9603d_l3GipFidHash(hashData.gip, hashData.fid, pHashValue)) != RT_ERR_OK)
            {
        		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
        ...
        case LUT_HASHTYPE_L3_GIPVID:
            RT_PARAM_CHK(!RTL9603D_HASH_FLAG_CHECK(hashData.dataFlags, LUT_HASHFLAG_GIP | LUT_HASHFLAG_VID), RT_ERR_INPUT);
            if((ret = _dal_rtl9603d_l3GipVidHash(hashData.gip, hashData.vid, pHashValue)) != RT_ERR_OK)
            {
        		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
        ...
        case LUT_HASHTYPE_L3_GIP:
            RT_PARAM_CHK(!RTL9603D_HASH_FLAG_CHECK(hashData.dataFlags, LUT_HASHFLAG_GIP), RT_ERR_INPUT);
            if((ret = _dal_rtl9603d_l3GipHash(hashData.gip, pHashValue)) != RT_ERR_OK)
            {
        		RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
	...
	return RT_ERR_OK;
} /* end of dal_rtl9603d_l2_hashValue_get */
