...
static int _rtl9607c_lan_sds0_modeV2_set(uint32 mode)
{
    ...
    case LAN_SDS_MODE_FIBER_1G:
        ...
        value = 0x1;
        if((ret = reg_field_write(RTL9607C_UTP_FIBER_AUTODETr, RTL9607C_SDS_CMD_STOP_GLI_CLKf, &value)) != RT_ERR_OK)
        {
            return ret;
        }
        /* Release MDIO reset */
...
static int _rtl9607c_lan_sds1_modeV1_set(uint32 mode)
{
    ...
    /* MAC link down (force link down) */
    if((ret = rtk_port_macForceAbility_get(7, &oldAbility)) != RT_ERR_OK)
    {
        return ret;
    }
    if((ret = rtk_port_macForceAbilityState_get(7, &oldState)) != RT_ERR_OK)
    {
        return ret;
    }
    newAbility = oldAbility;
    newAbility.linkStatus = PORT_LINKDOWN;
    if((ret = rtk_port_macForceAbility_set(7, newAbility)) != RT_ERR_OK)
    {
        return ret;
    }
...
//    if((ret = rtk_cpu_trapInsertTagByPort_set(7, DISABLED)) != RT_ERR_OK)
    {
        return ret;
    }
    ...
    /* MAC link restore (restore to original link state) */
    if((ret = rtk_port_macForceAbility_set(7, oldAbility)) != RT_ERR_OK)
    {
        return ret;
    }
    if((ret = rtk_port_macForceAbilityState_set(7, oldState)) != RT_ERR_OK)
    {
        return ret;
    }
...
static int _rtl9607c_lan_sds1_modeV2_set(uint32 mode)
{
    ...
    case LAN_SDS_MODE_HSGMII_MAC:
         ...
         * ANA0E[3:1]: REG_TX_DRV_VLDOP<2:0>=0x1
         */
        if((ret = _rtl9607c_lan_sds1_indirectAccess(0xc00e, 0xA3)) != RT_ERR_OK)
        {
            return ret;
        }
        /* ANA0F[15:15]: REG_CMU_SEL_CP<15:15>=0x1
