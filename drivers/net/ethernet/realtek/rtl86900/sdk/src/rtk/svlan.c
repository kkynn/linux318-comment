 ...
 *
 * $Revision: 66868 $
 * $Date: 2016-03-18 15:47:22 +0800 (Fri, 18 Mar 2016) $
 *
...
{
    int32   ret;
    if (NULL == RT_MAPPER->svlan_extPortPvid_get)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->svlan_extPortPvid_get(extPort, pPvid);
    RTK_API_UNLOCK();
    return ret;
...
rtk_svlan_extPortPvid_set(uint32 extPort, uint32 pvid)
{
    ...
    if (NULL == RT_MAPPER->svlan_extPortPvid_get)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->svlan_extPortPvid_set( extPort, pvid);
    RTK_API_UNLOCK();
    return ret;
