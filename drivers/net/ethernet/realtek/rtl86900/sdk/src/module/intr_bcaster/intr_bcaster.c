...
#if defined(CONFIG_GPON_FEATURE) && !defined(CONFIG_CA_RTK_GPON_FEATURE)
...
#if !defined(CONFIG_SDK_LUNA_G3) && !defined(CONFIG_SDK_CA8279)//link change intr register move to dal_luna_g3_intr_init
...
#if !defined(CONFIG_SDK_LUNA_G3) && !defined(CONFIG_SDK_CA8279) //link change intr register move to dal_luna_g3_intr_init, mark dying gasp intr 
    if (RT_ERR_OK != link_change_intr_register())
    {
        printk(KERN_ERR
            "%s: register link change interrupt failed!\n", __FUNCTION__);
