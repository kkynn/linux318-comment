...
static int epon_oam_orgSpec_set(unsigned char *pOui, unsigned char state)
{
    oam_orgSpecCbRec_t *pSearchPtr;
...
int epon_oam_database_init(void)
{
    ...
    for(i = 0;i < EPON_OAM_SUPPORT_LLID_NUM;i++)
    {
		...
		infoOam.oui[0] = llid_entry.mac.octet[0];
		infoOam.oui[1] = llid_entry.mac.octet[1];
		infoOam.oui[2] = llid_entry.mac.octet[2];
		...
		/* Set default keepalive time */
		oamConfig[i].keepaliveTime = 5000; /* ms */
    ...
    oam_oamInfo_t *pOamInfo)
{
	...
	/* patch: llid maybe modified unexpectedly, it is better to sync with llid entry here. */
	memset(&llid_entry, 0, sizeof(rtk_epon_llid_entry_t));
	llid_entry.llidIdx = llidIdx;
	rtk_epon_llid_entry_get(&llid_entry);
	localOamInfo[llidIdx].oui[0] = llid_entry.mac.octet[0];
	localOamInfo[llidIdx].oui[1] = llid_entry.mac.octet[1];
	localOamInfo[llidIdx].oui[2] = llid_entry.mac.octet[2];
    ...
    void *pCbData)
{
    ...
    case EPON_OAM_CBTYPE_ORGSPEC:
        ...
        {
            ((oam_orgSpecCb_t *) pCbData)->state = pOrgSpecCbRec->state;
...
/* Function Name:
 *      epon_oam_orgSpecCb_set
 * Description:
 *      Set callback function for organization specific OAM fields
 * Input:
 *      regType - state the type of callback function to be get
 *      pOui    - OUI of the callback function, this is the search key
 *      state   - enable / disable of the organization specific OAM
 * Output:
 *      None
 * Return:
 *      EPON_OAM_ERR_OK
    ...
    unsigned char state)
{
    ...
    case EPON_OAM_CBTYPE_EVENT_ORGSPEC:
        ...
        break;
    case EPON_OAM_CBTYPE_ORGSPEC:
        ret = epon_oam_orgSpec_set(pOui, state);
        break;
    default:
        ret = EPON_OAM_ERR_PARAM;
        break;
    ...
    void *pCbData)
{
    ...
    case EPON_OAM_CBTYPE_EVENT_ORGSPEC:
        ...
        break;
    case EPON_OAM_CBTYPE_ORGSPEC:
        defOrgSpecCb.state = ((oam_orgSpecCb_t *)pCbData)->state;
    ...
    void *pCbData)
{
    ...
    case EPON_OAM_CBTYPE_EVENT_ORGSPEC:
        ...
        break;
    case EPON_OAM_CBTYPE_ORGSPEC:
        ((oam_orgSpecCb_t *)pCbData)->state = defOrgSpecCb.state;
    ...
    unsigned short llidIdx)
{
    ...
    localOamInfo[llidIdx].venderSpecInfo[0] = 0;
    localOamInfo[llidIdx].venderSpecInfo[1] = 0;
    localOamInfo[llidIdx].venderSpecInfo[2] = 0;
    localOamInfo[llidIdx].venderSpecInfo[3] = 0;
    sem_post(&oamDbAccessSem);
}
