    ...
    unsigned short *pPushedLen)
{
    ...
    *pPushedLen = 0;
    ...
    if(localInfo.valid && (bufLen > EPON_INFO_OAMPDU_INFO_LEN))
    {
		...
		CTC_BUF_ENCODE16(pBuf+3, &localInfo.revision);
#else
...
#ifdef SUPPORT_OAM_BYTEORDER
		CTC_BUF_ENCODE16(pBuf+7, &localInfo.oamPduConfig);
#else
...
#ifdef SUPPORT_OAM_BYTEORDER
			CTC_BUF_ENCODE16(pBuf+3, &remoteInfo.revision);
#else
...
#ifdef SUPPORT_OAM_BYTEORDER
			CTC_BUF_ENCODE16(pBuf+7, &remoteInfo.oamPduConfig);
#else
    ...
    unsigned char *pFrameBuf)
{
    ...
    *pFrameBuf = pVarContainer->varDesc.varBranch;
    pFrameBuf++;
#ifdef SUPPORT_OAM_BYTEORDER
	CTC_BUF_ENCODE16(pFrameBuf, &pVarContainer->varDesc.varLeaf);
	pFrameBuf += 2;
#else
...
#ifdef SUPPORT_OAM_BYTEORDER
	CTC_BUF_ENCODE16(pFrameBuf, &oamPduFlag);
	pFrameBuf += 2;
#else
...
#ifdef SUPPORT_OAM_BYTEORDER
		CTC_BUF_ENCODE16(pPtr, &oamPduFlag);
		pPtr += 2;
#else
    ...
    oam_eventOrgSpec_t *pEvent)
{
    ...
    eventSeq[llidIdx]++;
#ifdef SUPPORT_OAM_BYTEORDER
	CTC_BUF_ENCODE16(pCurr, &eventSeq[llidIdx]);
	pCurr += 2;
#else
...
#ifdef SUPPORT_OAM_BYTEORDER
		CTC_BUF_ENCODE16(pCurr, &eventLen);
		pCurr += 2;
#else
...
/* send oam event to kernel, e.g. oam state change */
int epon_oam_notify_event_send(rtk_epon_oam_event_t * event)
{
    int ret;
	unsigned char *pPtr, *pCurr;
	...
	if(event->llidIdx >= EPON_OAM_SUPPORT_LLID_NUM)
    {
        return EPON_OAM_ERR_PARAM;
    }
    pPtr = (unsigned char *) malloc(sizeof(eventKey) + sizeof(rtk_epon_oam_event_t));
    ...
    /* Payload */
	pCurr = pPtr;
    memcpy(pCurr, eventKey, sizeof(eventKey));
    pCurr += sizeof(eventKey);

    memcpy(pCurr, event, sizeof(rtk_epon_oam_event_t));
	pCurr += sizeof(rtk_epon_oam_event_t);
