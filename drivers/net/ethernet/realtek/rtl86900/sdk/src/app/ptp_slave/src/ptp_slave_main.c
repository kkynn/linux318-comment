...
char delay_req_packet[DELAY_REQ_LEN]= {
    ...
    0x00,0x00,0x00,0x11,0x22,0xff,0xff,0x33,0x44,0x55,0x00,0x00,0x00,0x00,0x01,0x00,
    ...
    while(1)
    {
        ...
        switch(pmsg.mtype)
        {
            case MQ_SLAVE_RX_PACKET:
                ...
                if((pkt_ptr->pkt[PTP_ETHERTYPE_offset+sizeof(ptp_pkt_cputag_t)] == 0x88 ) && (pkt_ptr->pkt[PTP_ETHERTYPE_offset+1+sizeof(ptp_pkt_cputag_t)] == 0xf7))
                {
                    if((pkt_ptr->pkt[PTP_MTYPE_offset+sizeof(ptp_pkt_cputag_t)]&0xf)==PTP_Announce)
                    {
                        /*announce*/
                        DBG_PRINT(1,"[PTP Slave:%s:%d] receiving announce!\n",__FUNCTION__,__LINE__);

                        if(pkt_ptr->pkt_len < (ANNOUNCE_LEN+sizeof(ptp_pkt_cputag_t)))
                        {
                        ...
                        if(memcmp(bestClockId,&(pkt_ptr->pkt[PTP_CLOCKID_offset+sizeof(ptp_pkt_cputag_t)]),10)!=0)
                        {
                            /*changing best grand master*/
                            memcpy(bestClockId,&(pkt_ptr->pkt[PTP_CLOCKID_offset+sizeof(ptp_pkt_cputag_t)]),10);
                    ...
                    if((pkt_ptr->pkt[PTP_MTYPE_offset+sizeof(ptp_pkt_cputag_t)]&0xf)==PTP_Sync)
                    {                        
                        if((pkt_ptr->pkt[PTP_FLAG_offset+sizeof(ptp_pkt_cputag_t)]&(1<<1))==0)
                        {
                            /*one-step sync*/
                            DBG_PRINT(1,"[PTP Slave:%s:%d] receiving one-step sync!\n",__FUNCTION__,__LINE__);
                            
                            if(pkt_ptr->pkt_len < (SYN_LEN+sizeof(ptp_pkt_cputag_t)))
                            {
                                DBG_PRINT(1,"[PTP Slave:%s:%d] Not supporting packet len=%d!\n", __FUNCTION__, __LINE__,pkt_ptr->pkt_len);
                                for(i=0;i<pkt_ptr->pkt_len && i<PTP_SLAVE_MAX_PKT_SIZE;i++)
                                {
                            ...
                            /*store sequence id*/
                            slaveTs.sqid=ntohs(*((uint16 *)&(pkt_ptr->pkt[PTP_SQID_offset+sizeof(ptp_pkt_cputag_t)])));

                            /*store T1 and T2*/
                            memcpy(dTime_ptr->t1.sec,&(pkt_ptr->pkt[PTP_TSSEC_offset+sizeof(ptp_pkt_cputag_t)]),6);
                            memcpy(dTime_ptr->t1.nanosec,&(pkt_ptr->pkt[PTP_TSNSEC_offset+sizeof(ptp_pkt_cputag_t)]),4);

                            memcpy(dTime_ptr->t2.sec,cpuTagPtr->sec,6);
                            memcpy(dTime_ptr->t2.nanosec,cpuTagPtr->nanosec,4);
                            
                            /*send delay_req*/
                            memcpy(&delay_req_packet[PTP_SQID_offset],&(pkt_ptr->pkt[PTP_SQID_offset+sizeof(ptp_pkt_cputag_t)]),2);
                    ...
                    if((pkt_ptr->pkt[PTP_MTYPE_offset+sizeof(ptp_pkt_cputag_t)]&0xf)==PTP_Follow_Up)
                    {
                        /*two-step follow_up*/
                        DBG_PRINT(1,"[PTP Slave:%s:%d] receiving two-step follow_up!\n",__FUNCTION__,__LINE__);
                        
                        if(pkt_ptr->pkt_len < (FOLLOW_UP_LEN+sizeof(ptp_pkt_cputag_t)))
                        {
                            DBG_PRINT(1,"[PTP Slave:%s:%d] Not supporting packet len=%d!\n", __FUNCTION__, __LINE__,pkt_ptr->pkt_len);
                            for(i=0;i<pkt_ptr->pkt_len && i<PTP_SLAVE_MAX_PKT_SIZE;i++)
                            {
                        ...
                        /*check sequence id*/
                        sqid=ntohs(*((uint16 *)&pkt_ptr->pkt[PTP_SQID_offset+sizeof(ptp_pkt_cputag_t)]));
                    ...
                    if((pkt_ptr->pkt[PTP_MTYPE_offset+sizeof(ptp_pkt_cputag_t)]&0xf)==PTP_Delay_Resp)
                    {
                        /*delay_resp*/
                        DBG_PRINT(1,"[PTP Slave:%s:%d] receiving delay_resp!\n",__FUNCTION__,__LINE__);

                        if(pkt_ptr->pkt_len < (DELAY_RESP_LEN+sizeof(ptp_pkt_cputag_t)))
                        {
                            DBG_PRINT(1,"[PTP Slave:%s:%d] Not supporting packet len=%d!\n", __FUNCTION__, __LINE__,pkt_ptr->pkt_len);
                            for(i=0;i<pkt_ptr->pkt_len && i<PTP_SLAVE_MAX_PKT_SIZE;i++)
                            {
                        ...
                        if(memcmp(slaveClockid,&(pkt_ptr->pkt[PTP_REQCLOCKID_offset+sizeof(ptp_pkt_cputag_t)]),10)!=0)
                        {
                        ...
                        /*store T4*/
                        memcpy(dTime_ptr->t4.sec,&(pkt_ptr->pkt[PTP_TSSEC_offset+sizeof(ptp_pkt_cputag_t)]),6);
                        memcpy(dTime_ptr->t4.nanosec,&(pkt_ptr->pkt[PTP_TSNSEC_offset+sizeof(ptp_pkt_cputag_t)]),4);
...
static int32 ptp_configfile_set(int argc,char *argv[])
{
    ...
    if(help==1)
    {
        ...
        printf("%-5s  %-35s\n","-sn","Set Recording Offset Sample Num");
        printf("%-5s  %-35s\n","-ao","Using Average Offset of Recording Sample Adjust Offset 1:y 2:n");
