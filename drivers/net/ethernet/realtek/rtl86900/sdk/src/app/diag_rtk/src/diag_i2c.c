    ...
    cparser_context_t *context)
{
    ...
    if(NULL != i2c_write_buffer)
    {
        diag_util_printf("     00 01 02 03 04 05 06 07 08 09 0a 0b 0c 0d 0e 0f", i);
...
/* BEGIN: Added by huangyl, 2019/1/2; 添加BOSA校准参数调式命令  */
/*
 * i2c bosa_calibrate
 */
cparser_result_t
cparser_cmd_i2c_bosa_calibrate_show(
    cparser_context_t *context)
{
    int32 i;

    DIAG_UTIL_PARAM_CHK();
    rtusr_bosa_calibrate_show();
    diag_util_printf("\n");
...
cparser_result_t
cparser_cmd_i2c_bosa_calibrate_load(
    cparser_context_t *context)
{
    int32 i;

    DIAG_UTIL_PARAM_CHK();
    if(rtusr_bosa_calibrate_load())
        diag_util_printf("bosa_calibrate load failed !!!\n");
    else
        diag_util_printf("bosa_calibrate load success\n");

    return CPARSER_OK;
}

cparser_result_t
cparser_cmd_i2c_bosa_calibrate_save(
    cparser_context_t *context)
{
    int32 i;

    DIAG_UTIL_PARAM_CHK();
    if(rtusr_bosa_calibrate_save())
        diag_util_printf("bosa_calibrate save failed !!!\n");
    else
        diag_util_printf("bosa_calibrate save success\n");
        
    return CPARSER_OK;
}

cparser_result_t cparser_cmd_i2c_bosa_calibrate_set_dev_dev_id_reg_register_count_count_data_data(
    cparser_context_t *context,
    uint32_t  *dev_id_ptr,
    uint32_t  *register_ptr,
    uint32_t  *count_ptr,
    char * *data_ptr)
{
    ...
    uint16 payload_length;
    uint32 temp_data;
    uint8 write_buffer[BOSA_I2C_BUFFSIZE] = {0};
    ...
    payload_length = input_length >> 1;
    tmp[2] = '\0';
    for(i = *register_ptr, j = 0, pPtr = *data_ptr; j < payload_length && i < BOSA_I2C_BUFFSIZE; i++, j++, pPtr += 2)
    {
        tmp[0] = pPtr[0];
        tmp[1] = pPtr[1];
        write_buffer[i] = strtoul(&tmp[0], NULL, 16);
    }

    if(*count_ptr == (i - *register_ptr))
    {
        rtk_i2c_init(I2C_PORT_0);
        rtk_i2c_enable_set(I2C_PORT_0, ENABLED);
        rtk_i2c_clock_set(I2C_PORT_0, 400);
        
        for(i = 0; i < *count_ptr; i++)
        {
            temp_data = write_buffer[*register_ptr+i];
            ret = rtk_i2c_write(I2C_PORT_0, *dev_id_ptr, *register_ptr+i, temp_data);
            //usleep(100);
        }
        diag_util_printf("i2c bosa_calibrate set ok \n");
    }
    else
    {
        if((i - *register_ptr) > *count_ptr)
            diag_util_printf("Input the data is too large !!!!!!\n");        
        else
            diag_util_printf("Input the data is too short !!!!!!\n");   
    }

    return CPARSER_OK;
}

cparser_result_t cparser_cmd_i2c_bosa_calibrate_get_dev_dev_id_reg_register_count_count(
    cparser_context_t *context,
    uint32_t  *dev_id_ptr,
    uint32_t  *register_ptr,
    uint32_t  *count_ptr)
{
    int32 i, j, ret;
    uint32 temp_data;
    uint8 read_buffer[BOSA_I2C_BUFFSIZE] = {0};
    ...
    for(i = 0; i < *count_ptr; i++)
    {
        ret = rtk_i2c_read(I2C_PORT_0, *dev_id_ptr, *register_ptr+i, &temp_data);
        read_buffer[*register_ptr+i] = temp_data;
    }

    for(i = 0; i < *count_ptr; i++)
    {  
        diag_util_printf("%02x ", read_buffer[*register_ptr+i]);
        if((i+1)%16 == 0)
            diag_util_printf("\n");
    }
