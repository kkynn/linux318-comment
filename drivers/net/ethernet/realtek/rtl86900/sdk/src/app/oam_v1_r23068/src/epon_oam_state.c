    ...
    oam_oamInfo_t *pOamInfo)
{
...
#endif
        epon_oam_localInfo_get(llidIdx,&localInfo);
        if((localInfo.venderSpecInfo[0] == 0) &&
           (localInfo.venderSpecInfo[1] == 0) &&
           (localInfo.venderSpecInfo[2] == 0) &&
           (localInfo.venderSpecInfo[3] == 0))
        {
            llidEntry.llidIdx = llidIdx;
            ret = rtk_epon_llid_entry_get(&llidEntry);
            if(RT_ERR_OK == ret)
            {
                /* Save the registed MAC in local info vender specific info field */
				unsigned char vendorSpecInfo[4];
				if (ctc_oam_flash_var_get("HW_VENDOR_SPECINFO", vendorSpecInfo, sizeof(vendorSpecInfo)) != 
					EPON_OAM_ERR_OK)
				{
#ifdef YUEME_CUSTOMIZED_CHANGE
	                localInfo.venderSpecInfo[0] = 'T';
	                localInfo.venderSpecInfo[1] = 'M';
	                localInfo.venderSpecInfo[2] = 'B';
	                localInfo.venderSpecInfo[3] = 'B';
#else
	                localInfo.venderSpecInfo[0] = llidEntry.mac.octet[2];
	                localInfo.venderSpecInfo[1] = llidEntry.mac.octet[3];
	                localInfo.venderSpecInfo[2] = llidEntry.mac.octet[4];
	                localInfo.venderSpecInfo[3] = llidEntry.mac.octet[5];
#endif
				}
				else
				{
					memcpy(localInfo.venderSpecInfo, vendorSpecInfo, 4);
				}
                epon_oam_localInfo_set(llidIdx, &localInfo);
    ...
    case EPON_OAM_FSM_STATE_WAIT_REMOTE_OK:
		...
		epon_oam_register_number_increase(llidIdx);
        break;
