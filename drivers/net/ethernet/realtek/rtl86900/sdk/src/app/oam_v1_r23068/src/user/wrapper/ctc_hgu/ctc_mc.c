...
int igmpMcVlanClear(uint32 portId)
{
	...
	ctc_hgu_acl_rule_for_mcastvlan_clear(portId);
...
static int32 _ctc_getportVlanmodecfg(uint32 uiLPortId, ctc_wrapper_vlanCfg_t **pstCtcVlanMode)
{
	...
	if (RT_ERR_OK != ctc_hgu_vlanCfg_get(uiLPortId, pstCtcVlanMode))
	{        
...
static  int _igmpMcVlanAdd(uint32 portId, uint32 vlanId)
{
	...
	{
		case TAG_OPER_MODE_TRANSPARENT:
			/* add cf rule for MC Vlan transparent */
			ret = ctc_hgu_acl_rule_for_McastTransparent_create(portId, vlanId);
			if (RT_ERR_OK != ret)
				return SYS_ERR_FAILED;
			...
			break;
		case TAG_OPER_MODE_STRIP:
			/* add cf rule for MC Vlan strip */
			ret = ctc_hgu_acl_rule_for_McastStrip_create(portId, vlanId);
			if (RT_ERR_OK != ret)
				return SYS_ERR_FAILED;
...
int igmpMcVlanAdd(uint32 portId, uint32 vlanId)
{
	...
	RTK_CTC_DPRINTF("portId %d vlanId %d\n", portId, vlanId);
	...
	for(i=0; i<VLAN_TRANSLATION_ENTRY; i++)
	{
		if(igmpTagTranslationTable[i].enable==ENABLED)
		{
			if(igmpTagTranslationTable[i].portId == portId) 
			{
				RTK_PORTMASK_RESET(stLMask);
				RTK_PORTMASK_PORT_SET(stLMask, uiPPort);
				ret = drv_clf_translation_vlan_member_remove(igmpTagTranslationTable[i].userVid, stLMask);
				if (RT_ERR_OK != ret)
				{
					RTK_CTC_DPRINTF("failed to remove translation vlan member of port [%d]\n",portId);
					return SYS_ERR_FAILED;
				}
...
int32 drv_addport2mvlan(uint32 uiLPort, uint32 uiMcVid, mcvlan_tag_oper_mode_t enTagMode)
{
	...
	/********************************************************
	 * Martin ZHU note:
	 * MC_CTRL_GDA_MAC_FID should use VLAN_FID_IVL(FID), all other
	 * mode,including MC_CTRL_GDA_GDA_IP_VID, should use 
	 * VLAN_FID_IVL(VID) mode.but rg only support DMAC+FID, DMAC+VID and 
	 * GIP only for multicast,so, except MC_CTRL_GDA_MAC_VID 
	 * mode, we all use VLAN_FID_SVL(FID)mode.
	 ********************************************************/
	...
	{
		RTK_CTC_DPRINTF("%s[%d]:Failed to call drv_mc_vlan_member_add func.\n",__func__,__LINE__);
		return SYS_ERR_FAILED;
	}
	return SYS_ERR_OK;
}
...
int igspTransparentModeSetAction(uint32 portId)
{
	...
	for(j=1; j<MAX_VLAN_NUMBER; j++) 
	{
		if(igspVlanMapExist(portId, j))    
		{			
			ret = ctc_hgu_acl_rule_for_McastTransparent_create(portId, j);
			if (RT_ERR_OK != ret)
				return SYS_ERR_FAILED;
			
			ret = drv_addport2mvlan(portId, j, TAG_OPER_MODE_TRANSPARENT);
			if (SYS_ERR_OK != ret)
			{
				return SYS_ERR_FAILED;
			}		
...
int igspStripModeSetAction(uint32 portId)
{
	...
	for(j=1; j<MAX_VLAN_NUMBER; j++) 
	{
		if(igspVlanMapExist(portId, j)) 
		{
			ret = ctc_hgu_acl_rule_for_McastStrip_create(portId, j);
			if (RT_ERR_OK != ret)
				return SYS_ERR_FAILED;
			
			ret = drv_addport2mvlan(portId,j,TAG_OPER_MODE_STRIP);
			if (SYS_ERR_OK != ret)
			{
				return SYS_ERR_FAILED;
			}       
...
int igmpMcTagstripSet(uint32 portId, uint32 tagOper)
{
	...
	if(tagOper == igmpTagOperPerPortGet(portId))
	{
		RTK_CTC_DPRINTF("port[%d] tag mode[%d] not change, do nothing\n", portId, tagOper);
		return SYS_ERR_OK;
	}
...
int McTagTranslationCfAdd(uint32 portId, uint32 mcVid, uint32 userVid)
{
	...
	ret = ctc_hgu_acl_rule_for_McastTranslation_create(portId, mcVid, userVid);
	if (RT_ERR_OK != ret)
		return SYS_ERR_FAILED;
	...
	if (0 != ret) 
	{	
		RTK_CTC_DPRINTF("%s[%d]:call drv_addport2mvlan failed.\n", __func__,__LINE__);
		return SYS_ERR_FAILED;
	} 
	RTK_PORTMASK_RESET(stLMask);
	RTK_PORTMASK_RESET(stUntagLMsk);
	...
	drv_clf_translation_vlan_member_add(userVid, stLMask, stUntagLMsk);
	return SYS_ERR_OK;
}
...
int igmpMcTagTranslationSet(uint32 portId, uint32 mcVid, uint32 userVid)
{
	...
	RTK_CTC_DPRINTF("set port[%d] mc translation vlan table %d:%d\n", portId, mcVid, userVid);
...
int igmpMcCtrlGrpTypeSet(mc_control_e type)
{
	...
	/* clear software group entry */
	mcast_mcCtrl_group_entry_clear();

	if((type == MC_CTRL_GDA_MAC_VID) || (igmpctrlBlock.ctcControlType == MC_CTRL_GDA_MAC_VID))
	{
		int i;
		for(i = 1; i < MAX_VLAN_NUMBER; i++) 
		{
			...
			{
				if(type == MC_CTRL_GDA_MAC_VID)
				{
					//vlan fidmode from VLAN_FID_IVL to VLAN_FID_SVL
					drv_vlan_set_fidMode(igspVlanMap[i].vid, VLAN_FID_IVL);
				}
				else
				{
					//vlan fidmode from VLAN_FID_SVL to VLAN_FID_IVL
...
int igmpmcinit(void)
{
	...
	FOR_EACH_LAN_PORT(i)
	{
		...
		//cxy 2016-6-4: for hgu set port to strip mode
		igmpMcTagstripSet(i, TAG_OPER_MODE_STRIP);
	}
#if 0
	//cxy 2016-6-4: default add all lan port to mcast vlan of iptv wan vid
	for(i=0;i<8;i++)
	{
		int j;
		...
		snprintf(mibstr,sizeof(mibstr),"ATM_VC_TBL.%d.applicationtype", i);
		if(ctc_oam_flash_var_get(mibstr,value,sizeof(value)) != EPON_OAM_ERR_OK)
			continue;
		apptype = strtol(value, NULL, 10);
		if(apptype == 4)//iptv
		{
			snprintf(mibstr,sizeof(mibstr),"ATM_VC_TBL.%d.vid", i);
			if(ctc_oam_flash_var_get(mibstr,value,sizeof(value)) != EPON_OAM_ERR_OK)
				continue;
			vlan = strtol(value, NULL, 10);
			snprintf(mibstr,sizeof(mibstr),"ATM_VC_TBL.%d.itfGroup", i);
			if(ctc_oam_flash_var_get(mibstr,value,sizeof(value)) != EPON_OAM_ERR_OK )
				continue;
			itfgroup = strtol(value, NULL, 10);
			FOR_EACH_LAN_PORT(j)
			{
				if(itfgroup&(1<<j))
					igmpMcVlanAdd(j, vlan);
			}
		}
	}	
#endif	
	...
	igmpMcVlanAdd(0, 2299);
	igmpMcVlanAdd(1, 2299);
	
	igmpMcTagstripSet(0, TAG_OPER_MODE_TRANSPARENT);
	igmpMcTagstripSet(1, TAG_OPER_MODE_STRIP);
	igmpMcTagstripSet(2, TAG_OPER_MODE_TRANSLATION);
	if(igmpTagOperPerPortGet(2) == TAG_OPER_MODE_TRANSLATION)
	{
		igmpMcTagTranslationSet(2,2502, 252);
		igmpMcTagTranslationSet(2,2504, 254);
		igmpMcTagTranslationSet(2,2506, 256);
	}
