...
#include <DRV/platform/include/rtl9607_fc/omci_pf_rtl9607.h>
...
//static rtk_classify_field_t tagPri,interPri,InnerVlan, isCtag, isStag, etherType, tagVid, uni, tosGemId, dei, aclHit;
    ...
    if ((0x88a8 == systemTpid || 0x9100 == systemTpid) && APOLLOMP_CHIP_ID == gPlatformDb.chipId)
    {
            ...
            CLASSIFY_US_CACT_TRANSPARENT == cfg.act.usAct.cAct && 0xfff == vidMask)
        {
            ...
            else
            {
                ...
                {
                    rtk_vlan_port_set(vidData, &mbrmsk, &utgmask);
                }
            }
            ...
            CLASSIFY_US_CACT_TRANSPARENT == cfg.act.usAct.cAct))
        {
            ...
            for(idx = 0; idx <= 4095; idx++)
            {
                if(RT_ERR_OK == rtk_vlan_port_get(idx, &mbrmsk, &utgmask))
                {
                    ...
                    {
                        rtk_vlan_port_set(idx, &mbrmsk, &utgmask);
                    }
                }
            }
        }
...
static int _InitUsedTcontId(void)
{
    ...
    for(i=0;i<gPlatformDb.maxTcont;i++)
    {
        if(gPlatformDb.tCont[i].allocId!=0xff)
        {
            ...
            rtk_gpon_tcont_destroy_logical(&ind);
            gPlatformDb.tCont[i].allocId = 0xff;
        }
    }
    return 0;
...
static void _AllocateTcont(rtk_gpon_ploam_t *ploam)
{
    ...
    else if(ploam->msg[2]==1 /*&& ret !=RT_ERR_OK*/)
    {
       ...
       if(ret == OMCI_ERR_OK)
       {
            ret = rtk_gpon_tcont_create(&ind,&attr);
       }
    }
    return;
}
...
static int omci_SetClassifyDsAct(OMCI_VLAN_OPER_ts *pVlanRule,unsigned int uniMask,rtk_classify_ds_act_t *pDsAct)
{
    ...
    }else
    {
        ...
        omcidrv_feature_api(FEATURE_KAPI_BDP_00000100_DS, pDsAct, &gPlatformDb);
...
static int omci_SetClassifyUsRule(
    OMCI_VLAN_OPER_ts *pVlanRule, unsigned int srcUni, rtk_classify_cfg_t *pClassifyCfg, omci_rule_pri_t *pRulePri, int aclLatchIdx)
{
    ...
    else
    {
        ...
        switch (pDsRule->filterMode)
        {
            ...
            case VLAN_OPER_MODE_EXTVLAN:
                ...
                {
                    pDsRule->filterRule.filterCtagMode = VLAN_FILTER_CARE_TAG;//VLAN_FILTER_NO_TAG;
                }
                            ...
                            unsigned int        streamId)
{
    ...
    if (OMCI_ERR_OK != ret)
    {
        OMCI_LOG(OMCI_LOG_LEVEL_DBG, "set action for ds wan vlan cf fail!");
    ...
    if (OMCI_ERR_OK != ret)
    {
        OMCI_LOG(OMCI_LOG_LEVEL_DBG, "set rule for ds wan vlan cf fail!");
...
omcidrv_setVeipUsWanIntfCfContent(OMCI_VLAN_OPER_ts             *pVlanRule,
                                    ...
                                    omci_pon_wan_rule_t        *pRgAclEntry)
{
    //rtk_rg_cvlan_tag_action_t   *pEntryCtag;

    if (!pVlanRule || !pRgAclEntry)
        return OMCI_ERR_FAILED;

    memset(pRgAclEntry, 0, sizeof(omci_pon_wan_rule_t));
    pRgAclEntry->flowId = streamId;
    pRgAclEntry->wanVid = vid;
    pRgAclEntry->wanPri = pri;
            ...
            VLAN_ACT_TRANSPARENT == pVlanRule->cTagAct.vlanAct)
    {
        ...
        if (vid >= 0)
        {
            ...
            #else
            pRgAclEntry->ponVid = vid;
            //TBD, thinking pon pri
            #endif
        }
        else
        {
            //pEntryCtag->cvlanTagIfDecision = ACL_CVLAN_TAGIF_UNTAG;
            pRgAclEntry->ponVid = -1;
        }
    }
            ...
            VLAN_ACT_MODIFY == pVlanRule->cTagAct.vlanAct)
    {
        ...
        if (PRI_ACT_ASSIGN == pVlanRule->cTagAct.priAct)
        {
            ...
            // assign vid
            if (VID_ACT_ASSIGN == pVlanRule->cTagAct.vidAct)
                pRgAclEntry->ponVid = pVlanRule->cTagAct.assignVlan.vid;
            else
                pRgAclEntry->ponVid = vid;
    ...
    {
        OMCI_LOG(OMCI_LOG_LEVEL_DBG, "set action for us wan intf cf fail!");
                            ...
                            unsigned char       defaultCf)
{
    ...
    ret = omcidrv_setVeipUsWanIntfCfContent(pVlanRule,
                    vid, pri, netIfIdx, streamId, defaultCf, &rgAclEntry);
    if (OMCI_ERR_OK != ret)
    {
        OMCI_LOG(OMCI_LOG_LEVEL_DBG,
...
#else
    omci_pon_wan_rule_t    rgAclEntry;
    ...
    // invoke helper to set iptables rule
    // TBD, set iptables rules with netIfIdx and rgAclEntry
    ret = OMCI_ERR_OK;
    if (OMCI_ERR_OK != ret)
    {
        OMCI_LOG(OMCI_LOG_LEVEL_DBG,
...
#if 0
static void display_dm_entry(omci_dm_pon_wan_info_t a)
{
    unsigned int i;

    printk("wanIdx=%u, wanType=%u, gemPortId=%u, tcontId=%u\n",
            a.wanIdx, a.wanType, a.gemPortId, a.tcontId);

    for (i = 0; i < WAN_PONMAC_QUEUE_MAX; i++)
    {
        printk("index=%u, usFlowId=%u," \
                "qCfg.cir=%u, qCFg.pir=%u, qCfg.type=%u, qCfg.weight=%u, qCfg.egressDrop=%u, qSts=%u\n",
                i, a.usFlowId[i],
                a.queueCfg[i].cir, a.queueCfg[i].pir, a.queueCfg[i].type, a.queueCfg[i].weight, a.queueCfg[i].egrssDrop, a.queueSts[i]);
...
static int
omcidrv_setVeipRuleByDmm(veip_service_t *pVeipEntry,
                        int             wanIdx,
                        int             vid,
                        int             pri,
                        int             type,
                        ...
                        int             netIfIdx)
{
    ...
    {
        OMCI_LOG(OMCI_LOG_LEVEL_DBG, "dmm callback is not registered");
    ...
    if (OMCI_MODE_BRIDGE != type)
    {
        ...
        ret = omcidrv_setVeipUsWanIntfCfContent(&pVeipEntry->rule,
                vid, pri, netIfIdx, pVeipEntry->usStreamId, FALSE, &dmEntry.rgAclEntry);
        if (OMCI_ERR_OK != ret)
        {
            OMCI_LOG(OMCI_LOG_LEVEL_DBG,
        ...
        ret = omcidrv_setVeipUsWanIntfCfContent(&pVeipEntry->rule,
                vid, pri, netIfIdx, pVeipEntry->usStreamId, FALSE, &dmEntry.rgUsSwAclEntry);
        if (OMCI_ERR_OK != ret)
        {
            OMCI_LOG(OMCI_LOG_LEVEL_DBG,
            ...
            pVeipEntry->rule.outStyle.outVlan.vid <= 4095)
        {
                    ...
                    pVeipEntry->rule.outStyle.outVlan.vid <= 4095)
            {
                ...
                //    ((1 << gPlatformDb.cpuPort) | (1 << gPlatformDb.ponPort));
                //if (vid < 0)
                //    rgCvlan.untagPortMask.portmask = (1 << gPlatformDb.cpuPort);
    ...
    else
    {
        ...
        if (OMCI_ERR_OK !=
                _AssignNonUsedCfIndex(PF_CF_TYPE_L2_COMM, rule_pri, &pVeipEntry->pSwPathCfIdx[wanIdx]))
        {
            OMCI_LOG(OMCI_LOG_LEVEL_DBG, "allocate cf for b-wan sw path fail!");
        ...
        if (!isBinding)
        {
            ...
            if (1 == service)
            {
...
#if 0 //TBD, 9607c maybe remove
                // create default cf
                ret = omcidrv_setVeipUsWanIntfCfContent(&pVeipEntry->rule,
                        vid, pri, 0, pVeipEntry->usStreamId, TRUE, &dmEntry.rgAclEntry);
                if (OMCI_ERR_OK != ret)
                {
                    OMCI_LOG(OMCI_LOG_LEVEL_DBG,
                ...
                {
                    ret = omcidrv_setVeipUsWanIntfCfContent(&pVeipEntry->rule,
                            vid, pri, 0, pVeipEntry->usStreamId, FALSE, &dmEntry.rgUsSwAclEntry);
                    if (OMCI_ERR_OK != ret)
                    {
                        OMCI_LOG(OMCI_LOG_LEVEL_DBG,
        ...
        else
        {
            ...
            if (1 == service)
            {
                ...
                ret = omcidrv_setVeipUsWanIntfCfContent(&pVeipEntry->rule,
                        vid, pri, 0, pVeipEntry->usStreamId, TRUE, &dmEntry.rgUsSwAclEntry);
                if (OMCI_ERR_OK != ret)
                {
                    OMCI_LOG(OMCI_LOG_LEVEL_DBG,
                        "create cf content for b-wan default path fail, ret = %d", ret);
                ...
                pVeipEntry->rule.outStyle.outVlan.vid <= 4095)
        {
                    ...
                    pVeipEntry->rule.outStyle.outVlan.vid <= 4095)
            {
                ...
                //rgCvlan.vlanId = pVeipEntry->rule.outStyle.outVlan.vid;
                //rgCvlan.memberPortMask.portmask =
                //    (gPlatformDb.uniPortMask | (1 << gPlatformDb.ponPort));
                //if (vid < 0 ||
                    ...
                    int             netIfIdx)
{
    ...
    // TBD, temporarily workaround for not able to get the state at init
    ...
    if (OMCI_MODE_BRIDGE != type)
    {
            ...
            pVeipEntry->rule.outStyle.outVlan.vid <= 4095)
        {
                    ...
                    pVeipEntry->rule.outStyle.outVlan.vid <= 4095)
            {
                ...
                //    ((1 << gPlatformDb.cpuPort) | (1 << gPlatformDb.ponPort));
                //if (vid < 0)
                //    rgCvlan.untagPortMask.portmask = gPlatformDb.cpuPort;
        ...
        OMCI_VLAN_FILTER_ts *pFilter, rtk_rg_gpon_ds_bc_vlanfilterAndRemarking_t *pTagOp)
{
    ...
    {
        OMCI_LOG(OMCI_LOG_LEVEL_DBG, "filter pri is not supported\n");
    }
...
static int pf_rtl96xx_CreateTcont(OMCI_TCONT_ts *pTcont)
{
    ...
    if((ret = rtk_gpon_tcont_get(&ind,&attr))!=RT_ERR_OK)
    {
        if (RT_ERR_OK == rtk_gpon_tcont_get_physical(&ind, &attr))
        {
...
static int pf_rtl96xx_UpdateTcont(OMCI_TCONT_ts *pTcont)
{
    ...
    ind.alloc_id = pTcont->allocId;
    if (RT_ERR_OK != rtk_gpon_tcont_get(&ind, &attr))
        return OMCI_ERR_FAILED;
...
static int pf_rtl96xx_CfgGemFlow(OMCI_GEM_FLOW_ts *gemFlow)
{
    ...
    if(gemFlow->dir == PON_GEMPORT_DIRECTION_US || gemFlow->dir == PON_GEMPORT_DIRECTION_BI)
    {
        ...
        if (gemFlow->ena == TRUE)
        {
            ...
            /*create us gem flow*/
            ret = rtk_gpon_usFlow_set(gemFlow->flowId, &usFlow);
        ...
        else
        {
            ...
            ret = rtk_gpon_usFlow_set(flowId, &usFlow);
    ...
    if (gemFlow->dir == PON_GEMPORT_DIRECTION_DS || gemFlow->dir == PON_GEMPORT_DIRECTION_BI)
    {
        if (gemFlow->ena == TRUE)
        {
            ...
            ret = rtk_gpon_dsFlow_set(flowId, &dsFlow);
...
static int pf_rtl96xx_DeactiveBdgConn(int servId)
{
    ...
    //rtk_rg_initParam_get(&rgInitDb);
    //rtk_rg_svlanTpid_get(&systemTpid);
    ...
    if (pMBEntry)
    {
        ...
        if (pMBEntry->dsCfIndex <= gPlatformDb.veipFastStop)
        {
            ...
            {
                OMCI_LOG(OMCI_LOG_LEVEL_DBG,"del ds acl latch cf rule %x failed", pMBEntry->dsCfIndex);
                //return OMCI_ERR_FAILED;
            }
        }
        else
        ...
        if (OMCI_UNUSED_ACL != pMBEntry->referAclIdx)
        {
            ...
            {
                OMCI_LOG(OMCI_LOG_LEVEL_DBG,"del acl rule %x failed", pMBEntry->referAclIdx);
                //return OMCI_ERR_FAILED;
            }
            ...
            for(i = gPlatformDb.allPortMin; i <= gPlatformDb.allPortMax; i++){
                if(pL2Entry->pUsCfIndex[i] != OMCI_UNUSED_CF)
                {
                    ...
                    if(0x88a8 == systemTpid)
                        omci_delServicePort(pL2Entry->pUsCfIndex[i]);*/
...
static int pf_rtl96xx_SetL2Rule(OMCI_BRIDGE_RULE_ts *pBridgeRule)
{
    ...
    omci_rule_pri_t rule_pri;
    OMCI_BRIDGE_RULE_ts oriBridgeRule;
    ...
    /* Keep original BridgeRule for aggregated compare */
    memcpy(&oriBridgeRule, pBridgeRule, sizeof(OMCI_BRIDGE_RULE_ts));
...
static int pf_rtl96xx_SetSerialNum(char *serial)
{
    ...
    ret = rtk_gpon_serialNumber_set(&sn);
    return ret;
}
...
static int pf_rtl96xx_GetSerialNum(char *serial)
{
    ...
    ret = rtk_gpon_serialNumber_get(&sn);
...
static int pf_rtl96xx_SetGponPasswd(char *gponPwd)
{
    ...
    ret = rtk_gpon_password_set(&gpon_password);
    return ret;
}
...
static int pf_rtl96xx_ActivateGpon(int activate)
{
    ...
    if(activate)
    {
        ret = rtk_gpon_activate(RTK_GPONMAC_INIT_STATE_O1);
    }
    else
...
static int pf_rtl96xx_ResetMib(void)
{
    ...
    for(i=0;i<gPlatformDb.maxFlow;i++)
    {
        ...
        {
            ret = rtk_gpon_usFlow_set(i,&usAttr);
            ret = rtk_gpon_dsFlow_set(i,&dsAttr);
        }
    }

    #if 0 //TBD, sync linux kernel protocol stack
    ...
    for (i = 0; i < gPlatformDb.meterNum; i++)
    {
        ...
        // disable meters
        OMCI_DRV_ERR_CHK(OMCI_LOG_LEVEL_DBG,rtk_rate_shareMeter_set(gPlatformDb.stormCtrl[i].meterIdx, 0, DISABLED));
...
static int pf_rtl96xx_DumpMBServ(void)
{
    ...
    list_for_each_safe(next, tmp, &gPlatformDb.mbcastHead){
        ...
        printk("Application ID: %d, uniMask: %u, dsCf=%u \n", cur->index, cur->uniMask,  cur->dsCfIndex);
...
static int pf_rtl96xx_InitPlatform(void)
{
    ...
    /*initial chip*/
    if (OMCI_ERR_OK != rtk_gpon_initial(1))
        return OMCI_ERR_FAILED;
...
#if 0
static int pf_rtl96xx_GetPhysicalPort(OMCI_PORT_ts *pPort)
{
    int ret;
    ret = rtk_switch_phyPortId_get((rtk_switch_port_name_t)pPort->logical, &pPort->physical);
    if(ret != RT_ERR_OK)
    {
        printk ("%s: Failed to get Physical Port, ret 0x%x\n", __FUNCTION__, ret);
    }
    return 0;
}
#endif
...
static int pf_rtl96xx_SetDscpRemap(OMCI_DSCP2PBIT_ts *pDscp2PbitTable)
{
    ...
    {
        rtk_rg_qosDscpRemapToInternalPri_set(i, pDscp2PbitTable->pbit[i]);
    }

    /*Disable 1p remarking*/
    rtk_rg_qosDot1pPriRemarkByInternalPriEgressPortEnable_set(RTK_RG_MAC_PORT_PON, RTK_RG_DISABLED);
    /*Disable dscp remark, keep original DSCP*/
    rtk_rg_qosDscpRemarkEgressPortEnableAndSrcSelect_set(RTK_RG_MAC_PORT_PON, RTK_RG_DISABLED, RTK_RG_DSCP_RMK_SRC_INT_PRI);

    /*Set DSCP weight higher than port-based/dot1q in group 0*/
    rtk_rg_qosInternalPriDecisionByWeight_get(&weight);
    ...
    rtk_rg_qosInternalPriDecisionByWeight_set(weight);
...
static int pf_rtl96xx_GetDevCapabilities(omci_dev_capability_t *p)
{
   ...
   if( OMCIDRV_FEATURE_ERR_OK != omcidrv_feature_api(FEATURE_KAPI_ME_00100000, p))
    ...
    p->totalTContQueueNum = gPlatformDb.maxPonQueue - 1;
...
static int pf_rtl96xx_GetUsDBRuStatus(unsigned int *p)
{
    ...
    if (RT_ERR_OK != rtk_gpon_parameter_get(RTK_GPON_PARA_TYPE_US_DBR, &gponPara))
        return RT_ERR_FAILED;
...
static int pf_rtl96xx_GetUsFlowStat(omci_flow_stat_t *p)
{
    ...
    if (RT_ERR_OK != rtk_gpon_flowCounter_get(p->flow, RTK_GPON_PMTYPE_FLOW_US_GEM, &flowCntrs))
        return RT_ERR_FAILED;
...
static int pf_rtl96xx_GetDsFlowStat(omci_flow_stat_t *p)
{
    ...
    if (RT_ERR_OK != rtk_gpon_flowCounter_get(p->flow, RTK_GPON_PMTYPE_FLOW_DS_GEM, &flowCntrs))
        return RT_ERR_FAILED;
...
static int pf_rtl96xx_GetDsFecStat(omci_ds_fec_stat_t *p)
{
    ...
    if (RT_ERR_OK != rtk_gpon_globalCounter_get(RTK_GPON_PMTYPE_DS_PHY, &gponCntrs))
        return RT_ERR_FAILED;
...
{
    uint32 blockSize = 0;
    if (RT_ERR_OK != rtk_gpon_dbruBlockSize_get(&blockSize))
        return RT_ERR_FAILED;
...
{
    if (RT_ERR_OK != rtk_gpon_dbruBlockSize_set((uint32)gemBlkLen))
        return RT_ERR_FAILED;
...
static int pf_rtl96xx_GetDrvVersion(char *drvVersion)
{
    ...
    snprintf(drvVersion, 64, "%s for Flow Cache Driver", CONFIG_DRV_RELEASE_VERSION);
#endif
    return RT_ERR_OK;
}
...
static int pf_rtl96xx_SetDsBcGemFlow(unsigned int *pFlowId)
{
    ...
    if (*pFlowId < gPlatformDb.maxFlow &&
        ...
        enable = DISABLED;
#if 0 //TBD, sync linux kernel stack
...
{
    return  rtk_l2_aging_set(ageTime * 1000);
}
...
static pf_wrapper_t rtl96xx_mapper =
{
    ...
    .pf_SetDscpRemap                = NULL,//pf_rtl96xx_SetDscpRemap,
    ...
    .pf_SetMacLearnLimit            = NULL,//pf_rtl96xx_SetMacLearnLimit,
    ...
    //TBD, make sure rg and fc difference
    ...
    //TBD, sync linux kernel protocol stack
    .pf_SetSvlanTpid                = NULL,//pf_rtl96xx_SetSvlanTpid,
    .pf_GetSvlanTpid                = NULL,//pf_rtl96xx_GetSvlanTpid,
    ...
    //TBD, sync linux kernel protocol stack
    .pf_SetDot1RateLimiter          = NULL,//pf_rtl96xx_SetDot1RateLimiter,
    .pf_DelDot1RateLimiter          = NULL,//pf_rtl96xx_DelDot1RateLimiter,
