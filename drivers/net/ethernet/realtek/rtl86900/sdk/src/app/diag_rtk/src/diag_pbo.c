    ...
    uint32_t  *threshold_ptr)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_stopAllThreshold_set(*threshold_ptr), ret);
        }
        break;
#endif
        default:
    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_stopAllThreshold_get(&threshold), ret);
        }
        break;
#endif
        default:
    ...
    uint32_t  *sram_th_ptr)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usDscRunoutThreshold_set(*dram_th_ptr, *sram_th_ptr), ret);
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsDscRunoutThreshold_set(*dram_th_ptr, *sram_th_ptr), ret);
        }
        break;
#endif
        default:
    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usDscRunoutThreshold_get(&dramTh, &sramTh), ret);
            diag_util_mprintf("Upstrem Dsc run out threshold are DRAM: %u   SRAM: %u\n", dramTh, sramTh);	
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsDscRunoutThreshold_get(&dramTh, &sramTh), ret);
            diag_util_mprintf("Downstrem Dsc run out threshold are DRAM: %u   SRAM: %u\n", dramTh, sramTh);	
        }
        break;
#endif
    ...
    uint32_t  *threshold_ptr)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            if('f'==TOKEN_CHAR(4,1))
            {
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_globalThreshold_get(&onThresh, &offThresh), ret);
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_globalThreshold_set(onThresh, *threshold_ptr), ret);
                diag_util_printf("Set PBO Global Off Threshold to %u\n",*threshold_ptr);
            }
            else if('n'==TOKEN_CHAR(4,1))
            {
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_globalThreshold_get(&onThresh, &offThresh), ret);
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_globalThreshold_set(*threshold_ptr, offThresh), ret);
                diag_util_printf("Set PBO Global On Threshold to %u\n",*threshold_ptr);
            }
        }
        break;
    ...
    uint32_t  *threshold_ptr)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            if('f'==TOKEN_CHAR(5,1))
            {
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_sidThreshold_get(*sid_ptr, &onThresh, &offThresh), ret);
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_sidThreshold_set(*sid_ptr, onThresh, *threshold_ptr), ret);
                diag_util_printf("Set PBO SID Off Threshold to %u\n",*threshold_ptr);
            }
            else if('n'==TOKEN_CHAR(5,1))
            {
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_sidThreshold_get(*sid_ptr, &onThresh, &offThresh), ret);
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_sidThreshold_set(*sid_ptr, *threshold_ptr, offThresh), ret);
                diag_util_printf("Set PBO SID On Threshold to %u\n",*threshold_ptr);
            }
        }
        break;
...
#if defined(CONFIG_SDK_RTL9603D)
        case RTL9603D_CHIP_ID:
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usState_set(enable), ret);
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsState_set(enable), ret);
            break;
#endif
        default:
            diag_util_printf("%s\n", DIAG_STR_NOTSUPPORT);
...
#if defined(CONFIG_SDK_RTL9603D)
        case RTL9603D_CHIP_ID:
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usState_get(&enable), ret);
        	diag_util_printf("Upstream State %s\n",(enable == ENABLED) ? "enable" : "disable");
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsState_get(&enable), ret);
        	diag_util_printf("Downstream State %s\n",(enable == ENABLED) ? "enable" : "disable");
            break;
#endif
        default:
...
#if defined(CONFIG_SDK_RTL9603D)
        case RTL9603D_CHIP_ID:
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usStatus_get(&enable), ret);
        	diag_util_printf("Upstream status %s\n",(enable == ENABLED) ? "ready" : "not ready");
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsStatus_get(&enable), ret);
        	diag_util_printf("Downstream status %s\n",(enable == ENABLED) ? "ready" : "not ready");
            break;
#endif
        default:
    ...
    cparser_context_t *context)
{
    ...
    case RTL9603D_CHIP_ID:
    {
        ...
        DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usUsedPageCount_get(&sram, &dram), ret);
      	diag_util_printf("======== Upstream ========\n");
      	diag_util_printf("Current:\n");
        diag_util_printf("SRAM: %u       DRAM: %u\n", sram, dram);
        DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usMaxUsedPageCount_get(&sram, &dram), ret);
      	diag_util_printf("Maximum:\n");
        diag_util_printf("SRAM: %u       DRAM: %u\n", sram, dram);

        diag_util_printf("\n");

        DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsUsedPageCount_get(&sram, &dram), ret);
      	diag_util_printf("======= Downstream =======\n");
      	diag_util_printf("Current:\n");
        diag_util_printf("SRAM: %u       DRAM: %u\n", sram, dram);
        DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsMaxUsedPageCount_get(&sram, &dram), ret);
      	diag_util_printf("Maximum:\n");
        diag_util_printf("SRAM: %u       DRAM: %u\n", sram, dram);
        ...
        diag_util_printf("\n");
      	...
      	diag_util_printf("======= Per downstream queue =======\n");
      	diag_util_printf("Current:\n");
        DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsQUsedPageCount_get(RTL9603D_PBO_DSQUEUE_TYPE_NORMAL0, &normal0), ret);
        DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsQUsedPageCount_get(RTL9603D_PBO_DSQUEUE_TYPE_NORMAL1, &normal1), ret);
        DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsQUsedPageCount_get(RTL9603D_PBO_DSQUEUE_TYPE_HIGH, &high), ret);
        diag_util_printf("NORMAL0: %u    NORMAL1: %u    HIGH: %u\n", normal0, normal1, high);
      	diag_util_printf("Maximum:\n");
        DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsQMaxUsedPageCount_get(RTL9603D_PBO_DSQUEUE_TYPE_NORMAL0, &normal0), ret);
        DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsQMaxUsedPageCount_get(RTL9603D_PBO_DSQUEUE_TYPE_NORMAL1, &normal1), ret);
        DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsQMaxUsedPageCount_get(RTL9603D_PBO_DSQUEUE_TYPE_HIGH, &high), ret);
        diag_util_printf("NORMAL0: %u    NORMAL1: %u    HIGH: %u\n", normal0, normal1, high);
        break;
    }
#endif
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            ...
            for(sid = 0; sid < HAL_CLASSIFY_SID_NUM(); sid ++)
            {
                ...
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_flowcontrolStatus_get(sid, RTL9603D_PBO_FLOWCONTROL_STATUS_CURRENT, &enable), ret);
          	    diag_util_printf("%3c ", enable == ENABLED ? 'O' : 'X');
                if((sid % 16) == 15)
                {
               	    diag_util_printf("\n");
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            ...
            for(sid = 0; sid < HAL_CLASSIFY_SID_NUM(); sid ++)
            {
                ...
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_flowcontrolStatus_get(sid, RTL9603D_PBO_FLOWCONTROL_STATUS_LATCHED, &enable), ret);
          	    diag_util_printf("%3c ", enable == ENABLED ? 'O' : 'X');
                if((sid % 16) == 15)
                {
               	    diag_util_printf("\n");
...
#if defined(CONFIG_SDK_RTL9603D)
        case RTL9603D_CHIP_ID:
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usMaxUsedPageCount_clear(), ret);
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsMaxUsedPageCount_clear(), ret);
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsQMaxUsedPageCount_clear(), ret);
            break;
#endif
        default:
            diag_util_printf("%s\n", DIAG_STR_NOTSUPPORT);
    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            ...
            for(sid = 0;sid < HAL_CLASSIFY_SID_NUM();sid ++)
            {
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_flowcontrolLatchStatus_clear(sid), ret);
            }
            break;
        }
#endif
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            ...
            DIAG_UTIL_MASK_SCAN(groupList, group)
            {
                ...
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usGroupCounter_clear(group), ret);
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsGroupCounter_clear(group), ret);
            }
        }
        break;
#endif
    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        {
            DIAG_UTIL_OUTPUT_INIT();
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usCounter_clear(), ret);
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsCounter_clear(), ret);
        }
        break;
#endif
        default:
    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        {
            DIAG_UTIL_OUTPUT_INIT();
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usPonnicCounter_clear(), ret);
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsPonnicCounter_clear(), ret);
        }
        break;
#endif
        default:
    ...
    char * *sid_ptr)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usCounterGroupMember_clear(*grp_ptr), ret);
            DIAG_UTIL_MASK_SCAN(sidList, sid)
            {
                if(sid >= HAL_CLASSIFY_SID_NUM())
                {
                ...
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usCounterGroupMember_add(*grp_ptr, sid), ret);
            }

            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsCounterGroupMember_clear(*grp_ptr), ret);
            DIAG_UTIL_MASK_SCAN(sidList, sid)
            {
                if(sid >= HAL_CLASSIFY_SID_NUM())
                {
                ...
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsCounterGroupMember_add(*grp_ptr, sid), ret);
            }
        }
        break;
#endif
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
            ...
            for(j = 0;j < HAL_CLASSIFY_SID_NUM();j ++)
            {
                ...
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usCounterGroupMember_get(*grp_ptr, j, &enable), ret);
          	    diag_util_printf("%3c ", enable == ENABLED ? 'V' : ' ');
                if((j % 16) == 15)
                {
               	    diag_util_printf("\n");
            ...
            for(j = 0;j < HAL_CLASSIFY_SID_NUM();j ++)
            {
                ...
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsCounterGroupMember_get(*grp_ptr, j, &enable), ret);
          	    diag_util_printf("%3c ", enable == ENABLED ? 'V' : ' ');
                if((j % 16) == 15)
                {
               	    diag_util_printf("\n");
    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            rtl9603d_pbo_cnt_t pbo_cnt;
                
            DIAG_UTIL_OUTPUT_INIT();

            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usCounter_get(&pbo_cnt), ret);
          	diag_util_printf("======== Upstream ========\n");
          	diag_util_printf("RX_ERR_CNT: %u\n", pbo_cnt.rx_err_cnt);
          	diag_util_printf("TX_EMPTY_EOB: %u\n", pbo_cnt.tx_empty_eob_cnt);

            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsCounter_get(&pbo_cnt), ret);
          	diag_util_printf("======= Downstream ========\n");
          	diag_util_printf("RX_ERR_CNT: %u\n", pbo_cnt.rx_err_cnt);
          	diag_util_printf("RX_DROP_CNT: %u\n", pbo_cnt.rx_drop_cnt);
            break;
    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            rtl9603d_pbo_ponnic_cnt_t pbo_cnt;
                
            DIAG_UTIL_OUTPUT_INIT();

            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usPonnicCounter_get(&pbo_cnt), ret);
          	diag_util_printf("======== Upstream ========\n");
          	diag_util_printf("Tx PKT_OK_CNT: %u\n", pbo_cnt.pkt_ok_cnt_tx);
          	diag_util_printf("Rx PKT_OK_CNT: %u\n", pbo_cnt.pkt_ok_cnt_rx);
          	diag_util_printf("Tx PKT_ERR_CNT: %u\n", pbo_cnt.pkt_err_cnt_tx);
            ...
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsPonnicCounter_get(&pbo_cnt), ret);
          	diag_util_printf("======= Downstream ========\n");
          	diag_util_printf("Tx PKT_OK_CNT: %u\n", pbo_cnt.pkt_ok_cnt_tx);
          	diag_util_printf("Rx PKT_OK_CNT: %u\n", pbo_cnt.pkt_ok_cnt_rx);
          	diag_util_printf("Tx PKT_ERR_CNT: %u\n", pbo_cnt.pkt_err_cnt_tx);
    ...
    char * *group_ptr)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            rtl9603d_pbo_group_cnt_t group_cnt;
            ...
            DIAG_UTIL_MASK_SCAN(groupList, group)
            {
                ...
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usGroupCounter_get(group, &group_cnt), ret);
            	diag_util_printf("Group %u:\n", group);
                diag_util_printf("   RX_SID_GOOD_CNT:%u\n", group_cnt.rx_good_cnt);
                diag_util_printf("   RX_SID_BAD_CNT:%u\n", group_cnt.rx_bad_cnt);
                diag_util_printf("   RX_SID_CNT:%u\n", group_cnt.rx_cnt);
            ...
            DIAG_UTIL_MASK_SCAN(groupList, group)
            {
                ...
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsGroupCounter_get(group, &group_cnt), ret);
            	diag_util_printf("Group %u:\n", group);
                diag_util_printf("   RX_SID_GOOD_CNT:%u\n", group_cnt.rx_good_cnt);
                diag_util_printf("   RX_SID_BAD_CNT:%u\n", group_cnt.rx_bad_cnt);
                diag_util_printf("   RX_SID_FRAG_CNT:%u\n", group_cnt.rx_frag_cnt);
    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
        	...
        	diag_util_printf("US Memory Config\n");
		    DIAG_UTIL_ERR_CHK(reg_read(RTL9603D_IP_MSTBASE_USr, &value), ret);
        	diag_util_printf("\tBase Addr: 0x%08x\n", value);
		    DIAG_UTIL_ERR_CHK(reg_field_read(RTL9603D_PON_DSC_CFG_USr, RTL9603D_CFG_PAGE_SIZEf, &value), ret);
        	diag_util_printf("\tPage Size: %u\n", value == 0 ? 128 : (value == 1 ? 256 : 512));
		    DIAG_UTIL_ERR_CHK(reg_field_read(RTL9603D_PON_DSC_CFG_USr, RTL9603D_CFG_SRAM_NOf, &value), ret);
        	diag_util_printf("\tSRAM DSC Cnt: %u\n", value + 1);
		    DIAG_UTIL_ERR_CHK(reg_field_read(RTL9603D_PON_DSC_CFG_USr, RTL9603D_CFG_RAM_NOf, &value), ret);
        	diag_util_printf("\tDRAM DSC Cnt: %u\n", value + 1);

        	diag_util_printf("\n");

        	diag_util_printf("DS Memory Config\n");
		    DIAG_UTIL_ERR_CHK(reg_read(RTL9603D_IP_MSTBASE_DSr, &value), ret);
        	diag_util_printf("\tBase Addr: 0x%08x\n", value);
		    DIAG_UTIL_ERR_CHK(reg_field_read(RTL9603D_PON_DSC_CFG_DSr, RTL9603D_CFG_PAGE_SIZEf, &value), ret);
        	diag_util_printf("\tPage Size: %u\n", value == 0 ? 128 : (value == 1 ? 256 : 512));
		    DIAG_UTIL_ERR_CHK(reg_field_read(RTL9603D_PON_DSC_CFG_DSr, RTL9603D_CFG_SRAM_NOf, &value), ret);
        	diag_util_printf("\tSRAM DSC Cnt: %u\n", value + 1);
		    DIAG_UTIL_ERR_CHK(reg_field_read(RTL9603D_PON_DSC_CFG_DSr, RTL9603D_CFG_RAM_NOf, &value), ret);
        	diag_util_printf("\tDRAM DSC Cnt: %u\n", value + 1);
			
        	diag_util_printf("\n");

        	diag_util_printf("SW Memory Config\n");
		    DIAG_UTIL_ERR_CHK(reg_read(RTL9603D_SW_PBO_MSTBASEr, &value), ret);
        	diag_util_printf("\tBase Addr: 0x%08x\n", value);
		    DIAG_UTIL_ERR_CHK(reg_field_read(RTL9603D_SW_PBO_DSCCTRLr, RTL9603D_CFG_PAGE_SIZEf, &value), ret);
        	diag_util_printf("\tPage Size: %u\n", value == 0 ? 128 : (value == 1 ? 256 : (value == 2 ? 512 : 1024)));
        	diag_util_printf("\tSRAM DSC Cnt: %u\n", value == 0 ? 16 : (value == 1 ? 8 : (value == 2 ? 4 : 2)));
		    DIAG_UTIL_ERR_CHK(reg_field_read(RTL9603D_SW_PBO_DSCCTRLr, RTL9603D_CFG_PAGE_NUMBERf, &value), ret);
    ...
    char * *sid_ptr)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            rtl9603d_pbo_dsQueue_type_t qType;
            DIAG_UTIL_ERR_CHK(DIAG_UTIL_EXTRACT_MASK128(sidList, 5), ret);
            if(!strcmp("normal0", TOKEN_STR(7)))
            {
                qType = RTL9603D_PBO_DSQUEUE_TYPE_NORMAL0;
            }
            else if(!strcmp("normal1", TOKEN_STR(7)))
            {
                qType = RTL9603D_PBO_DSQUEUE_TYPE_NORMAL1;
            }
            else
            {
                qType = RTL9603D_PBO_DSQUEUE_TYPE_HIGH;
            }
            ...
            DIAG_UTIL_MASK_SCAN(sidList, sid)
            {
                ...
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsSidToQueueMap_set(sid, qType), ret);
            }
        }
        break;
#endif
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            ...
            DIAG_UTIL_MASK_SCAN(sidList, sid)
            {
                ...
                qType = 0;
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsSidToQueueMap_get(sid, &qType), ret);
                diag_util_printf("%3d %s\n", sid, diagStr_dsQueueType[qType]);
            }
        }
        break;
    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            rtl9603d_pbo_dsQueueQos_type_t qosType;

            DIAG_UTIL_PARAM_CHK();

            if('r' == TOKEN_CHAR(4,0))
            {
                qosType = RTL9603D_PBO_DSQUEUEQOS_TYPE_ROUNDROBIN;
            }
            else
            {
                qosType = RTL9603D_PBO_DSQUEUEQOS_TYPE_STRICTPRIORITY;
            }

            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsQueueQos_set(qosType), ret);
        }
        break;
#endif
        default:
    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            ...
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsQueueQos_get(&qosType), ret);
            diag_util_printf("DS queue scheduling: ");
            if(RTL9603D_PBO_DSQUEUEQOS_TYPE_ROUNDROBIN == qosType)
            {
                diag_util_printf("round-robin\n");
            }
            else
    ...
    uint32_t  *threshold_ptr)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            ...
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsFcIgnoreThreshold_get(&stopTh, &startTh), ret);
            if('a' == TOKEN_CHAR(4,2))
            {
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsFcIgnoreThreshold_set(stopTh, *threshold_ptr), ret);
            }
            else
            {
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsFcIgnoreThreshold_set(*threshold_ptr, startTh), ret);
            }
        }
        break;
#endif
    ...
    cparser_context_t *context)
{
    ...
    switch(DIAG_UTIL_CHIP_TYPE)
    {
        ...
        case RTL9603D_CHIP_ID:
        {
            ...
            /* Reset PBO */
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usState_set(DISABLED), ret);
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsState_set(DISABLED), ret);

            /* Release reset PBO */
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_usState_set(ENABLED), ret);
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_pbo_dsState_set(ENABLED), ret);
    ...
    uint32_t  *dram_th_ptr)
{
    ...
    case RTL9603D_CHIP_ID:
        DIAG_UTIL_PORTMASK_SCAN(portlist, port)
        {
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_swPbo_dscRunoutThreshold_set(port, *sram_th_ptr, *dram_th_ptr), ret);
        }
        break;
#endif
    default:
    ...
    char * *ports_ptr)
{
    ...
    case RTL9603D_CHIP_ID:
        ...
        DIAG_UTIL_PORTMASK_SCAN(portlist, port)
        {
            DIAG_UTIL_ERR_CHK(rtl9603d_raw_swPbo_dscRunoutThreshold_get(port, &sramTh, &dramTh), ret);
            diag_util_mprintf("%4u %4u %4u\n", port, sramTh, dramTh);
        }
        diag_util_printf("---- ---- ----\n");
        break;
