 ...
 */
int epon_oam_maxOamPduSize_get(
    ...
    unsigned short *pExtractLen)
{
	...
	CTC_BUF_PARSE16(pParsePtr, &pOamPdu->flag);
	pParsePtr += 2;
#else
    ...
    unsigned short *pExtractLen)
{
    ...
    pParsePtr += 1;
#ifdef SUPPORT_OAM_BYTEORDER
	CTC_BUF_PARSE16(pParsePtr, &pOamInfo->revision);
	pParsePtr += 2;
#else
    ...
    pParsePtr += 1;
#ifdef SUPPORT_OAM_BYTEORDER
	CTC_BUF_PARSE16(pParsePtr, &pOamInfo->oamPduConfig);
	pParsePtr += 2;
#else
...
#ifdef SUPPORT_OAM_BYTEORDER
		CTC_BUF_PARSE16(pProcPtr, &seqNumber);
		pProcPtr += 2;
#else
    ...
    unsigned short *pReplyLen)
{
    ...
    *ppReplyOamPdu = pReplyPtr;
    ...
    while(remainLen >= EPON_VAR_OAMPDU_DESC_LEN)
    {
        ...
        pProcPtr ++;
#ifdef SUPPORT_OAM_BYTEORDER
		CTC_BUF_PARSE16(pProcPtr, &varCb.varDesc.varLeaf);
#else
...
static int epon_oam_stdLoopback_process(
    ...
    unsigned short *pReplyLen)
{
	if (0x1 == pOamPdu->pData[0])//Enable
		CTC_WRAPPER(ponLoopbackSet, ENABLED);
	else if (0x2 == pOamPdu->pData[0])//Disable
		CTC_WRAPPER(ponLoopbackSet, DISABLED);
    ...
    unsigned short *pReplyLen)
{
    ...
    *ppReplyOamPdu = pReplyPtr;
    ...
    replyLen = 0;
    if((NULL != orgSpecCb.processor) && (orgSpecCb.state))
    {
    ...
    unsigned short length)
{
	...
	#ifndef YUEME_CUSTOMIZED_CHANGE
        ret = epon_oam_stdLoopback_process(&oamPdu, &pReplyOamPdu, &replyLen);
	#else
		ret = EPON_OAM_ERR_OK;  /* ignore oam loopback command */
	#endif
        break;
        ...
        else if(ret > 0)
        {
            ...
            if(4 == dataLen)
            {
                ...
                if((payload[0] == 0x15) && (payload[1] == 0x68))
                {
					...
					else if ((payload[2] == 0x55) && (payload[3] == 0x55))
					{
						/* Exit LOS message */
						epon_oam_event_send(0, EPON_OAM_EVENT_LOS_RECOVER);
					}
					else if((payload[2] == 0x66) && (payload[3] == 0x66))
					{
						/* Open ctc silent mode message*/
						epon_ctc_open_silent_mode();
					}
                }
            }
        }
