 ...
 *
 * $Revision: 88131 $
 * $Date: 2018-05-18 19:07:20 +0800 (Fri, 18 May 2018) $
 *
    ...
    char * *ports_ptr)
{
    ...
    diag_util_mprintf("Port 10000F 1000F 100F 100H 10F 10H FlowControl AsyFC\n");
    diag_util_mprintf("---- ------ ----- ---- ---- --- --- ----------- -----\n");

    DIAG_UTIL_PORTMASK_SCAN(portlist, port)
    {
        DIAG_UTIL_ERR_CHK(rtk_port_phyAutoNegoAbility_get(port, &ability), ret);
        diag_util_mprintf("%-4d %-6s %-5s %-4s %-4s %-3s %-3s %-11s %-5s\n",
                            port,
    ...
    DIAG_UTIL_PORTMASK_SCAN(portlist, port)
    {
        switch(DIAG_UTIL_CHIP_TYPE)
        {
            ...
            case RTL9607C_CHIP_ID:
                ...
                DIAG_UTIL_ERR_CHK(diag_util_lPortMask2str(portStr, &portmask), ret);
                diag_util_printf("\n port %d force DMP: %s State: %s", port, portStr, diagStr_enable[enable]);
                break;
#endif
#if defined(CONFIG_SDK_RTL9603D)
            case RTL9603D_CHIP_ID:
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_port_forceDmpMask_get(port, &portmask), ret);
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_port_forceDmp_get(port, &enable), ret);
                DIAG_UTIL_ERR_CHK(diag_util_lPortMask2str(portStr, &portmask), ret);
                diag_util_printf("\n port %d force DMP: %s State: %s", port, portStr, diagStr_enable[enable]);
                break;
#endif
...
#if defined(CONFIG_SDK_RTL9603D)
            case RTL9603D_CHIP_ID:
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_port_forceDmpMask_set(port, dmp_portlist.portmask), ret);
                break;
#endif
...
#if defined(CONFIG_SDK_RTL9603D)
            case RTL9603D_CHIP_ID:
                DIAG_UTIL_ERR_CHK(rtl9603d_raw_port_forceDmp_set(port, enable), ret);
                break;
#endif
 ...
 */
cparser_result_t
cparser_cmd_port_set_auto_nego_port_ports_all_ability_10h_10f_100h_100f_1000f_10000f_flow_control_asy_flow_control(
    cparser_context_t *context,
    char * *ports_ptr)
{
    int32 ret;
    ...
    return CPARSER_OK;
}    /* end of cparser_cmd_port_set_auto_nego_port_ports_all_ability_10h_10f_100h_100f_1000f_10000f_flow_control_asy_flow_control */
			...
			case RTL9607C_CHIP_ID:
				...
				break;
                        ...
                        case RTL9603D_CHIP_ID:
                                if(!osal_strcmp(TOKEN_STR(6),"speed"))
                                {
                                        DIAG_UTIL_ERR_CHK((reg_array_field_write(RTL9603D_ABLTY_FORCE_MODEr, port, REG_ARRAY_INDEX_NONE, RTL9603D_FORCE_SPEED1_ABLTYf, &enable)), ret);
                                        DIAG_UTIL_ERR_CHK((reg_array_field_write(RTL9603D_ABLTY_FORCE_MODEr, port, REG_ARRAY_INDEX_NONE, RTL9603D_FORCE_SPEED0_ABLTYf, &enable)), ret);
                                }
                                else if(!osal_strcmp(TOKEN_STR(6),"duplex"))
                                {
                                        DIAG_UTIL_ERR_CHK((reg_array_field_write(RTL9603D_ABLTY_FORCE_MODEr, port, REG_ARRAY_INDEX_NONE, RTL9603D_FORCE_DUPLEX_ABLTYf, &enable)), ret);
                                }
                                else if(!osal_strcmp(TOKEN_STR(6),"tx-pause"))
                                {
                                        DIAG_UTIL_ERR_CHK((reg_array_field_write(RTL9603D_ABLTY_FORCE_MODEr, port, REG_ARRAY_INDEX_NONE, RTL9603D_FORCE_TXPAUSE_ABLTYf, &enable)), ret);
                                }
                                else if(!osal_strcmp(TOKEN_STR(6),"rx-pause"))
                                {
                                        DIAG_UTIL_ERR_CHK((reg_array_field_write(RTL9603D_ABLTY_FORCE_MODEr, port, REG_ARRAY_INDEX_NONE, RTL9603D_FORCE_RXPAUSE_ABLTYf, &enable)), ret);
                                }
                                else if(!osal_strcmp(TOKEN_STR(6),"nway"))
                                {
                                        DIAG_UTIL_ERR_CHK((reg_array_field_write(RTL9603D_ABLTY_FORCE_MODEr, port, REG_ARRAY_INDEX_NONE, RTL9603D_FORCE_NWAY_ABLTYf, &enable)), ret);
                                }
                                else if(!osal_strcmp(TOKEN_STR(6),"link-state"))
                                {
                                        DIAG_UTIL_ERR_CHK((reg_array_field_write(RTL9603D_ABLTY_FORCE_MODEr, port, REG_ARRAY_INDEX_NONE, RTL9603D_FORCE_LINK_ABLTYf, &enable)), ret);
                                }
                                else if(!osal_strcmp(TOKEN_STR(6),"lpi-100M"))
                                {
                                        DIAG_UTIL_ERR_CHK((reg_array_field_write(RTL9603D_ABLTY_FORCE_MODEr, port, REG_ARRAY_INDEX_NONE, RTL9603D_FORCE_LPI_100_ABLTYf, &enable)), ret);
                                }
                                else if(!osal_strcmp(TOKEN_STR(6),"lpi-giga"))
                                {
                                        DIAG_UTIL_ERR_CHK((reg_array_field_write(RTL9603D_ABLTY_FORCE_MODEr, port, REG_ARRAY_INDEX_NONE, RTL9603D_FORCE_LPI_1000_ABLTYf, &enable)), ret);
                                }
                                break;
#endif
...
#if defined(CONFIG_SDK_RTL9603D)
            case RTL9603D_CHIP_ID:
                DIAG_UTIL_ERR_CHK(reg_array_field_read(RTL9603D_SC_P_CTRL_0r, port, REG_ARRAY_INDEX_NONE, RTL9603D_CGST_TMRf, &cgstTimer), ret);
                break;
#endif
            default:
                diag_util_printf("%s\n", DIAG_STR_NOTSUPPORT);
 ...
 */
cparser_result_t
cparser_cmd_port_set_serdes_port_ports_all_fiber_1g_sgmii_mac_hsgmii_mac_n_way_force_auto(
    cparser_context_t *context,
    char * *ports_ptr)
{
    ...
    if(serdPortCnt > 0)
    {
        ...
        if(!osal_strcmp(TOKEN_STR(5),"sgmii-mac"))
        {
            mode=LAN_SDS_MODE_SGMII_MAC;
        }
        if(!osal_strcmp(TOKEN_STR(5),"hsgmii-mac"))
        {
...
{
    int32 serdPortCnt = HAL_GET_SERDES_PORTNUM(); 
    int32 ret = RT_ERR_FAILED, serdIdx = 0;
    rtk_port_t port = 0;
    diag_portlist_t  portlist;
    uint8 mode = LAN_SDS_MODE_FIBER_1G,nway = LAN_SDS_NWAY_AUTO;
    rtk_portmask_t check_portmask;
    ...
    if(serdPortCnt > 0)
    {
        ...
        DIAG_UTIL_PORTMASK_SCAN(portlist, port)
        {
                if(HAL_IS_SERDES_PORT(port))
                {
                    DIAG_UTIL_ERR_CHK(rtk_port_serdesMode_get(serdIdx, &mode), ret);
                    DIAG_UTIL_ERR_CHK(rtk_port_serdesNWay_get(serdIdx, &nway), ret);
