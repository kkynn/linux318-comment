    ...
    while(1)
    {
        ...
        switch(pmsg.mtype)
        {
            ...
            case MQ_RX_PACKET:
                ...
                {
                    if((pkt_ptr->pkt[PTP_MTYPE_offset+sizeof(ptp_pkt_cputag_t)]&0xf)==PTP_Delay_Req)
                    {
                        /*delay_req*/
                        ...
                        if(pkt_ptr->pkt_len < (DELAY_REQ_LEN+sizeof(ptp_pkt_cputag_t)))
                        {
                        ...
                        memcpy(&delay_resp_packet[PTP_DMAIN_offset],&(pkt_ptr->pkt[PTP_DMAIN_offset+sizeof(ptp_pkt_cputag_t)]),1);
                        memcpy(&delay_resp_packet[PTP_SQID_offset],&(pkt_ptr->pkt[PTP_SQID_offset+sizeof(ptp_pkt_cputag_t)]),2);
                        memcpy(&delay_resp_packet[PTP_CFNS_offset],&(pkt_ptr->pkt[PTP_CFNS_offset+sizeof(ptp_pkt_cputag_t)]),8);
                        ...
                        memcpy(&delay_resp_packet[PTP_REQCLOCKID_offset],&(pkt_ptr->pkt[PTP_CLOCKID_offset+sizeof(ptp_pkt_cputag_t)]),10);
                    ...
                    if(ptp_peer==ENABLED)
                    {
                        if((pkt_ptr->pkt[PTP_MTYPE_offset+sizeof(ptp_pkt_cputag_t)]&0xf)==PTP_Pdelay_Req)
                        {
                            if(ptp_cur_mode == PTP_MODE_ONE_STEP)
                            {
                                ...
                                if(pkt_ptr->pkt_len < (PDELAY_REQ_LEN+sizeof(ptp_pkt_cputag_t)))
                                {
                                    DBG_PRINT(1,"[PTP Master:%s:%d] Not supporting packet len=%d!\n", __FUNCTION__, __LINE__,pkt_ptr->pkt_len);
                                    for(i=0;i<pkt_ptr->pkt_len && i<PTP_MAX_PKT_SIZE;i++)
                                    {
                                ...
                                memcpy(&pdelay_resp_packet[PTP_DMAIN_offset],&(pkt_ptr->pkt[PTP_DMAIN_offset+sizeof(ptp_pkt_cputag_t)]),1);
                                memcpy(&pdelay_resp_packet[PTP_SQID_offset],&(pkt_ptr->pkt[PTP_SQID_offset+sizeof(ptp_pkt_cputag_t)]),2);
                                memcpy(&pdelay_resp_packet[PTP_CFNS_offset],&(pkt_ptr->pkt[PTP_CFNS_offset+sizeof(ptp_pkt_cputag_t)]),8);
                                
                                memcpy(&pdelay_resp_packet[PTP_REQCLOCKID_offset],&(pkt_ptr->pkt[PTP_CLOCKID_offset+sizeof(ptp_pkt_cputag_t)]),10);                                
