...
/*__DRAM*/ unsigned int debug_enable=RTL8686_RX_WARN;
/*__DRAM*/ //unsigned int padding_enable=0;
...
//20141114LUKE: disable for protocol stack should carry ctag itself.
	...
	if( !padding_enable && pTxInfo->opts1.bit.cputag_psel != 1)
		pTxInfo->opts3.bit.tx_dst_stream_id = 0x7f;
	#endif
}
...
static void re8670_rx (struct re_private *cp)
{
	...
	*/
	...
	unsigned char higherq[MAX_RXRING_NUM] = {0x06,0x04,0x0,0x0,0x0,0x0};
	#else
	unsigned char pri_map[MAX_RXRING_NUM] = {5,4,3,2,1,0};
	unsigned char higherq[MAX_RXRING_NUM] = {0x3e,0x3c,0x38,0x30,0x20,0x00};
	#endif
	int i = 0;
	...
	#ifdef CONFIG_YUEME
	// we need kick watchdog when receive packet
	REG32(0xB8003260) |= (1<<31);
	#endif
#if RX_NOT_ONLY_RING1
	...
	for(i=0;rx_ring_bitmap_current && i<MAX_RXRING_NUM;i++){
		...
		while (rx_work--)
		{
			...
			if (higherq[ring_num] && (retrive_isr1_status()&higherq[ring_num])) {
...
rx_to_software:
			...
			// Software flow control for Jumbo Frame RX issue. 
			{
				...
				else
				{
					...
					{
						RX_WARN("NIC Full2 tail=%d cdo=%d\n",rx_Mtail,cdo);
						dev_kfree_skb_any(skb);
						goto rx_next;				
					}
				}			
...
static inline void updateTxStatic(struct sk_buff *skb){
	if (skb->dev && DEVPRIV(skb->dev)) {
                DEVPRIV(skb->dev)->net_stats.tx_packets++;
                DEVPRIV(skb->dev)->net_stats.tx_bytes += skb->len;
        }else {
                printk("%s(%d): skb dev %p %s \n", __func__,__LINE__,skb->dev, skb->dev ? skb->dev->name : "none");
                dump_stack();
        }
}
...
static int port_to_rxfunc_write(struct file *file, const char __user *buff, size_t len, loff_t *ppos)
{
	char 	tmpbuf[512];
	char		*strptr;	
...
//	dev->features |= NETIF_F_SG |  NETIF_F_GSO; //: this configuration will crash in skb_segment
	...
	strcpy(LCDev_mapping[5].ifname, "eth0");
        LCDev_mapping[5].phy_dev = eth_net_dev;
	#else
