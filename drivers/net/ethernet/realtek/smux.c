...
//printk("%s %d \n", __func__, __LINE__);
		...
		list_for_each(lh, &grp->virtual_devs)
		{
			...
			if (((skb->vlan_tci&VLAN_VID_MASK) && !(vdev->priv_flags&IFF_VSMUX)) ||
				((vdev->priv_flags&IFF_VSMUX) && 
				 (!(skb->vlan_tci&VLAN_VID_MASK) || ((((skb->vlan_tci&VLAN_VID_MASK)-1)!=dev_info->vid) 
#if defined(CONFIG_YUEME)
					&& (((skb->vlan_tci&VLAN_VID_MASK)-1)!=dev_info->mvid)
#endif
				 ))))
	  			continue;
#if 0 
...
int smux_dev_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	//to bypass fwdengin: store assigned flow id to skb qos mark. and flow id will be retrieved in fwdengine
	if (skb->mark & BYPASS_FWDENGINE_MARK_MASK)
	{
		int qid = (skb->mark & QOS_SWQID_MASK)>>QOS_SWQID_OFFSET;
		if(qid==0)
			qid=8;
		skb->mark = ((skb->mark)&(~FLOWID_MARK_OFFSET))|
				(((dev_info->gpon_flow_id[qid-1])<<FLOWID_MARK_OFFSET)&FLOWID_MARK_MASK);
	}
#endif
...
#endif
#if defined(CONFIG_GPON_FEATURE) && (defined(CONFIG_FON_GRE) || defined(CONFIG_XFRM))
			if(parg->args.setflag & 2)
			{
				//for gpon, unnumber pkts processed by protocol stack must assign gpon flow id
				//so smux gpon_flow_id store useable flow id, and priority: gpon_flow_id[0]>...>gpon_flow_id[7]
				int i;
				for(i=0; i<8; i++)
				{
					if(parg->args.gponusflowid[i]==-1 && i!=0)
						sdev->gpon_flow_id[i]=sdev->gpon_flow_id[i-1];
					else
						sdev->gpon_flow_id[i] = parg->args.gponusflowid[i];
					//printk(" %d %d\n",i,sdev->gpon_flow_id[i]);
				}
			}
#endif
...
static int smux_ioctl_handler(void __user *arg)
{
	...
	switch (pargs->cmd) {
		...
		case IGN_SMUX_CMD:
			if(pargs->vid == 0){
			...
			}else if((pargs->vid == 1) || (pargs->vid == 2)){
				/* 1: add ignore list */
				/* 2: del ignore list */
				if(!list_empty(&SMUX_IGNORE_HEAD)){
					list_for_each_entry_safe(pEntry, next, &SMUX_IGNORE_HEAD, smuxIgnoreList){
						if(pEntry->proto == pargs->proto){
							if(pargs->vid == 2){
				...
				if(pargs->vid == 1){
					...
					if(pargs->proto <= 0xFFFF){
						pEntry->proto = pargs->proto;
