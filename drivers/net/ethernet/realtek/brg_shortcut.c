...
static int fastbridge_read_proc(struct seq_file *seq, void *v)
{
	int i;
	char srcportbuf[16];
	char dstportbuf[16];
...
static int fastbridge_write_proc(struct file *file, const char __user *buffer, size_t len, loff_t *ppos)
{
	unsigned char flag;
...
static int fastbridge_open(struct inode *inode, struct file *file)
{
        return single_open(file, fastbridge_read_proc, inode->i_private);
}
static const struct file_operations fastbridge_fops = {
        .owner          = THIS_MODULE,
        .open           = fastbridge_open,
        .read           = seq_read,
        .write          = fastbridge_write_proc,
        .llseek         = seq_lseek,
...
static int __init fastbridge_init(void) 
{
	...
	proc_create_data("fastbridge", 0, NULL,&fastbridge_fops, NULL);
