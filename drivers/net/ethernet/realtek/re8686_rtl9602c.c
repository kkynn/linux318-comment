...
#define DRV_NAME		"RTL9600"
#define DRV_VERSION		"1.0.0"
#define DRV_RELDATE		"Oct 17, 2015"
...
#define NIC_DESC_ACCELERATE_FOR_SG //: many pages which have the continuious address are using the same descriptor, for accelerating.

#if	GMAC_MODE==MODE_PURE_SW
#undef HW_CHECKSUM_OFFLOAD //: for hardware checksum offload.
#undef LINUX_LSO_ENABLE //: enable new version kernel lso(sw & hw LSO are required)
#undef LINUX_SG_ENABLE //: enable kernel SG.
#undef HW_LSO_ENABLE //: enable hw lso (new or old kernel are required)
...
#ifdef CONFIG_DUALBAND_CONCURRENT
unsigned int iocmd1_reg=CMD1_CONFIG | (RX_NOT_ONLY_RING1<<25) | TX_RR_scheduler | 0x3f<<16  | txq1_h ; // set RR scheduler and tx ring 1 high queue
#else
unsigned int iocmd1_reg=CMD1_CONFIG | (RX_NOT_ONLY_RING1<<25) | RX_MULTIRING_BITMAP << 16 | txq1_h; // set TX ring 1 to high queue
#endif

unsigned int debug_enable=0;
unsigned int padding_enable=0;
...
unsigned int txJumboFrameEnabled = 1;
	...
	if(pMsgData->intrType == MSG_TYPE_LINK_CHANGE) // Link Change Interrupt
	{
		if(pMsgData->intrBitMask < CPU_PORT && pMsgData->intrBitMask >= 0)
		{
			if(pMsgData->intrSubType == 1 && LCDev_mapping[pMsgData->intrBitMask].status == 0) // Link Up
			{
				...
				LCDev_mapping[pMsgData->intrBitMask].linkchangetimes++;
			}
...
static int change_tx_jumbo_setup(int enabled)
{
	if(enabled)
	{
		RTL_W32(TCR, RTL_R32(TCR)|(1<<15));
		RTL_W32(IO_CMD, RTL_R32(IO_CMD)|(1<<28));
		RTL_W32(CONFIG_REG, RTL_R32(CONFIG_REG)|(1<<16));
	}
	else
	{
		RTL_W32(TCR, RTL_R32(TCR)&~(1<<15));
...
static int re8686_set_mtu(struct net_device *dev, int new_mtu)
{
	...
	dev->mtu = new_mtu;
	txJumboFrameEnabled = (new_mtu > SKB_BUF_SIZE) ? 1 : 0;
	change_tx_jumbo_setup((new_mtu > SKB_BUF_SIZE) ? 1 : 0);
...
#define RX_WARN( comment ,arg...) \
do {\
		if(debug_enable&RTL8686_RX_WARN)\
		{\
			int mt_trace_i;\
			sprintf( mt_watch_tmp, comment,## arg);\
			for(mt_trace_i=0;mt_trace_i<512;mt_trace_i++) \
...
#define TX_WARN( comment ,arg...) \
do {\
		if(debug_enable&RTL8686_TX_WARN)\
		{\
			int mt_trace_i;\
			sprintf( mt_watch_tmp, comment,## arg);\
			for(mt_trace_i=0;mt_trace_i<512;mt_trace_i++) \
...
int re8670_host_speedup (struct re_private *cp, struct sk_buff *skb, struct rx_info *pRxInfo)
{
	...
	int isPPPoE=0;
	unsigned char *pL2Hdr, *pPPPoEHdr=NULL, *pL3Hdr;
    ...
    if (isHostSpeedUpEnable())
    {
            ...
            (*(unsigned short *)pData == 0x0800))
        {
            ...
            if ((pHostTrack = shouldSpeedUp(0, iph->daddr, iph->saddr, tcph->dest, tcph->source)) != NULL)
            {
            	...
            	ret = speedtest_stream_shortcut_check(pHostTrack, iph->daddr, iph->saddr, tcph->dest, tcph->source, &seqNo, &ip_id, rcvBytes);
		...
		while (rx_work--)
		{
			...
			if(rxInfo.opts1.bit.fs==1)	//FS=1
			{
				...
				else //FS=1, LS=0
				{
					...
					if(re_private_data.jumboLength>0)		//flush jumbo skb
					{
						if(re_private_data.jumboFrame) 
							dev_kfree_skb_any(re_private_data.jumboFrame);
						re_private_data.jumboLength=0;
						re_private_data.jumboFrame=NULL;
					}
...
//			printk("wake\n");
		...
		data=kmalloc(65535,GFP_KERNEL); //: don't free this memory manually, this memory is free in skbuff.c:skb_release_data()		
		if(data==NULL) printk("alloc failed\n");
...
__IRAM_NIC void re8670_tx (struct re_private *cp,int ring_num)
{
	...
	while (!((status = (cp->tx_Mhqring[ring_num][tx_tail].opts1))& DescOwn)) {
		...
		while((u32)skb==0xffffffff) //skb is last_frag in mfrag_skb.
		{
			...
			tx_tail = NEXT_TX(tx_tail,RE8670_TX_MRING_SIZE[ring_num]); //: this skb is many frags skb, just free once.
			skb = cp->tx_skb[ring_num][tx_tail].skb;
			//printk("skb=%x tx_tail=%d\n",(u32)skb,tx_tail);			
...
void re8670_mFrag_xmit (struct sk_buff *skb, struct re_private *cp, unsigned *entry,int ring_num)
{
	...
	//first_mapping = (u32)skb->data|UNCACHE_MASK; //:disable uncache access
	first_mapping = (u32)skb->data;
#ifdef CONFIG_REALTEK_HW_LSO			
	// : skb and desc is one-by-one mapping in old nic driver, but in new nic driver, many descs are using the same skb.
	...
	*entry = NEXT_TX(*entry,RE8670_TX_MRING_SIZE[ring_num]);
	...
	for (frag = 0; frag < skb_shinfo(skb)->nr_frags; frag++) {
		...
		len+=this_frag->size;
		//first_data_mapping |= UNCACHE_MASK; //: disable uncache
		mapping=first_data_mapping;
#else
		len = this_frag->size;
		//mapping = (u32)page_address(this_frag->page)+(u32)this_frag->page_offset|UNCACHE_MASK;
		mapping = (u32)page_address(this_frag->page.p)+(u32)this_frag->page_offset; //: disable uncache
#endif
...
//wmb();
	...
	if(skb->len-14>CP_LS_MTU)
	{
		...
		*(u32 *)((u32)&txd->opts1|0xa0000000) = ctrl; //: own must write at last.
#endif
		...
		if(frag == skb_shinfo(skb)->nr_frags-1)
			...
			cp->tx_skb[ring_num][*entry].skb = (struct sk_buff *)0xffffffff;  // : make sure the same skb is not free many times.
...
//	CP_VLAN_TX_TAG(txd, vlan_tag);
	...
	*(u32 *)((u32)&txd->opts1|0xa0000000) =temp_opt1; //: own must write at last.		
#endif		
...
static inline void kick_tx(int ring_num){
	...
	switch(ring_num){
		...
		case 2:
		case 3:
			RTL_W32(IO_CMD, RTL_R32(IO_CMD)|(1 << ring_num));
			break;
		case 4:
			RTL_W32(IO_CMD1, RTL_R32(IO_CMD1)|TX_POLL5);
			break;
		default:
...
__IRAM_NIC int do_icmpreplay_check(struct sk_buff *skb)
{
	u16 off=12;
	if((*(u16*)(skb->data+off))==htons(0x8100))//CTAG
		off+=4;
	...
	if(((*(u16*)(skb->data+off))==htons(0x0800))||((*(u16*)(skb->data+off))==htons(0x0021)))//IPv4 or IPv4 with PPPoE
	{
		if(skb->data[off+11]  == 0x1 && skb->data[off+22] == 0x0) // ICMP
		{
                        int idx = 0;
                        int ipLen = ((skb->data[off+2]&0xf)<<2);
                        int l4Len = ((skb->data[off+4] << 16) + skb->data[off+5])-ipLen;
                        // if header and payload are all zero , set checksum to 0xffff directly.(special case)
                        off+=ipLen+2; // ipv4 header + packet type (0x0800 or 0x0021)
                        if(*(u16*)(skb->data+off)!=0x0)return 0;        //icmp type and code
                        for(idx = 4 ; idx<l4Len; idx++)
                                if(skb->data[off+idx] != 0x0)
                                        return 0;
                        skb->data[off+2] = skb->data[off+3] = 0xff;
                        return 1;
		}
		else
			return 0;
...
int re8686_send_with_txInfo_and_mask(struct sk_buff *skb, struct tx_info* ptxInfo, int ring_num, struct tx_info* ptxInfoMask)
{
	...
	if (skb_shinfo(skb)->nr_frags == 0) {
		...
		if(do_icmpreplay_check(skb) == 1) 
		{
			ptxInfo->opts1.dw &= ~(L4CS);
			if(len > 0)
				dma_cache_wback_inv((unsigned long)skb->data, len);
		}
		cp->tx_skb[ring_num][entry].skb = skb;
...
*	ptxInfo: pointer of tx_info(not to be used)
*	reg_num: 0 to set VLAN_REG, otherwise illegal reg_num value
*	value: the value to set VLAN_REG
*/
__IRAM_NIC
...
{	
	...
	{
		case 0:
			re_private_data.stag_pid = value;
			RTL_W32(VLAN_REG, value);
			break;
		case 1:
		default:
...
*	ptxInfo: pointer of tx_info(not to be used)
*	reg_num: 0 to get VLAN_REG, otherwise illegal reg_num value
*	value_p: the pointer to get content of VLAN_REG
*/
__IRAM_NIC
...
#endif
{
	...
	if (skb_shinfo(skb)->nr_frags == 0) {
		...
		//[step1] opts1 is in local, set init value
...
static void multi_rtx_ring_init(struct re_private *cp)
{
	...
	for(i=0;i<MAX_TXRING_NUM;i++)
	{
		...
		#endif
		RTL_W32(TxFDP1+(ADDR_OFFSET*i),CPHYSADDR(cp->tx_Mhqring[idx_sw2hw(i)]));
...
//#endif
	...
	//cpu tag function
	cputag_info = (CTEN_RX | 2<<CT_RSIZE_L | 2<<CT_TSIZE | CT_APPLO_FE | CTPM_8370 | CTPV_8370);
	RTL_W32(CPUtagCR,cputag_info); /* Turn on the cpu tag adding function */  //czyao 8672c
	RTL_W32(CPUtag1CR, (VDSL_PORT << 4) | (PON_PORT) | (CT1_SID));/*spa_dsl no need now, spa_pon is 3[from switch team]*/
...
static int re8670_alloc_rings (struct re_private *cp)
{
	...
	{    
		...
		if(j == 0)
			...
			pBuf = kzalloc(RE8670_RXRING_BYTES(RE8670_RX_MRING_SIZE[j]), GFP_KERNEL);
#else
		pBuf = kzalloc(RE8670_RXRING_BYTES(RE8670_RX_MRING_SIZE[j]), GFP_KERNEL);
#endif
		if (!pBuf)
	...
	for(j=0;j<MAX_TXRING_NUM;j++)
	{
		...
		pBuf = s_begin + 0x1000;
		#else
		pBuf = kzalloc(RE8670_TXRING_BYTES(RE8670_TX_MRING_SIZE[j]), GFP_KERNEL);
		#endif
		if (!pBuf)
			goto ErrMem;
...
static int re8670_open (struct net_device *dev)
{
	...
	if(dev_num == 0){
		...
		if (rc)
			goto err_out_hw;
		RTL_R16(VLAN_REG) = re_private_data.stag_pid;
	}
...
static int re8670_close (struct net_device *dev)
{
	...
	if(dev_num == 0){
		...
		#endif
		printk("%s %d\n", __func__, __LINE__);
		re_private_data.stag_pid = RTL_R16(VLAN_REG);
	}
...
static int dbg_level_read(struct seq_file *seq, void *v)
{
	seq_printf(seq, "[debug_enable = 0x%08x]\n", debug_enable);
	seq_printf(seq, "RTL8686_PRINT_NOTHING\t0x%08x\n", RTL8686_PRINT_NOTHING);
	seq_printf(seq, "RTL8686_SKB_RX\t\t0x%08x\n", RTL8686_SKB_RX);
	seq_printf(seq, "RTL8686_SKB_TX\t\t0x%08x\n", RTL8686_SKB_TX);
	seq_printf(seq, "RTL8686_RXINFO\t\t0x%08x\n", RTL8686_RXINFO);
	seq_printf(seq, "RTL8686_TXINFO\t\t0x%08x\n", RTL8686_TXINFO);
	seq_printf(seq, "RTL8686_RX_TRACE\t\t0x%08x\n", RTL8686_RX_TRACE);
	seq_printf(seq, "RTL8686_TX_TRACE\t\t0x%08x\n", RTL8686_TX_TRACE);
	seq_printf(seq, "RTL8686_RX_WARN\t\t0x%08x\n", RTL8686_RX_WARN);
	seq_printf(seq, "RTL8686_TX_WARN\t\t0x%08x\n", RTL8686_TX_WARN);

	return 0;
}
static int dbg_level_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
...
static int memrw_read(struct seq_file *seq, void *v)
{
	return 0;
}
static int memrw_write(struct file *file, const char __user *buff, size_t len, loff_t *ppos)
{
	char 		tmpbuf[64];
...
{	  
	  switch(SWITCH_MODE)
	  {
		case RTL8686_Switch_Mode_Trap2Cpu:
			seq_printf(seq, "Asic switch mode : trap2cpu\n");
			break;
		case RTL8686_Switch_Mode_Normal:
			seq_printf(seq, "Asic switch mode : normal\n");
			break;
		default:
			seq_printf(seq, "Asic switch mode : Unknown\n");
	  }      
      return 0;
}
...
static int switch_mode_write(struct file *file, const char __user *buff, size_t len, loff_t *ppos)
{
	char 	tmpbuf[512];
	char		*strptr;	
	int retval = -1;
...
static int misc_read(struct seq_file *seq, void *v)
{
	printk("iocmd_reg 0x%x\n", iocmd_reg);
	printk("iocmd1_reg 0x%x\n", iocmd1_reg);
	printk("TX jumbo frame: %s\n", txJumboFrameEnabled?"Enabled":"Disabled");
	return 0;
}

static int misc_write(struct file *file, const char __user *buff, size_t len, loff_t *ppos)
{
	char 	tmpbuf[64];
	char	*strptr, *var;
	char	*tokptr;
	...
	unsigned int* pVar[VAR_NUM] = {&iocmd_reg, &iocmd1_reg, &txJumboFrameEnabled, NULL};
...
static int dev_port_mapping_read(struct seq_file *seq, void *v)
{
	unsigned int i;
	...
	{
		seq_printf(seq,"Port%d => ifname:%s , dev:%s(0x%p) , status:%d , changes:%d\n" , 
			i , LCDev_mapping[i].ifname , LCDev_mapping[i].phy_dev->name, LCDev_mapping[i].phy_dev, LCDev_mapping[i].status, LCDev_mapping[i].linkchangetimes);
	}
#endif
	return 0;
}

static int dev_port_mapping_write(struct file *file, const char __user *buff, size_t len, loff_t *ppos)
{
	char 		tmpbuf[64];
	char		*strptr;
	...
	buf_size = min(len, (sizeof(tmpbuf)-1));
	if (buff && !copy_from_user(tmpbuf, buff, buf_size)) {
		tmpbuf[len] = '\0';
		strptr=tmpbuf;
		tokptr = strsep(&strptr," ");
		if (tokptr==NULL)
...
static int port_to_rxfunc_read(struct seq_file *seq, void *v)
{
...
static int hw_reg_read(struct seq_file *seq, void *v)
{
	int len = 0;
	int i;
...
static int sw_cnt_write(struct file *file, const char __user *buff, size_t len, loff_t *ppos)
{
	...
	if (buff && !copy_from_user(tmpbuf, buff, len))
	{
		...
		{
			re_private_data.cp_stats.rx_hw_num=0;
			re_private_data.cp_stats.rx_sw_num=0;
			re_private_data.cp_stats.rer_runt=0;
			re_private_data.cp_stats.rer_ovf=0;
			re_private_data.cp_stats.rdu=0;
			re_private_data.cp_stats.frag=0;
			re_private_data.cp_stats.toobig=0;
			re_private_data.cp_stats.crcerr=0;
			re_private_data.cp_stats.rcdf=0;
			re_private_data.cp_stats.rx_no_mem=0;
			re_private_data.cp_stats.tx_sw_num=0;
			re_private_data.cp_stats.tx_hw_num=0;
			re_private_data.cp_stats.tx_no_desc=0;
...
static int rx_ring_write(struct file *file, const char __user *buff, size_t len, loff_t *ppos)
{
	char 	tmpbuf[64];
	char	*strptr;
	char	*tokptr;
...
static int tx_ring_write(struct file *file, const char __user *buff, size_t len, loff_t *ppos)
{
	char 	tmpbuf[64];
	char	*strptr;
	char	*tokptr;
...
static int nic_tx_ring_dump_read(struct seq_file *seq, void *v)
{
	nic_tx_ring_dump();
	return 0;
}

static int nic_tx_ring_dump_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	nic_tx_ring_dump();
	return 0;
}
...
static int nic_rx_ring_dump_read(struct seq_file *seq, void *v)
{
	nic_rx_ring_dump();
	return 0;
}

static int nic_rx_ring_dump_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	nic_rx_ring_dump();
	return 0;
}

static int padding_enable_read(struct seq_file *seq, void *v)
{
	int len=0;

	//printk("[padding_enable = %d]\n", padding_enable);
	switch (padding_enable) {
	case 0:
		seq_printf(seq, "0, (padding is disable)\n");
		break;
	case 1:
		seq_printf(seq, "1, (padding is enable)");
		break;
	default:
		seq_printf(seq, "Error, (padding_enable is invalid)\n");
	}

	return len;
}

static int padding_enable_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	unsigned char tmpBuf[16] = {0};
	int i=0;

	if (buffer && count==2 && !copy_from_user(tmpBuf, buffer, count))
	{
		tmpBuf[count] = '\0';
		...
		case 0:
			padding_enable=0;
			REG32(0xB8012040) |= 0x1;
			printk("write 0 to disable padding\n");
			break;
		case 1:
			padding_enable=1;
			REG32(0xB8012040) &= (~0x1);
			printk("write 1 to enable padding\n");
			break;
		default:
...
static int dbglv_open(struct inode *inode, struct file *file)
{
        return single_open(file, dbg_level_read, inode->i_private);
}
...
{
        return single_open(file, hw_reg_read, inode->i_private);
}
...
{
        return single_open(file, sw_cnt_read, inode->i_private);
}
...
{
        return single_open(file, rx_ring_read, inode->i_private);
}
...
{
        return single_open(file, tx_ring_read, inode->i_private);
}
...
{
        return single_open(file, memrw_read, inode->i_private);
}
...
static int devport_open(struct inode *inode, struct file *file)
{
        return single_open(file, dev_port_mapping_read, inode->i_private);
}
...
{
        return single_open(file, misc_read, inode->i_private);
}
...
static int port2rx_open(struct inode *inode, struct file *file)
{
        return single_open(file, port_to_rxfunc_read, inode->i_private);
}
...
{
        return single_open(file, nic_tx_ring_dump_read, inode->i_private);
}
...
{
        return single_open(file, nic_rx_ring_dump_read, inode->i_private);
}
...
{
        return single_open(file, padding_enable_read, inode->i_private);
}
...
static void rtl8686_proc_debug_init(void){
	...
	if(rtl8686_proc_dir)
	{
		proc_create_data("dbg_level", 0, rtl8686_proc_dir,&dbglv_fops, &re_private_data);
		proc_create_data("hw_reg", 0, rtl8686_proc_dir,&hwreg_fops, &re_private_data);
		proc_create_data("sw_cnt", 0, rtl8686_proc_dir,&swcnt_fops, &re_private_data);
		proc_create_data("rx_ring", 0, rtl8686_proc_dir,&rxring_fops, &re_private_data);
		proc_create_data("tx_ring", 0, rtl8686_proc_dir,&txring_fops, &re_private_data);
		proc_create_data("mem", 0, rtl8686_proc_dir,&mem_fops, &re_private_data);
		proc_create_data("dev_port_mapping", 0, rtl8686_proc_dir,&devport_fops, &re_private_data);
		proc_create_data("misc", 0, rtl8686_proc_dir,&misc_fops, &re_private_data);
		proc_create_data("port2rxfunc", 0, rtl8686_proc_dir,&port2rx_fops, &re_private_data);
		#ifdef CONFIG_RTL8686_SWITCH
		#ifndef CONFIG_RTK_L34_ENABLE
		proc_create_data("switch_mode", 0, rtl8686_proc_dir,&swmode_fops, &re_private_data);
		#endif
		#endif
		#ifdef RE8686_VERIFY
		proc_create_data("verify", 0, rtl8686_proc_dir,&verify_fops, &re_private_data);
		#endif
		proc_create_data("rx_ring_detail", 0, rtl8686_proc_dir,&rxdetail_fops, &re_private_data);
		proc_create_data("tx_ring_detail", 0, rtl8686_proc_dir,&txdetail_fops, &re_private_data);
		proc_create_data("padding_enable", 0, rtl8686_proc_dir,&padding_fops, &re_private_data);
	}
...
//	dev->features |= NETIF_F_SG |  NETIF_F_GSO; //: this configuration will crash in skb_segment
	...
	change_tx_jumbo_setup(txJumboFrameEnabled);
	...
	// handle port 0 (eth0.2)
	memcpy(&LCDev_mapping[rtl8686_dev_table[1].phyPort].ifname,dev->name , strlen(dev->name));
	LCDev_mapping[rtl8686_dev_table[1].phyPort].phy_dev = dev;
	// handle port 1 (eth0.3)
	memcpy(&LCDev_mapping[rtl8686_dev_table[2].phyPort].ifname,dev->name , strlen(dev->name));
	LCDev_mapping[rtl8686_dev_table[2].phyPort].phy_dev = dev;
	// handle port 2 (nas0, pon port)
	memcpy(&LCDev_mapping[rtl8686_dev_table[3].phyPort].ifname,dev->name , strlen(dev->name));
	LCDev_mapping[rtl8686_dev_table[3].phyPort].phy_dev = dev;
