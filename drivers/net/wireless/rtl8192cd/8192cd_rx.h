    ...
    )
{
    ...
    phw->rx_infoL[i].pbuf   = (void *)pskb;
			...
			phw->rx_infoL[i].map_len = (RX_BUF_LEN - sizeof(struct rx_frinfo) - offset);
		#ifdef DEBUG_DMA_MAP
			#ifdef CONFIG_DUAL_CPU_SLAVE
				phw->rx_infoL[i].paddr	= get_physical_addr_1(priv, pskb->data, phw->rx_infoL[i].map_len, PCI_DMA_FROMDEVICE,RX_MAP_TYPE) + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL;
			#else
			    phw->rx_infoL[i].paddr  = get_physical_addr_1(priv, pskb->data, phw->rx_infoL[i].map_len, PCI_DMA_FROMDEVICE,RX_MAP_TYPE);
			#endif
		#else/*DEBUG_DMA_MAP*/
			#ifdef CONFIG_DUAL_CPU_SLAVE
				phw->rx_infoL[i].paddr	= get_physical_addr(priv, pskb->data, phw->rx_infoL[i].map_len, PCI_DMA_FROMDEVICE) + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL;
			#else
			    phw->rx_infoL[i].paddr  = get_physical_addr(priv, pskb->data, phw->rx_infoL[i].map_len, PCI_DMA_FROMDEVICE);
			#endif
		#endif /*DEBUG_DMA_MAP*/
	...
	#ifdef DEBUG_DMA_MAP
		#ifdef CONFIG_DUAL_CPU_SLAVE
			phw->rx_infoL[i].paddr	  = get_physical_addr_1(priv , pskb->data, (RX_BUF_LEN - sizeof(struct rx_frinfo) - offset), PCI_DMA_FROMDEVICE,RX_MAP_TYPE ) + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL;
		#else
		    phw->rx_infoL[i].paddr  = get_physical_addr_1(priv , pskb->data, (RX_BUF_LEN - sizeof(struct rx_frinfo) - offset), PCI_DMA_FROMDEVICE,RX_MAP_TYPE);
		#endif
	#else/*DEBUG_DMA_MAP*/
		#ifdef CONFIG_DUAL_CPU_SLAVE
...
static __inline__ void init_rxdesc(struct sk_buff *pskb, int i, struct rtl8192cd_priv *priv)
{
	...
	phw->rx_infoL[i].pbuf  = (void *)pskb;
#ifdef DEBUG_DMA_MAP
	phw->rx_infoL[i].paddr = get_physical_addr_1(priv, pskb->data, (RX_BUF_LEN - sizeof(struct rx_frinfo) - 64), PCI_DMA_FROMDEVICE,RX_MAP_TYPE);
#else
	...
	if (pfrinfo->physt) {
...

			{
				if (pfrinfo->rf_info.mimorssi[0] != 0 || pfrinfo->rf_info.mimorssi[1] != 0) {
					...
					// skip the bogus SNR values (63, 63, 63, 63)
					if (is_CCK_rate(pfrinfo->rx_rate) || (pfrinfo->rf_info.RxSNRdB[0] == 63 && pfrinfo->rf_info.RxSNRdB[1] == 63)) {
						memcpy(&pstat->rf_info.mimosq[0], &pfrinfo->rf_info.mimosq[0], 8);
					} else {
						memcpy(&pstat->rf_info.mimosq[0], &pfrinfo->rf_info.mimosq[0], 12);
					}
				}
			}
