...
#ifdef RTK_ATM_EXT
#define SIOC92DSETSTAATMSTATE		0x8BFC
#endif
#endif
...
#ifdef RTK_ATM_EXT
	{ SIOC92DSETSTAATMSTATE, IW_PRIV_TYPE_CHAR | 128, 0, "set_sta_atmstate" },
#endif
#endif
#if defined(CONFIG_OFFLOAD_FUNCTION) || defined(SDIO_AP_OFFLOAD)
...
static struct iwpriv_arg mib_table[] = {
	...
	{"curAmsduNum",	INT_T,		_OFFSET(dot11nConfigEntry.dot11nCurAMSDUSendNum), _SIZE(dot11nConfigEntry.dot11nCurAMSDUSendNum), 0},
...
//PRINT_SINGL_ARG("    dgaf_disable:  ",priv->dgaf_disable, "%d");
	...
	{"mc2u_match_ip",	RFFT_T,	_OFFSET_RFFT(mc2u_match_ip), _SIZE_RFFT(mc2u_match_ip), 0},
	...
	{"acs_not60_inc",		RFFT_T,	_OFFSET_RFFT(acs_not60_inc), _SIZE_RFFT(acs_not60_inc), 5000},
#endif
#ifdef EN_EFUSE
	...
	{"vomaxagg",	RFFT_T, _OFFSET_RFFT(vomaxagg), _SIZE_RFFT(vomaxagg), 0},
	{"vimaxagg",	RFFT_T, _OFFSET_RFFT(vimaxagg), _SIZE_RFFT(vimaxagg), 0},
	{"bemaxagg",	RFFT_T, _OFFSET_RFFT(bemaxagg), _SIZE_RFFT(bemaxagg), 0},
	{"bkmaxagg",	RFFT_T, _OFFSET_RFFT(bkmaxagg), _SIZE_RFFT(bkmaxagg), 0},
...
//	{"antSw_dump",			RFFT_T, _OFFSET_RFFT(antSw_dump), _SIZE_RFFT(antSw_dump), 0},
	...
	{"pri_sum_safe_th",			RFFT_T,	_OFFSET_RFFT(pri_sum_safe_th), _SIZE_RFFT(pri_sum_safe_th), 40},
	...
	{"pri_sum_g1_th",			RFFT_T,	_OFFSET_RFFT(pri_sum_g1_th), _SIZE_RFFT(pri_sum_g1_th), 2},
	...
	{"swqdbg2",        RFFT_T, _OFFSET_RFFT(swq_dbg2), _SIZE_RFFT(swq_dbg2), 2},
    ...
    {"swqedcathd",     RFFT_T, _OFFSET_RFFT(swq_edca_thd), _SIZE_RFFT(swq_edca_thd), 2133},
#else
	{"swq_max_len",		RFFT_T, _OFFSET_RFFT(swq_max_enqueu_len), _SIZE_RFFT(swq_max_enqueu_len), 8192},
    {"swqedcathd",     RFFT_T, _OFFSET_RFFT(swq_edca_thd), _SIZE_RFFT(swq_edca_thd), 6827},
#endif
#endif
...
#ifdef RTK_ATM_EXT
	{"atm_11abg_ratio",	RFFT_T, _OFFSET_RFFT(atm_11abg_ratio), _SIZE_RFFT(atm_11abg_ratio), 100},
	{"atm_11n_ratio",	RFFT_T, _OFFSET_RFFT(atm_11n_ratio), _SIZE_RFFT(atm_11n_ratio), 100},
	{"atm_11ac_ratio",	RFFT_T, _OFFSET_RFFT(atm_11ac_ratio), _SIZE_RFFT(atm_11ac_ratio), 100},

	{"atm_distr_type",	RFFT_T, _OFFSET_RFFT(atm_distribution_type), _SIZE_RFFT(atm_distribution_type), 0},//dynamic:0, static:1 
	{"atm_wait_th",		RFFT_T, _OFFSET_RFFT(atm_wait_threshold), _SIZE_RFFT(atm_wait_threshold), 10}, 
	{"atm_ts_enable",	RFFT_T, _OFFSET_RFFT(atm_ts_enable), _SIZE_RFFT(atm_ts_enable), 0},//time slice
	{"atm_ts_plus",		RFFT_T, _OFFSET_RFFT(atm_ts_plus), _SIZE_RFFT(atm_ts_plus), 0},//time slice, unit : ms
	{"atm_ts_ratio",	RFFT_T, _OFFSET_RFFT(atm_ts_ratio), _SIZE_RFFT(atm_ts_ratio), 100},//time slice
#endif
#endif
	...
	{"brcm_amsdu_pkt_th",    RFFT_T, _OFFSET_RFFT(brcm_amsdu_pkt_th), _SIZE_RFFT(brcm_amsdu_pkt_th), 40000},
    ...
    {"stactrl_param_3",      INT_T,    _OFFSET(staControl.stactrl_param_3),       _SIZE(staControl.stactrl_param_3), 600},  //timerX
    {"stactrl_param_4",      INT_T,    _OFFSET(staControl.stactrl_param_4),       _SIZE(staControl.stactrl_param_4), 10}, //retry Y
#else
...
#endif
#ifdef TXPWR_LMT
#if defined(CONFIG_SDIO_HCI) || defined(CONFIG_RTL8672)
...
// Tx Power Diff
	...
	{"mu_monitor", RFFT_T, _OFFSET_RFFT(mu_monitor), _SIZE_RFFT(mu_monitor), 0},
#endif		
	...
	{"symbol_det_dm",	RFFT_T,	_OFFSET_RFFT(symbol_det_dm), _SIZE_RFFT(symbol_det_dm), 1},
	...
	{"ldpc_92e", RFFT_T, _OFFSET_RFFT(ldpc_92e), _SIZE_RFFT(ldpc_92e), 3},	
#endif
...
//	{"mp_dig_enable",	RFFT_T, _OFFSET_RFFT(mp_dig_enable), _OFFSET_RFFT(mp_dig_enable), 0},
	...
	{"odmdebugcom",	ODM_DM_4UT,	_OFFSET_ODM_DM(DebugComponents), _SIZE_ODM_DM(DebugComponents), 0},// test for ODM DM (long long)
...
#ifdef CONFIG_RTL8672
	{"EDCCA_enable",	ODM_DM_1UT,	_OFFSET_ODM_DM(EDCCA_enable), _SIZE_ODM_DM(EDCCA_enable), 0},// for odm_Adaptivity()
#else
...
static int get_atm_info(struct rtl8192cd_priv *ppriv)
{
...
#ifdef RTK_ATM_EXT	
	panic_printk("	atm_sta_num_active: %d\n",ppriv->atm_sta_num_active);	
	panic_printk("	atm_sta_num_pause: %d\n",ppriv->atm_sta_num_pause);	
	panic_printk("	atm_ttl_stanum_pause: %d\n",priv->pshare->atm_ttl_stanum_pause);	
	panic_printk("	atm_ttl_match_stanum_pause: %d, atm_ttl_match_statime_pause: %d\n",
		priv->pshare->atm_ttl_match_stanum_pause, priv->pshare->atm_ttl_match_statime_pause);	
	panic_printk("	atm_11abg_ratio: %d, atm_11n_ratio: %d, atm_11ac_ratio: %d\n", priv->pshare->rf_ft_var.atm_11abg_ratio, priv->pshare->rf_ft_var.atm_11n_ratio, priv->pshare->rf_ft_var.atm_11ac_ratio);
	panic_printk("	atm_ts_enable=%d, atm_ts_plus=%d ms, atm_ts_ratio=%d\n", priv->pshare->rf_ft_var.atm_ts_enable, priv->pshare->rf_ft_var.atm_ts_ratio);
#endif
	panic_printk("\n\n");
	...
	for(i=0; i<NUM_STAT; i++) {
		if (priv->pshare->aidarray[i] && (priv->pshare->aidarray[i]->used == TRUE)){
			...
			panic_printk("	atm_adj_factor: %d \n",pstat->atm_adj_factor);
#ifdef RTK_ATM_EXT
			panic_printk("	atm_burst_num_adj_ratio: %d \n",pstat->atm_burst_num_adj_ratio);
			panic_printk("	tx time from assoc: %d ms\n",pstat->atm_tx_time_from_assoc);
#endif
			panic_printk("	atm_is_maxsta: %d \n",pstat->atm_is_maxsta);
			panic_printk("	is matched sta:%d\n", (pstat->atm_match_sta_time>0)?1:0); 			
#ifdef RTK_ATM_EXT
			priv_t = priv->pshare->aidarray[i]->priv;
			if (priv_t) {
				memcpy(tmpbuf, priv_t->pmib->dot11StationConfigEntry.dot11DesiredSSID, priv_t->pmib->dot11StationConfigEntry.dot11DesiredSSIDLen);
				tmpbuf[priv_t->pmib->dot11StationConfigEntry.dot11DesiredSSIDLen] = '\0';
				panic_printk("	ssid connected to: %s (%s: %d%%)\n", tmpbuf, priv_t->dev->name, priv_t->atm_time_per_ssid);
			}

			panic_printk("	atm_tx_len_byte: %d bytes\n",pstat->atm_tx_len_byte);
			panic_printk("	atm_tx_time_us: %d us\n",pstat->atm_tx_time_us);
#endif
			printk("\n");
		}
	}
...
#ifdef RTK_ATM_EXT
		if(priv->pshare->rf_ft_var.atm_mode == 2 || priv->pshare->rf_ft_var.atm_mode == 4 || priv->pshare->rf_ft_var.atm_mode == 5){
			if(!priv->pshare->rf_ft_var.atm_distribution_type) { //dynamic
					panic_printk("__ATM_IF_TIME_DYNAMIC__\n");
					//printk("\n _if_		_atmtime_\n");
					panic_printk("%10s	%d%%\n", GET_ROOT(priv)->dev->name, GET_ROOT(priv)->atm_iftime_dynamic);
#ifdef MBSSID	
					//if (GET_ROOT(priv)->pmib->miscEntry.vap_enable){
						for (i=0; i<RTL8192CD_NUM_VWLAN; i++){
							//if (IS_DRV_OPEN(GET_ROOT(priv)->pvap_priv[i]))
								panic_printk("%10s	%d%%\n", 
									GET_ROOT(priv)->pvap_priv[i]->dev->name, GET_ROOT(priv)->pvap_priv[i]->atm_iftime_dynamic);
						}
					//}
#endif
#ifdef UNIVERSAL_REPEATER
					panic_printk("%10s	%d%%\n", GET_ROOT(priv)->pvxd_priv->dev->name, GET_ROOT(priv)->pvxd_priv->atm_iftime_dynamic);
#endif
					panic_printk("\n");
...
#ifdef RTK_ATM_EXT
static int set_sta_atmstate(struct rtl8192cd_priv *priv, unsigned char *data)
{
	char tmpbuf[100], macid[13] = {'\0'}, macaddr[MACADDRLEN];
	int i=0, next_state;
	struct stat_info *pstat;

	// format: iwpriv interface set_sta_atmstate mac,next_state
	...
	if(strlen(macid) != 12){
		panic_printk("set_sta_atmstate : wrong mac addr\n");
		return -1;
	}else{
		for(i=0; i<12; i++){
        	if('0' <= macid[i]  && macid[i] <= '9')
	...
	//state
	next_state = _atoi(data, 10);	
	if (next_state!=0 && next_state!=1)
	{
		panic_printk("set_sta_atmstate : wrong parameters\n");
		return -1;
	}
	...
	for(i=0; i<NUM_STAT; i++) {
		if (priv->pshare->aidarray[i] && (priv->pshare->aidarray[i]->used == TRUE)){
			...
			if(pstat->hwaddr[0] == (unsigned char)macaddr[0] &&
				pstat->hwaddr[1] == (unsigned char)macaddr[1] &&
				pstat->hwaddr[2] == (unsigned char)macaddr[2] &&
				pstat->hwaddr[3] == (unsigned char)macaddr[3] &&
				pstat->hwaddr[4] == (unsigned char)macaddr[4] &&
				pstat->hwaddr[5] == (unsigned char)macaddr[5]) {
				pstat->atm_state_next = next_state;
			}
		}
	}
...
void AddDelMCASTGroup2STA(struct rtl8192cd_priv *priv, unsigned char *mac2addr, int add)
{
    ...
    if(found) {
        ...
        for (i=0; i < pstat->ipmc_num; i++) {
			...
			((!priv->pshare->rf_ft_var.mc2u_match_ip) || (priv->pshare->rf_ft_var.mc2u_match_ip && !memcmp(pstat->ipmc[i].mc_ip, mac2addr+12, 2)))) {
                found = 1;
                break;
            }
        }
        ...
        if (found) {
            if (!add) { // delete entry
                if (i != pstat->ipmc_num-1) {
					...
					memcpy(pstat->ipmc[i].mc_ip, pstat->ipmc[pstat->ipmc_num-1].mc_ip, 2);
                }
                pstat->ipmc_num--;
            }
        } else { // not found
            if (add) { // add entry
                if (pstat->ipmc_num < MAX_IP_MC_ENTRY) {
					...
					if(priv->pshare->rf_ft_var.mc2u_match_ip)
						memcpy(pstat->ipmc[pstat->ipmc_num].mc_ip, mac2addr+12, 2);
                    pstat->ipmc_num++;
                }
            }
        }
...
#ifdef RTK_ATM_EXT
case SIOC92DSETSTAATMSTATE:
	if ((wrq->u.data.length > sizeof_tmpbuf) ||
		copy_from_user(tmpbuf, (void *)wrq->u.data.pointer, wrq->u.data.length))
		break;
	ret = set_sta_atmstate(priv, tmpbuf);
	break;
#endif
#endif
