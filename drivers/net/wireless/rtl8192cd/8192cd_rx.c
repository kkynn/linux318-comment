...
static int deauth_filter(struct rtl8192cd_priv *priv, unsigned char *sa);
...
__inline__ void translate_CRC32_outsrc(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo, BOOLEAN CRC32,u2Byte PKT_LEN)
{
	...
	odm_AntselStatistics(pDM_Odm, pPhyInfo, pDM_FatTable->antsel_rx_keep_0, 1 , weighting , CRC32_METHOD, 0);
...
static void ___ConvertMulticatIPtoMacAddr(__u32 group, unsigned char *gmac)
{
	...
	gmac[12] = (group & 0xFF000000) >> 24;
	gmac[13] = (group & 0x00FF0000) >> 16;
}
...
static void ___ConvertMulticatIPv6toMacAddr(unsigned char* icmpv6_McastAddr, unsigned char *gmac)
{
	...
	gmac[12] = icmpv6_McastAddr[10];
	gmac[13] = icmpv6_McastAddr[11];
}
...
#ifdef RTK_ATM_EXT
	|| priv->pshare->rf_ft_var.atm_mode==5
#endif
	))
	{
...
#if defined(WDS)
                if ((pfrinfo->to_fr_ds == 3 && priv->pmib->dot11WdsInfo.wdsEnabled) || (dst_pstat == NULL) || !(dst_pstat->state & WIFI_ASOC_STATE))
#else
	...
	if (GET_CHIP_VER(priv) == VERSION_8192E) {
        ...
        if ( RT_STATUS_SUCCESS != GET_HAL_INTERFACE(priv)->QueryRxDescHandler(priv, q_num, pskb->data, &rx_desc_status) ) {
			...
			if (IS_PCIBIOS_TYPE) {
				...
				#ifdef DEBUG_DMA_MAP
					rtl_dbg_pci_unmap_single(priv->pshare->pdev, phw->rx_infoL[cur_q->cur_host_idx].paddr, phw->rx_infoL[cur_q->cur_host_idx].map_len , PCI_DMA_FROMDEVICE,RX_MAP_TYPE);
				#else
					pci_unmap_single(priv->pshare->pdev, phw->rx_infoL[cur_q->cur_host_idx].paddr, phw->rx_infoL[cur_q->cur_host_idx].map_len , PCI_DMA_FROMDEVICE);
				#endif /*DEBUG_DMA_MAP*/
			#else
				#ifdef DEBUG_DMA_MAP
					rtl_dbg_pci_unmap_single(priv->pshare->pdev, phw->rx_infoL[cur_q->cur_host_idx].paddr, (RX_BUF_LEN - sizeof(struct rx_frinfo)), PCI_DMA_FROMDEVICE,RX_MAP_TYPE);
				#else
		...
		if (IS_PCIBIOS_TYPE) {
			...
			#else
				#ifdef DEBUG_DMA_MAP
							rtl_dbg_pci_unmap_single(priv->pshare->pdev, phw->rx_infoL[cur_q->cur_host_idx].paddr, rx_desc_status.PKT_LEN, PCI_DMA_FROMDEVICE,RX_MAP_TYPE);
				#else
...
#if	0 /*eric-8822 check csi*/
{
	...
	if (IS_SUPPORT_HW_FILL_MACID(priv)) {
			...
			------------------------------------------------------*/
#if (defined(CONFIG_RTL_QOS_PATCH) || defined(CONFIG_RTK_VOIP_QOS) || defined(CONFIG_RTK_VLAN_WAN_TAG_SUPPORT) ||defined(CONFIG_RTL_HW_QOS_SUPPORT_WLAN)) && (!(defined(CONFIG_RTL8672) && defined(__LINUX_3_18__)))
...
void rtl8192cd_rx_isr(struct rtl8192cd_priv *priv)
{
...
#endif //CONFIG_WLAN_HAL
	{
		...
		while (1)
		{
			...
			#ifdef DMA_MAP_REFINE
				#ifdef DEBUG_DMA_MAP
					rtl_dbg_pci_unmap_single(priv->pshare->pdev, phw->rx_infoL[tail].paddr, phw->rx_infoL[tail].map_len , PCI_DMA_FROMDEVICE,RX_MAP_TYPE);
				#else
					pci_unmap_single(priv->pshare->pdev, phw->rx_infoL[tail].paddr, phw->rx_infoL[tail].map_len , PCI_DMA_FROMDEVICE);
				#endif /*DEBUG_DMA_MAP*/
			#else
				#ifdef DEBUG_DMA_MAP
								rtl_dbg_pci_unmap_single(priv->pshare->pdev, phw->rx_infoL[tail].paddr, (cmd & RX_PktLenMask), PCI_DMA_FROMDEVICE,RX_MAP_TYPE);
				#else
	...
	rx_reuse:
...
#endif	/*	defined(DELAY_REFILL_RX_BUF)	*/
			{
				...
				if (IS_PCIBIOS_TYPE) {
					#ifdef DEBUG_DMA_MAP
										phw->rx_infoL[tail].paddr = get_physical_addr_1(priv, pskb->data, (RX_BUF_LEN - sizeof(struct rx_frinfo)), PCI_DMA_FROMDEVICE , RX_MAP_TYPE);
					#else
...
static struct rx_frinfo *defrag_frame_main(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	...
	if (OPMODE & WIFI_AP_STATE) {
		...
		if(pfrinfo->to_fr_ds == 3 && (priv->pmib->dot11WdsInfo.wdsEnabled || priv->pmib->dot1180211sInfo.mesh_enable)) {
			...
			if (pstat == NULL){
				priv->ext_stats.rx_data_drops++;
				DEBUG_ERR("RX Drop: WDS/MESH rx data with pstat == NULL\n");
	...
	if (pstat == NULL)
	{
		status = _RSON_CLS3_;
		priv->ext_stats.rx_data_drops++;
		DEBUG_ERR("RX DROP: class 3 error (STA info is empty)!\n");
...
check_privacy:
	...
	if (OPMODE & WIFI_AP_STATE) {
		...
		if(pfrinfo->to_fr_ds == 3 && pfrinfo->is_11s)
...
#if (defined(CONFIG_RTK_VOIP_QOS) || defined(CONFIG_RTK_VLAN_WAN_TAG_SUPPORT)) && (!(defined(CONFIG_RTL8672) && defined(__LINUX_3_18__)))
#ifdef MBSSID
...
void process_amsdu(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct rx_frinfo *pfrinfo)
{
    ...
    struct sk_buff	*pskb  = NULL, *pnewskb = NULL, *pnewmcastskb = NULL;;
...
#if defined(_SINUX_) && defined(CONFIG_RTL865X_ETH_PRIV_SKB)
					extern struct sk_buff *priv_skb_copy(struct sk_buff *skb);
					pnewmcastskb = priv_skb_copy(pnewskb);
#else
					pnewmcastskb = skb_copy(pnewskb, GFP_ATOMIC);
#endif
					if(pnewmcastskb)
					{
#ifdef TX_SCATTER
						pnewmcastskb->list_num = 0;
#endif
						if ( 0 
							#ifdef GBWC
							|| GBWC_forward_check(priv, pnewmcastskb, pstat)
							#endif
							#ifdef SBWC
							|| SBWC_forward_check(priv, pnewmcastskb, pstat)
							#endif
						) {
							// packet is queued, nothing to do
						}
						else{
							if (rtl8192cd_start_xmit(pnewmcastskb, priv->dev))
                        		rtl_kfree_skb(priv, pnewmcastskb, _SKB_TX_);
                        }
					}
