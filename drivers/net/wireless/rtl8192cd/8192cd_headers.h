...
#define EXTERN
#endif
EXTERN u32 getAvailableTXBD(struct rtl8192cd_priv * priv, u32 qNum);
...
EXTERN int issue_BSS_Trans_Req(struct rtl8192cd_priv *priv, unsigned char *da, unsigned char dialog_token, unsigned char *prefered_BSSID, unsigned char prefered_channel);
...
#ifdef IQK_THREAD
EXTERN void iqk_task_kick(struct rtl8192cd_priv *priv);
EXTERN void iqk_task_block(struct rtl8192cd_priv *priv);
EXTERN void iqk_task_unblock(struct rtl8192cd_priv *priv);
EXTERN int iqk_task_init(struct rtl8192cd_priv *priv);
EXTERN void iqk_task_stop(struct rtl8192cd_priv *priv);
#endif
...
EXTERN void mp_kfree(struct rtl8192cd_priv *priv, unsigned char *data);
#endif
#endif
...
EXTERN void mesh_check_proxy_table(struct rtl8192cd_priv* priv);
...
EXTERN void stactrl_status_check(struct rtl8192cd_priv *priv);
#endif
#ifdef CONFIG_RTL_PROC_NEW
...
#ifdef CONFIG_WLAN_IPC
EXTERN int aipc_wlan_set(int id, char* buf, int offset, int size, char sleep_en);
EXTERN int aipc_wlan_get(int id, char* buf, int offset, int size, char sleep_en);
#endif // CONFIG_WLAN_IPC
#undef EXTERN
...
#if defined(DETECT_STA_EXISTANCE) && !defined(RATEADAPTIVE_BY_ODM)
