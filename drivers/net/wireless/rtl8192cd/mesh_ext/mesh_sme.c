...
void mesh_check_proxy_table(DRV_PRIV* priv)
{
#ifdef SMP_SYNC
	unsigned long		flags;
#endif
	...
	if(GET_MIB(priv)->dot1180211sInfo.mesh_enable == 1) {
		...
		for(i = 0; i < (1 << priv->proxy_table->table_size_power); i++) {
			if(priv->proxy_table->entry_array[i].dirty != 0) {
				...
				pstat = get_stainfo(priv, ptable_entry->sta);
				if(pstat && pstat->idle_count2 > 8 && memcmp(ptable_entry->owner, priv->pmib->dot11OperationEntry.hwaddr, MACADDRLEN)) {
					...
					printk("Detect STA (%02x%02x%02x%02x%02x%02x) connect with neighbor Mesh AP(%02x%02x%02x%02x%02x%02x).\n", 
						pstat->hwaddr[0], pstat->hwaddr[1], pstat->hwaddr[2], pstat->hwaddr[3], pstat->hwaddr[4], pstat->hwaddr[5],
						ptable_entry->owner[0], ptable_entry->owner[1], ptable_entry->owner[2],
						ptable_entry->owner[3], ptable_entry->owner[4], ptable_entry->owner[5]);
				}
			}
		}	
		SMP_UNLOCK_MESH_PROXY(flags);
