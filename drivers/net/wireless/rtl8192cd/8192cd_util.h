...
#ifdef DEBUG_DMA_MAP
extern int map_cnt[TOTAL_MAP_TYPE+1];
extern int map_total_size[TOTAL_MAP_TYPE+1];
extern int unmap_cnt[TOTAL_MAP_TYPE+1];
extern int unmap_total_size[TOTAL_MAP_TYPE+1];

static __inline__ unsigned long get_physical_addr_1(struct rtl8192cd_priv *priv, void *ptr,
				unsigned int size, int direction , int map_type )
{
	...
	if ((IS_PCIBIOS_TYPE) && (0 != size))
	{
			...
			map_cnt[TOTAL_MAP_TYPE]++;
			map_total_size[TOTAL_MAP_TYPE]+=size;
			
			if( (map_type == TX_MAP_TYPE)       || (map_type == RX_MAP_TYPE)      || (map_type == BEACON_MAP_TYPE) ||
			    (map_type == BUS1_TX_MAP_TYPE)  || (map_type == BUS1_RX_MAP_TYPE) || (map_type == BUS1_BEACON_MAP_TYPE) )
			{
				map_cnt[map_type]++;
				map_total_size[map_type]+=size;
			}
			else
			{
				map_cnt[UNKNOW_MAP_TYPE]++;
				map_total_size[UNKNOW_MAP_TYPE]+=size;
			}

			phyaddr = pci_map_single(priv->pshare->pdev, ptr, size, direction);
			//DMA_MAP_BCN_PRINTK("phyaddr=%x\n",phyaddr);
			return phyaddr;
	}
	else
#endif
...
static __inline__ int	rtl_dbg_pci_unmap_single(struct pci_dev *hwdev, dma_addr_t dma_addr, size_t size, int direction, int map_type )
{
			unmap_cnt[TOTAL_MAP_TYPE]++;
			unmap_total_size[TOTAL_MAP_TYPE]+=size;
			
			if( (map_type == TX_MAP_TYPE)       || (map_type == RX_MAP_TYPE)      || (map_type == BEACON_MAP_TYPE) ||
			    (map_type == BUS1_TX_MAP_TYPE)  || (map_type == BUS1_RX_MAP_TYPE) || (map_type == BUS1_BEACON_MAP_TYPE) )
			{
				unmap_cnt[map_type]++;
				unmap_total_size[map_type]+=size;
			}
			else
			{
				unmap_cnt[UNKNOW_MAP_TYPE]++;
				unmap_total_size[UNKNOW_MAP_TYPE]+=size;
			}
...
static __inline__ void rx_sum_up(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct rx_frinfo *pfrinfo)
{
	...
	if (priv) {
		...
		if (pfrinfo->rx_rate >= VHT_RATE_ID && (pfrinfo->rx_rate <= (_NSS4_MCS9_RATE_))) {
			...
			p_dbg->dNumQryVhtPkt[pfrinfo->rx_rate-VHT_RATE_ID]++;
			p_dbg->dNumQryVhtPktBytes[pfrinfo->rx_rate-VHT_RATE_ID] += pfrinfo->pktlen;
			if (pfrinfo->retry)
				p_dbg->dNumQryVhtPktRetry[pfrinfo->rx_rate-VHT_RATE_ID]++;
		}
#endif		
	}
