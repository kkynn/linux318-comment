...
{
	int val = -1;
	// |1~14|36, 38, 40, ..., 64|100, 102, ..., 140, 144|149, 151, ..., 165|
	if (ch <= 14)
		val = ch - 1;
	else if (ch <= 64)
		val = ((ch - 36) >> 1) + 14;
...
int PHY_ConfigTXLmtWithParaFile_new(struct rtl8192cd_priv *priv)
{
	...
	if (read_bytes > MAX_CONFIG_FILE_SIZE) {
		printk("[%s] exceed in size (%d)\n", __func__, read_bytes);
		return -1;
	}
...
//_TXPWR_REDEFINE in MP Tool, 3 Groups: 36-99 100-148 149-165
		...
		printk("[%s] reg_file %d Not support!!\n", __func__, reg_file);
		return -1;
	}
...
int PHY_ConfigMACWithParaFile(struct rtl8192cd_priv *priv)
{
...

	{
		...
		if (IS_HAL_CHIP(priv)) {
			...
			GET_HAL_INTERFACE(priv)->GetHwRegHandler(priv, HW_VAR_MACREGFILE_START, (pu1Byte)&next_head);
		} else
#endif
#ifdef CONFIG_RTL_92D_SUPPORT
		if (GET_CHIP_VER(priv) == VERSION_8192D) {
			...
			next_head = data_MACPHY_REG_start;
		} else
#endif
#ifdef CONFIG_RTL_92C_SUPPORT
		if ((GET_CHIP_VER(priv) == VERSION_8192C) || (GET_CHIP_VER(priv) == VERSION_8188C)) {
			...
			next_head = data_MACPHY_REG_92C_start;
		} else
#endif
...
#endif
		curMaxRFPath = get_curMaxRFPath(priv) ? get_curMaxRFPath(priv) : RF92CD_PATH_MAX;
...
void SelectTXPowerOffset(struct rtl8192cd_priv *priv)
{
...
#ifdef TX_SHORTCUT
	unsigned char txpwr_offset = priv->pshare->phw->TXPowerOffset;
#endif
	...
	if (priv->pshare->rf_ft_var.tx_pwr_ctrl
			&& (txpwr_offset != priv->pshare->phw->TXPowerOffset))
...
//	SAVE_INT_AND_CLI(x);
	...
	if ((GET_CHIP_VER(priv) == VERSION_8192D) && (priv->pmib->dot11RFEntry.macPhyMode == DUALMAC_DUALPHY))
		...
		if(GET_CHIP_VER(priv) == VERSION_8814A)
			NumTotalRFPath++;
#endif
	for (eRFPath = RF92CD_PATH_A; (eRFPath < NumTotalRFPath) && (eRFPath < RF92CD_PATH_MAX); eRFPath++) {
...
#ifdef CONFIG_ZIGBEE_COEXIST	
void zigbee_coex_init(struct rtl8192cd_priv *priv)
{
	if(GET_CHIP_VER(priv) == VERSION_8192E) {
		RTL_W8(0x76e, 0x2d);
		RTL_W8(0x790, 0x5);
		...
		RTL_W8(0x101, 0x16);
//		RTL_W32(0x778, 0x0);
		RTL_W32(0x778, 0x2);  // pin 46 , 47 exchange
		RTL_W32(0x6c0, 0x55555555);
		RTL_W32(0x6c4, 0x55555555);
		RTL_W32(0x6c8, 0x00ffffff);
		RTL_W32(0x6cc, 0x3);
	}
}
#endif
...
//eric-8822 ?? odm ability
	...
	panic_printk("[%s][%d] BoardType = 0x%x rfe_type=%d\n", __FUNCTION__, __LINE__, BoardType, 
	...
	{		
		...
		} else if(priv->pmib->dot11RFEntry.rfe_type == 8){
			panic_printk("PHY paratemters: RFE type 8\n");
			ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_GPA, TYPE_GPA0);
			ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_APA, TYPE_APA0);
			ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_GLNA, TYPE_GLNA0);
			ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_ALNA, TYPE_ALNA0);	
		} else if(priv->pmib->dot11RFEntry.rfe_type == 10){
			panic_printk("PHY paratemters: RFE type 10\n");
			ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_GPA, TYPE_GPA0);
			ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_APA, TYPE_APA0);
			ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_GLNA, TYPE_GLNA0);
			ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_ALNA, TYPE_ALNA0);	
...
// dummy
	...
	ODM_CmnInfoHook(ODMPTR, ODM_CMNINFO_ANT_TEST, &priv->pshare->dummy);
...
//	RTL_W32(0x300, tmp32);
    ...
    if((priv->pmib->dot11RFEntry.rfe_type == 1) ||(priv->pmib->dot11RFEntry.rfe_type == 6)
...
void spread_spectrum(struct rtl8192cd_priv *priv)
{
...
#endif
	unsigned char val;
	unsigned int PCIE0_H_CFG, PCIE1_H_CFG;

#ifdef CONFIG_RTL8672
	PCIE0_H_CFG = BSP_PCIE0_H_CFG;
	PCIE1_H_CFG = BSP_PCIE1_H_CFG;
#else
	PCIE0_H_CFG = 0xb8b00000;
	PCIE1_H_CFG = 0xb8b20000;
#endif
	...
	if(GET_CHIP_VER(priv) == VERSION_8814A){
		...
		if(priv->pshare->wlandev_idx == (1^WLANIDX)){  //slot1
			...
			REG32(PCIE1_H_CFG + 0x1000) = 0x00040101; //PCIe PORT1		
			delay_ms(1);
			val = REG32(PCIE1_H_CFG + 0x1000);
			REG32(PCIE1_H_CFG + 0x1000) = 0x45990b01; //PCIe PORT1
			delay_ms(1);
			val = REG32(PCIE1_H_CFG + 0x1000);
		}else{  //slot 0
			printk("Spread Spectrum set in slot0\n");
			REG32(PCIE0_H_CFG + 0x1000) = 0x00040101; //PCIe PORT0
			delay_ms(1);
			val = REG32(PCIE0_H_CFG + 0x1000);
			REG32(PCIE0_H_CFG + 0x1000) = 0x45990b01; //PCIe PORT0
			delay_ms(1);
			val = REG32(PCIE0_H_CFG + 0x1000);
		}
...
void get_swr_mode(struct rtl8192cd_priv *priv)
{    
#ifdef CONFIG_WLAN_HAL_8192EE
    if (GET_CHIP_VER(priv) == VERSION_8192E) {
        if (RTL_R32(REG_SYS_CFG1) & BIT24)
            priv->pshare->swr_mode = LDO_MODE;
        else
            priv->pshare->swr_mode = SPS_MODE;        
    }
#endif
}
...
//	MacConfigAfterFwDownload(priv);
	...
	if (priv->pmib->dot11OperationEntry.lifetime) {
		unsigned short val=0;
		if (priv->pmib->dot11OperationEntry.lifetime > 200)
			priv->pmib->dot11OperationEntry.lifetime = 200;

		if (GET_CHIP_VER(priv) == VERSION_8812E || GET_CHIP_VER(priv) == VERSION_8814A
			|| GET_CHIP_VER(priv) == VERSION_8822B || GET_CHIP_VER(priv) == VERSION_8197F)
		{
			val = RTL_MILISECONDS_TO_US256(priv->pmib->dot11OperationEntry.lifetime);
		} else {
			val = RTL_MILISECONDS_TO_US32(priv->pmib->dot11OperationEntry.lifetime);
		}
		RTL_W8(0x426, RTL_R8(0x426) | 0xf);
		RTL_W32(0x4C0, ((val<<16) | val));    //unit:32us, 
	} else {
		RTL_W8(0x426, RTL_R8(0x426) & 0xf0);
		RTL_W32(0x4C0, 0x10001000);    //MAC default
    }
#endif
...
//	RTL_W8(AGGR_BK_TIME, 0x10);
	...
	priv->pshare->ant_orig = ODMPTR->antdiv_select;
#endif
...
#if defined(SW_TX_QUEUE) || defined(RTK_ATM)
            priv->pshare->InterruptMask |= HIMR_88E_GTINT4;
#endif
        } else
#endif
#if defined(CONFIG_RTL_8812_SUPPORT)  || defined(CONFIG_RTL_8723B_SUPPORT)
...
//		phy_BB8192CD_Check_ParaFile(priv);
			...
			if (GET_CHIP_VER(priv) == VERSION_8814A) {
				...
				// refine the parameters of channel tracking
				RTL_W8(0x865, RTL_R8(0x865) & ~BIT2);	// disable pilot channel tracking loop
				RTL_W32(0x85c, 0x74210168);
				RTL_W32(0x8e0, 0x31316507);
				RTL_W32(0x19d8, 0xccc);
			}
#endif
...
#endif
	priv->pshare->rf_ft_var.rcr_val = RTL_R32(RCR);
...
void get_txsc_AC(struct rtl8192cd_priv *priv, unsigned char channel)
{
	...
	if (priv->pshare->CurrentChannelBW == HT_CHANNEL_WIDTH_80) {
		...
		case 0:
			//printk("case 0 \n");
#if defined(CONFIG_WLAN_HAL_8822BE)
			/* brain fix bad mcs0 @ch42 */
			if (GET_CHIP_VER(priv) == VERSION_8822B && priv->pshare->rf_ft_var.mp_specific == 1) {  
...
#ifdef CONFIG_WLAN_HAL
	if ( IS_HAL_CHIP(priv) ) {
		GET_HAL_INTERFACE(priv)->AddInterruptMaskHandler(priv, HAL_INT_TYPE_PSTIMEOUT);
#if defined(CONFIG_WLAN_HAL_8192EE)
		RTL_W32(REG_92E_HIMR, RTL_R32(REG_92E_HIMR) | HIMR_92E_PSTIMEOUT);
#else
...
#if defined(CONFIG_VERIWAVE_CHECK)
void setup_timer1(struct rtl8192cd_priv *priv, unsigned int timeout, unsigned int current_value)
{
				   ...
				   unsigned long encAlg, unsigned long useDK, unsigned char *pKey)
{
	...
	unsigned char force_gk_search = 0;
...
#else
		camIndex = keyId;
#ifndef HW_ENC_FOR_GROUP_CIPHER
		force_gk_search = 1;
#endif
#endif
	}
...
//for WAPI, camIndex 0 and 1 are used to store broadcast keys (crypto key and mic key)	
	...
	if (camIndex < set_dk_margin || force_gk_search) {
#ifdef  CONFIG_WLAN_HAL
		if (IS_HAL_CHIP(priv)) {
			SECURITY_CONFIG_OPERATION  SCO;
			GET_HAL_INTERFACE(priv)->GetHwRegHandler(priv, HW_VAR_SECURITY_CONFIG, (pu1Byte)&SCO);
...
int MIMO_Mode_1Plus1(struct rtl8192cd_priv *priv)
{
	...
	if(GET_CHIP_VER(priv) == VERSION_8814A) {
		...
		priv->pshare->rf_phy_bb_backup[1] = RTL_R32(0x80c);
		priv->pshare->rf_phy_bb_backup[2] = RTL_R32(0xa04);
		priv->pshare->rf_phy_bb_backup[3] = RTL_R32(0x93c);

		RTL_W8(0x808, 0x22);        // Rx path B
		RTL_W8(0x80c, 0x22);        // Tx path B for CCK
		RTL_W8(0xa07, 0x45);        // Tx path B for CCK
		RTL_W16(0x93e, 0x002c);     // Tx path B for 1SS HT/VHT

		priv->pshare->idle_txpwr_reduction = priv->pshare->rf_ft_var.idle_txpwr_reduction;
	}
#endif
...
{
	if(priv->pshare->rf_status == MIMO_2T2R && get_rf_mimo_mode(priv) == MIMO_2T2R)
		return 1;
	...
	if(GET_CHIP_VER(priv) == VERSION_8814A) {
		...
		RTL_W8(0x808, 0x66);        // Rx path B/C for OFDM
		RTL_W8(0x80c, 0x66);        // Tx path B/C for CCK
		RTL_W8(0xa07, 0x66);        // Tx path B/C for CCK
		RTL_W16(0x93e, 0x106c);     // Tx path B/C for 1SS HT/VHT

		priv->pshare->idle_txpwr_reduction = 0;
	}
#endif
	return 1;
}
...
{
	if(priv->pshare->rf_status == MIMO_3T3R && get_rf_mimo_mode(priv) == MIMO_3T3R)
		return 1;
	//panic_printk("switch to 3x3\n");
#ifdef CONFIG_WLAN_HAL_8814AE
	if(GET_CHIP_VER(priv) == VERSION_8814A) {
		PHY_SetBBReg(priv, 0x808, bMaskDWord, priv->pshare->rf_phy_bb_backup[0]);
		PHY_SetBBReg(priv, 0x80c, bMaskDWord, priv->pshare->rf_phy_bb_backup[1]);
		PHY_SetBBReg(priv, 0xa04, bMaskDWord, priv->pshare->rf_phy_bb_backup[2]);
		PHY_SetBBReg(priv, 0x93c, bMaskDWord, priv->pshare->rf_phy_bb_backup[3]);
...
void set_MIMO_Mode(struct rtl8192cd_priv *priv, unsigned pref_mode)
{
	 if(MIMO_Mode_Switch(priv, pref_mode)) {
		if((pref_mode == MIMO_2T2R && get_rf_mimo_mode(priv) == MIMO_2T2R) || (pref_mode == MIMO_3T3R && get_rf_mimo_mode(priv) == MIMO_3T3R)) {
			...
			if (GET_CHIP_VER(priv) == VERSION_8192D) {
				...
				for (i = 0; i < MAX_5G_CHANNEL_NUM; i++) {
					if (i >= 35 && i <= 63) { // ch 36 ~ 64
						...
						} else if (i >= 127 && i <= 143) { // ch 128 ~ 144
	...
	)
{
	...
	switch(TxABiaOffset)
	{
		...
		case 0xF0:
		default:
			panic_printk("No need to do TxA bias current calibration\n");
			return;
			break;
	}
...
#ifdef CONFIG_WLAN_HAL
	if (IS_HAL_CHIP(priv)) {
		return __getAvailableTXBD_HAL(priv, qNum);
		}
	else 
#endif
	{
		return __getAvailableTXBD_nonHAL(priv, qNum);
	}
...
/* CLM [channel loading measurement] */
void channel_loading_measurement(struct rtl8192cd_priv *priv, unsigned short clm_sample_num)
{
    ...
    if (IS_OUTSRC_CHIP(priv)) {
        ...
        if (clm_count > 0) {
    		...
    		priv->ext_stats.ch_utilization = (clm_count * 255) / clm_sample_num;
    		// GDEBUG("busy time[%d]ms\n", RTK80211_SAMPLE_NUM_TO_TIME(clm_count));
    	} else
            priv->ext_stats.ch_utilization = 0;
...
#endif
    {
        ...
        if (clm_count > 0) {
            ...
            priv->ext_stats.ch_utilization = 0;
    	start_bbp_ch_load(priv, clm_sample_num); /* trigger next time CLM */	
    }
}
...
#define	ODM_REG_CLM_TIME_PERIOD_11N		0x894
	...
	if (ODMPTR->SupportICType & ODM_IC_11AC_SERIES) {
	...
	} else if (ODMPTR->SupportICType & ODM_IC_11N_SERIES) {
