	...
	)
{
	...
	else
	{
		...
		unsigned int cw_max = 2;
		unsigned int cw_min = 1;
		aifs_time = 0x1f;
#else
		...
		if (priv->pshare->txop_enlarge == 0xe || priv->pshare->txop_enlarge == 0xd){   //if intel/ralink case
			...
			if ( priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G)                                                    //if other case
					...
					if (pstat) {
						if((pstat->IOTPeer == HT_IOT_PEER_REALTEK || pstat->IOTPeer == HT_IOT_PEER_RTK_APCLIENT || pstat->IOTPeer == HT_IOT_PEER_REALTEK_8812
							...
							if (priv->pshare->th_mode == 2) { // cliff test
								if ( (ODMPTR->FalseAlmCnt.Cnt_all > 300 && (RTL_R8(0xc50) & 0x7f) >= 0x32) )
									RTL_W32(0x508, (0x138 << 16) | (6 << 12) | (4 << 8) | 0x19);
								else
...
#else
						RTL_W32(0x508, (txop << 16) | (5 << 12) | (3 << 8) | (sifs_time + 2 * slot_time));
#endif
					}
				}
				#endif
...
//	}
	...
	if(GET_CHIP_VER(priv) == VERSION_8192E) {
		if(priv->pshare->rf_ft_var.symbol_det_dm) {
			if(pstat && pstat->IOTPeer == HT_IOT_PEER_INTEL && (priv->pmib->dot11BssType.net_work_type & WIRELESS_11N))
				RTL_W8(0xC69, 0xb);
			else
				RTL_W8(0xC69, 0x5);
		}
	...
	if ((priv->pshare->rf_ft_var.spec_env_test_en & BIT(1)) && priv->assoc_num >= 1 ) {
		if ((UINT32_DIFF(priv->pshare->rx_packets_total, priv->pshare->rx_packets_pre2) < 2)
			...
			&& (ODMPTR->FalseAlmCnt.Cnt_all <= priv->pshare->rf_ft_var.spec_env_test_fa)))
		{
			...
			if (((UINT32_DIFF(priv->pshare->rx_packets_total, priv->pshare->rx_packets_pre2) > 3)
				...
				|| (((priv->pshare->spirent_sta_num+priv->pshare->veriwave_sta_num) > 0)
				&& (ODMPTR->FalseAlmCnt.Cnt_all > (priv->pshare->rf_ft_var.spec_env_test_fa + 200))))
			{
...
//#ifdef CONFIG_WLAN_HAL_8881A
	...
	if ((priv->up_time % 2) == 0) {
		...
		//Disable CFEnd if detect STA is under 11G mode
		if( !pstat && priv->assoc_num == 1 && priv->pmib->dot11ErpInfo.nonErpStaNum ==0 && priv->ht_legacy_sta_num == 1 &&
			priv->pshare->current_rx_bytes >> 17 > 1 &&  (priv->pshare->current_tx_bytes / (priv->pshare->current_rx_bytes+1)) < 4)
		{
			if(!priv->pshare->iot_11g_discfe){
				RTL_W16(RD_CTRL, RTL_R16(RD_CTRL) | DIS_TXOP_CFE);
				priv->pshare->iot_11g_discfe=1;
			}
		}
		else
		{
				...
				if(pstat && pstat->IOTPeer == HT_IOT_PEER_BROADCOM) {
					if(curr_tx_pkts > 4000) {
						if(priv->pshare->CurrentChannelBW == HT_CHANNEL_WIDTH_80 && priv->pmib->dot11BssType.net_work_type & WIRELESS_11AC) {
							...
							if(sta_curr_tx_pkts < 28000 && curr_tx_pkts < 28000 && priv->assoc_num < 3) {
								unsigned int  ra_cnt_sum = 0;
								unsigned char ra_max = 0x00;
								unsigned char ra_min = 0xff;
								int i;
								for(i = 0; i < 3; i++) {
									if(pstat->ra_max_window[i] != 0x00)
										if(ra_max < pstat->ra_max_window[i])
											ra_max = pstat->ra_max_window[i];
									if(pstat->ra_min_window[i] != 0xff)
										if(ra_min > pstat->ra_min_window[i])
											ra_min = pstat->ra_min_window[i];
									ra_cnt_sum += pstat->ra_cnt_window[i];
								}
								if(ra_max != 0x00 && ra_min != 0xff && ra_cnt_sum >= 5 && ra_max - ra_min >= 2)
									pstat->is_legacy_USB = 1;
							}
							else {
	...
	if(pstat) {
		...
		if (UINT32_DIFF(cur_queue_pkt, priv->pshare->swq_residual_drop_pkt) > priv->pshare->rf_ft_var.swq_edca_thd) {
			if (priv->pshare->th_mode != 3) {
				priv->pshare->th_mode = 3; // Turbo-TX mode
				switch_turbo++;
			}
		} else 
#endif
		...
		} else if((pstat->tx_avarage>>17) > 15 && (priv->pshare->th_mode!=1)) {				
			priv->pshare->th_mode = 1; // TX mode
			switch_turbo++;
