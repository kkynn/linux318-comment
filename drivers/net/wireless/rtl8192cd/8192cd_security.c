	...
	if (priv->MIC_timer_on) {
		...
		if (pstat_del) {
			...
			DEBUG_INFO("%s() disassoc %02X:%02X:%02X:%02X:%02X:%02X\n",__func__,
				pstat_del->hwaddr[0],pstat_del->hwaddr[1],pstat_del->hwaddr[2],
				pstat_del->hwaddr[3],pstat_del->hwaddr[4],pstat_del->hwaddr[5]);
...
int rtl8192cd_ioctl_priv_daemonreq(struct net_device *dev, struct iw_point *data)
{
	...
	switch(req.EventId)
	{
		...
		case DOT11_EVENT_WSC_GET_MIB:
						...
						break;			
#endif
		case DOT11_EVENT_WSC_NEED_DISASSOC_SET:
		{
			DOT11_WSC_CHECK_STA_IND wsc_check_sta_ind;
			struct stat_info *pstat;

			if (copy_from_user((void *)(&wsc_check_sta_ind), (void *)(data->pointer), data->length)) {
				DEBUG_ERR("copy_from_user fail!\n");
				return -1;
			}

			pstat = get_stainfo(priv, wsc_check_sta_ind.staAddr);
			if (!pstat) {
				DEBUG_ERR("get_stainfo return NULL\n");
				return -1;
			}

			pstat->wsc_need_disassoc = wsc_check_sta_ind.need_disassoc;
		}
			break;

		case DOT11_EVENT_WSC_NEED_DISASSOC_GET:
		{
			DOT11_WSC_CHECK_STA_IND wsc_check_sta_ind;
			struct stat_info *pstat;
			...
			wsc_check_sta_ind.need_disassoc = pstat->wsc_need_disassoc;

			if (copy_to_user((void *)(data->pointer), (void *)(&wsc_check_sta_ind), sizeof(DOT11_WSC_CHECK_STA_IND))) {
				DEBUG_ERR("copy_to_user fail!\n");
				return -1;
			}
			data->length = sizeof(DOT11_WSC_CHECK_STA_IND);
		}
			break;
#endif	//WIFI_SIMPLE_CONFIG
