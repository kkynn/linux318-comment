...
void TxLMT_OFDM_Jaguar_N(IN  HAL_PADAPTER	Adapter)
{
	...
	char tmp_lmt_rate54=0;
	char tmp_lmt_idx_ht1s=0;
	char tmp_lmt_idx_ht2s=0; 
	...
	tmp_lmt_rate54 = idx_rate54 + lmt_pg_idx_rate;
	max_lmt_rate54 = POWER_RANGE_CHECK(tmp_lmt_rate54);

	//Cal Max tx pwr idx for HT1S
	tmp_dw = HAL_RTL_R32(REG_BB_TXAGC_A_MCS07_MCS04_N);
	idx_ht_mcs7 = HAL_get_byte_from_dw(tmp_dw, 3);
	tmp_lmt_idx_ht1s = idx_ht_mcs7 + lmt_pg_idx_ht1s;
	max_lmt_idx_ht1s = POWER_RANGE_CHECK(tmp_lmt_idx_ht1s);

	//Cal Max tx pwr idx for HT2S
	tmp_dw = HAL_RTL_R32(REG_BB_TXAGC_A_MCS15_MCS12_N);
	idx_ht_mcs15 = HAL_get_byte_from_dw(tmp_dw, 3);
	tmp_lmt_idx_ht2s = idx_ht_mcs15 + lmt_pg_idx_ht2s;
	max_lmt_idx_ht2s = POWER_RANGE_CHECK(tmp_lmt_idx_ht2s);
...
void TxLMT_CCK_Jaguar_N(IN  HAL_PADAPTER	Adapter)
{
	...
	unsigned char	max_lmt_idx_cck_A = 0;
	unsigned char	max_lmt_idx_cck_B = 0;
	char tmp_lmt_cck_A=0, tmp_lmt_cck_B=0;
	...
	tmp_lmt_cck_A = idx_cck_11m_A + lmt_pg_idx_cck_A;
	max_lmt_idx_cck_A = POWER_RANGE_CHECK(tmp_lmt_cck_A);

	tmp_dw = HAL_RTL_R32(REG_BB_TXAGC_A_CCK11_2_B_CCK11_N);
	idx_cck_11m_B = HAL_get_byte_from_dw(tmp_dw, 0);
	tmp_lmt_cck_B = idx_cck_11m_B + lmt_pg_idx_cck_B;
	max_lmt_idx_cck_B = POWER_RANGE_CHECK(tmp_lmt_cck_B);
...
#if 1 //DAVIDadd 1108
void degrade_power_control(
	IN  HAL_PADAPTER                Adapter
)
{
	unsigned char path, rate, power;

	if (IS_HARDWARE_TYPE_8814A(Adapter)) {
		for (path=1;path<4;path++) {
			for (rate=TX_AGC_CCK_1M;rate<=TX_AGC_CCK_11M;rate++) {
				PHYSetTxPower88XX(Adapter,rate,rate,(unsigned char)path,POWER_RANGE_CHECK(Adapter->pshare->phw->CurrentTxAgcCCK[path][rate]-6*Adapter->pshare->rf_ft_var.power_level));
			}
			for (rate=TX_AGC_OFDM_6M;rate<=TX_AGC_OFDM_54M;rate++) {
				PHYSetTxPower88XX(Adapter,rate,rate,(unsigned char)path,POWER_RANGE_CHECK(Adapter->pshare->phw->CurrentTxAgcOFDM[path][rate-TX_AGC_OFDM_6M]-6*Adapter->pshare->rf_ft_var.power_level));
			}
			for (rate=TX_AGC_HT_NSS1_MCS0;rate<=TX_AGC_HT_NSS3_MCS23;rate++) {
				PHYSetTxPower88XX(Adapter,rate,rate,(unsigned char)path,POWER_RANGE_CHECK(Adapter->pshare->phw->CurrentTxAgcMCS[path][rate-TX_AGC_HT_NSS1_MCS0]-6*Adapter->pshare->rf_ft_var.power_level));
			}		
			for (rate=TX_AGC_VHT_NSS1_MCS0;rate<=TX_AGC_VHT_NSS3_MCS9;rate++) {
				PHYSetTxPower88XX(Adapter,rate,rate,(unsigned char)path,POWER_RANGE_CHECK(Adapter->pshare->phw->CurrentTxAgcVHT[path][rate-TX_AGC_VHT_NSS1_MCS0]-6*Adapter->pshare->rf_ft_var.power_level));
			}	
		}
	}
}
	...
	if (Band == RF88XX_BAND_ON_2_4G) { // 2.4G band
		...
		if (GET_CHIP_VER(Adapter) == VERSION_8881A) {
			...
			if (Adapter->pshare->rf_ft_var.use_ext_pa && Adapter->pshare->rf_ft_var.use_ext_lna) {
				...
				case 8:
				case 10:
					panic_printk("TRSWITCH: RFE TYPE 2 or 4 or 8 or 10\n");
					PHY_SetBBReg_88XX(Adapter, 0x1994, 0xf, 0xf); // (ANTSW/ANTSWB controlled by S3)
					PHY_SetBBReg_88XX(Adapter, REG_GPIO_MUXCFG, 0x00f00000, 0xf); // (S3 RFE output enable) , 0x40[23:20]
	...
	if (phy_band == PHY_BAND_2G) {
        ...
        for(i = 0; i < 4; i++) {
            ...
            if(bPathXEn[i]) {
                ...
                Adapter->pshare->tgpwr_idx_CCK[i] = HAL_VAR_pwrlevelCCK_X[i];
            }
        }  
    }
...
)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        if(bPathXEn[i]) {
            ...
            Adapter->pshare->tgpwr_idx_OFDM[i] = tmp_TPI;
        }
    }
	...
	if (HAL_VAR_CurrentChannelBW == 0) {
        ...
        for (i = 0; i < 4; i++) {
            if(bPathXEn[i]) {
                ...
                Adapter->pshare->tgpwr_idx_HT1S[i] = tmp_TPI;
                Adapter->pshare->tgpwr_idx_VHT1S[i] = tmp_TPI;
                ...
                Adapter->pshare->tgpwr_idx_HT2S[i] = tmp_TPI;
                Adapter->pshare->tgpwr_idx_VHT2S[i] = tmp_TPI;
                ...
                Adapter->pshare->tgpwr_idx_HT3S[i] = tmp_TPI;
                Adapter->pshare->tgpwr_idx_VHT3S[i] = tmp_TPI;
#endif
            }
        }
	//3 40M
