...
#ifdef DEBUG_DMA_MAP 
#undef HAL_VIRT_TO_BUS1
#define HAL_VIRT_TO_BUS1(Adapter, ptr, size, direction)         get_physical_addr_1(Adapter, ptr, size, direction,TX_MAP_TYPE)
#endif
...
)
{
...
#endif
	{
		if (q_num != HCI_TX_DMA_QUEUE_BCN) {
		    ...
		    u4Byte beacon_offset = TXBD_BEACON_OFFSET_V1;
...
//#ifdef DEBUG_DMA_MAP 
//#undef HAL_VIRT_TO_BUS1
//#define HAL_VIRT_TO_BUS1(Adapter, ptr, size, direction)         get_physical_addr_1(Adapter, ptr, size, direction,BEACON_MAP_TYPE)
//#endif
...
#else
#ifdef DEBUG_DMA_MAP
        HAL_CACHE_SYNC_WBACK(Adapter, HAL_VIRT_TO_BUS1_BEACON_DBG(Adapter, (PVOID)pTXBD, sizeof(TX_BUFFER_DESCRIPTOR), HAL_PCI_DMA_TODEVICE),
...
)
{
    ...
    if( pTXBD->TXBD_ELE[1].Dword1 != 0 )
    {
#ifdef DEBUG_DMA_MAP
    			rtl_dbg_pci_unmap_single(Adapter->pshare->pdev, get_desc(pTXBD->TXBD_ELE[1].Dword1), get_desc(pTXBD->TXBD_ELE[1].Dword0) , PCI_DMA_TODEVICE,BEACON_MAP_TYPE);
#else
    			pci_unmap_single(Adapter->pshare->pdev, get_desc(pTXBD->TXBD_ELE[1].Dword1), get_desc(pTXBD->TXBD_ELE[1].Dword0) , PCI_DMA_TODEVICE);
#endif
    }
    ...
    SET_DESC_FIELD_CLR(pTXBD->TXBD_ELE[1].Dword1,
                HAL_VIRT_TO_BUS1_BEACON_DBG(Adapter, (PVOID)beaconbuf, frlen, HAL_PCI_DMA_TODEVICE) + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL,
                TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
#else
    SET_DESC_FIELD_CLR(pTXBD->TXBD_ELE[1].Dword1,
