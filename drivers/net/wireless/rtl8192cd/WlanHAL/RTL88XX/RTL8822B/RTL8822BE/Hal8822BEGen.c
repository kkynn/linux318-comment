...
#else
	ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_TXBF, ODM_DBG_TRACE, ("[%s] BFerBFeeIdx=%d, BFerIdx=%d, BFeeIdx=%d, beamformee_su_cnt=%d, beamformee_su_reg_maping=%d, beamformee_mu_reg_maping\n", __func__, BFerBFeeIdx, BFerIdx, BFeeIdx, pBeamformingInfo->beamformee_su_cnt, pBeamformingInfo->beamformee_su_reg_maping));
#endif
	/*************SU BFer Entry Init*************/
	...
	)
{
	...
	if(pSoundingInfo->State == SOUNDING_STATE_SU_SOUNDDOWN) {
		...
		if(pBeamformEntry->pSTA->error_csi == 1 && pBeamformEntry->pSTA->current_tx_rate >= _NSS2_MCS0_RATE_ && (priv->pshare->rf_ft_var.applyVmatrix)) {
		...
		if (1){//(pBeamformEntry->BeamformEntryState == BEAMFORMING_ENTRY_STATE_PROGRESSED) {
			...
			if(priv->pshare->rf_ft_var.swq_dbg==79) {
				GDEBUG("snd_result=%x, isApplyMU=%x, RegMUTxCtrl=%x\n", snd_result, isApplyMU, pBeamformingInfo->RegMUTxCtrl);
			}
