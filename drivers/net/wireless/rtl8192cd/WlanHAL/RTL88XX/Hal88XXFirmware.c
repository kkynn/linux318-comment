	...
	)
{
	...
	if(BW == 0)
	{
		...
		phydm_UpdateHalRAMask(&(Adapter->pshare->_dmODM), WirelessMode, rf_mimo_mode, BW, MimoPs_enable, disable_cck_rate, &ratr_bitmap_msb, &ratr_bitmap, rssi_level);
...
//			if(MimoPs <= MIMO_PS_DYNAMIC)
	...
	if(GET_CHIP_VER(priv) == VERSION_8192E) {
		if(BW == HT_CHANNEL_WIDTH_20_40 && ratr_index == RATR_INX_WIRELESS_NGB && pstat->veriwave_2G_RvR_test) {
