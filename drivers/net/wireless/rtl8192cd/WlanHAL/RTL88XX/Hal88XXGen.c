...
)
{
    ...
    if ( _GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v2) {
		...
		HAL_RTL_W32(REG_CR, HAL_RTL_R32(REG_CR) | (BIT_MACTXEN | BIT_MACRXEN));
			...
			// Update current Tx FIFO page status.
			//
			HalQueryTxBufferStatus8822BSdio(Adapter);
			HalQueryTxOQTBufferStatus8822BSdio(Adapter);
	
			// Enable MACTXEN/MACRXEN block
			HAL_RTL_W32(REG_CR, HAL_RTL_R32(REG_CR) | (BIT_MACTXEN | BIT_MACRXEN));
#endif // IS_EXIST_RTL8822BS
	}
	break;
...
#if (IS_RTL8192E_SERIES || IS_RTL8881A_SERIES)
        *((pu4Byte)(val)) = PlatformEFIORead4Byte(Adapter, REG_TSFTR1);
#else
	 *((pu4Byte)(val)) = PlatformEFIORead4Byte(Adapter, REG_FREERUN_CNT);
#endif
    }
    break;
...
)
{
	...
	HAL_RTL_W16(REG_FIFOPAGE_INFO_5, TX_PAGE_CNT_PUBQ_V1);
        ...
        //------------Tx FIFO page number setting------------------
        // Queue page number setting
        //HAL_RTL_W16(REG_FIFOPAGE_INFO_1, TX_PAGE_CNT_HPQ);
        //HAL_RTL_W16(REG_FIFOPAGE_INFO_2, TX_PAGE_CNT_LPQ);
        //HAL_RTL_W16(REG_FIFOPAGE_INFO_3, TX_PAGE_CNT_NPQ);
        //HAL_RTL_W16(REG_FIFOPAGE_INFO_4, TX_PAGE_CNT_EPQ);
        //HAL_RTL_W16(REG_FIFOPAGE_INFO_5, TX_PAGE_CNT_PUBQ);
...
#if IS_EXIST_RTL8822BE || IS_EXIST_RTL8822BS  /* 8822B temp Etic remove*/
