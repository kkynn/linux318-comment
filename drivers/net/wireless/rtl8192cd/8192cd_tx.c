...
#ifdef DEBUG_DMA_MAP
#define get_physical_addr(Adapter, ptr, size, direction)  get_physical_addr_1(Adapter, ptr, size, direction,TX_MAP_TYPE)
#define pci_unmap_single(Adapter, ptr, size, direction) rtl_dbg_pci_unmap_single(Adapter, ptr, size, direction , TX_MAP_TYPE )
#endif
...
#ifdef SW_TX_QUEUE
#define SWQ_DEQ_ALL			0
#define SWQ_DEQ_LIMIT		1
#define SWQ_MU_DEQ_LIMIT	2
#endif
...
skip_check_eap:
	...
	if (skb_queue_len(&priv->pshare->skb_xmit_queue) >= priv->pshare->rf_ft_var.swq_max_enqueu_len) {
		printk(KERN_WARNING "TX DROP: exceed the skb xmit queue!\n");
		priv->ext_stats.tx_drops++;
		dev_kfree_skb_any(skb);
		return 0; 
	}
#else
	if (getAvailableTXBD(priv, VI_QUEUE) < (NUM_TX_DESC/10) ||
		getAvailableTXBD(priv, VO_QUEUE) < (NUM_TX_DESC/10) ||
		getAvailableTXBD(priv, BE_QUEUE) < (NUM_TX_DESC/10) ||
		getAvailableTXBD(priv, BK_QUEUE) < (NUM_TX_DESC/10)) {
...
int amsdu_timer_add(struct rtl8192cd_priv *priv, struct stat_info *pstat, int tid, int from_timeout)
{
	...
	if (priv->pmib->dot11nConfigEntry.dot11nAMSDUSendTimeout) {
		...
		if (priv->pshare->in_spec_env_test_to)
			timeout = now + 15000;
		else if (pstat->amsdu_dropdupack_en)
			timeout = now + 400;
		else
			timeout = now + 15000;
	}
...
int mlcst2unicst(struct rtl8192cd_priv *priv, struct sk_buff *skb)
{
	...
	if( isSpecialFloodMac(priv,skb) /*|| IS_MDNSV4_MAC(skb->data)||IS_MDNSV6_MAC(skb->data)*/||IS_IGMP_PROTO(skb->data) || isICMPv6Mng(skb) || IS_ICMPV6_PROTO(skb->data)|| isMDNS(skb->data))
	{
		...
		while ((plist = asoc_list_get_next(priv, plist)) != phead && priv->ext_stats.tx_flood_mcast_pkts_cnt < 1000) {
				...
				&& (newskb = skb_clone(skb, GFP_ATOMIC)))
			{
				...
				priv->ext_stats.tx_flood_mcast_pkts_cnt++;
...
//#ifdef VIDEO_STREAMING_REFINE
	...
	while ((plist = asoc_list_get_next(priv, plist)) != phead) {
		...
		for (i=0; i < pstat->ipmc_num; i++) {
			...
			((!priv->pshare->rf_ft_var.mc2u_match_ip) || (priv->pshare->rf_ft_var.mc2u_match_ip && !memcmp(pstat->ipmc[i].mc_ip, skb->data+30, 2)))) {
				pstat_found = pstat;
				m2uCnt++;
				break;
			}
	...
	while ((plist = asoc_list_get_next(priv, plist)) != phead) {
...
#ifdef CONFIG_PCI_HCI
		{
			if (priv->stop_tx_mcast2uni) {
				...
				if (getAvailableTXBD(priv, BE_QUEUE) < 20) {
					asoc_list_unref(priv, pstat);
					#ifdef __ECOS
					#else
		...
		*bRtsEnable = TRUE;
			...
			(priv->ht_protection /*|| txcfg->pstat->is_rtl8190_sta*/))
		{
			*bNProtect = 1;
			...
			if( ((GET_CHIP_VER(priv) == VERSION_8192E) || (GET_CHIP_VER(priv)==VERSION_8812E)) 
				&& priv->pshare->total_assoc_num > 10 )
			{
				*bRtsEnable = FALSE;
				*bRtsEnable = TRUE;
			} else			
		...
		{	// when HT MIMO Dynamic power save is set, RTS is needed
			*bRtsEnable = TRUE;
			...
			if (is_MCS_rate(txcfg->tx_rate) && (/*(txcfg->pstat->IOTPeer!=HT_IOT_PEER_INTEL) ||*/ txcfg->pstat && !txcfg->pstat->no_rts)) 
			{
				*bNProtect = 1;
				*bHwRts = TRUE;
...
#ifdef MULTI_STA_REFINE 					
				else if ( (GET_CHIP_VER(priv)==VERSION_8812E) && priv->pshare->total_assoc_num > 10 )
...
#ifdef COCHANNEL_RTS
	if (priv->pmib->miscEntry.auto_rts_mode == 1)
	{
...
#endif
#ifdef MULTI_STA_REFINE
		else if((GET_CHIP_VER(priv)==VERSION_8812E) && priv->pshare->total_assoc_num > 30) {
		        pdesc->Dword4 |= set_desc(TXdesc_92E_RtyLmtEn);
...
//	BOOLEAN			bCts2SelfEnable;
    ...
    if (is_MCS_rate(txcfg->tx_rate))    // HT rates
    {
        ...
        if (
            ...
            if (txcfg->pstat && txcfg->pstat->ADDBA_ready[TID] && !txcfg->pstat->low_tp_disable_ampdu) {
				...
				switch(txcfg->q_num) {
					case VO_QUEUE:
						if(priv->pshare->rf_ft_var.vomaxagg) 
							pdesc_data->maxAggNum = priv->pshare->rf_ft_var.vomaxagg;
						break;
					case VI_QUEUE:
						if(priv->pshare->rf_ft_var.vimaxagg) 
							pdesc_data->maxAggNum = priv->pshare->rf_ft_var.vimaxagg;
						break;		
					case BE_QUEUE:
						if(priv->pshare->rf_ft_var.bemaxagg) 
							pdesc_data->maxAggNum = priv->pshare->rf_ft_var.bemaxagg;
						break;		
					case BK_QUEUE:
						if(priv->pshare->rf_ft_var.bkmaxagg) 
							pdesc_data->maxAggNum = priv->pshare->rf_ft_var.bkmaxagg;
						break;		
					default:
						break;
				}
	...
	if (priv->pmib->miscEntry.auto_rts_mode == 1)
	{
		...
		if( txcfg->pstat && priv->pmib->dot11RFEntry.txbf_mu && txcfg->pstat->muPartner){
...
#if (BEAMFORMING_SUPPORT == 1)/*eric-8822 ?? assign gid of txdesc = 0x3f*/
...
{
    if (getAvailableTXBD(priv, qnum) < qlen)
		return 1;
	else
    	return 0;
}
...
#ifdef CONFIG_WLAN_HAL	 	 
    if(IS_HAL_CHIP(priv)) {
        if (getAvailableTXBD(priv, qnum) < (num+4))
            return 1;
...
#endif
__IRAM_IN_865X
__inline__ static int rtl8192cd_swq_dequeue(struct rtl8192cd_priv *priv, struct stat_info *pstat,  unsigned char qnum, int mode, unsigned int dqnum)
{
	int count = 0;
	...
	while (1)
	{
		if(rtl8192cd_swq_bdfull(priv, pstat, qnum, 5) && mode == SWQ_DEQ_LIMIT)
			break;
		...
		priv->pshare->swq_xmit_out_pkt++;
		priv->pshare->swq_real_deque_pkt++;
...
#if (MU_BEAMFORMING_SUPPORT == 1)
		if(mode == SWQ_MU_DEQ_LIMIT && count >= dqnum){
			break;
		}
		else
#endif	
	...
	if(priv->pshare->swq_use_hw_timer) {
		...
		if(psta_mu) {
			if(pstat->swq.swq_en[q_num]) {
				...
				if(pstat->tx_avarage < (priv->pshare->rf_ft_var.mutp_th_lower * 125000) || psta_mu->tx_avarage < (priv->pshare->rf_ft_var.mutp_th_lower * 125000)) 
				{
					...
					if(pstat->swq.swq_empty[q_num]) {
						rtl8192cd_swq_dequeue(priv, pstat, q_num, SWQ_DEQ_ALL, 0);
					}
					if(psta_mu->swq.swq_empty[q_num]) {
						rtl8192cd_swq_dequeue(priv, psta_mu, q_num, SWQ_DEQ_ALL, 0);
            		}
				}
			} else {
				if(pstat->tx_avarage > (priv->pshare->rf_ft_var.mutp_th_up * 125000) && psta_mu->tx_avarage > (priv->pshare->rf_ft_var.mutp_th_up * 125000)) {
					pstat->swq.swq_en[q_num] = 1; /* enable swq */ 
					pstat->swq.swq_prev_timeout[q_num] = 1;
					pstat->swq.swq_timeout_change[q_num] = 0;
					pstat->swq.swq_keeptime[q_num] = 0;   
		...
		{	
			...
			/* (Assoc number > 4 and TP > 200KB) or (5.5Mb < TP < 155Mb) */
			else if((assoc_num > assoc_num_low && pstat->tx_avarage >= tp_avg_low) || (687500 < pstat->tx_avarage && pstat->tx_avarage < 19375000)) {
				if(!pstat->swq.swq_en[q_num]) {
...
#ifdef CONFIG_RTL8672
				if(pstat->swq.swq_en[q_num]) {
					if(!rtl8192cd_swq_bdfull(priv, pstat, q_num, CURRENT_NUM_TX_DESC/2)) {
						change = 1;
					}
				} else {
					if(rtl8192cd_swq_bdfull(priv, pstat, q_num, 32)) {
						change = 2;
					}
				}
				...
				if(change) {
					if(pstat->swq.swq_en[q_num]) {
						...
						priv->pshare->swq_real_enque_pkt++;
						...
						rtl8192cd_swq_dequeue(priv, pstat, q_num, SWQ_DEQ_LIMIT, 0xffffffff);
...
#else
				/* 5Mb > TP || 160Mb < TP */
				if(assoc_num <= assoc_num_low && (pstat->tx_avarage < 625000 || 20000000 < pstat->tx_avarage)) {
					if(pstat->swq.swq_en[q_num]) {
						pstat->swq.swq_en[q_num] = 0;
						if (pstat->swq.swq_timer_id[q_num])
							rtl8192cd_swq_deltimer(priv, pstat, q_num);
	...
	if(change != 1) {
				...
				if (priv->pshare->rf_ft_var.dqnum) {
					if(qlen >= psta[0]->mu_deq_num && qlen2 >= psta[1]->mu_deq_num) {
						...
						while (1)
					    {
					    	...
					    	if(rtl8192cd_swq_bdfull(priv, pstat, q_num, 5)) {
								break;
					    	}
							if(count > 0 && count2 > 0) {
								if(count == count2) {
								...
								if(rtl8192cd_swq_bdfull(priv, pstat, q_num, sta_deq_num[0]+sta_deq_num[1])) {
									break;
					    		}
								rtl8192cd_swq_dequeue(priv, psta[0], q_num, SWQ_MU_DEQ_LIMIT, sta_deq_num[0]);
								rtl8192cd_swq_dequeue(priv, psta[1], q_num, SWQ_MU_DEQ_LIMIT, sta_deq_num[1]);
							} else if(count > 0) {
								rtl8192cd_swq_dequeue(priv, psta[0], q_num, SWQ_MU_DEQ_LIMIT, count);
								count = 0;
								if(drop_tmp != priv->ext_stats.tx_drops) {
									GDEBUG("drop\n");
								}
							} else if(count2 > 0) {
								rtl8192cd_swq_dequeue(priv, psta[1], q_num, SWQ_MU_DEQ_LIMIT, count2);
								count2 = 0;
								if(drop_tmp != priv->ext_stats.tx_drops) {
									GDEBUG("drop\n");
								}
						...
						priv->cnt_sta1_only++;
						rtl8192cd_swq_dequeue(priv, psta[0], q_num, SWQ_MU_DEQ_LIMIT, psta[0]->mu_deq_num);
					} else if(qlen2 >= (priv->pshare->rf_ft_var.dqnum)) {
						priv->cnt_sta2_only++;
						rtl8192cd_swq_dequeue(priv, psta[1], q_num, SWQ_MU_DEQ_LIMIT, psta[1]->mu_deq_num);
					}
				} else {
...
#endif
		{
			...
			if(need_deque) {
				if (pstat->swq.swq_timer_id[q_num])
					rtl8192cd_swq_deltimer(priv, pstat, q_num);
				if(rtl8192cd_swq_bdfull(priv, pstat, q_num, pstat->swq.q_aggnum[q_num] * 2)) {
#ifdef SW_TXQ_RSVD_DESC
					if (pstat->ADDBA_ready[pri])
		...
		{		   
			if(priv->pshare->swq_use_hw_timer) {
				...
				if(priv->pshare->swq_numActiveSTA > priv->pshare->rf_ft_var.swq_turbo_sta_th && (priv->up_time - priv->pshare->swq_turbo_time) < priv->pshare->rf_ft_var.swqmaxturbotime)
					tri_time = 20;
				else if (pstat->tx_avarage > 13750000)	/* 110Mb up */
					tri_time = priv->pshare->rf_ft_var.tri_time1;
				else if (pstat->tx_avarage > 1875000)	/* 15Mb ~ 110Mb */
					tri_time = priv->pshare->rf_ft_var.tri_time2;
				else if (pstat->tx_avarage > 625000)	/* 5Mb ~ 15Mb */
					tri_time = priv->pshare->rf_ft_var.tri_time3;
				else if (pstat->tx_avarage > 250000)	/* 2Mb ~ 5Mb */
					tri_time = priv->pshare->rf_ft_var.tri_time4;
				else if (pstat->tx_avarage > 100000)	/* 800Kb ~ 2Mb */
					tri_time = priv->pshare->rf_ft_var.tri_time5;
				else if (pstat->tx_avarage > 25000)		/* 200Kb ~ 800Kb */
				...
				if (pstat->tx_avarage > 1875000)     /* 15Mb up */
					tri_time = 1;
				else if (pstat->tx_avarage > 500000) /* 4Mb ~ 15Mb */
					tri_time = 30;
				else if (pstat->tx_avarage > 250000) /* 2Mb ~ 4Mb */
					tri_time = 90;
				else if (pstat->tx_avarage > 100000) /* 800Kb ~ 2Mb */
					tri_time = 120;			  
				else
			...
			if(need_dequeue) {  
#if (MU_BEAMFORMING_SUPPORT == 1)
				if(pstat->muPartner) {
					bdfull = rtl8192cd_swq_bdfull(priv, pstat, qnum, 6);
				} else {
				...
				if(bdfull) {
					...
					if(priv->pshare->swq_use_hw_timer) {
						...
						rtl8192cd_swq_dequeue(priv_this, pstat, qnum, SWQ_DEQ_LIMIT, 0xffffffff);
						pstat->swq.q_TOCount[qnum]++;						
					}
					else {
						pstat->swq.q_TOCount[qnum] += rtl8192cd_swq_dequeue(priv_this, pstat, qnum, SWQ_DEQ_LIMIT, 0xffffffff); 
					}
					...
					if(priv->pshare->swq_use_hw_timer)
						...
						current_time = jiffies;
#if defined(WIFI_LOGO_11N_4_2_47) && !defined(SMP_LOAD_BALANCE_SUPPORT)
					if(priv->is_ssid_ps && list_empty(&pstat->sleep_list) && qlen) {
						add_timer = 1; 
						if(priv->pshare->swq_use_hw_timer) {        
							add_timer_timeout = 1;   
							add_timer_timeout = current_time + 500; 
						}
					} else
#endif					
					if (qlen == 0) {
...
#ifdef RTK_ATM_EXT
__inline__ static int rtl8192cd_atm_swq_dequeue_ext(struct rtl8192cd_priv *priv);
#endif
...
void rtl8192cd_atm_swq_timeout(unsigned long task_priv)
{
	...
	//atm swq dequeue
#ifdef RTK_ATM_EXT
	if(priv->pshare->rf_ft_var.atm_ts_enable)
		rtl8192cd_atm_swq_dequeue_ext(priv);
	else
#endif
	...
	for(qnum = VO_QUEUE; qnum >= BK_QUEUE ; qnum--){
		for(i=0; i<NUM_STAT; i++) {
			if (priv->pshare->aidarray[i] && (priv->pshare->aidarray[i]->used == TRUE)){
				...
				if(rtl8192cd_swq_bdfull(priv, pstat, qnum, 4))
	...
	for(qnum = VO_QUEUE; qnum >= BK_QUEUE ; qnum--){
		...
		while(sta_cnt < NUM_STAT){
			...
			if (priv->pshare->aidarray[(i%NUM_STAT)] && (priv->pshare->aidarray[(i%NUM_STAT)]->used == TRUE)){
				...
				bd_full = rtl8192cd_swq_bdfull(priv, pstat, qnum, 4);
				if(!bd_full){					
					burst_num = (priv->pshare->rf_ft_var.atm_aggmax*pstat->atm_burst_size)/priv->pshare->atm_max_burstsize;
					if(burst_num < priv->pshare->rf_ft_var.atm_aggmin)
						burst_num = priv->pshare->rf_ft_var.atm_aggmin;
...
__inline__ static int atm_skb_dequeue_ext(struct rtl8192cd_priv *priv, struct stat_info *pstat, int qnum)
{	
	int count = 0, ret = 0;
	struct sk_buff *tmpskb;
	int packet_time = 0; //us
	...
	{        
		...
		if (pstat->atm_tx_rate != 0) {
			packet_time = (tmpskb->len*8)/pstat->atm_tx_rate; //us

			pstat->atm_tx_len_byte += tmpskb->len;
			pstat->atm_tx_time_us += packet_time;
		} else {
			printk("err : pstat->atm_tx_rate is zero~\n");
			break;
		}

		if (pstat->atm_time_slice >= packet_time) {	
			pstat->atm_time_slice -= packet_time;
		} else {
			pstat->atm_time_slice = 0;
		}	
...
__inline__ static int rtl8192cd_atm_swq_dequeue_ext(struct rtl8192cd_priv *priv)
{
	...
	for(qnum = VO_QUEUE; qnum >= BK_QUEUE ; qnum--){
		if (priv->pshare->atm_sta_idx[qnum-1] < NUM_STAT) {
			time_slice_reset = 0;
			break;
		}
	}
	...
	if (time_slice_reset)
	{
		...
		for(i=0; i<NUM_STAT; i++) {
			if (priv->pshare->aidarray[(i%NUM_STAT)] && (priv->pshare->aidarray[(i%NUM_STAT)]->used == TRUE)){
				...
				pstat->atm_time_slice = (priv->pshare->atm_timer*priv->pshare->rf_ft_var.atm_ts_ratio/100+priv->pshare->rf_ft_var.atm_ts_plus)*1000*pstat->atm_sta_time/100; // ms * 1000 * atm_time_per_sta : us

				//adjust by different speed standard
				if (pstat->atm_burst_num_adj_ratio != 0)
					pstat->atm_time_slice = (pstat->atm_time_slice*pstat->atm_burst_num_adj_ratio)/100;
			}
		}

		for(qnum = VO_QUEUE; qnum >= BK_QUEUE ; qnum--){
			priv->pshare->atm_sta_idx[qnum-1] = 0;
		}
	}
	
	for(qnum = VO_QUEUE; qnum >= BK_QUEUE ; qnum--){

		i = priv->pshare->atm_sta_idx[qnum-1];
		while(i < NUM_STAT){
			
			if (priv->pshare->aidarray[(i%NUM_STAT)] && (priv->pshare->aidarray[(i%NUM_STAT)]->used == TRUE)){
				pstat = &(priv->pshare->aidarray[(i%NUM_STAT)]->station);

				if (atm_skb_dequeue_ext(priv, pstat, qnum) == 1)
					return 0;	
			}

			//next
			i++;
			priv->pshare->atm_sta_idx[qnum-1] = i;
		}
	}
...
check_swq_enq:
	...
	if (swq_out == 0
		...
		|| (priv->pmib->dot11nConfigEntry.dot11nAMSDUSmallPkts &&
		   (skb->len <= priv->pmib->dot11nConfigEntry.dot11nAMSDUSmallPktLen || (IPV4_TCP_ACK_PACKET(skb->data) && pstat->amsdu_tcpack_en)))
#endif
		...
		unsigned long cur_queue_pkt = priv->pshare->swq_real_enque_pkt - priv->pshare->swq_real_deque_pkt;
		if (UINT32_DIFF(cur_queue_pkt, priv->pshare->swq_residual_drop_pkt) > priv->pshare->rf_ft_var.swq_max_enqueu_len) 
		{
			DEBUG_ERR("TX DROP: exceed the SW tx queue!\n");
			priv->ext_stats.tx_drops++;
			priv->pshare->swq_overflow_drop_pkt++;
			goto free_and_stop;
		} else {
			priv->pshare->swq_enque_pkt++;
		...
		for(temp = BK_QUEUE; temp <= VO_QUEUE; temp++)
		{
			if(pstat->swq.swq_empty[temp]) {				
				if (pstat->swq.swq_timer_id[temp])
					...
					rtl8192cd_swq_dequeue(priv, pstat, temp, SWQ_DEQ_ALL, 0);
			}
		}
		goto xmit_out;
	}
...
) //struct net_device *dev)
{
	...
	if ((OPMODE & WIFI_AP_STATE)
			...
			&& (!(skb->cb[47] & (BIT(FG_AGGRE_MSDU_FIRST)|BIT(FG_AGGRE_MSDU_MIDDLE)|BIT(FG_AGGRE_MSDU_LAST))))
#endif
		) {
#ifdef MCAST2UI_REFINE
...
//		printk("%s %d cb[47] %X\n", __func__, __LINE__, skb->cb[47]);
	...
	if (IS_SUPPORT_TX_AMSDU(priv) 
		...
		&& !block_mesh
#endif
		&& pstat && (pstat->amsdu_level > 0) &&	((pstat->aggre_mthd & AGGRE_MTHD_MSDU)
			...
			&& ((k = get_tx_sc_index(priv, pstat, skb->data)) >= 0))
	{
			...
			)
		{
...
#endif
			{
...
#ifdef TX_EARLY_MODE
					if (GET_TX_EARLY_MODE && GET_EM_SWQ_ENABLE) {
						bUnAvail = (getAvailableTXBD(priv, q_num) < (num_rsvd + num_swq_rsvd));
					}
					else
#endif
					{
						bUnAvail = (getAvailableTXBD(priv, q_num) < (needTxBD + num_swq_rsvd));
					}
				}
				else
#endif
				{
#ifdef TX_SCATTER
					bUnAvail = ((ptxsc_entry->has_desc3 && (getAvailableTXBD(priv, q_num) < 6)) ||
...
static int check_txdesc(struct rtl8192cd_priv *priv, struct tx_insn* txcfg)
{
	...
	if (txcfg->fr_type == _SKB_FRAME_TYPE_)
	{
			...
			)
		{
			...
			if ((!txcfg->is_test) && (((protocol == 0x888E) && ((GET_UNICAST_ENCRYP_KEYLEN == 0)
#ifdef WIFI_SIMPLE_CONFIG
