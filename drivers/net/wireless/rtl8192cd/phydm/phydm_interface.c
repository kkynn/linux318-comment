...
ODM_delay_ms(IN u4Byte	ms)
{
	...
	// All ODM functions is executed in process context, so we use msleep instead of mdelay to improve CPU loading
	if (!in_atomic() && ms >= (MSEC_PER_SEC / HZ))
		msleep(ms);
	else
#endif
	delay_ms(ms);
...
#elif(DM_ODM_SUPPORT_TYPE & ODM_AP)	
#if ((RTL8881A_SUPPORT == 1) || (RTL8192E_SUPPORT == 1) || (RTL8814A_SUPPORT == 1) || (RTL8822B_SUPPORT == 1) || (RTL8197F_SUPPORT == 1)) 
		if (pDM_Odm->SupportICType == ODM_RTL8881A || pDM_Odm->SupportICType == ODM_RTL8192E || pDM_Odm->SupportICType == ODM_RTL8822B || pDM_Odm->SupportICType & PHYDM_IC_3081_SERIES) 
