	...
	)
{
	...
	if(pDM_FatTable->AntDiv_OnOff != swch)
	{
		...
		else if(pDM_Odm->SupportICType & ODM_AC_ANTDIV_SUPPORT)
		{
			...
			if (pDM_Odm->SupportICType & (ODM_RTL8812 | ODM_RTL8822B)) {
	...
	)
{
	...
	} else if (pDM_Odm->SupportICType & (ODM_RTL8821 | ODM_RTL8822B)) {
...
)
{
	...
	if(pDM_Odm->mp_mode == TRUE)
	{
		...
		if(pDM_Odm->antdiv_select == ODM_FIX_MAIN_ANT)
			ODM_SetBBReg(pDM_Odm, 0xc50 , BIT9, 0);  // 0:CG, 1:CS
		else if(pDM_Odm->antdiv_select == ODM_FIX_AUX_ANT)
			ODM_SetBBReg(pDM_Odm, 0xc50 , BIT9, 1);  // 0:CG, 1:CS
		return;
	}
	#endif
	...
	)
{
	...
	/*CGCS Setting*/
	ODM_SetBBReg(pDM_Odm, 0xC1C , (BIT7|BIT6), 0); /*force to CG*/ 
	...
	/*GPIO Setting*/
	ODM_SetMACReg(pDM_Odm, 0x64 , (BIT18 | BIT17 | BIT16), 0); 
	ODM_SetMACReg(pDM_Odm, 0x44 , BIT25|BIT24, 0);	/*config P_GPIO[3:2] to data port*/
	ODM_SetMACReg(pDM_Odm, 0x44 , BIT17|BIT16, 0x3);	/*enable_output for P_GPIO[3:2]*/
	/*ODM_SetMACReg(pDM_Odm, 0x44 , BIT9|BIT8, 0);*/ /*P_GPIO[3:2] output value*/
	ODM_SetMACReg(pDM_Odm, 0x40 , BIT1|BIT0, 0);		/*GPIO function*/
	...
	/*Mapping table*/
	ODM_SetBBReg(pDM_Odm, 0xCA4 , bMaskDWord, 0x03020100);
	ODM_SetBBReg(pDM_Odm, 0xCA8 , bMaskDWord, 0x07060504);
	...
	ODM_SetMACReg(pDM_Odm, 0x7b4, (BIT17|BIT16), 0x3);	/*Reg7B4[16]=1 enable antenna training */
														/*Reg7B4[17]=1 enable  match MAC Addr*/
	ODM_SetMACReg(pDM_Odm, 0x7b4, 0xFFFF, 0);		/*Match MAC ADDR part1*/
	ODM_SetMACReg(pDM_Odm, 0x7b0, bMaskDWord, 0);	/*Match MAC ADDR part2*/
	...
	)
{
	...
	//Timming issue
	ODM_SetBBReg(pDM_Odm, 0x818 , BIT23|BIT22|BIT21|BIT20, 0); /*keep antidx after tx for ACK ( unit x 3.2 mu sec)*/
	ODM_SetBBReg(pDM_Odm, 0x8CC , BIT20|BIT19|BIT18, 3); //settling time of antdiv by RF LNA = 100ns
	...
	)
{
	...
	if((pDM_Odm->SupportICType & ODM_EVM_ENHANCE_ANTDIV_SUPPORT_IC))
	{
		if(pDM_Odm->bOneEntryOnly)
		{
			...
			{
				ODM_RT_TRACE(pDM_Odm,ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("> TH_H || EVM_method_en==1\n"));
...
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	ODM_RT_TRACE(pDM_Odm,ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("******AntDiv_Callback****** \n"));
	...
	)
{
	...
	ODM_RT_TRACE(pDM_Odm,ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("***AntDiv End***\n\n"));
}
	...
	)
{
	...
	for (i = 0; i <= (codeword_length-1); i++) {
		...
		if(pDM_Odm->SupportICType == ODM_RTL8821) {
			...
			reg44_tmp_p = reg44_ori & (~(BIT9|BIT8)); /*clean bit 9 & 8*/
			reg44_tmp_p |= ((1<<9) | (beam_ctrl_signal<<8));
			reg44_tmp_n = reg44_ori & (~(BIT9|BIT8));
	...
	)
{
	...
	/* [TRAINING STATE] */
		...
		pdm_sat_table->pre_fast_training_beam_num = pdm_sat_table->fast_training_beam_num;
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("[prepare state] Update Pre_Beam =(( %d ))\n", pdm_sat_table->pre_fast_training_beam_num));
	}
	/*  [Prepare State] */
	/*=======================================================================================*/
	...
	)
{
	...
	//2 [---Set TX Antenna---]
	if (pDM_FatTable->pForceTxAntByDesc == NULL)
	{
	...
	if (*(pDM_FatTable->pForceTxAntByDesc) == TRUE)
		odm_Tx_By_TxDesc_or_Reg(pDM_Odm, TX_BY_DESC);
	else
		odm_Tx_By_TxDesc_or_Reg(pDM_Odm, TX_BY_REG);
	...
	else if(pDM_Odm->SupportICType == ODM_RTL8822B)
	{
		...
		if (pDM_Odm->AntDivType == HL_SW_SMART_ANT_TYPE1) {
			...
			phydm_hl_smart_ant_type1_init_8822b(pDM_Odm);
		}
		#endif
	}
	#endif
	...
	)
{
	...
	#ifdef ODM_EVM_ENHANCE_ANTDIV
	if (pDM_Odm->bLinked) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV,ODM_DBG_LOUD,("TP_active_occur=((%d)), EVM_Method_enable=((%d))\n",
			pDM_Odm->tp_active_occur, pDM_FatTable->EVM_method_enable));
		
		if ((pDM_Odm->tp_active_occur == 1) && (pDM_FatTable->EVM_method_enable == 1)) {

			pDM_FatTable->idx_AntDiv_counter_5G = pDM_Odm->antdiv_period;
			pDM_FatTable->idx_AntDiv_counter_2G = pDM_Odm->antdiv_period;
		}
	}
	#endif
