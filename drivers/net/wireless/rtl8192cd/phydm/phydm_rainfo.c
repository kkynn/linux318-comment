...
)
{
	...
	pEntry = get_macidinfo(pDM_Odm->priv, macid);
	if(IS_STA_VALID(pEntry)) {
		if(rate_idx != pEntry->ra_cur)
			pEntry->ra_cnt++;
		
		pEntry->ra_cur = rate_idx;
		
		if(pEntry->ra_max < rate_idx)
			pEntry->ra_max = rate_idx;
		if(pEntry->ra_min > rate_idx)
			pEntry->ra_min = rate_idx;
	}
#endif
...
{
#if (DM_ODM_SUPPORT_TYPE == ODM_AP)
#if (RTL8812A_SUPPORT || RTL8881A_SUPPORT || RTL8192E_SUPPORT || RTL8814A_SUPPORT || RTL8197F_SUPPORT || RTL8822B_SUPPORT)
		...
		if (IS_STA_VALID(pstat)) {
			...
			if ((priv->pmib->dot11nConfigEntry.dot11nSTBC) &&
				...
				if(pstat->aggre_mthd == AGGRE_MTHD_MPDU_AMSDU)
					H2C_Parameter[4] = (16 & 0x7f) | (0<<8); 
				else
					H2C_Parameter[4] = (0 & 0x7f) | (0<<8); 
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD, ("RA_threshold_offset = (( %s%d ))\n", ((pRA_Table->RA_threshold_offset == 0) ? " " : ((pRA_Table->RA_offset_direction) ? "+" : "-")),pRA_Table->RA_threshold_offset));
