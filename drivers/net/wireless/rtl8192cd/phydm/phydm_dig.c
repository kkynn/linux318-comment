	...
	)
{
	...
	//1 Boundary Decision
	{
		...
		if ((((*pDM_Odm->pChannel >= 52) && (*pDM_Odm->pChannel <= 64)) ||
			((*pDM_Odm->pChannel >= 100) && (*pDM_Odm->pChannel <= 144)))
	...
	if(pDM_Odm->bLinked && bPerformance)
	{
		...
		ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("FA %d wifi_rx_pkts_2 %u wifi_rx_pkts_1 %u \n", pFalseAlmCnt->Cnt_all, priv->pshare->wifi_rx_pkts[2], priv->pshare->wifi_rx_pkts[0]));
		if (priv->assoc_num == 1 && ((UINT32_DIFF(priv->pshare->wifi_rx_pkts[2], priv->pshare->wifi_rx_pkts[0]) < priv->pshare->rf_ft_var.vwchk_rxpkt) || (priv->pshare->vw_dig_cnt))
#if (RTL8814A_SUPPORT == 1)
			&& !((pDM_Odm->SupportICType & ODM_RTL8814A) && priv->pshare->highTP_found_pstat && priv->pshare->highTP_found_pstat->current_tx_rate >= _NSS3_MCS7_RATE_)
#endif
			)
		{
			if (pFalseAlmCnt->Cnt_all <= priv->pshare->rf_ft_var.vwchk_fa) {
				if (CurrentIGI > (DM_DIG_MIN_AP + 2))
					pDM_DigTable->rx_gain_range_max = CurrentIGI-2;
				else 
					pDM_DigTable->rx_gain_range_max = DM_DIG_MIN_AP;
			} else {
				pDM_DigTable->rx_gain_range_max = CurrentIGI + 2;
			}
			ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("CurrentIGI %d, pDM_DigTable->rx_gain_range_max:%d\n", CurrentIGI, pDM_DigTable->rx_gain_range_max));
		} else 
#endif
	...
	if ((pDM_Odm->priv->pmib->miscEntry.manual_thru_mode == THRU_OPER_MODE_EXTREME) && (pDM_Odm->priv->pshare->active_thru_mode == THRU_OPER_MODE_EXTREME))		
	{	
		if (pDM_Odm->priv->pmib->miscEntry.fixed_igi > pDM_DigTable->rx_gain_range_max)
			CurrentIGI = pDM_DigTable->rx_gain_range_max;
		else
			CurrentIGI = pDM_Odm->priv->pmib->miscEntry.fixed_igi;
	}
...
//	if (pDM_Odm->bDualMacSmartConcurrent == FALSE)
	...
	if (pDM_Odm->bLinked) {
		...
		if ((pDM_Odm->priv->pmib->miscEntry.manual_thru_mode == THRU_OPER_MODE_MODERATE) || (pDM_Odm->priv->pmib->miscEntry.manual_thru_mode == THRU_OPER_MODE_EXTREME))
		{
			if ((pDM_Odm->RSSI_Min > pDM_Odm->priv->pmib->miscEntry.rssi_th) && (pDM_DigTable->TxTP >= pDM_Odm->priv->pmib->miscEntry.cckpd_txtp))
			{
				if (pDM_Odm->priv->pmib->miscEntry.manual_thru_mode == THRU_OPER_MODE_MODERATE)
				{
					ODM_Write1Byte(pDM_Odm, 0xA09, (ODM_Read1Byte(pDM_Odm,0xA09)&0xf0)|0xa);
					pDM_Odm->priv->pshare->active_thru_mode = THRU_OPER_MODE_MODERATE;
				}
				else
				{
					CurCCK_CCAThres = pDM_Odm->priv->pmib->miscEntry.cckpd_enhance;
					pDM_Odm->priv->pshare->active_thru_mode = THRU_OPER_MODE_EXTREME;				
				}
			}	
			else
			{
				pDM_Odm->priv->pshare->active_thru_mode=THRU_OPER_MODE_NORMAL;
				if (pDM_Odm->priv->pmib->miscEntry.manual_thru_mode == THRU_OPER_MODE_MODERATE)
					ODM_Write1Byte(pDM_Odm, 0xA09, (ODM_Read1Byte(pDM_Odm,0xA09)&0xf0)|0x3);							
			}
		}
#endif
	} else {
