...
#endif
	u8									date_rate = (pPktinfo->DataRate & 0x7f);

	if (date_rate <= ODM_RATE54M) {
		
		pPhydm->PhyDbgInfo.num_qry_legacy_pkt[date_rate]++;
		/**/	
	} else if (date_rate <= ODM_RATEMCS31) {
		
		pPhydm->PhyDbgInfo.num_qry_ht_pkt[date_rate - ODM_RATEMCS0]++;
		pPhydm->PhyDbgInfo.ht_pkt_not_zero = true;
			
	} 
	#if ODM_IC_11AC_SERIES_SUPPORT
	else if (date_rate <= ODM_RATEVHTSS4MCS9) {

		#if 0//(ODM_PHY_STATUS_NEW_TYPE_SUPPORT == 1)
		if ((pPhyStaRpt->gid != 0) && (pPhyStaRpt->gid != 63) && (pPhydm->SupportICType & ODM_IC_PHY_STATUE_NEW_TYPE)) {

			pPhydm->PhyDbgInfo.NumQryMuVhtPkt[date_rate - ODM_RATEVHTSS1MCS0]++;
			//pPhydm->PhyDbgInfo.num_of_ppdu[pPktinfo->ppdu_cnt] = date_rate | BIT(7);
			//pPhydm->PhyDbgInfo.gid_num[pPktinfo->ppdu_cnt] = pPhyStaRpt->gid;
			
		} else 
		#endif
		{
			pPhydm->PhyDbgInfo.num_qry_vht_pkt[date_rate - ODM_RATEVHTSS1MCS0]++;
			pPhydm->PhyDbgInfo.vht_pkt_not_zero = true;
			#if (ODM_PHY_STATUS_NEW_TYPE_SUPPORT == 1)
			//pPhydm->PhyDbgInfo.num_of_ppdu[pPktinfo->ppdu_cnt] = date_rate;
			//pPhydm->PhyDbgInfo.gid_num[pPktinfo->ppdu_cnt] = pPhyStaRpt->gid;
			#endif
		}
	}
...
//1 All platforms support
	...
	if(pDM_Odm->SupportICType == ODM_RTL8814A) 
	{
		if(pDM_Odm->RFEType == 0)
			...
			READ_AND_CONFIG_MP(8814A,_TxPowerTrack_Type10);
		else
...
*/
		if ((pPhyStaRpt->gid != 0) && (pPhyStaRpt->gid != 63)) {
			if (((pPktinfo->DataRate & 0x7f) >= ODM_RATEVHTSS1MCS0) && ((pPktinfo->DataRate & 0x7f) <= ODM_RATEVHTSS4MCS9))
	...
	)
{
	...
	phydm_rx_rate_for_soml(pDM_Odm, pPktinfo);
#endif
	if (pPktinfo->bPacketToSelf || pPktinfo->bPacketBeacon) {
		u4Byte RSSI_linear = 0;
...
*/
	...
	if (pPktinfo->bPacketMatchBSSID) {
		phydm_rx_statistic_cal(pPhydm, pPhyStatus, pPktinfo);
	}
	#if RTL8723D_SUPPORT == 1
