...
//	tx_power_control(priv, NULL, FALSE);
	...
	if (priv->pshare->rf_ft_var.use_ext_lna && GET_CHIP_VER(priv) == VERSION_8192E) {
		...
		PHY_SetBBReg(priv, 0xa2c, 0x010000, 0x0);
	}
#endif
#endif
...
//#endif
	...
	if (GET_CHIP_VER(priv) == VERSION_8192E) {
		...
		{
			/* reset to default */
			PHY_SetBBReg(priv, 0xd3c, 0x000001C0, 0x5);
			PHY_SetBBReg(priv, 0xc40, BIT(9), 0);
			PHY_SetBBReg(priv, 0xc40, 0x1F000000, 0x1F);

			if (channel == 13)
				PHY_SetBBReg(priv, 0xc40, BIT(9), 1);

			if ((priv->pshare->swr_mode == SPS_MODE) &&
				(priv->pshare->_dmODM.AntDivType == CGCS_RX_HW_ANTDIV))
			{
				if (channel == 13) {                
					PHY_SetBBReg(priv, 0xd3c, 0x000001C0, 0x2);
				} else if (channel == 12) {                
					PHY_SetBBReg(priv, 0xd18, BIT(27), 1);
					PHY_SetBBReg(priv, 0xd2C, BIT(28), 1);
					PHY_SetBBReg(priv, 0xd40, BIT(26), 1);
...
void mp_pmac_tx(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	if(is_CCK_rate(PMacTxInfo.TX_RATE))	{
		...
		if ((GET_CHIP_VER(priv) == VERSION_8814A)||(GET_CHIP_VER(priv) == VERSION_8822B))
			...
			PHY_SetBBReg(priv, 0xf18, 0xffff0000, u4bTmp);
...
#ifdef EN_EFUSE
#if defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8197F)
void mp_kfree(struct rtl8192cd_priv *priv, unsigned char *data)
{
	char *value;
	u8 val,valb;
	u32 res,resb;
	...
	if (value) {
		if (priv->pshare->mp_kfree_on)
			return;
		
		priv->pshare->mp_kfree_on = 1;
		efuse_kfree(priv, 0);			
	...
	if (value) {
		if (priv->pshare->mp_kfree_on == 0)
			return;
		priv->pshare->mp_kfree_on = 0;
		efuse_kfree(priv, 1); 		
...
static void mp_chk_sw_ant(struct rtl8192cd_priv *priv)
{
...
#ifdef __OSK__
		r_ant_select_cck_val = r_ant_select_cck_val | 0x05;
		//printk("RX B p_cck_txrx->r_cckrx_enable:%d, r_ant_select_cck_val:%d\n", p_cck_txrx->r_cckrx_enable, r_ant_select_cck_val);
#endif
		chgRx = 1;
	...
	case ANTENNA_AB:	// For 8192S and 8192E/U...
		...
		p_cck_txrx->r_cckrx_enable_2= 0x1;		// option:B
#ifdef __OSK__
		r_ant_select_cck_val = r_ant_select_cck_val | 0x01;
#endif
		chgRx = 1;
		break;
	}
