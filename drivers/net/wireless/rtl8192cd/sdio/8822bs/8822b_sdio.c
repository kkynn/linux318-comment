...
#if defined(CONFIG_XMITBUF_TXAGG_ADV) || defined(SDIO_STATISTICS)
...
void set_ap_32k(struct rtl8192cd_priv *priv, BOOLEAN en_32K)
{
	u4Byte PreRpwmVal =0 ;
	u4Byte PreCpwmVal = 0;
	u4Byte PreCpwmVal_org = 0;
	u1Byte wait_times = 0;

	sdio_local_read(priv, (SDIO_REG_HCPWM1-1), 4, (u8*)&PreCpwmVal_org);
	PreCpwmVal_org = le32_to_cpu(PreCpwmVal_org);
	//printk("[%s,%d] : PreCpwmVal_org=0x%08x\n", __FUNCTION__, __LINE__, PreCpwmVal_org);

	/* set rpwm */
	sdio_local_read(priv, SDIO_REG_HRPWM1, 4, (u8*)&PreRpwmVal);
	PreRpwmVal = le32_to_cpu(PreRpwmVal);
	...
	PreRpwmVal = cpu_to_le32(PreRpwmVal);
	sdio_local_write(priv, SDIO_REG_HRPWM1, 4, (u8*)&PreRpwmVal);
	...
	while (wait_times <= 50) {
		sdio_local_read(priv, (SDIO_REG_HCPWM1-1), 4, (u8*)&PreCpwmVal);
		PreCpwmVal = le32_to_cpu(PreCpwmVal);
		//printk("[%s,%d] : PreCpwmVal=0x%08x\n", __FUNCTION__, __LINE__, PreCpwmVal);
		if (((PreCpwmVal&BIT(15)) ^(PreCpwmVal_org&BIT(15))) == BIT(15))
			break;
		udelay(500);
		wait_times++;
	}
	...
	do {
		...
		if (pHalData->SdioTxIntStatus & BIT(SDIO_TX_INT_SETUP_TH))
		{
			if (sdio_hisr & SDIO_HISR_AVAL)
			{
				...
				HalQueryTxOQTBufferStatus8822BSdio(priv);
				...
				if (test_and_clear_bit(SDIO_TX_INT_SETUP_TH, &pHalData->SdioTxIntStatus)) {
					...
					RTL_W16(reg_freepage_thres[pHalData->SdioTxIntQIdx], 0xFFFF);
					
					HalQueryTxBufferStatus8822BSdio2(priv);
		...
		else if (pshare->offload_function_ctrl)
		{
					...
					&& (_rtw_queue_empty(&pshare->pending_xmitbuf_queue) == FALSE))
			{
				...
				if (!pHalData->WaitSdioTxOQT
						|| rtw_sdio_check_TxOQT_space(priv, pHalData->WaitSdioTxOQTQNum,
...
#ifdef CONFIG_POWER_SAVE
	if (priv->pshare->ap_ps_handle.suspend_processing) {
...
//
u8 HalQueryTxOQTBufferStatus8822BSdio(struct rtl8192cd_priv *priv)
{
	HAL_INTF_DATA_TYPE *pHalData = GET_HAL_INTF_DATA(priv);
	u16 oqtFreeSpace;
...
//
s32 HalQueryTxBufferStatus8822BSdio(struct rtl8192cd_priv *priv)
{
	HAL_INTF_DATA_TYPE *pHalData = GET_HAL_INTF_DATA(priv);
...
#ifdef CONFIG_SDIO_TX_IN_INTERRUPT
s32 HalQueryTxBufferStatus8822BSdio2(struct rtl8192cd_priv *priv)
{
	struct priv_shared_info *pshare = priv->pshare;
	HAL_INTF_DATA_TYPE *pHalData = GET_HAL_INTF_DATA(priv);
	unsigned long seq;
...
//	NumOfFreePage = SdioLocalCmd53Read4Byte(priv, SDIO_REG_FREE_TXPG);
	...
	if ((!pshare->freepage_updated) && (seq == pshare->freepage_updated_seq)) {
...
#if 0
	printk("%s: Free page for HIQ(%#x),MIQ(%#x),LOQ(%#x),EXQ(%#x),PUBQ(%#x),Update(%d)\n",
			__FUNCTION__,
			pHalData->SdioTxFIFOFreePage[HI_QUEUE_IDX],
			pHalData->SdioTxFIFOFreePage[MID_QUEUE_IDX],
			pHalData->SdioTxFIFOFreePage[LOW_QUEUE_IDX],
