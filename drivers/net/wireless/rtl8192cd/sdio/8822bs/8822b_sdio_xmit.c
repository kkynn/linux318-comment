...
static int rtw_sdio_wait_TxOQT_space(struct rtl8192cd_priv *priv, u8 q_num, u8 agg_num)
{
	...
	while (pHalData->SdioTxOQTFreeSpace[oqt_idx] < agg_num) {
		if (TRUE == priv->pshare->bSurpriseRemoved) {
...
static int rtw_sdio_try_wait_TxOQT_space(struct rtl8192cd_priv *priv, u8 q_num, u8 agg_num)
{
	...
	if (pHalData->SdioTxOQTFreeSpace[oqt_idx] < agg_num) {
		HalQueryTxBufferStatus8822BSdio(priv);
		if (pHalData->SdioTxOQTFreeSpace[oqt_idx] < agg_num) {
...
#ifdef SDIO_AP_OFFLOAD
  // if((GET_ROOT(priv)->offload_function_ctrl & RTW_PM_PREPROCESS)) 
	{
 	   //printk("hal_fill_bcn_desc set beacon HW SN=%d txLength=%d priv->tx_beacon_len=%d forceUpdate=%d\n ",GetSequence(dat_content),txLength,priv->tx_beacon_len,forceUpdate);
...
void ap_offload_process(struct rtl8192cd_priv *priv, unsigned int frlen)
{
	...
	pdesc->Dword4 &= ~ set_desc(TX_DW4_DATA_RT_LMT_MSK << TX_DW4_DATA_RT_LMT_SH);
	pdesc->Dword4 |= set_desc((1 << TX_DW4_RTY_LMT_EN_SH) | (6 << TX_DW4_DATA_RT_LMT_SH));
	...
	signin_beacon_desc(priv, pshare->offload_buf, total_len);
