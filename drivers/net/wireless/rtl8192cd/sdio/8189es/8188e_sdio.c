...
void DisableApOffload(struct rtl8192cd_priv *priv)
{
	u32 PreRpwmVal ;
	PreRpwmVal = cpu_to_le32(0);
	PreRpwmVal = sdio_local_read(priv, SDIO_REG_HRPWM1,4,&PreRpwmVal);
	PreRpwmVal = PreRpwmVal ^ BIT(31);
	PreRpwmVal = PreRpwmVal & ~BIT(16);
...
#if 0
	printk("%s: Free page for HIQ(%#x),MIQ(%#x),LOQ(%#x),PUBQ(%#x),Update(%d)\n",
			__FUNCTION__,
			pHalData->SdioTxFIFOFreePage[HI_QUEUE_IDX],
			pHalData->SdioTxFIFOFreePage[MID_QUEUE_IDX],
			pHalData->SdioTxFIFOFreePage[LOW_QUEUE_IDX],
