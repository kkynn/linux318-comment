...
#endif	
		{
			DEBUG_ERR("NULL Wlan Interrupt !?\n");
			DEBUG_ERR("Reg_HISR0:	%08x, Reg_HISR1:	%08x\n", RTL_R32(REG_HISR0), RTL_R32(REG_HISR1));//should back up halisr to printk
			DEBUG_ERR("Reg_HIMR0:	%08x, Reg_HISM1:	%08x\n", RTL_R32(0xb0), RTL_R32(0xb8));
			DEBUG_ERR("IntMask0:	%08x, IntMask1:	%08x\n",_GET_HAL_DATA(priv)->IntMask[0], _GET_HAL_DATA(priv)->IntMask[1]);
		}
...
#ifdef DEBUG_DMA_MAP
int map_cnt[TOTAL_MAP_TYPE+1];
int map_total_size[TOTAL_MAP_TYPE+1];
int unmap_cnt[TOTAL_MAP_TYPE+1];
int unmap_total_size[TOTAL_MAP_TYPE+1];
#endif /* DEBUG_DMA_MAP end*/
...
#ifdef DEBUG_DMA_MAP
	memset(map_cnt,sizeof(map_cnt),0);
	memset(map_total_size,sizeof(map_total_size),0);
	memset(unmap_cnt,sizeof(unmap_cnt),0);
	memset(unmap_total_size,sizeof(unmap_total_size),0);
#endif /*DEBUG_DMA_MAP  end*/
...
&& (GET_VXD_PRIV(priv)->simple_config_status==0)
...
#endif
	{
		...
		/* To set the DMA address for both RX/TX ring */
		{
			...
			for (i=0; i<NUM_RX_DESC_IF(priv); i++) {
#ifdef DEBUG_DMA_MAP
				phw->rx_descL_dma_addr[i] = get_physical_addr_1(priv, (void *)(&phw->rx_descL[i]),
					sizeof(struct rx_desc), PCI_DMA_TODEVICE,RX_MAP_TYPE);
#else
			...
			while (tx_desc_array[txDescRingIdx] != 0) {
				...
				for (i=0; i<CURRENT_NUM_TX_DESC; i++) {
#ifdef DEBUG_DMA_MAP
					tx_desc_dma_ptr[i] = get_physical_addr_1(priv, (void *)(&(tx_desc_ptr[i])),
						sizeof(struct tx_desc), PCI_DMA_TODEVICE,TX_MAP_TYPE);
#else
...
*/
			...
			init_WMM_Para_Element(priv, priv->pmib->dot11QosEntry.WMM_IE, sizeof(priv->pmib->dot11QosEntry.WMM_IE));  //  WMM STA
		}
#endif
...
//------------------------- david+2007-5-31
	...
	if (GET_CHIP_VER(priv) == VERSION_8814A) {
			...
			if(priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_5G) {
				...
				priv->pmib->dot11RFEntry.MIMO_TR_mode = MIMO_2T2R; // 2T2R
			}
		#endif	
...
/* No idea how to process AMSDU shortcut with MESH or WDS, so disable AMSDU shortcut when MESH or WDS enabled */
    ...
    if(STACTRL_SHM_SUPPORT) {
        if(priv->pshare->stactrl_groupID_used[priv->stactrl.stactrl_id] != priv->vap_id + 1) {
            priv->pmib->staControl.stactrl_enable = 0;
        }
        stactrl_init(priv);
    }
#else
...
#ifdef DMA_MAP_REFINE
//map debug
static int free_txdesc( struct rtl8192cd_priv   *priv , int tx_queue )
{
	...
	unsigned int                halQnum = GET_HAL_INTERFACE(priv)->MappingTxQueueHandler(priv, tx_queue);
	head = GET_HAL_INTERFACE(priv)->GetTxQueueHWIdxHandler(priv, tx_queue);
	tail = ptx_dma->tx_queue[halQnum].hw_idx;
    
	pdescinfoH	= get_txdesc_info(priv->pshare->pdesc_info, tx_queue);
	while (CIRC_CNT_RTK(head, tail, CURRENT_NUM_TX_DESC ))
	{
		...
		if (IS_PCIBIOS_TYPE) {
			...
			{				
#ifdef DEBUG_DMA_MAP
				rtl_dbg_pci_unmap_single(priv->pshare->pdev, pdescinfo->paddr, pdescinfo->len, PCI_DMA_TODEVICE,TX_MAP_TYPE);
#else
...
#ifdef DEBUG_DMA_MAP
				rtl_dbg_pci_unmap_single(priv->pshare->pdev,
...
//					rtl_kfree_skb(pdescinfo->priv, (struct sk_buff *)(pdescinfo->pframe), _SKB_TX_IRQ_);
		...
		for (cnt =0; cnt<WLAN_HAL_TX_AMSDU_MAX_NUM; cnt++) {
			if (pdescinfo->buf_type_amsdu[cnt] == _SKB_FRAME_TYPE_)
			{
				...
				if (IS_PCIBIOS_TYPE) {
#ifdef DEBUG_DMA_MAP
					rtl_dbg_pci_unmap_single(priv->pshare->pdev, pdescinfo->buf_paddr_amsdu[cnt], pdescinfo->buf_len_amsdu[cnt], PCI_DMA_TODEVICE,TX_MAP_TYPE);
#else
...
//					rtl_kfree_skb(pdescinfo->priv, (struct sk_buff *)(pdescinfo->pframe), _SKB_TX_IRQ_);
		...
		if (RSVQ_ENABLE && IS_RSVQ(tx_queue))
			pdescinfo->priv->use_txdesc_cnt[RSVQ(tx_queue)]--;
#endif
		...
		for (i=0; i<NUM_RX_DESC_IF(priv); i++)
		{
			if (phw->rx_infoL[i].pbuf != NULL) {
				...
				if (IS_PCIBIOS_TYPE)
				{
					#ifdef DMA_MAP_REFINE
						#ifdef DEBUG_DMA_MAP
							rtl_dbg_pci_unmap_single(priv->pshare->pdev, phw->rx_infoL[i].paddr, phw->rx_infoL[i].map_len , PCI_DMA_FROMDEVICE,RX_MAP_TYPE);
						#else
							pci_unmap_single(priv->pshare->pdev, phw->rx_infoL[i].paddr, phw->rx_infoL[i].map_len , PCI_DMA_FROMDEVICE);
						#endif /*DEBUG_DMA_MAP*/
					#else
						#ifdef DEBUG_DMA_MAP
							rtl_dbg_pci_unmap_single(priv->pshare->pdev, phw->rx_infoL[i].paddr, (RX_BUF_LEN - sizeof(struct rx_frinfo)), PCI_DMA_FROMDEVICE,RX_MAP_TYPE);
						#else
...
int max_qnum = HIGH_QUEUE;
		for (tx_queue=0; tx_queue<=max_qnum; tx_queue++) {
			free_txdesc(priv,tx_queue);
		}
#else  //DMA_MAP_REFINE   // TX unmap ¨SÄÀ©ñ°®²b
		for (i=0; i<CURRENT_NUM_TX_DESC; i++)
		{
			// free tx queue skb
			struct tx_desc_info *tx_info;
...
//				if (i <tail || i >= head)
                ...
                if (IS_HAL_CHIP(priv)) {
    				if (tx_info[i].buf_pframe[0] && (tx_info[i].buf_type[0] == _SKB_FRAME_TYPE_)) { // should be buf_paddr
    					...
    					if (IS_PCIBIOS_TYPE && (tx_info[i].buf_len[0]!=0 && tx_info[i].buf_paddr[0]!=0))
    						rtl_dbg_pci_unmap_single(priv->pshare->pdev, tx_info[i].buf_paddr[0],(tx_info[i].buf_len[0])&0xffff, PCI_DMA_TODEVICE,TX_MAP_TYPE);
#else
    					if (IS_PCIBIOS_TYPE && (tx_info[i].buf_len[0]!=0 && tx_info[i].buf_paddr[0]!=0))
...
#ifdef DEBUG_DMA_MAP
    					if (IS_PCIBIOS_TYPE )
    						rtl_dbg_pci_unmap_single(priv->pshare->pdev, tx_info[i].paddr, (tx_info[i].len), PCI_DMA_TODEVICE,TX_MAP_TYPE);
#else
    					if (IS_PCIBIOS_TYPE)
		...
		if (IS_HAL_CHIP(priv)) {
		    if (IS_PCIBIOS_TYPE) {
				...
				#ifdef DMA_MAP_REFINE 
					#ifdef DEBUG_DMA_MAP
										rtl_dbg_pci_unmap_single(priv->pshare->pdev, get_desc(cur_txbd->TXBD_ELE[1].Dword1), get_desc(cur_txbd->TXBD_ELE[1].Dword0), PCI_DMA_TODEVICE,BEACON_MAP_TYPE);
					#else
										pci_unmap_single(priv->pshare->pdev, get_desc(cur_txbd->TXBD_ELE[1].Dword1), get_desc(cur_txbd->TXBD_ELE[1].Dword0) , PCI_DMA_TODEVICE);
					#endif /*DEBUG_DMA_MAP*/
				#else
					#ifdef DEBUG_DMA_MAP
										rtl_dbg_pci_unmap_single(priv->pshare->pdev, get_desc(cur_txbd->TXBD_ELE[1].Dword1), 128*sizeof(unsigned int), PCI_DMA_TODEVICE,BEACON_MAP_TYPE);
					#else
...
#endif
		{
		    ...
		    if (IS_PCIBIOS_TYPE) {
				...
				if(GET_CHIP_VER(priv)== VERSION_8812E) {
					...
					#endif	
#ifdef DEBUG_DMA_MAP
					rtl_dbg_pci_unmap_single(priv->pshare->pdev, get_desc(phw->tx_descB->Dword10),
							128*sizeof(unsigned int), PCI_DMA_TODEVICE,BEACON_MAP_TYPE);
#else
...
#endif
				{
					...
					#endif	
#ifdef DEBUG_DMA_MAP
					rtl_dbg_pci_unmap_single(priv->pshare->pdev, get_desc(phw->tx_descB->Dword8), 128*sizeof(unsigned int), PCI_DMA_TODEVICE,BEACON_MAP_TYPE);
#else
...
//			priv->pmib->dot11OperationEntry.keep_rsnie = 1;
    ...
    if (!priv->reset_hangup)
        ...
        if(STACTRL_SHM_SUPPORT) {
            if(priv->pshare->stactrl_groupID_used[priv->stactrl.stactrl_id] == priv->vap_id + 1) {
        stactrl_deinit(priv);
                priv->pshare->stactrl_groupID_used[priv->stactrl.stactrl_id] = -1;
            }
    }
#else
        stactrl_deinit(priv);
...
#ifdef DEBUG_DMA_MAP
	//printk("\n=============================statistic DEBUG_DMA_MAP \n=============================\n");
	//printk("rtl8192cd_stop_sw   map_cnt=%d   tx_map_cnt=%d   rx_map_cnt=%d   beacon_map_cnt=%d beacon_txBD_cnt=%d\n",    map_cnt ,   tx_map_cnt ,   rx_map_cnt ,   beacon_map_cnt , beacon_txBD_cnt );
	//printk("rtl8192cd_stop_sw unmap_cnt=%d tx_unmap_cnt=%d rx_unmap_cnt=%d beacon_unmap_cnt=%d\n",  unmap_cnt , tx_unmap_cnt , rx_unmap_cnt , beacon_unmap_cnt  );
	//DMA_PRINTK("rtl8192cd_stop_sw map_total_size=%d  unmap_total_size=%d\n", map_total_size , unmap_total_size );

	printk("       map_cnt=%d                   map_total_size=%d\n", map_cnt[TOTAL_MAP_TYPE]       , map_total_size[TOTAL_MAP_TYPE] );
	printk("    tx_map_cnt=%d                tx_map_total_size=%d\n", map_cnt[TX_MAP_TYPE]          , map_total_size[TX_MAP_TYPE] );
	printk("beacon_map_cnt=%d            beacon_map_total_size=%d\n", map_cnt[BEACON_MAP_TYPE]      , map_total_size[BEACON_MAP_TYPE] );
	printk("    rx_map_cnt=%d                rx_map_total_size=%d\n", map_cnt[RX_MAP_TYPE]          , map_total_size[RX_MAP_TYPE] );
	printk("bus1_tx_map_cnt=%d          bus1_tx_map_total_size=%d\n", map_cnt[BUS1_TX_MAP_TYPE]     , map_total_size[BUS1_TX_MAP_TYPE] );
	printk("bus1_rx_map_cnt=%d          bus1_rx_map_total_size=%d\n", map_cnt[BUS1_RX_MAP_TYPE]     , map_total_size[BUS1_RX_MAP_TYPE] );
	printk("bus1_beacon_map_cnt=%d  bus1_beacon_map_total_size=%d\n", map_cnt[BUS1_BEACON_MAP_TYPE] , map_total_size[BUS1_BEACON_MAP_TYPE] );
	printk("unknow_map_cnt=%d            unknow_map_total_size=%d\n", map_cnt[UNKNOW_MAP_TYPE]      , map_total_size[UNKNOW_MAP_TYPE] );

	printk("       unmap_cnt=%d         unmap_total_size=%d\n", unmap_cnt[TOTAL_MAP_TYPE]  , unmap_total_size[TOTAL_MAP_TYPE] );
	printk("    tx_unmap_cnt=%d      tx_unmap_total_size=%d\n", unmap_cnt[TX_MAP_TYPE]     , unmap_total_size[TX_MAP_TYPE] );
	printk("beacon_unmap_cnt=%d  beacon_unmap_total_size=%d\n", unmap_cnt[BEACON_MAP_TYPE] , unmap_total_size[BEACON_MAP_TYPE] );
	printk("    rx_unmap_cnt=%d      rx_unmap_total_size=%d\n", unmap_cnt[RX_MAP_TYPE]     , unmap_total_size[RX_MAP_TYPE] );
	printk("unknow_unmap_cnt=%d  unknow_unmap_total_size=%d\n", unmap_cnt[UNKNOW_MAP_TYPE] , unmap_total_size[UNKNOW_MAP_TYPE] );
...
int rtl8192cd_open(struct net_device *dev)
{
    ...
    unsigned char opmode_buf[4];
             char dev_name_buf[16];
    unsigned char drv_state_buf;
#endif
...
#if defined(STA_CONTROL) && defined(STA_CONTROL_AIPC)
    priv->stactrl.stactrl_id = priv->pmib->staControl.stactrl_groupID;
    if (STACTRL_SHM_SUPPORT &&
        priv->pshare->stactrl_groupID_used[priv->stactrl.stactrl_id] == -1 && priv->pmib->staControl.stactrl_enable) {

        priv->pshare->stactrl_groupID_used[priv->stactrl.stactrl_id] = priv->vap_id + 1;
    }
#endif // STA_CONTROL && STA_CONTROL_AIPC
...
priv->drv_state |= DRV_STATE_OPEN;      // set driver as has been opened, david
    ...
    if(STACTRL_SHM_SUPPORT) {
        if(priv->pshare->stactrl_groupID_used[priv->stactrl.stactrl_id] == priv->vap_id + 1) {
            drv_state_buf = 1;
            aipc_wlan_set(STACTRL_WLAN_ID, &drv_state_buf, STACTRL_SHM_DRV_STATE, sizeof(drv_state_buf), 0);
        }
    }
#endif // STA_CONTROL && STA_CONTROL_AIPC
...
static void MDL_DEVINIT set_mib_default(struct rtl8192cd_priv *priv)
{
	...
	strncpy((char *)priv->pmib->dot11StationConfigEntry.dot11DesiredSSID, "RTL8186-default", 32);
...
//Only ONE SLOT, always enable HIGH Power 
...
#if 0
	if (GET_CHIP_VER(priv)==VERSION_8192E) {
	    priv->pmib->dot11nConfigEntry.dot11nAMPDURevSz = 42;
	}
#endif
	...
	if(is_11AC_Chips(priv) || GET_CHIP_VER(priv) == VERSION_8192E)
		AMSDU_ENABLE = 2;
	else
		AMSDU_ENABLE = 0;

	if (RX_BUF_LEN > 3800) { //size more than amsdu size, it could recevie bigger packets in one rx-buffer 
...
/* Adaptivity */
...
#ifdef CONFIG_RTL8672
	priv->pshare->rf_ft_var.dig_cov_enable = 2;
#endif
	...
	if (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G) {
		...
		if (GET_CHIP_VER(priv) == VERSION_8814A) {
			...
			/* Enable it for Intel 8260/8265, PCIE AC68 */
			priv->pmib->dot11nConfigEntry.dot11nAMSDUTCPAckDropDup = 1;
			/* Enable it for Intel 8260/8265, PCIE AC68 */
			priv->pmib->dot11nConfigEntry.dot11nAMSDUSmallPkts = 0;
			/* This value can be adjusted to get better performance */
			// priv->pmib->dot11nConfigEntry.dot11nAMSDUSendTimeout = 15000;
		}
	}
#endif
...
#endif // CONFIG_NET_PCI
	{
        ...
        if (((priv->pshare->type>>TYPE_SHIFT) & TYPE_MASK) == TYPE_PCI_DIRECT)
        {
            ...
            if (BSP_PCIE0_D_CFG0 == wdev->conf_addr)
            {
                ...
                #endif				
                printk("CFG0: wdev->conf_addr:%08x,base_addr:%08x\n", wdev->conf_addr, wdev->base_addr);
            ...
            else if (BSP_PCIE1_D_CFG0 == wdev->conf_addr)
			{
                ...
                printk("CFG1 wdev->conf_addr:%08x,base_addr:%08x\n", wdev->conf_addr, wdev->base_addr);
...
#define BSP_RTL8198_REVISION_B	0xC0000001 
            ...
            if (GET_CHIP_VER(priv)==VERSION_8192E) {
                ...
                if (PHY_QueryBBReg(priv,0xf0,BIT(23)) == 0) {
                    ...
                    REG32(wdev->conf_addr + 0x78)= 0x00100010;	// burst size        
                    #else
                    if (((RTL_R32(0xf0) >> 12 ) & 0xf) == 0){ 			

                        unsigned int tmp_reg = REG32(wdev->conf_addr + 0x78);
...
#if defined(CONFIG_WLAN_HAL_8197F) 
            if(REG32(0xB8000000)== 0x8197F000)
            {
...
register_driver:
...
#if defined(CONFIG_RG_WLAN_HWNAT_ACCELERATION) && !defined(CONFIG_ARCH_LUNA_SLAVE)
#if defined(RTK_NL80211)
	if (dev->name[0] == 't' && dev->name[1] == 'm' && dev->name[2] == 'p')  //dev->name="tmp.wlanX"
	{
		printk("#### %s (scan) = %x ####\n", dev->name, (u32)dev);
	...
	else if (dev->name[strlen(dev->name)-2] == '-' && dev->name[strlen(dev->name)-1] != '4')  //dev->name="wlanX-X"
	{
		switch (dev->name[strlen(dev->name)-1])
		{
			case '1':
				printk("#### %s (vap1) = %x ####\n", dev->name, (u32)dev);
				if (dev->name[strlen(dev->name)-3] == '0')
	...
	{
		int vap_netdev_idx = dev->name[strlen(dev->name)-1]-'0';
		if(dev->name[strlen(dev->name)-6]=='0')
			wlan_vap_netdev[vap_netdev_idx]=dev;
		else
			wlan1_vap_netdev[vap_netdev_idx]=dev;
	}
#endif
#ifdef CONFIG_OPENWRT_SDK
	else if (dev->name[strlen(dev->name)-2] == '-' && dev->name[strlen(dev->name)-1] == '4')  //dev->name="wlanX-4"
	{
		printk("#### %s (vxd) = %x ####\n", dev->name, (u32)dev);
		if (dev->name[strlen(dev->name)-3] == '0')
			wlan_vxd_netdev = dev;
	...
	{
		printk("#### vxd = %x ####\n", (u32)dev);
		if (dev->name[strlen(dev->name)-5] == '0')
			wlan_vxd_netdev = dev;
		else
			wlan1_vxd_netdev = dev;
	}
#endif
	else if ((dev->name[strlen(dev->name)-2] == 'n') && (strlen(dev->name) == 5))  //dev->name="wlanX"
	{
		printk("#### root = %x ####\n", (u32)dev);
		if (dev->name[strlen(dev->name)-1] == '0')
...
#endif
	{
		...
		for (i=0; i<wds_num; i++) {
			...
			dev->name[sizeof(dev->name)-1] = '\0';
			strncat(dev->name, "-wds%d", sizeof(dev->name)-strlen(dev->name)-1);
...
#ifdef STA_CONTROL_AIPC
        if(IS_ROOT_INTERFACE(priv)) {
            for(stactrl_i = 0; stactrl_i < STA_CONTROL_SUPPORT_NUM; stactrl_i++) {
                priv->pshare->stactrl_groupID_used[stactrl_i] = -1;
            }
        }
#endif  
#endif
...
#if defined(NOT_RTK_BSP)
				_GET_HAL_DATA(priv)->desc_dma_buf_addr_amsdu[0] = _GET_HAL_DATA(priv)->desc_dma_buf_addr_amsdu[0] + page_align_phy;  
#else
				_GET_HAL_DATA(priv)->desc_dma_buf_addr_amsdu[0]= HAL_VIRT_TO_BUS(page_ptr);
#endif
#endif
...
#ifdef IQK_THREAD
void _PHY_IQCalibrate(struct rtl8192cd_priv *priv);

int iqk_thread(void *p)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)p;
	...
	while (1) {
		rc = wait_event_interruptible(pshare->iqk_task_wait, 
			(pshare->iqk_task_begin != pshare->iqk_task_end)
			|| kthread_should_stop());

		if (kthread_should_stop() || rc == -ERESTARTSYS)
			break;

		// mitigate the frequency of doing IQK
		if (netif_running(priv->dev))
			mdelay(1000);

		spin_lock_irqsave(&pshare->iqk_task_lock, flags);

		if (pshare->iqk_task_freeze) {
			pshare->iqk_task_end = pshare->iqk_task_begin;
		} else if (pshare->iqk_task_begin != pshare->iqk_task_end) {
			pshare->iqk_task_end = pshare->iqk_task_begin;
			if (netif_running(priv->dev))
				_PHY_IQCalibrate(priv);
		}

		spin_unlock_irqrestore(&pshare->iqk_task_lock, flags);
	}

	return 0;
}

void iqk_task_kick(struct rtl8192cd_priv *priv)
{
	if (priv->pshare->iqk_task) {
		priv->pshare->iqk_task_begin++;
		wake_up(&priv->pshare->iqk_task_wait);
	}
}

void iqk_task_block(struct rtl8192cd_priv *priv)
{
	struct priv_shared_info *pshare = priv->pshare;
	unsigned long flags;
	...
	if (pshare->iqk_task) {
		spin_lock_irqsave(&pshare->iqk_task_lock, flags);
		pshare->iqk_task_freeze = 1;
		spin_unlock_irqrestore(&pshare->iqk_task_lock, flags);
	}
}

void iqk_task_unblock(struct rtl8192cd_priv *priv)
{
	struct priv_shared_info *pshare = priv->pshare;
	
	if (pshare->iqk_task) {
		pshare->iqk_task_freeze = 0;
	}
}

int iqk_task_init(struct rtl8192cd_priv *priv)
{
	struct priv_shared_info *pshare = priv->pshare;
	char name[16] = {0};
	...
	snprintf(name, sizeof(name), "IQK/%s", priv->dev->name);
	
	pshare->iqk_task = kthread_run(iqk_thread, (void *)priv, name);
	if (IS_ERR(pshare->iqk_task)) {
		printk("[%s] kthread_run failed!\n", __func__);
		pshare->iqk_task = NULL;
		return -1;
	}
	
	init_waitqueue_head(&pshare->iqk_task_wait);
	spin_lock_init(&pshare->iqk_task_lock);
	pshare->iqk_task_begin = 0;
	pshare->iqk_task_end = 0;
	pshare->iqk_task_freeze = 0;
	
	return 0;
}

void iqk_task_stop(struct rtl8192cd_priv *priv)
{	
	if (priv->pshare->iqk_task) {
		printk("[%s] waiting...\n", __func__);
		kthread_stop(priv->pshare->iqk_task);
		printk("[%s] finish\n", __func__);
		priv->pshare->iqk_task = NULL;
	}
}
#endif // IQK_THREAD
