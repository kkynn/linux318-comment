...
enum amsdu_dm_state {
	AMSDU_DM_LEARNING = 0x1,
	AMSDU_DM_TP_GOUP = 0x2,
	AMSDU_DM_DISABLE = 0x4,
};
...
struct ip_mcast_info {
	...
	unsigned char		mc_ip[2]; // only store most significient 2 bytes of IP addr
};
#endif
...
struct dot11k_neighbor_info
{
    ...
    unsigned char source; //0: beacon listened 1:beacon_report from STA
...
struct stat_info {
	...
	unsigned int		atm_match_sta_time;//sta tx time for mode 4 & 5
...
#ifdef RTK_ATM_EXT
	unsigned int		atm_burst_num_adj_ratio;
#endif
#endif//RTL_ATM
	...
	unsigned char		tim_clear_cntdown;

#ifdef WIFI_SIMPLE_CONFIG
	unsigned char		wsc_need_disassoc;
#endif
	...
	unsigned short		spec_env_UDP_Tx_PktSz; 
	...
	unsigned char		idle_count2;
	unsigned char		rssi;
	unsigned char		sq;
	unsigned char		rx_rate;
	unsigned char		rx_bw;
	unsigned char		rf_mimo_mode;
...
#endif
	unsigned short		tx_tp_peak;
	unsigned short		tx_tp_valley;
	unsigned short		tx_tp_bounce;
	unsigned short		amsdu_dm_state;
	unsigned char		amsdu_learn_cntdown;
...
#ifdef RTK_ATM
#ifdef RTK_ATM_EXT
	unsigned int	atm_state_current;//pause:1, active :0
	unsigned int	atm_state_next;//pause:1, active :0
	unsigned int	atm_state_wait_count;
	unsigned int	atm_tx_time_from_assoc;//real transmission time//ms

	unsigned int	atm_time_slice;//us
	unsigned int	atm_tx_len_byte;
	unsigned int	atm_tx_time_us;
#endif
#endif
...
struct extra_stats {
	...
	unsigned long		tx_ucast_pkts_cnt_pre;
...
#endif
	unsigned long		tx_flood_mcast_pkts_cnt;
};
...
//#endif
	...
	unsigned int		lmt2;
	unsigned int		lmt3;
	unsigned char		vomaxagg;
	unsigned char		vimaxagg;
	unsigned char		bemaxagg;
	unsigned char		bkmaxagg;
    ...
    int                 atm_en;
    int                 atm_swqf;
    int                 atm_mode;// 1:if auto 2:if manual 3:sta auto 4:sta manual 5:if&sta manual
	...
	int		atm_distribution_type;//dynamic:0, static:1 
	int		atm_wait_threshold;
	int		atm_ts_enable;//time slice
	int		atm_ts_plus;//time slice
	int		atm_ts_ratio;//time slice
#endif
#endif	
...
// power constraint
	...
	unsigned int		mu_total_ok;
	unsigned int		mu_total_fail;
	unsigned char		mu_monitor;
...
__PACK struct stactrl_prefer_entry_aipc {
    u8 used;
    u8 mac[MACADDRLEN];
    u8 rssi;
    u32 aging; 
...
__PACK struct stactrl_block_entry_aipc {
    u8 used;
    u8 mac[MACADDRLEN];
    u16 timerX;
    u8 retryY; 
...
__PACK struct stactrl_shm_common {
    u8 enable;
    u8 group_id;
    u8 prefer;              /* 0: not prefer band, 1: is prefer band */
    u8 status;              /* 0: sta control disable, 1: sta control enable */
    u8 driver_state;
    u32 opmode;
...
__PACK struct stactrl_prefer_shm {
    u8 enable;
    u8 group_id;
    u8 prefer;              /* 0: not prefer band, 1: is prefer band */
    u8 status;              /* 0: sta control disable, 1: sta control enable */
    ...
    u32 assoc_num;
    u8 prefer_num;
    struct stactrl_prefer_entry_aipc entry[MAX_STACTRL_PREFERBAND_NUM];
} __WLAN_ATTRIB_PACK__;

__PACK struct stactrl_block_shm {
    u8 enable;
    u8 group_id;
    u8 prefer;              /* 0: not prefer band, 1: is prefer band */    
    u8 status;              /* 0: sta control disable, 1: sta control enable */
    ...
    u32 assoc_num;
    u8 block_num;
    struct stactrl_block_entry_aipc entry[MAX_STACTRL_BLOCK_NUM];
} __WLAN_ATTRIB_PACK__;

#ifdef CONFIG_ARCH_LUNA_SLAVE
#define STACTRL_WLAN_ID         1
#define STACTRL_WLAN_ID_SC      0
#else
#define STACTRL_WLAN_ID         0
#define STACTRL_WLAN_ID_SC      1
#endif
// STA CONTROL Shared Memory Support
#define STACTRL_SHM_SUPPORT     ((priv->stactrl.stactrl_id < STA_CONTROL_SUPPORT_NUM) && (priv->stactrl.stactrl_id >= 0)) ? 1 : 0
// STA CONTROL Shared Memory Address
#define STACTRL_SHM_BASE        (STA_CONTROL_SHM_SIZE * priv->stactrl.stactrl_id)
#define STACTRL_SHM_ENABLE      (STACTRL_SHM_BASE + offsetof(struct stactrl_shm_common, enable))
#define STACTRL_SHM_GROUPID     (STACTRL_SHM_BASE + offsetof(struct stactrl_shm_common, group_id))
#define STACTRL_SHM_PREFER      (STACTRL_SHM_BASE + offsetof(struct stactrl_shm_common, prefer))
#define STACTRL_SHM_STATUS      (STACTRL_SHM_BASE + offsetof(struct stactrl_shm_common, status))
#define STACTRL_SHM_DRV_STATE   (STACTRL_SHM_BASE + offsetof(struct stactrl_shm_common, driver_state))
#define STACTRL_SHM_OPMODE      (STACTRL_SHM_BASE + offsetof(struct stactrl_shm_common, opmode))
#define STACTRL_SHM_DEV_NAME    (STACTRL_SHM_BASE + offsetof(struct stactrl_shm_common, dev_name))
#define STACTRL_SHM_ASSOC_NUM   (STACTRL_SHM_BASE + offsetof(struct stactrl_shm_common, assoc_num))
#define STACTRL_SHM_PREFER_NUM  (STACTRL_SHM_BASE + offsetof(struct stactrl_prefer_shm, prefer_num))
#define STACTRL_SHM_BLOCK_NUM   (STACTRL_SHM_BASE + offsetof(struct stactrl_block_shm, block_num))
#define STACTRL_SHM_PREFER_LIST (STACTRL_SHM_BASE + offsetof(struct stactrl_prefer_shm, entry))
#define STACTRL_SHM_BLOCK_LIST  (STACTRL_SHM_BASE + offsetof(struct stactrl_block_shm, entry))


// STA CONTROL Size
#define STACTRL_PREFER_ENTRY_SIZE   (sizeof(struct stactrl_prefer_entry_aipc))
#define STACTRL_PREFERLIST_SIZE     (STACTRL_PREFER_ENTRY_SIZE * MAX_STACTRL_PREFERBAND_NUM)
#define STACTRL_PREFER_SIZE         (sizeof(struct stactrl_prefer_shm))
#define STACTRL_BLOCK_ENTRY_SIZE    (sizeof(struct stactrl_block_entry_aipc))
#define STACTRL_BLOCKLIST_SIZE      (STACTRL_BLOCK_ENTRY_SIZE * MAX_STACTRL_BLOCK_NUM)
#define STACTRL_BLOCK_SIZE          (sizeof(struct stactrl_block_shm))
...
struct priv_shared_info {
   ...
   /*=for========remain on channel=============*/    
	...
	unsigned char			tgpwr_idx_CCK[4];   //index for CCK 11M target power
	unsigned char			tgpwr_idx_OFDM[4];  //index for OFDM 54M target power
	unsigned char			tgpwr_idx_HT1S[4];  //index for MCS7 target power
	unsigned char			tgpwr_idx_HT2S[4];  //index for MCS15 target power
	unsigned char			tgpwr_idx_HT3S[4];  //index for MCS23 target power
	unsigned char			tgpwr_idx_HT4S[4];  //index for MCS31 target power
	unsigned char			tgpwr_idx_VHT1S[4]; //index for VHT MCS7 target power
	unsigned char			tgpwr_idx_VHT2S[4]; //index for VHT NSS2 MCS7 target power
	unsigned char			tgpwr_idx_VHT3S[4]; //index for VHT NSS3 MCS7 target power
	unsigned char			tgpwr_idx_VHT4S[4]; //index for VHT NSS4 MCS7 target power
#ifdef _TRACKING_TABLE_FILE
...
#if defined(CONFIG_WLAN_HAL_8814AE) ||	defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8192EE)
...
/*cfg p2p cfg p2p*/	
...
#endif
#ifdef IQK_THREAD
	struct task_struct		*iqk_task;
	wait_queue_head_t	iqk_task_wait;
	unsigned char			iqk_task_begin;
	unsigned char			iqk_task_end;
	unsigned char			iqk_task_freeze;
	spinlock_t			iqk_task_lock;
#endif
	...
	short			stactrl_groupID_used[STA_CONTROL_SUPPORT_NUM];
#endif
...
#if 0//def CH_LOAD_CAL
	unsigned long		ch_utilization;
#endif
	struct ch_load_survey_info ch_load_ss_info[SUPPORT_CH_NUM];
...
#ifdef RTK_ATM_EXT
    unsigned int                atm_ttl_stanum_pause;// num of pause-cli that in client list of interface 2.4G/5G
    unsigned int                atm_ttl_match_stanum_pause;// num of matched pause-cli that in client list of interface 2.4G/5G
    unsigned int                atm_ttl_match_statime_pause;// total matched pause-sta time
    unsigned int                atm_sta_idx[4];
#endif
#endif//RTK_ATM
...
#ifdef RTK_ATM_EXT
    unsigned int		atm_iftime_dynamic;
    unsigned int		atm_time_per_ssid;
    unsigned int		atm_sta_num_pause;//num of pause cli of every SSID		
    unsigned int		atm_sta_num_active;//num of active cli of every SSID		
#endif
#endif
