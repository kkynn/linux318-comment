...
void init_stainfo(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	if(is_FON_SSID(idx) && !FON_getClient_byMac(pstat->hwaddr, &pFonClient))
	{
		pFonClient =  (Fonclient *)malloc(sizeof(Fonclient));
		memset(pFonClient, 0, sizeof(Fonclient));
		memcpy(pFonClient->mac, pstat->hwaddr, MACADDRLEN);
...
//		wapiAssert(pstat->wapiInfo!=NULL);
	...
	pstat->ra_cnt_window[0] = pstat->ra_cnt_window[1] = pstat->ra_cnt_window[2] = 0;
	pstat->ra_max_window[0] =  pstat->ra_max_window[1] = pstat->ra_max_window[2] = 0x00;
	pstat->ra_min_window[0] =  pstat->ra_min_window[1] = pstat->ra_min_window[2] = 0xff;
	pstat->ra_cnt = 0;
	pstat->ra_max = 0x00;
	pstat->ra_min = 0xff;
	pstat->ra_cur = 0x00;
	pstat->is_legacy_USB = 0;
...
void free_sta_tx_skb(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
    ...
    for(i = BK_QUEUE; i < HIGH_QUEUE; i++) {
        while (skb_queue_len(&pstat->swq.swq_queue[i])) {
            ...
            priv->pshare->swq_residual_drop_pkt++;
            rtl_kfree_skb(priv, pskb, _SKB_TX_);
        }
    }
#endif
...
void release_stainfo(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	if(tmp_pstat && isEqualMACAddr(tmp_pstat->hwaddr, pstat->hwaddr))
...
#if defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_RTL_8812_SUPPORT)
	if(GET_CHIP_VER(priv) == VERSION_8192E || GET_CHIP_VER(priv) == VERSION_8822B || GET_CHIP_VER(priv) == VERSION_8812E)
		return 2;
	else
#endif
...
int get_bssrate_set(struct rtl8192cd_priv *priv, int bssrate_ie, unsigned char **pbssrate, unsigned int *bssrate_len)
{
	...
	if (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_5G)
#else
	if ((priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_5G)||(priv->pmib->dot11nConfigEntry.dot11nUse40M == HT_CHANNEL_WIDTH_80)) //AC2G_256QAM
#endif
	{
...
{
    unsigned int _40m_stamap, _80m_stamap;
    int FA_counter;
    int do_switch = 0;
    int switch_20m = 0;
...
/*  expired time already check mp mode and assoc does not enter this function
#ifdef MP_TEST
...
*/
...
#endif
        FA_counter = priv->pshare->FA_total_cnt;

    _40m_stamap = orSTABitMap(&priv->pshare->_40m_staMap);
    _80m_stamap = orSTABitMap(&priv->pshare->_80m_staMap);
    ...
    if( priv->pmib->dot11nConfigEntry.dot11nCoexist) {
        ...
        if ((priv->pshare->CurrentChannelBW == HT_CHANNEL_WIDTH_20)
            ...
            && (priv->pshare->rf_ft_var.bws_enable & BIT2)
#ifdef MBSSID
        ...
        {
            switch_20m = 1;
            priv->pshare->bws_cntdown = priv->pshare->rf_ft_var.bws_keeptime;
        }
        else if (((FA_counter > priv->pshare->rf_ft_var.bws_Thd) && (IGI >= 0x3a))
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
            || (IGI >= 0x42)
#endif
            )
		{
            ...
            if ((priv->pshare->rf_ft_var.bws_enable & BIT2)
#ifdef MBSSID
                && (!priv->pmib->miscEntry.vap_enable)
#endif
                )
            ...
            if (priv->pshare->bws_cntdown) {
                priv->pshare->bws_cntdown--;
                // keep current bandwidth settings
                if (priv->pshare->CurrentChannelBW == HT_CHANNEL_WIDTH_20)
                    switch_20m = 1;
        } 
            if (!priv->pshare->bws_cntdown && !orSTABitMap(&priv->pshare->intel_sta_bitmap)) {
    ...
    } else if( _40m_stamap && (priv->pshare->CurrentChannelBW == HT_CHANNEL_WIDTH_20)  &&
...
{
    if( channelVal >= 52  && channelVal <= 144 ){    
        return 1;
    }else{
        return 0;
    }
...
int is_passive_channel(struct rtl8192cd_priv *priv , int domain, int chan)
{
    ...
    if( (chan >= 52  && chan <= 144) && !priv->pmib->dot11DFSEntry.disable_DFS){  
        return 1;
    }
#endif    	
...
#endif
#if defined(CONFIG_RTL_88E_SUPPORT) && !defined(RATEADAPTIVE_BY_ODM)
		if (GET_CHIP_VER(priv) == VERSION_8188E)
...
#ifdef CONFIG_SPECIAL_ENV_TEST
		if (priv->pshare->in_spec_env_test_to && pstat->spec_env_UDP_Tx_PktSz == VERIWAVE_PKT_SIZE_BIG){ 
			pstat->swq.q_aggnum[j] = priv->pshare->rf_ft_var.swqSEaggnum;
		} else 
#endif				
...
#endif			
        if(priv->pshare->swq_use_hw_timer && priv->pshare->swq_numActiveSTA > priv->pshare->rf_ft_var.swq_turbo_sta_th && 
           (priv->up_time - priv->pshare->swq_turbo_time) < priv->pshare->rf_ft_var.swqmaxturbotime) {
#ifdef CONFIG_SPECIAL_ENV_TEST
		if (priv->pshare->in_spec_env_test_to && pstat->spec_env_UDP_Tx_PktSz == VERIWAVE_PKT_SIZE_BIG){ 
			pstat->swq.q_aggnum[i] = priv->pshare->rf_ft_var.swqSEaggnum;
		} else 
#endif	
...
#ifdef CONFIG_SPECIAL_ENV_TEST
			if (priv->pshare->in_spec_env_test_to && pstat->spec_env_UDP_Tx_PktSz == VERIWAVE_PKT_SIZE_BIG){ 
				maxAggNum = priv->pshare->rf_ft_var.swqSEaggnum;
			} else 
#endif	
...
#endif
		if(assoc_num) {
			if(get_rf_mimo_mode(priv) == MIMO_3T3R && (priv->pshare->rf_status == MIMO_2T2R || priv->pshare->rf_status == MIMO_1T1R))
	...
	if(GET_ROOT(priv)->pmib->dot11OperationEntry.opmode & (WIFI_STATION_STATE|WIFI_ADHOC_STATE)) {
		...
		if(get_ss_level(priv) < SS_LV_ROOTFUNCOFF) {
			if((priv->site_survey->ss_channel == 100) && (priv->pmib->miscEntry.ss_delay)) {
				ret = 1;
				DEBUG_INFO("%s Site survey defered\n",priv->dev->name);
			}
		}
	}
