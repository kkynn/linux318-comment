...
#endif			
{
	...
	if(WNM_ENABLE) {
		...
		PRINT_SINGL_ARG("	 BssDbEnable:  ",	priv->pmib->wnmEntry.dot11vDriverBasedEnable, "%d");
		PRINT_SINGL_ARG("	 BssDbIntval:  ",	priv->pmib->wnmEntry.dot11vDbCheckInterval, "%d");
		PRINT_SINGL_ARG("	 BssDbRedirMask:  ", priv->pmib->wnmEntry.dot11vDbRedirectMask, "%d");
		PRINT_SINGL_ARG("	 BssDbApTxLoadThrd:  ", priv->pmib->wnmEntry.dot11vDbApTxLoadThreshold, "%d");
		PRINT_SINGL_ARG("	 BssDbStaTxLoadThrd:  ", priv->pmib->wnmEntry.dot11vDbStaTxLoadThreshold, "%d");
		PRINT_SINGL_ARG("	 BssDbApRxLoadThrd:  ", priv->pmib->wnmEntry.dot11vDbApRxLoadThreshold, "%d");
		PRINT_SINGL_ARG("	 BssDbStaRxLoadThrd:  ", priv->pmib->wnmEntry.dot11vDbStaRxLoadThreshold, "%d");
#endif 
	}
#endif
...
#endif			
{
    ...
    if(priv->ss_req_ongoing)	{
        ...
        if (IS_ROOT_INTERFACE(priv)) {    
            ...
            CHECK_LEN;
            len = pos;
            PRINT_ONE("    channel utilization", "%s", 1);
            CHECK_LEN;
            len = pos;
            for(i1=0; i1<SUPPORT_CH_NUM ;i1++){
                if (priv->pshare->ch_load_ss_info[i1].channel !=0 ) {
                    sprintf(tmp2, "    idx:%d, Channel:%d, busyness:%d\n", i1+1, 
                        priv->pshare->ch_load_ss_info[i1].channel, 
                        priv->pshare->ch_load_ss_info[i1].chbusytime);
                    PRINT_ONE(tmp2, "%s", 1);
                    CHECK_LEN;
                    len = pos;
                } else {
...
#endif			
{
	...
	PRINT_SINGL_ARG("    bws_enable: ", priv->pshare->rf_ft_var.bws_enable, "%d");
	PRINT_SINGL_ARG("    bws_cntdown: ", priv->pshare->bws_cntdown, "%d");
#endif
#ifdef CONFIG_WLAN_HAL
...
#if defined (HW_ANT_SWITCH) && !defined(CONFIG_RTL_92C_SUPPORT) && !defined(CONFIG_RTL_92D_SUPPORT)
	if (priv->pshare->_dmODM.AntDivType) {
...
#else
    size = sprintf(pTmp, "  Make info: v%d.%d.%d (%s)\n", DRV_VERSION_H, DRV_VERSION_L, DRV_VERSION_SUBL, DRV_RELDATE);
    CHECK_LEN_E;
#endif
#else  /* CONFIG_RTL8672 */
#ifdef __ECOS
...
#endif			
{
...
#if defined(CONFIG_VERIWAVE_CHECK)
	memcpy(priv->wlan_addr, pstat->hwaddr, WLAN_ADDR_LEN);
#endif
	...
	avg = (pstat->rf_info.RxSNRdB[1] ? ((pstat->rf_info.RxSNRdB[0] + pstat->rf_info.RxSNRdB[1])/2):pstat->rf_info.RxSNRdB[0]); 	
	PRINT_ONE(avg, "%d", 0);
...
//8812_client tx_bw rx_bw ??
	...
	if (pstat->is_realtek_sta && (pstat->IOTPeer == RTK_CAP_IE_8814))
		snprintf((char *)tmpbuf, sizeof(tmpbuf), "Realtek_8814");
	else if (pstat->is_realtek_sta)
		snprintf((char *)tmpbuf, sizeof(tmpbuf), "Realtek");
...
//AC capability
	...
	for (idx=0; idx < pstat->ipmc_num; idx++) {
		...
		PRINT_ONE( "    mcmac: ", "%s", 0);
		PRINT_ARRAY(pstat->ipmc[idx].mcmac, "%02x", MACADDRLEN,0);
		if(priv->pshare->rf_ft_var.mc2u_match_ip){
			PRINT_ONE( "   ", "%s", 0);
			PRINT_ONE( pstat->ipmc[idx].mc_ip[0], "%d", 0);
			PRINT_ONE( ".", "%s", 0);
			PRINT_ONE( pstat->ipmc[idx].mc_ip[1], "%d", 0);
			PRINT_ONE( ".", "%s", 0);
			PRINT_ONE( pstat->ipmc[idx].mcmac[4], "%d", 0);
			PRINT_ONE( ".", "%s", 0);
			PRINT_ONE( pstat->ipmc[idx].mcmac[5], "%d", 1);
		}
		else{
			PRINT_ONE( "", "%s", 1);
		}
...
/* Hotspot 2.0 Release 1 */
	...
	if ((pstat->WirelessMode & WIRELESS_MODE_AC_5G) || (pstat->WirelessMode & WIRELESS_MODE_AC_24G))
		snprintf((char *)tmpbuf, sizeof(tmpbuf), "AC");
		...
		snprintf(tmpbuf, sizeof(tmpbuf), "Unknown WirelessMode:%x\n",pstat->WirelessMode);
   ...
   CHECK_LEN_B;
...
#ifdef RTK_ATM
#ifdef RTK_ATM_EXT
	if(priv->pshare->rf_ft_var.atm_en && !priv->pshare->rf_ft_var.atm_distribution_type) //dynamic
	{
		PRINT_SINGL_ARG("    atm_state_current: ", pstat->atm_state_current, "%d");
		CHECK_LEN_B;
		PRINT_SINGL_ARG("    atm_state_next: ", pstat->atm_state_next, "%d");
		CHECK_LEN_B;
		PRINT_SINGL_ARG("    atm_state_wait_count: ", pstat->atm_state_wait_count, "%d");
		CHECK_LEN_B;
	}
#endif
#endif
	...
	if(priv->pshare->in_spec_env_test_to){
		if(pstat->spec_env_UDP_Tx_PktSz == VERIWAVE_PKT_SIZE_BIG) {
			PRINT_ONE("    spec_env UDP Tx test: big pkts", "%s", 1);
		}
		else if(pstat->spec_env_UDP_Tx_PktSz == VERIWAVE_PKT_SIZE_SML) {
	...
	PRINT_SINGL_ARG("    disable_amsdu: ", (pstat->amsdu_dm_state & AMSDU_DM_DISABLE)? 1:0, "%d");
	CHECK_LEN_B;
#ifdef SUPPORT_TX_AMSDU_SMALL_PKTS_ONLY
...
#endif			
{
	...
	PRINT_SINGL_ARG("    tx_fail_continue:   ", pstat->tx_fail_continue, "%u");
	PRINT_SINGL_ARG("    tx_fail_addba_cnt:   ", pstat->tx_fail_addba_cnt, "%u");

	PRINT_ARRAY_ARG("    ra_cnt_window: ", pstat->ra_cnt_window, "%u ", 3);
	PRINT_ARRAY_ARG("    ra_max_window: ", pstat->ra_max_window, "%02x ", 3);
	PRINT_ARRAY_ARG("    ra_min_window: ", pstat->ra_min_window, "%02x ", 3);
	PRINT_SINGL_ARG("    is_legacy_USB:   ", pstat->is_legacy_USB, "%u");
...
#ifdef SW_TX_QUEUE
	PRINT_SINGL_ARG("    swq_enque_pkt:    ", priv->pshare->swq_enque_pkt, "%lu");
	PRINT_SINGL_ARG("    swq_xmit_out_pkt:    ", priv->pshare->swq_xmit_out_pkt, "%lu");
	PRINT_SINGL_ARG("    swq_real_enque_pkt:    ", priv->pshare->swq_real_enque_pkt, "%lu");
	PRINT_SINGL_ARG("    swq_real_deque_pkt:    ", priv->pshare->swq_real_deque_pkt, "%lu");
	PRINT_SINGL_ARG("    swq_residual_drop_pkt:    ", priv->pshare->swq_residual_drop_pkt, "%lu");
	PRINT_SINGL_ARG("    swq_overflow_drop_pkt:    ", priv->pshare->swq_overflow_drop_pkt, "%lu");
#endif
...
#endif			
{
	...
	for (i=0; i<12; i++) {
		if (priv->pshare->phw->GPIO_dir[i] == 0x01) {
			snprintf(tmp, sizeof(tmp), "GPIO%d %d", i, RTLWIFINIC_GPIO_read(i));
			PRINT_ONE(tmp, "%s", 1);
		}
	}
...
#ifdef PROC_STA_CONN_FAIL_INFO
    {"sta_conn_fail", 		rtl8192cd_proc_sta_conn_fail},
#endif
#ifdef STA_RATE_STATISTIC
	{"sta_rateinfo",		rtl8192cd_proc_sta_rateinfo},
#endif        
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
...
#endif
#ifdef SBWC
	{"mib_sbwc",			rtl8192cd_proc_mib_sbwc},
#endif
