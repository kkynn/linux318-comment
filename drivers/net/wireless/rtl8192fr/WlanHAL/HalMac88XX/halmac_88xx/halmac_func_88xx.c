...
)
{
	...
	while (pkt_size_tmp != 0) {
		...
		rsvd_buf = (u8*)PLATFORM_RTL_MALLOC(pDriver_adapter, send_pkt_size);		
		PLATFORM_RTL_MEMSET(pDriver_adapter, rsvd_buf, 0x00, send_pkt_size);
		PLATFORM_RTL_MEMCPY(pDriver_adapter, rsvd_buf, pCode_ptr + mem_offset, send_pkt_size);			
				
		if (HALMAC_RET_SUCCESS != halmac_send_fwpkt_88xx(pHalmac_adapter, rsvd_buf, send_pkt_size)) {			
			PLATFORM_MSG_PRINT(pDriver_adapter, HALMAC_MSG_INIT, HALMAC_DBG_ERR, "halmac_send_fwpkt_88xx fail!!");
			PLATFORM_RTL_FREE(pDriver_adapter, rsvd_buf, send_pkt_size);		
			rsvd_buf = NULL;			
...
)
{
	...
	// On TK1, the first read operation might get wrong value
	HALMAC_REG_READ_32(pHalmac_adapter, REG_H2C_PKT_WRITEADDR);
#endif
#endif
