...
#if defined(_TEGRA_TK1_)
    pHalmac_adapter->sdio_cmd53_4byte = HALMAC_SDIO_CMD53_4BYTE_MODE_RW;
    pHalmac_adapter->sdio_hw_info.io_hi_speed_flag = 0;
    pHalmac_adapter->sdio_hw_info.spec_ver = HALMAC_SDIO_SPEC_VER_3_00;
  #if HALMAC_8822B_SUPPORT
	pHalmac_adapter->sdio_hw_info.clock_speed = 200;
  #elif HALMAC_8821C_SUPPORT
    pHalmac_adapter->sdio_hw_info.clock_speed = 81;
  #else
...
#endif
	} else if (pHalmac_adapter->chip_id == HALMAC_CHIP_ID_HW_DEF_8197F) {
...
#if defined(_TEGRA_TK1_) && HALMAC_8822B_SUPPORT
#else
...
)
{
	...
	halmac_api_record_id_88xx(pHalmac_adapter, HALMAC_API_READ_EFUSE_BT);
...
)
{
	...
	halmac_api_record_id_88xx(pHalmac_adapter, HALMAC_API_CFG_EFUSE_AUTO_CHECK);
...
)
{
	...
	/* Enter 32K */
	if (pHalmac_adapter->halmac_interface == HALMAC_INTERFACE_SDIO) {
		if (pHalmac_adapter->fwlps_option.enter_32K == _TRUE) {
