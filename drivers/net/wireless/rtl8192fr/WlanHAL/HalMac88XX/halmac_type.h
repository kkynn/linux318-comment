...
/*
jim 20161026
The SWAPxx definition in OSK is transforming data to big-endian format, but if CPU or data is already in BE mode, keep the orignal one.
then the definition is different with below one in wireless driver.
*/
#define SWAP32_wlan(x) ((u32)( \
			   (((u32)(x) & (u32)0x000000ff) << 24) | \
			   (((u32)(x) & (u32)0x0000ff00) << 8) | \
			   (((u32)(x) & (u32)0x00ff0000) >> 8) | \
			   (((u32)(x) & (u32)0xff000000) >> 24)))

#define SWAP16_wlan(x) ((u16)( \
			   (((u16)(x) & (u16)0x00ff) << 8) | \
			   (((u16)(x) & (u16)0xff00) >> 8)))			   
...
#define rtk_cpu_to_be32(x)              SWAP32_wlan((x))
#define rtk_be32_to_cpu(x)              SWAP32_wlan((x))
#define rtk_cpu_to_be16(x)              SWAP16_wlan((x))
#define rtk_be16_to_cpu(x)              SWAP16_wlan((x))
#endif
#else
#ifndef rtk_le16_to_cpu
#define rtk_cpu_to_le32(x)              SWAP32_wlan((x))
#define rtk_le32_to_cpu(x)              SWAP32_wlan((x))
#define rtk_cpu_to_le16(x)              SWAP16_wlan((x))
#define rtk_le16_to_cpu(x)              SWAP16_wlan((x))
...
typedef struct _HALMAC_INTF_INTGRA {
	u8 refclk_cal_div;
	u8 refclk_cal_margin;
} HALMAC_INTF_INTGRA, *PHALMAC_INTF_INTGRA;
...
typedef struct _HALMAC_API {
	...
	HALMAC_RET_STATUS(*halmac_interface_integration_tuning)(PHALMAC_ADAPTER pHalmac_adapter, PHALMAC_INTF_INTGRA pIntf_intgra);
