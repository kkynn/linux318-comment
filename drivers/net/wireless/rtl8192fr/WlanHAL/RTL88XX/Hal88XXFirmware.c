...
#if (IS_RTL8881A_SERIES || IS_RTL8192E_SERIES || IS_RTL8814A_SERIES || IS_RTL8197F_SERIES || IS_RTL8822B_SERIES || IS_RTL8821C_SERIES)
...
#if (IS_EXIST_RTL8822BE || IX_EXIST_RTL8821CE)
        if ( IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8821C(Adapter))  {
            TXDESCSize = SIZE_TXDESC_88XX_V1;
        }
#endif //IS_EXIST_RTL8822BE
...
#if 1
	pEntry->cmn_info.mimo_type = rf_mimo_mode;
...
#if (IS_RTL8822B_SERIES || IS_RTL8821C_SERIES)
...
)
{
	...
	case C2H_88XX_RA_PARA_RPT:
		odm_c2h_ra_para_report_handler(&(Adapter->pshare->_dmODM), tmpBuf, c2hCmdLen);
		break;
		
	case C2H_88XX_RA_DYNAMIC_TX_PATH_RPT:
		phydm_c2h_dtp_handler(&(Adapter->pshare->_dmODM), tmpBuf, c2hCmdLen);
		//dbg_print("[C2H] C2H_8192E_RA_PARA_RPT \n");
		break;
...
)
{
    ...
    if(cmdsubID == EXTEND_C2H_88XX_ATM_RPT){
        tmpBuf = tmpBuf+8;                 //ATM header extended to 8 bytes   
        c2hCmdLen = c2hCmdLen-8;
    }
    else if(cmdsubID != EXTEND_C2H_88XX_DBG_PRINT){
        tmpBuf = tmpBuf+4;
        c2hCmdLen = c2hCmdLen-4;
    }
        
    switch(cmdsubID)
    {
        case EXTEND_C2H_88XX_DBG_PRINT:
#if 0 //seems format changed, need check            
        //for debug message use
	while(c2hissued_len < c2hCmdLen-1)
	{
        ...
        break;
        default:
            printk("[C2H EXT], Unkown packet!! CmdId(%#X)!\n", c2hCmdId);
