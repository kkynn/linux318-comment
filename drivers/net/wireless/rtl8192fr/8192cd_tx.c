...
)
{
    ...
    pkt_Qos = pdescinfo->SWPS_pkt_Qos;
		...
		if ((QOS_ENABLE) && (APSD_ENABLE) && (pdescinfo->swps_pstat->QosEnabled) && (pdescinfo->swps_pstat->apsd_bitmap & 0x0f)) {
            if(pkt_Qos < 0){
                GDEBUG("Error Qos when reprepare insert to dzqueue.\n");
                return FALSE;
            }
...
static int SWPS_reprepare_pktinfo_enqueue(struct rtl8192cd_priv *priv, struct tx_desc_info *pdescinfo, int pkt_Qos, int insert_position, u1Byte AMSDU_flag)
{
    ...
    if ((QOS_ENABLE) && (APSD_ENABLE) && (pdescinfo->swps_pstat->QosEnabled) && (pdescinfo->swps_pstat->apsd_bitmap & 0x0f)) {
        ...
        if(pkt_Qos < 0){
            GDEBUG("Error Qos when reprepare pktinfo enqueue.\n");
            return FALSE;
        }
...
#endif
{
    ...
    {	    
        struct wlan_ethhdr_t *wlan_eth = (struct wlan_ethhdr_t *)hdr;	   
		
        for (i=0; i<TX_SC_ENTRY_NUM; i++, idx = ((idx+1) % TX_SC_ENTRY_NUM)) {        
#if 0
			if (isEqualMACAddr(wlan_eth->saddr, (unsigned char *)(&ptxsc_entry[idx].ethhdr.saddr)) && (wlan_eth->type == ptxsc_entry[idx].ethhdr.type))
#else
...
int rtl8192cd_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if(skb->len>1300)
		priv->ext_stats.start_xmit_pkt_1518++;
#endif
    ...
    if ((eth->h_proto != __constant_htons(ETH_P_PAE)) ||
        (skb->len < 9))
        goto skip_check_eap;
...
skip_check_eap:
	...
	if (skb_queue_len(&priv->pshare->skb_xmit_queue) >= priv->pshare->rf_ft_var.swq_max_enqueu_len) {
		printk_ratelimited(KERN_WARNING "TX DROP: exceed the skb xmit queue!\n");
		priv->ext_stats.tx_drops++;
		dev_kfree_skb_any(skb);
		return 0; 
	}
...
static int amsdu_xmit(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct tx_insn *txcfg, int tid,
				int from_timeout, struct net_device *wdsDev, struct net_device *dev)
{
	...
	while (skb_queue_len(&pstat->amsdu_tx_que[tid]) > 0) {
		...
		if (is_first) {
			if (skb_queue_len(&pstat->amsdu_tx_que[tid]) > 0) {
				...
				pskb->cb[47] |=(BIT(FG_AGGRE_MSDU_FIRST)|BIT(FG_AGGRE_MSDU_MIDDLE)|BIT(FG_AGGRE_MSDU_LAST));//Normal packet without AMSDU
				txcfg->aggre_en = 0;
			}
		}
...
#ifdef TX_AMSDU_TIMER_LIST
unsigned int amsdu_timer_now(struct rtl8192cd_priv *priv)
{
	unsigned int now;
...
{
	unsigned int now, timeout, next_timeout=0;
	int setup_timer = 0;
	int idx;
	struct list_head *timer_list=&priv->pshare->amsdu_timer_list;
	struct amsdu_timer *new_entry, *first_entry;

	for (idx=0; idx<AMSDU_TIMER_NUM; idx++)
		if (priv->pshare->amsdu_timer[idx].pstat == NULL)
			break;

	if (idx >= AMSDU_TIMER_NUM) {
		pstat->tx_amsdu_timer_ovf++;
		return -1;
	}

	new_entry = &priv->pshare->amsdu_timer[idx];
	if (list_empty(timer_list)) {
		setup_timer = 1;
	} else {
		first_entry = list_entry(timer_list->next, struct amsdu_timer, list);
		next_timeout = first_entry->timeout;
	}

    now = amsdu_timer_now(priv);
	timeout = now + priv->pmib->dot11nConfigEntry.dot11nAMSDUSendTimeout;
	new_entry->timeout = timeout;
	new_entry->priv = priv;
	new_entry->pstat = pstat;
	new_entry->tid = (unsigned char)tid;
	list_add_tail(&new_entry->list, timer_list);

	if (!from_timeout) {
		if (setup_timer)
			setup_timer2(priv, timeout);
		else if (TSF_LESS(next_timeout, now))
			setup_timer2(priv, timeout);
		else
			setup_timer2(priv, next_timeout);
	}

	return idx;
}

int amsdu_timer_del(struct rtl8192cd_priv *priv, struct amsdu_timer *entry)
{
	if (entry->pstat) {
	    entry->pstat->amsdu_timer_id[entry->tid] = 0;
		entry->pstat = NULL;
	}
...
void amsdu_timeout(struct rtl8192cd_priv *priv, unsigned int current_time)
{
	...
	list_for_each_safe (pos, n, timer_list) {
		entry = list_entry(pos, struct amsdu_timer, list);
		pstat = entry->pstat;
		amsdu_timer_del(priv, entry);
		if (pstat) {
			tid = entry->tid;
			priv_this = entry->priv;
#ifdef WDS
			wdsDev = NULL;
			...
			{
				int ret;
				priv->pshare->rf_ft_var.amsduTOPktNum++;
    ...
    now = amsdu_timer_now(priv);
	if (!list_empty(timer_list)) {
		entry = container_of(timer_list->next, struct amsdu_timer, list);
		if (TSF_LESS(entry->timeout, now))
			setup_timer2(priv, (now+priv->pmib->dot11nConfigEntry.dot11nAMSDUSendTimeout));
		else
			setup_timer2(priv, entry->timeout);
	} else {
		cancel_timer2(priv);
	}
	SMP_UNLOCK_XMIT(flags);	
...
int amsdu_timer_add(struct rtl8192cd_priv *priv, struct stat_info *pstat, int tid, int from_timeout)
{
	...
	if (CIRC_CNT(current_idx, priv->pshare->amsdu_timer_tail, AMSDU_TIMER_NUM) == 0) {
		...
		if( from_timeout == 2 ){
			...
			if(priv->pshare->amsdu_timer[priv->pshare->amsdu_timer_tail].pstat && priv->pshare->amsdu_timer_tail != arrange_idx){
				priv->pshare->amsdu_timer[arrange_idx] = priv->pshare->amsdu_timer[priv->pshare->amsdu_timer_tail];
		                priv->pshare->amsdu_timer[arrange_idx].pstat->amsdu_timer_id[tid] = arrange_idx + 1;
...
//----------------------
	...
	if(priv->pmib->dot11nConfigEntry.dot11nAMSDUSmallPkts && 
		...
		IPV4_TCP_ACK_PACKET(skb->data))
	{
		struct sk_buff *queueskb = pstat->amsdu_tx_que[priority].next;
		struct sk_buff *queueskb2 = queueskb;
		while((unsigned long)queueskb != (unsigned long)&pstat->amsdu_tx_que[priority]){
			queueskb2 = queueskb->next;
			//Drop duplicate TCP Ack if it's at the same tcp connection
			if(!memcmp(queueskb->data+26, skb->data+26, 12)){
				skb_unlink(queueskb, &pstat->amsdu_tx_que[priority]);
				pstat->amsdu_size[priority] -= (queueskb->len + sizeof(struct llc_snap));
				rtl_kfree_skb(priv, queueskb, _SKB_TX_);
			}
			 queueskb= queueskb2;
		}
	}
	...
	if ((!pstat->is_realtek_sta && pstat->amsdu_size[priority] >= pstat->amsdu_level) ||
			 ...
			 (skb_queue_len(&pstat->amsdu_tx_que[priority]) >= priv->pmib->dot11nConfigEntry.dot11nAMSDUSendNum))
	{
		...
		if (pstat->amsdu_timer_id[priority] > 0) {
			int timer_id = pstat->amsdu_timer_id[priority] - 1;
#ifdef TX_AMSDU_TIMER_LIST
			struct list_head *entry = &priv->pshare->amsdu_timer[timer_id].list;
			amsdu_timer_del(priv, entry);
#else
			priv->pshare->amsdu_timer[timer_id].pstat = NULL;
			pstat->amsdu_timer_id[priority] = 0;
#endif
		}
		txcfg->q_num = q_num;
		priv->pshare->rf_ft_var.amsduChkPktNum++;
		ret = amsdu_xmit(priv, pstat, txcfg, priority, 0, wdsDev, priv->dev);
		if (ret == 0) // not finish
			add_timer = 1;
		else
			add_timer = 0;
...
#ifdef TX_AMSDU_TIMER_LIST
	else if (list_empty(&priv->pshare->amsdu_timer_list))
		cancel_timer2(priv);
#endif
    ...
    if (is_MCS_rate(txcfg->tx_rate))    // HT rates
    {
        ...
        if (
            ...
            if (txcfg->pstat && txcfg->pstat->ADDBA_ready[TID] && !txcfg->pstat->low_tp_disable_ampdu) {
                ...
                // assign aggr density
				if (0 != priv->pshare->rf_ft_var.diffAmpduSz ) {
					pdesc_data->ampduDensity =  priv->pshare->rf_ft_var.diffAmpduSz;  // cliff test
...
#endif
			else if ((GetFrameSubType(txcfg->phdr) == WIFI_WMM_ACTION) &&
				txcfg->pstat->is_trigger_addba) {
				//printk("is_trigger_addba!!, dataRtyLmt = 16\n");
				pdesc_data->dataRtyLmt = 48;
				pdesc_data->useRate  = TRUE;
				pdesc_data->dataRate = 11; /* 54m */
			}
            else {
...
#if defined(NEAR_END_RX_REFINE) && defined(CONFIG_WLAN_HAL_8822BE)
	if ((GET_CHIP_VER(priv) == VERSION_8822B) && priv->pshare->ner_rx_mode) {
		pdesc_data->TXPowerOffset = 2;
	}
#endif
...
//		priv->pshare->phw->bErpProtection = TRUE;
	...
	if (txcfg->need_ack) {
...
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		else if (priv->pshare->rf_ft_var.low_retry_rssi_thd && !txcfg->fixed_rate && txcfg->pstat->rssi
				&& (txcfg->pstat->rssi < priv->pshare->rf_ft_var.low_retry_rssi_thd)) {
			if ((txcfg->tx_rate <= _9M_RATE_) ||
				((_MCS0_RATE_ == txcfg->tx_rate) || (_MCS1_RATE_ == txcfg->tx_rate))) {
				pdesc->Dword5 |= set_desc(TX_RtyLmtEn);
		...
		} else if (priv->pshare->rf_ft_var.low_rate_low_retry && !txcfg->fixed_rate
				&& ((txcfg->tx_rate <= _9M_RATE_)
				|| (_MCS0_RATE_ == txcfg->tx_rate) || (_MCS1_RATE_ == txcfg->tx_rate))) {
			pdesc->Dword5 |= set_desc(TX_RtyLmtEn);
			pdesc->Dword5 |= set_desc((0x08 & TX_DataRtyLmtMask) << TX_DataRtyLmtSHIFT);
...
#else
	PRI_TO_QNUM(pri, ret, priv->pmib->dot11OperationEntry.wifi_specific);
#endif
#if 0
	if (pstat)
	{
...
//	if (txcfg->is_ICMP_req) 
//		GDEBUG(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ICMP REQ seq=0x%x\n", txcfg->icmp_seq);
//	else	if (txcfg->is_ICMP_reply) 
//		GDEBUG(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>> ICMP REPLY seq=0x%x\n", txcfg->icmp_seq);	
...
swenc_end:
    ...
    if (IS_SUPPORT_AP_SWPS_OFFLOAD(priv)) {
        ...
        pdescinfo->Drop_ID = txcfg->pstat->Drop_ID;
         GDEBUG("macid =%x,aid = %x,tx pkt swps seq=%x, drop id=%x\n",txcfg->pstat->cmn_info.aid,txcfg->pstat->cmn_info.aid,pdescinfo->SWPS_sequence,pdescinfo->Drop_ID);
        }
    }
...
#endif
#ifdef WLAN_HAL_TX_AMSDU
            if ((AMSDU_ENABLE >= 2) && IS_SUPPORT_TX_AMSDU(priv) && (txcfg->aggre_en == FG_AGGRE_MSDU_FIRST) && (halQNum-1 < HCI_TX_AMSDU_DMA_QUEUE_MAX_NUM)) {
                ptx_dma_amsdu  = (PHCI_TX_AMSDU_DMA_MANAGER_88XX)(_GET_HAL_DATA(priv)->PTxDMAAMSDU88XX);
                cur_txbd_amsdu = ptx_dma_amsdu->tx_amsdu_queue[halQNum-1].pTXBD_head_amsdu + cur_q->host_idx;
...
#ifdef WLAN_HAL_TX_AMSDU
    if(IS_SUPPORT_TX_AMSDU(priv)){
    	pdescinfo->buf_paddr_amsdu[priv->amsdu_num]=get_desc(cur_txbd_amsdu->TXBD_ELE[priv->amsdu_num].Dword1);
		pdescinfo->buf_len_amsdu[priv->amsdu_num]=get_desc(cur_txbd_amsdu->TXBD_ELE[priv->amsdu_num].Dword0) & 0xffff;
		priv->amsdu_num++;
    }
#endif
...
__inline__ static int rtl8192cd_swq_bdfull(struct rtl8192cd_priv *priv, unsigned char qnum, int qlen)
{
    int *tx_head=NULL, *tx_tail=NULL;
...
#endif
    {
        ...
        if((qlen) > CIRC_SPACE_RTK(*tx_head, *tx_tail, CURRENT_NUM_TX_DESC))
            return 1;
    }
...
#endif
__IRAM_IN_865X
__inline__ static int rtl8192cd_swq_dequeue(struct rtl8192cd_priv *priv, struct stat_info *pstat,  unsigned char qnum, unsigned int dqnum, unsigned char mode)
{
	int count = 0;
	
	if(pstat->swq.swq_queue_len_max[qnum] < skb_queue_len(&pstat->swq.swq_queue[qnum]))
		pstat->swq.swq_queue_len_max[qnum] = skb_queue_len(&pstat->swq.swq_queue[qnum]);
	
	while (1)
	{
		if(rtl8192cd_swq_bdfull(priv, qnum, 5) && mode == SWQ_DEQ_LIMIT)
			break;
		...
		if(mode == SWQ_DEQ_LIMIT && priv->pshare->rf_ft_var.swq_max_xmit)
			if(count == priv->pshare->rf_ft_var.swq_max_xmit)
				break;
...
__inline__ static UINT32 get_swq_tri_time_using_hw_timer(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info *pstat)
{
...
#if defined(WIFI_LOGO_11N_4_2_47) && !defined(SMP_LOAD_BALANCE_SUPPORT)
	if(priv->is_ssid_ps && list_empty(&pstat->sleep_list)) {
		tri_time = RTL_R32(TSFTR1) + 500;		
	} else
#endif
    if(priv->pshare->swq_numActiveSTA > priv->pshare->rf_ft_var.swq_turbo_sta_th && (priv->up_time - priv->pshare->swq_turbo_time) < priv->pshare->rf_ft_var.swqmaxturbotime)
		tri_time = 20;
	...
	else if (pstat->tx_avarage > 275000){      //2.2M~5M
		GET_TRI_TIME_VALUE(tri_time,skb,priv,4);
	}
    else if (pstat->tx_avarage > 250000){        //2M~2.2M
...
__inline__ static int rtl8192cd_swq_enqueue(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info	*pstat)
{
	...
	if(priv->pshare->swq_use_hw_timer) {
		...
		if (GET_ROOT(priv)->pmib->miscEntry.vap_enable) {
			...
			for (i=0; i<RTL8192CD_NUM_VWLAN; ++i) {
				...
				if (vap_priv && IS_DRV_OPEN(vap_priv) && (GET_MIB(vap_priv)->dot11OperationEntry.opmode & WIFI_AP_STATE)) 
...
#endif	
		{
			...
			if ((1250000 < pstat->rx_avarage) && ((pstat->tx_avarage * 5) < pstat->rx_avarage))
		    {
				...
				if(assoc_num <= 4) {
					if(pstat->swq.swq_en[q_num]) {
						if(!rtl8192cd_swq_bdfull(priv, q_num, CURRENT_NUM_TX_DESC/2))
							change = 1;
					} else {
						if(rtl8192cd_swq_bdfull(priv, q_num, 32))
							change = 2;
					}
				    if(change) {
					    if(pstat->swq.swq_en[q_num]) {
						    ...
						    rtl8192cd_swq_dequeue(priv, pstat, q_num, 0xffffffff, SWQ_DEQ_LIMIT);
				...
				if(assoc_num <= assoc_num_low && (pstat->tx_avarage < MBPS_TO_BIT(5) || MBPS_TO_BIT(160) < pstat->tx_avarage)) {
					...
					if(assoc_num > 4 || (687500 < pstat->tx_avarage && pstat->tx_avarage < 19375000)) {	/* 5.5M < TP < 155M*/
						pstat->swq.swq_en[q_num] = 1;                                                   /* enable swq */ 
						pstat->swq.swq_prev_timeout[q_num] = 1;
						pstat->swq.swq_timeout_change[q_num] = 0;
						pstat->swq.swq_keeptime[q_num] = 0;		 
	...
	if(change != 1) {
				...
				if (priv->pshare->rf_ft_var.dqnum){
					if(qlen >= psta[0]->mu_deq_num && qlen2 >= psta[1]->mu_deq_num){
					    ...
					    {						
					    	...
					    	if(rtl8192cd_swq_bdfull(priv, q_num, 5)) {           
								break;
					    	}
							if(count > 0 && count2 > 0) {
								if(count == count2) {
								...
								if(rtl8192cd_swq_bdfull(priv, q_num, sta_deq_num[0]+sta_deq_num[1])) {
									break;
					    		}
	    ...
	    if(need_deque) {
	        if (pstat->swq.swq_timer_id[q_num])
	            rtl8192cd_swq_deltimer(priv, pstat, q_num);
	        if(rtl8192cd_swq_bdfull(priv, q_num, (pstat->swq.q_aggnum[q_num]+1) * 2)) {
#ifdef SW_TXQ_RSVD_DESC
				if (pstat->ADDBA_ready[pri])
...
void rtl8192cd_swq_timeout(unsigned long task_priv)
{
	...
	while (head != priv->pshare->swq_timer_tail) /* check if queue is empty */
	{
		...
		if (swq_timer->pstat) {
			...
			add_timer = 0;
			if(need_dequeue) {
                bdfull = rtl8192cd_swq_bdfull(priv, qnum, (pstat->swq.q_aggnum[qnum] * 2));
                if(bdfull) {
					add_timer = 1;
					if(priv->pshare->swq_use_hw_timer) {
						add_timer_timeout = 1;   
	...
	for(qnum = VO_QUEUE; qnum >= BK_QUEUE ; qnum--){
		...
		while(sta_cnt < NUM_STAT){
			...
			if (priv->pshare->aidarray[(i%NUM_STAT)] && (priv->pshare->aidarray[(i%NUM_STAT)]->used == TRUE)){
				...
				bd_full = rtl8192cd_swq_bdfull(priv, qnum, 4);
				if(!bd_full){					
					burst_num = (priv->pshare->rf_ft_var.atm_aggmax*pstat->atm_burst_size)/priv->pshare->atm_max_burstsize;
					if(burst_num < priv->pshare->rf_ft_var.atm_aggmin)
						burst_num = priv->pshare->rf_ft_var.atm_aggmin;
...
check_swq_enq:
...
#ifdef TCP_ACK_ACC
		if (pstat && priv->pmib->miscEntry.tcpack_acc && (pstat->tcpack_acc_en || pstat->veriwave_test) && tx_flag != TX_NO_TCPACK_ACC) {
	...
	if (swq_out == 0
		...
		|| (priv->pmib->dot11nConfigEntry.dot11nAMSDUSmallPkts &&
		   (skb->len <= priv->pmib->dot11nConfigEntry.dot11nAMSDUSmallPktLen || IPV4_TCP_ACK_PACKET(skb->data))
		   && pstat->IOTPeer == HT_IOT_PEER_INTEL) //Intel only
#endif
		...
		unsigned long cur_queue_pkt = priv->ext_stats.swq_real_enque_pkt - priv->ext_stats.swq_real_deque_pkt;
		if (UINT32_DIFF(cur_queue_pkt, priv->ext_stats.swq_residual_drop_pkt) > priv->pshare->rf_ft_var.swq_max_enqueu_len) 
		{
...
//if(IS_ICMPV4_PROTO(skb->data)) {		
//	if (skb->data[34] == 0x08) {
//		txcfg->is_ICMP_req = 1;
//		txcfg->icmp_seq = ((skb->data[40] << 8)|skb->data[41]);
//	}
//	else if (skb->data[34] == 0) {
//		txcfg->is_ICMP_reply = 1;
//		txcfg->icmp_seq = ((skb->data[40] << 8)|skb->data[41]);
//	}
//}
//if (txcfg->is_ICMP_req) 
...
//	GDEBUG(" >>>>>>>>>>>>>>>>>>> ICMP REPLY seq=0x%x\n", txcfg->icmp_seq);	
    ...
    if (( skb->cb[33] != 0x11 )
        ...
        skb->cb[34] = pri & 0xFF;
        skb->cb[35] = q_num & 0xFF;  
		txcfg->q_pri = skb->cb[34];
        txcfg->q_num = skb->cb[35];
    }   
#endif // TX_SKB_REFINE
...
#ifdef SUPPORT_TX_AMSDU_SHORTCUT
	if(skb->cb[47] & (BIT(FG_AGGRE_MSDU_FIRST)|BIT(FG_AGGRE_MSDU_MIDDLE)|BIT(FG_AGGRE_MSDU_LAST))){
		// from amsdu_xmit, don't go to amsdu_check again
		if((skb->cb[47] & (BIT(FG_AGGRE_MSDU_FIRST)|BIT(FG_AGGRE_MSDU_MIDDLE)|BIT(FG_AGGRE_MSDU_LAST))) !=
			 (BIT(FG_AGGRE_MSDU_FIRST)|BIT(FG_AGGRE_MSDU_MIDDLE)|BIT(FG_AGGRE_MSDU_LAST))){
		...
		if(skb->cb[47]&BIT(FG_AGGRE_MSDU_FIRST))
			...
			txcfg->aggre_en = FG_AGGRE_MSDU_LAST;
		}//if flag is equal to First|MIDDLE|LAST, transmit as normal packet without AMSDU
	}else 
#endif    
	if (IS_SUPPORT_TX_AMSDU(priv) 
#if defined(CONFIG_RTK_MESH)
    ...
    if (( skb->cb[33] != 0x11 )
            && ((priv->assoc_num > 1) && pstat)) {
#ifdef CONFIG_RTK_MESH
		txcfg->q_pri = get_skb_priority3(priv, skb, txcfg->is_11s, pstat);
#else		
		txcfg->q_pri = get_skb_priority(priv, skb, pstat);
#endif
    }
#endif
			...
			&& ((k = get_tx_sc_index(priv, pstat, skb->data)) >= 0))
	{
		...
		if (
			...
			(txsc_txcfg->fr_len > 0) &&
#ifdef TX_SKB_REFINE
             (txcfg->q_pri == ptxsc_entry->pktpri) &&
#else
#ifdef CONFIG_RTK_MESH
...
#endif
			{
				...
				if (IS_HAL_CHIP(priv)) {
					bUnAvail = 
						...
						compareAvailableTXBD(priv, num_rsvd + num_swq_rsvd, q_num, 2) :
#endif
						COMPAREAVAILABLETXBD(ptx_dma, q_num, needTxBD + num_swq_rsvd) ;
				} else
#endif
				{
#ifdef TX_SCATTER
...
#else
					bUnAvail = ((num_rsvd + num_swq_rsvd) > CIRC_SPACE_RTK(*tx_head, *tx_tail, CURRENT_NUM_TX_DESC));
#endif
				}
			}
...

			{
				...
				if(txcfg->aggre_en >= FG_AGGRE_MSDU_FIRST){
					...
					if (skb_headroom(skb) < sizeof(struct llc_snap)||skb_tailroom(skb)< needed_tailroom) {
						...
						if (skb->cb[33] == 0x11) {
							skb2->cb[33] = 0x11;
							skb2->cb[34] = skb->cb[34];
							skb2->cb[35] = skb->cb[35];
						}
#endif
#ifdef MCAST2UI_REFINE
...
static int check_txdesc(struct rtl8192cd_priv *priv, struct tx_insn* txcfg)
{
	...
	if (txcfg->fr_type == _SKB_FRAME_TYPE_)
	{
			...
			)
		{
...
#endif			
			if ((!txcfg->is_test) && protocol == 0x888e ){	/*EAP type packets*/
			...
			if ((!txcfg->is_test) && (OPMODE & WIFI_AP_STATE) && !IS_MCAST(da) && pskb && (isDHCPpkt(pskb) == TRUE))
				is_dhcp++;
...
#ifdef CONFIG_WLAN_HAL
			if (IS_HAL_CHIP(priv))
				bUnAvail = compareAvailableTXBD(priv, val32+1+num_swq_rsvd, q_num, 2);
			else
#endif
#ifdef OSK_LOW_TX_DESC
				bUnAvail = ((val32 + 2 + num_swq_rsvd) > CIRC_SPACE_RTK(*tx_head, *tx_tail, ((q_num==BE_QUEUE || q_num==HIGH_QUEUE)?BE_TXDESC:NONBE_TXDESC))); //per mpdu, we need 2 desc...
#else
				bUnAvail = ((val32 + 2 + num_swq_rsvd) > CIRC_SPACE_RTK(*tx_head, *tx_tail, CURRENT_NUM_TX_DESC)); //per mpdu, we need 2 desc...
#endif
		}

		if (bUnAvail) {
				if(IS_HAL_CHIP(priv)) {
					DEBUG_ERR("%s:%d: tx drop: %d hw Queue desc not available!\n", __FUNCTION__, __LINE__, q_num);
				} else
					DEBUG_ERR("%d hw Queue desc not available! head=%d, tail=%d request %d\n",q_num,*tx_head,*tx_tail,2);
				//SMP_UNLOCK_XMIT(x);
				return CONGESTED;
...
)
{
    ...
    GDEBUG("head seq=%x, tail seq=%x, SWPS_sequence=%x,RPT_rep=%x, RPT_TxDone=%x\n",head,tail,SWPS_sequence,RPT_reprepare_SEQ,RPT_TxDone_SEQ);
    if(head >= tail){
        if((SWPS_sequence <= RPT_TxDone_SEQ) && ((RPT_TxDone_SEQ-SWPS_sequence) < (MAX_SWPS_SEQ/2)))//to prevent get old report txdone, if not drop done when wakeup, will get old info
            ...
            result=STOP_RECYCLE;    //STOP_RECYCLE //wait for new report
            ...
            result=REPREPARE;       //REPREPARE
        else
            result=STOP_RECYCLE;   //STOP_RECYCLE
    }
    else if(tail > head){
	...
	while (CIRC_CNT_RTK(head, tail, ptx_dma->tx_queue[halQnum].total_txbd_num))
	{
        ...
        if (IS_SUPPORT_AP_SWPS_OFFLOAD(priv)) {
            ...
            pkt_Qos = pdescinfo->SWPS_pkt_Qos;
				...
				GDEBUG("pdescinfo->SWPS_sequence=%x, Pkt_Qos=%x,head=%x,tail=%x\n",
				pdescinfo->SWPS_sequence,pkt_Qos,head,tail);  //yllinSWPSprint
            ...
            if(pkt_Qos == -2)
                ...
                recycle_condition = SWPS_check_recycle_condition(priv,pdescinfo->SWPS_sequence,
					                                             pdescinfo->swps_pstat->RPT_TxDone_SEQ[pkt_Qos],
					                                             pdescinfo->swps_pstat->RPT_reprepare_SEQ[pkt_Qos],
					                                             pdescinfo->swps_pstat->SWPS_seq_head[pkt_Qos],
					                                             pdescinfo->swps_pstat->SWPS_seq_tail[pkt_Qos]);
                if((recycle_condition==RECYCLE) || (recycle_condition==REPREPARE)) //recycle
                {
                   ...
                   GDEBUG("recycle condition=0...ERROR!\n");
		...
		if (pdescinfo->type == _SKB_FRAME_TYPE_)
		{
...
#endif
			{
                ...
                if (IS_SUPPORT_AP_SWPS_OFFLOAD(priv)) {
                    ...
                    if(pkt_Qos < 0){
                        GDEBUG("recycle not AC packet, packet Qos =%d\n", pkt_Qos);
                    ...
                    if(pkt_Qos < 0){
                        ...
                        if(SWPS_dz_queue_insert(priv,pdescinfo,pdescinfo->pframe,&pkt_insert_position)!=TRUE)
                            GDEBUG("insert packet at AC dz_queue head fail.\n");
                        else
                            pdescinfo->pframe = NULL;
                    }
                    ...
                    if (IS_SUPPORT_AP_SWPS_OFFLOAD(priv)) {
                        ...
                        if(pkt_Qos < 0){
                            ...
                            if(SWPS_dz_queue_insert(priv,pdescinfo,pdescinfo->buf_pframe[cnt],&pkt_insert_position)!=TRUE)
                                GDEBUG("insert frame into dz_queue list fail,pkt_insert_position=%x, pkt_qos=%x.\n",pkt_insert_position,pdescinfo->SWPS_pkt_Qos);
                            else
                                pdescinfo->buf_pframe[cnt]=NULL;
                        }else if(pdescinfo->swps_pstat->SWPS_last_seq[pkt_Qos]==0 && pdescinfo->swps_pstat->PS_get_SWPSRPT==1){
                            pdescinfo->swps_pstat->prepare_done[pkt_Qos]=1;
                            GDEBUG("no packet to prepare, set prepare_done[%x]=1\n",pkt_Qos);
                        }
                        else 
                            priv->ext_stats.freeskb_err++;
                        //RESTORE_INT(flags);
...
#ifdef WLAN_HAL_TX_AMSDU
#ifndef SUPPORT_TX_AMSDU_SMALL_PKTS_ONLY
		if((AMSDU_ENABLE >= 2) && IS_SUPPORT_TX_AMSDU(priv))
#endif
		{
...
#ifdef AP_SWPS_OFFLOAD 
        if(recycle_condition == REPREPARE){
            GDEBUG("pkt_insert_position=%d,pkt_Qos=%d,AMSDU_flag=%d\n",pkt_insert_position,pkt_Qos,AMSDU_flag);  //yllinSWPSprint
            pkt_info_result = SWPS_reprepare_pktinfo_enqueue(priv,pdescinfo,pkt_Qos,pkt_insert_position,AMSDU_flag);
            if(!pkt_info_result)
                GDEBUG("Reprepare pkt info fail.......\n");//debug
...
int tx_tcp_ack_accelerate(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct sk_buff *skb)
{
	...
	unsigned int timeout = priv->pmib->miscEntry.tcpack_to;
	unsigned long seq;
	unsigned int ack_th = (pstat->veriwave_test ? 0 : 
