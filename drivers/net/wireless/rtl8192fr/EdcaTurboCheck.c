...
//				odm_write_2byte(p_dm_odm, RD_CTRL, odm_read_2byte(p_dm_odm, RD_CTRL) & ~(DIS_TXOP_CFE));
#endif
			...
			else 
			{
				...
				if(GET_CHIP_VER(priv) == VERSION_8814A || GET_CHIP_VER(priv) == VERSION_8822B || GET_CHIP_VER(priv) == VERSION_8197F)
				{
...
#else					
					{
						...
						} else if ( (pstat && (pstat->IOTPeer == RTK_CAP_IE_8814)) //parameter for DWA-192
							&& (priv->pshare->th_mode) 
							&& (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_5G))
						{
								if(priv->pshare->th_mode == 1){
...
#ifdef CONFIG_RTL8672
	if (priv->pshare->th_mode == 2 && priv->pshare->txop_enlarge == TXOP_ENLARGE_DEFAULT) {
