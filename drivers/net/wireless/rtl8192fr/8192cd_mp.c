...
#if (IC_LEVEL >= LEVEL_8814) || (IC_LEVEL == LEVEL_92E)
	if(CHECKICIS8814() || CHECKICIS97F() ||CHECKICIS8822() || CHECKICIS8821() || (GET_CHIP_VER(priv) == VERSION_8192E) || (GET_CHIP_VER(priv) == VERSION_8192F)) //wish
		mp_set_tmac_tx(priv);
#endif	
	// get count
...
void mp_set_tmac_tx(struct rtl8192cd_priv *priv)
{
	...
	if(!CHECKICIS97F() && (GET_CHIP_VER(priv) != VERSION_8192E) && (GET_CHIP_VER(priv) != VERSION_8192F)){
...
#endif
		{
			odm_false_alarm_counter_statistics(ODMPTR);
...
#if defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8881A) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8821CE)
	if ((GET_CHIP_VER(priv)==VERSION_8814A) || (GET_CHIP_VER(priv)==VERSION_8812E) || (GET_CHIP_VER(priv)==VERSION_8881A) || (GET_CHIP_VER(priv)==VERSION_8822B) || (GET_CHIP_VER(priv)==VERSION_8821C))
		phy_set_rf_reg(priv, RF_PATH_A, 0x42, BIT(17), 0x1);
	else
#endif	
#if defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8192FE) || defined(CONFIG_RTL_8723B_SUPPORT) || defined(CONFIG_WLAN_HAL_8197F)
	if ((GET_CHIP_VER(priv)==VERSION_8188E) || (GET_CHIP_VER(priv)==VERSION_8192E) || (GET_CHIP_VER(priv)==VERSION_8192F) || (GET_CHIP_VER(priv) == VERSION_8723B) || (GET_CHIP_VER(priv) == VERSION_8197F))
...
int mp_get_txpwr(struct rtl8192cd_priv *priv, unsigned char *data, unsigned int data_len)
{
	...
	snprintf(data, data_len, "%d %d", pwrA, pwrB);
	return strlen(data)+1;
}
...
#endif
#if defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8192FE)
