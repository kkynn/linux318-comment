...
# _PC_ , _BRLADLIGHT_ , _PUMA6_ , _MNDSPEED_ , _POWERPC_, _ARM_BALONG_, ARM_QUALCOMM_9x15, _TEGRA_TK1_
...
ifeq ($(CONFIG_PLATFORM),_TEGRA_TK1_)
	EXTRA_CFLAGS += -D_PC_
	EXTRA_CFLAGS += -D_TEGRA_TK1_
	EXTRA_CFLAGS += -D_LITTLE_ENDIAN_
	EXTRA_CFLAGS += -DSMP_SYNC

	SUBARCH := $(shell uname -m | sed -e s/v7l//)
	ARCH ?= $(SUBARCH)
	CROSS_COMPILE ?=
	KVER  := $(shell uname -r)
	KSRC := /lib/modules/$(KVER)/build
