...
#endif
unsigned char		is_ICMP_req;
unsigned char		is_ICMP_reply;
u2Byte			icmp_seq;
...
#endif
	unsigned short		veriwave_test; // 0x00: none, 0x01: veriwave 5G throughput TCP Rx
...
//#endif
	...
	unsigned char		is_trigger_addba;
...
//	UINT8		IOTPeer;			// Enum value.	HT_IOT_PEER_E
...
#ifdef STA_CONTROL_BAND_TRANSITION
	unsigned int stactrl_steer_detect_counter;  // time (sec) that STA keeps on satisfying band transition requirement
#endif  // STA_CONTROL_BAND_TRANSITION
	...
	unsigned char is_near_2ss_sta;
	unsigned char is_near_1ss_sta;
#endif
};
...
#endif
#ifdef CONFIG_SPECIAL_ENV_TEST
	unsigned long       start_xmit_pkt_1518;
	unsigned long       netif_rx_pkt_1518;
#endif	
...
//#endif
	...
	unsigned int		amsduTOPktNum;		// AMSDU xmit via amsduTimeout
	unsigned int		amsduChkPktNum;		// AMSDU xmit via amsduCheck
...
#ifdef NEAR_END_RX_REFINE
	unsigned char		ner_en;
	unsigned char		ner_force;	// 1: apply; 2: not apply; others: auto decision
	unsigned char		ner_rssi;	// if rssi is greater than or equal to this threshold, it will be treated as near end
	unsigned char		ner_learning_time;
	unsigned char		ner_learning_rate;
#endif
};
...
#endif
#if defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_RTL_8197F)	|| defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8192FE)
...
#if defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8192FE)	
	UINT64					mp_rx_mac_ok;
...
#endif
#if defined(NEAR_END_RX_REFINE)
	unsigned int			ht_1ss_sta_num;
	unsigned int			ht_2ss_sta_num;
	unsigned char			ner_rx_mode;
	unsigned char			ner_learning_end;
	unsigned char			ner_learning_start;
	unsigned char			ner_learning_time;
	unsigned char			ner_learning_state;
	unsigned long			ner_rx_bytes;
	unsigned int			ner_rx_tput_mode0;
	unsigned int			ner_rx_tput_mode1;
	unsigned int			rfe_sel_backup[4];
	unsigned int			tx_pwr_ctrl_backup;
#endif
...
//	unsigned int			pkt_in_hiQ;
	...
	struct amsdu_timer			amsdu_timer[AMSDU_TIMER_NUM];
#ifdef TX_AMSDU_TIMER_LIST
	struct list_head			amsdu_timer_list;
#else
...
typedef struct rtl8192cd_priv {
	...
	struct rtl8192cd_priv *dualband_rootpriv; //another band's root priv
#endif
...
#if defined(CONFIG_IEEE80211R) || defined(CONFIG_IEEE80211R_CLI)
#if defined (SUPPORT_FAST_CONFIG)
#define MDID_LEN					2		
...
#define IPV4_TCP_ACK_PACKET(eth_packet)  ( eth_packet[12] ==0x08 	&& eth_packet[13] ==0x00 \
													&&  eth_packet[23] ==0x06    \
													&&  eth_packet[47] == 0x10 \
													&&  (*(unsigned short *)(eth_packet+16) == (((*(unsigned char *)(eth_packet+14)&0xF)<<2)+(*(unsigned char *)(eth_packet+46)>>4<<2))))
