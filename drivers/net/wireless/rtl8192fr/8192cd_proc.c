...
#if defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8881A) || defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8821CE)//FOR_8812_MP
#ifdef CONFIG_RTL_PROC_NEW
...
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8881A) || defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8821CE) //FOR_8812_MP
...
#endif
#if defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8821CE)
...
#endif			
{
	...
	PRINT_SINGL_ARG("    amsduTOPktNum: ", priv->pshare->rf_ft_var.amsduTOPktNum, "%d");
	PRINT_SINGL_ARG("    amsduChkPktNum: ", priv->pshare->rf_ft_var.amsduChkPktNum, "%d");
...
#endif			
{
	...
	PRINT_ONE(priv->pshare->rf_ft_var.ner_force, "    rx mode(force%d): ", 0);
	PRINT_ONE(priv->pshare->ner_rx_mode, "%d", 1);
#endif
	return pos;
}
...
#else
    size = snprintf(pTmp, length, "  Make info: v%d.%d (%s)\n", DRV_VERSION_H, DRV_VERSION_L, DRV_RELDATE);
    CHECK_LEN_E;
#endif
...
#endif
    {
        ...
        if (GET_CHIP_VER(priv) == VERSION_8814A)	
            ...
            seq_printf(s, "  RTL8192 firmware version: %d.%d,  built time: %02x-%02x %02x:%02x\n", priv->pshare->fw_version,
                ...
                priv->pshare->fw_date_hour, priv->pshare->fw_date_minute);
#else
        size = snprintf(pTmp, length-len, "  RTL8192 firmware version: %d.%d,  built time: %02x-%02x %02x:%02x\n", priv->pshare->fw_version,
            priv->pshare->fw_sub_version, priv->pshare->fw_date_month, priv->pshare->fw_date_day,
            priv->pshare->fw_date_hour, priv->pshare->fw_date_minute);
        CHECK_LEN_E;
#endif
...
//#endif
	...
	if(pstat->muPartner) {
		PRINT_SINGL_ARG("	 mu Partner aid: ", pstat->muPartner->cmn_info.aid, "%d");
	} else {
...
/* Hotspot 2.0 Release 1 */
	...
	if(pstat->current_tx_pkt_size == VERIWAVE_PKT_SIZE_BIG) {
		PRINT_ONE("    veriwave test: 2G big pkts", "%s", 1);
	}
	else if(pstat->current_tx_pkt_size == VERIWAVE_PKT_SIZE_SML) {
		PRINT_ONE("    veriwave test: 2G small pkts", "%s", 1);
	}
	CHECK_LEN_B;
...
#endif
#ifdef CONFIG_SPECIAL_ENV_TEST
       PRINT_SINGL_ARG("    start_xmit_pkt_1518:    ", priv->ext_stats.start_xmit_pkt_1518, "%lu");
        PRINT_SINGL_ARG("    netif_rx_pkt_1518:    ", priv->ext_stats.netif_rx_pkt_1518, "%lu");
#endif
