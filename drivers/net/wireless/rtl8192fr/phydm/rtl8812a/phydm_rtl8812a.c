...
void odm_update_tx_path_8812a(struct dm_struct *dm, u8 path)
{
	...
	if (dm_path_div->resp_tx_path != path) {
		...
		if (path == RF_PATH_A) {
			...
			odm_set_bb_reg(dm, R_0x80c, 0xFFF0, 0x222); /* Tx by Reg */
			odm_set_bb_reg(dm, R_0x6d8, BIT(7) | BIT6, 2); /* Resp Tx by Txinfo */
		}
	}
...
void odm_path_diversity_init_8812a(
	struct dm_struct *dm)
{
	u32 i;
	struct _ODM_PATH_DIVERSITY_ *dm_path_div = &dm->dm_path_div;
	...
	for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
		...
		if (is_sta_active(entry)) {
			...
			PHYDM_DBG(dm, DBG_PATH_DIV,
				  "mac_id=%d, path_a_sum=%d, path_a_cnt=%d\n",
				  i, dm_path_div->path_a_sum[i],
				  dm_path_div->path_a_cnt[i]);
			PHYDM_DBG(dm, DBG_PATH_DIV,
				  "mac_id=%d, path_b_sum=%d, path_b_cnt=%d\n",
				  i, dm_path_div->path_b_sum[i],
				  dm_path_div->path_b_cnt[i]);
			PHYDM_DBG(dm, DBG_PATH_DIV,
				  "mac_id=%d, rssi_avg_a= %d, rssi_avg_b= %d\n",
			...
			PHYDM_DBG(dm, DBG_ANT_DIV,
				  "Tx from TxInfo, target_path=%s\n",
				  (target_path == RF_PATH_A) ? "RF_PATH_A" :
				  "RF_PATH_B");
			PHYDM_DBG(dm, DBG_ANT_DIV,
				  "dm_path_div->path_sel[%d] = %d\n", i,
				  dm_path_div->path_sel[i]);
		}
...
void odm_set_tx_path_by_tx_info_8812a(
	struct dm_struct *dm,
	u8 *desc,
	u8 mac_id)
{
