...
#else
/*Originally p_config->do_iqk is hooked phy_iq_calibrate_8198f, but do_iqk_8198f and phy_iq_calibrate_8198f have different arguments*/
void do_iqk_8198f(
	void *dm_void,
	u8 delta_thermal_index,
	u8 thermal_value,
...
void _phydm_get_read_counter_8198f(struct dm_struct *dm)
{
	u32 counter = 0x0;
#if 0
	while (1) {
	...
	u8 num_backup_bb_reg)
{
	...
	for (i = 0; i < IQK_MAC_REG_NUM_8198f; i++)
...
void _iqk_backup_rf_8198f(
	struct dm_struct *dm,
	u32 RF_backup[][SS_8198F],
	u32 *backup_rf_reg)
{
	u32 i, j;

	for (i = 0; i < IQK_RF_REG_NUM_8198F; i++)
	...
	struct dm_struct *dm)
{
	...
	odm_write_4byte(dm, 0x1b00, 0x00000008);
	odm_write_4byte(dm, 0x1b00, 0x00057008);
	odm_write_4byte(dm, 0x1b00, 0x000b0008);
	odm_write_4byte(dm, 0x1b00, 0x00000008);
}
	...
	boolean do_iqk)
{
	if (do_iqk) {
		...
		odm_write_4byte(dm, 0xc60, 0x70000000);
		odm_write_4byte(dm, 0xc60, 0x700B005A);
	}
	...
	u32 RF_backup[][SS_8198F])
{
	...
	for (i = 2; i < (IQK_RF_REG_NUM_8198F - 1); i++)
...
void _iqk_configure_macbb_8198f(
	struct dm_struct *dm)
{
	...
	u8 uPADindex)
{
	...
	u32 LOK0x56_2G = 0x12ee3;
	u32 LOK0x56_5G = 0x12eec;
	...
	case ODM_BAND_2_4G:
		...
		odm_write_4byte(dm, 0x1bcc, 0x12);
		...
		odm_set_rf_reg(dm, path, 0x8f, RFREGOFFSETMASK, 0xafc00);
		odm_set_rf_reg(dm, path, 0x56, RFREGOFFSETMASK, LOK0x56_2G);
		odm_set_rf_reg(dm, path, 0x33, BIT(3) | BIT(2) | BIT(1) | BIT(0), (~BIT(3)) & LOK0x33);
		break;
	case ODM_BAND_5G:
		//odm_write_1byte(dm, 0x1b2b, 0x00);
		odm_set_rf_reg(dm, path, 0x57, 0x00007000, 0x0);// TAG
		odm_set_rf_reg(dm, path, 0x57, 0x00000700, 0x2); //TXA
		odm_write_4byte(dm, 0x1bcc, 0x09);
		LOK0x56_5G = LOK0x56_5G & (0xfff1f | ((u32)uPADindex << 5));
		odm_set_rf_reg(dm, path, 0x56, RFREGOFFSETMASK, LOK0x56_5G);
		odm_set_rf_reg(dm, path, 0x8f, RFREGOFFSETMASK, 0xafc00);
		odm_set_rf_reg(dm, path, 0x33, BIT(3) | BIT(2) | BIT(1) | BIT(0), BIT(3) | LOK0x33);
		break;
	}
	...
	u8 path)
{
	...
	case ODM_BAND_2_4G:
		odm_set_rf_reg(dm, path, 0x57, 0x00007000, 0x0);// TAG
		odm_set_rf_reg(dm, path, 0x57, 0x00000700, 0x0);// TXA
		odm_set_rf_reg(dm, path, 0x56, RFREGOFFSETMASK, 0x14ee3);
		odm_set_rf_reg(dm, path, 0x8f, RFREGOFFSETMASK, 0xafc00);
		//odm_write_1byte(dm, 0x1b2b, 0x00);
		break;
	case ODM_BAND_5G:
		odm_set_rf_reg(dm, path, 0x57, 0x00007000, 0x0);// TAG
		odm_set_rf_reg(dm, path, 0x57, 0x00000700, 0x2); //TXA
		odm_set_rf_reg(dm, path, 0x56, RFREGOFFSETMASK, 0x12eec);
		odm_set_rf_reg(dm, path, 0x8f, RFREGOFFSETMASK, 0xafc00);
		//odm_write_1byte(dm, 0x1b2b, 0x00);
		break;
	}
	...
	u8 path)
{
	...
	odm_set_rf_reg(dm, path, 0xde, BIT(16), 0x1);
	odm_set_rf_reg(dm, path, 0xee, BIT(18), 0x0);
//	odm_set_rf_reg(dm, path, 0x57, 0x00007000, 0x0);
//	odm_set_rf_reg(dm, path, 0x57, 0x00000700, 0x0);
	odm_set_mac_reg(dm, 0x1b18, BIT(1), 0x0);

	switch (*dm->band_type) {
	case ODM_BAND_2_4G:
		odm_write_1byte(dm, 0x1bcc, 0x1b);
		odm_write_4byte(dm, 0x1b20, 0x00050008);
		odm_write_4byte(dm, 0x1b24, 0x00061068);
		odm_set_rf_reg(dm, path, 0x56, RFREGOFFSETMASK, 0x12eec);
		odm_set_rf_reg(dm, path, 0x8f, RFREGOFFSETMASK, 0xafc00);
		break;
	case ODM_BAND_5G:
		...
		odm_write_1byte(dm, 0x1b2b, 0x00);
		odm_write_4byte(dm, 0x1b20, 0x00050008);
		odm_write_4byte(dm, 0x1b24, 0x00061468);
		odm_set_rf_reg(dm, path, 0x56, RFREGOFFSETMASK, 0x12eec);
		odm_set_rf_reg(dm, path, 0x8f, RFREGOFFSETMASK, 0xafc00);
		break;
	}
	...
	u8 step)
{
	...
	if (step == RXIQK1)
		RF_DBG(dm, DBG_RF_IQK, "[IQK]============ RXIQK GainSearch ============\n");
	...
	ODM_delay_ms(GS_delay_8198F);
	fail = _iqk_check_cal_8198f(dm, IQK_CMD);
...
void _iqk_rxk2setting_by_toneindex_8198f(
	struct dm_struct *dm,
	u8 path,
	boolean is_gs,
	u8 toneindex)
...
boolean
_iqk_rx_iqk_gain_search_fail_by_toneindex_8198f(
	struct dm_struct *dm,
	u8 path,
	u8 step,
	u8 tone_index)
{
	...
	_iqk_rxk2setting_by_toneindex_8198f(dm, path, RXIQK1, tone_index);
	odm_write_4byte(dm, 0x1b00, 0x00000008 | path << 1);
	...
	)
{
	...
	if (LOK_notready)
		...
		RF_DBG(dm, DBG_RF_IQK, "In LOK, RF08 = %x, RF00=%x\n", odm_get_rf_reg(dm, (enum rf_path)path, 0x8, RFREGOFFSETMASK), odm_get_rf_reg(dm, (enum rf_path)path, 0x0, RFREGOFFSETMASK));
		RF_DBG(dm, DBG_RF_IQK, "In LOK, RF58 = %x\n", odm_get_rf_reg(dm, (enum rf_path)path, 0x58, RFREGOFFSETMASK));
	...
	u8 idx)
{
	...
	if (idx == TX_IQK)
		RF_DBG(dm, DBG_RF_IQK, "[IQK]============ WBTXIQK ============\n");
	else if (idx == RXIQK1)
		RF_DBG(dm, DBG_RF_IQK, "[IQK]============ WBRXIQK STEP1============\n");
	else
		RF_DBG(dm, DBG_RF_IQK, "[IQK]============ WBRXIQK STEP2============\n");
	...
	//	if (odm_get_rf_reg(dm, path, 0x08, RFREGOFFSETMASK) != 0x0)
	//		odm_set_rf_reg(dm, path, 0x8, RFREGOFFSETMASK, 0x0);
	...
	fail = _iqk_check_nctl_done_8198f(dm, path, IQK_CMD);
	ODM_delay_ms(10);
	...
	if (dm->debug_components & DBG_RF_IQK) {
		...
		if (idx != TXIQK)
			RF_DBG(dm, DBG_RF_IQK,
				...
				odm_get_rf_reg(dm, path, 0x0, RFREGOFFSETMASK),  0x56, odm_get_rf_reg(dm, path, 0x56, RFREGOFFSETMASK));
	}
	...
	u8 path)
{
	...
	case 1: /*gain search_RXK1*/
		_iqk_rxk1setting_8198f(dm, path);
		gonext = false;
		while (1) {
	...
	case 2: /*gain search_RXK2*/
		_iqk_rxk2setting_8198f(dm, path, true);
	...
	case 3: /*RXK1*/
		...
		break;
	case 4: /*RXK2*/
		_iqk_rxk2setting_8198f(dm, path, false);
		gonext = false;
		while (1) {
	...
	while (1) {
		...
		case 1: /*S0 LOK*/
			for (i = 7; i >= 0; i--) { /* the LOK Cal in the each PAD stage*/
				_iqk_lok_setting_8198f(dm, RF_PATH_A, i);
				_lok_one_shot_8198f(dm, RF_PATH_A, i);
				if (i==0)
					break;
			}
			dm->rf_calibrate_info.iqk_step++;
...
#else
			/*S0 TXIQK*/
			RF_DBG(dm, DBG_RF_IQK, "S0 TXIQK 0, RF08 = %x, RF00=%x\n", odm_get_rf_reg(dm, 0x0, 0x8, RFREGOFFSETMASK), odm_get_rf_reg(dm, 0x0, 0x0, RFREGOFFSETMASK));
			RF_DBG(dm, DBG_RF_IQK, "S0 TXIQK 0, RF58 = %x\n", odm_get_rf_reg(dm, 0x0, 0x58, RFREGOFFSETMASK));

			_iqk_txk_setting_8198f(dm, RF_PATH_A);
//			RF_DBG(dm, DBG_RF_IQK, "S0 TXIQK 1, RF08 = %x, RF00=%x\n", odm_get_rf_reg(dm, 0x0, 0x8, RFREGOFFSETMASK), odm_get_rf_reg(dm, 0x0, 0x0, RFREGOFFSETMASK));
//			RF_DBG(dm, DBG_RF_IQK, "S0 TXIQK 1, RF58 = %x\n", odm_get_rf_reg(dm, 0x0, 0x58, RFREGOFFSETMASK));


			KFAIL = _iqk_one_shot_8198f(dm, RF_PATH_A, TXIQK);

			
//			RF_DBG(dm, DBG_RF_IQK, "S0 TXIQK 2, RF08 = %x, RF00=%x\n", odm_get_rf_reg(dm, 0x0, 0x8, RFREGOFFSETMASK), odm_get_rf_reg(dm, 0x0, 0x0, RFREGOFFSETMASK));
//			RF_DBG(dm, DBG_RF_IQK, "S0 TXIQK 2, RF58 = %x\n", odm_get_rf_reg(dm, 0x0, 0x58, RFREGOFFSETMASK));
			iqk_info->kcount++;
...
//			RF_DBG(dm, DBG_RF_IQK, "S0 TXIQK 3, RF08 = %x, RF00=%x\n", odm_get_rf_reg(dm, 0x0, 0x8, RFREGOFFSETMASK), odm_get_rf_reg(dm, 0x0, 0x0, RFREGOFFSETMASK));
//			RF_DBG(dm, DBG_RF_IQK, "S0 TXIQK 3, RF58 = %x\n", odm_get_rf_reg(dm, 0x0, 0x58, RFREGOFFSETMASK));
			break;
#endif
		case 3: /*S0 RXIQK*/
#if 0
...
#else
			while (1) {
				KFAIL = _iqk_rxiqkbystep_8198f(dm, RF_PATH_A);
	...
	struct dm_struct *dm)
{
	...
	RF_DBG(dm, DBG_RF_IQK, "_iq_calibrate_8198f_init \n");
	if (firstrun) {
		firstrun = false;
	...
	u8 path)
{
	...
	for (i = 0x0; i <= imr_limit; i++) {
		...
		imr_result[RF_PATH_A] = _iqk_tximr_selfcheck_8198f(dm, tone_index, RF_PATH_A);
	...
	for (path = 0; path < SS_8198F; path++) {
		for (i = 0; i <= imr_limit; i++) {
			tone_index = (u8)(0x08 | i << 4);
			KFAIL = _iqk_rx_iqk_gain_search_fail_by_toneindex_8198f(dm, path, RXIQK1, tone_index);
			if (!KFAIL) {
				odm_write_4byte(dm, 0x1b1c, 0xa2193c32);
	...
	for (path = 0; path < SS_8198F; path++) {
		for (i = 0x0; i <= imr_limit; i++) {
			tone_index = (u8)(0x08 | i << 4);
			imr_result[RF_PATH_A] = _iqk_rximr_selfcheck_8198f(dm, tone_index, RF_PATH_A, rxk1_tmp1b38[path][i]);
			RF_DBG(dm, DBG_RF_IQK, "[IQK]toneindex = %x, RXIMR = %d\n", tone_index, imr_result[RF_PATH_A]);
		}
	}
	...
	boolean do_imr_test)
{
	...
	u32 MAC_backup[IQK_MAC_REG_NUM_8198F], BB_backup[IQK_BB_REG_NUM_8198F], RF_backup[IQK_RF_REG_NUM_8198F][1];
	u32 backup_mac_reg[IQK_MAC_REG_NUM_8198F] = {0x520, 0x550, 0x1518};
	u32 backup_bb_reg[IQK_BB_REG_NUM_8198F] = {0x808, 0x90c, 0xc00, 0xcb0, 0xcb4, 0xcbc, 0x1990, 0x9a4, 0xa04, 0x1c44, 0xc94};
	u32 backup_rf_reg[IQK_RF_REG_NUM_8198F] = {0xdf, 0xde, 0x8f, 0x0, 0x1, 0xef};
	boolean is_mp = false;
	...
	RF_DBG(dm, DBG_RF_IQK,
	       "[IQK]dm->channel = %d, dm->band_width = %x\n", *dm->channel, *dm->band_width);
	...
	_iqk_backup_mac_bb_8198f(dm, MAC_backup, BB_backup, backup_mac_reg, backup_bb_reg, IQK_BB_REG_NUM_8198F);
...
//		_iqk_rfsetting_8198f(dm);
		_iqk_start_iqk_8198f(dm, segment_iqk);
		_iqk_afe_setting_8198f(dm, false);
		_iqk_restore_mac_bb_8198f(dm, MAC_backup, BB_backup, backup_mac_reg, backup_bb_reg, IQK_BB_REG_NUM_8198F);
	...
	if (!do_imr_test) {
		...
		if (!is_mp)
			RF_DBG(dm, DBG_RF_IQK, "[IQK]Total IQK progressing_time = %lld ms\n",
...
void _phy_iqk_XYM_Read_Using_0x1b38_8198f(
	struct dm_struct *dm,
	u8 path)
{
	u32 tmp = 0x0;
...
void _phy_iqk_debug_inner_lpbk_psd_8198f(
	struct dm_struct *dm,
	u8 path)
{
	s16 tx_x;
	...
	void *dm_void)
{
	...
	RF_DBG(dm, DBG_RF_IQK,
	       "[IQK]  ************IMR Test *****************\n");
	_phy_iq_calibrate_8198f(dm, false, false, true);
	RF_DBG(dm, DBG_RF_IQK,
	       "[IQK]  **********End IMR Test *******************\n");
}
	...
	boolean segment_iqk)
{
	...
	RF_DBG(dm, DBG_RF_IQK, "[IQK]  phy_iq_calibrate_8198f \n");
	_iq_calibrate_8198f_init(dm);
	_phy_iq_calibrate_8198f(dm, clear, segment_iqk, false);
	_iqk_fail_count_8198f(dm);

//	if (*(dm->mp_mode))
//		halrf_iqk_hwtx_check(dm, false);
#if (dm_SUPPORT_TYPE & ODM_AP)
	_iqk_iqk_fail_report_8198f(dm);
#endif
	//halrf_iqk_dbg(dm);

//	odm_write_4byte(dm, 0x10c0, 0xaaaa5555);
}
#endif
