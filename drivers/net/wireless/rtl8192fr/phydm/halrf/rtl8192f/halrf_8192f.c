		...
		struct dm_struct *dm)
{
	...
	* 	RF_DBG(dm,DBG_RF_IQK, "path-B IQK setting!\n"); */
	...
	if ((dm->rfe_type == 7) | (dm->rfe_type == 8))
		...
		odm_set_rf_reg(dm, RF_PATH_B, RF_0x56, 0x003ff, 0xe9);
	...
	boolean is2T)
{
	...
	_phy_path_adda_on_92f(dm, ADDA_REG, true, is2T);
	...
	boolean is_recovery)
{
	...
	if ((rege94 != 0) /*&&(regea4 != 0)*/)
		_phy_path_a_fill_iqk_matrix_92f(dm, is_patha_ok, result, final_candidate, (regea4 == 0));
	_phy_path_b_fill_iqk_matrix_92f(dm, is_pathb_ok, result, final_candidate, (regec4 == 0));
