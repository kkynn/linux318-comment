...
struct _device_info_ wlan_device[] =
{
    ...
    #if defined(CONFIG_USE_PCIE_SLOT_0) && defined(CONFIG_USE_PCIE_SLOT_1) && \
   	    (defined(CONFIG_SLOT_0_8812) || defined(CONFIG_SLOT_0_92D)) && !defined(RTK_129X_PLATFORM)
...
#ifdef DUAL_BAND_SUPPORT
static struct rtl8192cd_priv *dualband_tmp_priv = NULL;
#endif
...
&& (GET_VXD_PRIV(priv)->simple_config_status==0)
...
#endif
	{
		...
		INIT_LIST_HEAD(&priv->pshare->amsdu_timer_list);
#else
...
#ifdef SUPPORT_TX_AMSDU
		/* dot11nConfigEntry.dot11curAMSDUSendNum = 0, then assign default value */
...
//		priv->ps_ctrl = 0x11;
        ...
        if (priv->pshare->rf_ft_var.rf_mode < 4) {
...
#if defined(CONFIG_RTL8672) && (defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8192FE))
	if (GET_CHIP_VER(priv)==VERSION_8192E) {
	    priv->pmib->dot11nConfigEntry.dot11nAMPDURevSz = 42;
	} else if (GET_CHIP_VER(priv)==VERSION_8192F) {
	    priv->pmib->dot11nConfigEntry.dot11nAMPDURevSz = 32;
	}
#endif
...
#if defined(NEAR_END_RX_REFINE) && defined(HIGH_POWER_EXT_LNA)
	if (priv->pshare->rf_ft_var.use_ext_lna) { 
		priv->pshare->rf_ft_var.ner_en = 1;
	}
	priv->pshare->tx_pwr_ctrl_backup = priv->pshare->rf_ft_var.tx_pwr_ctrl;
#endif
...
#ifdef SW_TX_QUEUE
#ifndef WIFI_BUFFER_LIMITED
	if(is_11AC_Chips(priv)){
		priv->pshare->rf_ft_var.swq_max_enqueu_len = 8192;
	}
#endif
#endif
...
#ifdef TCP_ACK_ACC
	if(GET_CHIP_VER(priv) != VERSION_8192E && GET_CHIP_VER(priv) != VERSION_8822B && GET_CHIP_VER(priv) != VERSION_8192F)
...
/* Low power consumption in idle mode */
...
#if defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8192FE) || defined(CONFIG_WLAN_HAL_8822BE)
	if ((GET_CHIP_VER(priv) == VERSION_8192E) || (GET_CHIP_VER(priv) == VERSION_8192F) || (GET_CHIP_VER(priv) == VERSION_8822B)) {
		/* Because RTL8192E opens TCP_ACK_ACC function */
	...
	if (GET_CHIP_VER(priv) == VERSION_8822B) {
		...
		// priv->pmib->dot11nConfigEntry.dot11nAMSDUSendTimeout = 1500;
	}
#endif
#endif
#endif
...
#elif defined(CONFIG_RTL8672) || defined(__OSK__) || defined(CONFIG_ARCH_CORTINA)
...
#elif defined(CONFIG_USE_PCIE_SLOT_0) && defined(CONFIG_USE_PCIE_SLOT_1) && (defined(CONFIG_ARCH_CORTINA_G3) || defined(CONFIG_ARCH_CORTINA_G3HGU))
...
#if defined (CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_RTL_8723B_SUPPORT) || defined(CONFIG_WLAN_HAL_8822BE)|| defined(CONFIG_WLAN_HAL_8821CE) || defined(CONFIG_WLAN_HAL_8192FE)
#ifdef CONFIG_RTL_92D_SUPPORT
                if (vendor_deivce_id == ((unsigned long)((0x8193<<16)|PCI_VENDOR_ID_REALTEK)))
                    priv->pshare->version_id = VERSION_8192D;
                else
...
#if defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_RTL_8812_SUPPORT)|| defined(CONFIG_WLAN_HAL_8821CE)
			if(priv->pshare->version_id == VERSION_8822B || priv->pshare->version_id == VERSION_8812E|| priv->pshare->version_id == VERSION_8821C) 
...
#ifndef NOT_RTK_BSP
#if defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8192FE)
			if ((GET_CHIP_VER(priv)!=VERSION_8188E) && (GET_CHIP_VER(priv)!=VERSION_8192E) && (GET_CHIP_VER(priv)!=VERSION_8812E) && (GET_CHIP_VER(priv)!=VERSION_8814A) && (GET_CHIP_VER(priv)!=VERSION_8822B) && (GET_CHIP_VER(priv)!=VERSION_8192F))
#endif
			{
...
#ifdef DUAL_BAND_SUPPORT
        if(dualband_tmp_priv == NULL) {
            dualband_tmp_priv = priv;
        }
        else {
            dualband_tmp_priv->dualband_rootpriv = priv;
            priv->dualband_rootpriv = dualband_tmp_priv;
        }
#endif
...
#else
				page_ptr_amsdu = dma_alloc_coherent(priv->pshare->cma_dev, DESC_DMA_PAGE_SIZE_HAL_FOR_AMSDU, &addr, GFP_KERNEL);
#endif
				_GET_HAL_DATA(priv)->desc_dma_buf_amsdu = page_ptr_amsdu;
