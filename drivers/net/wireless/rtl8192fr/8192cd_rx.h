...
#if defined(CONFIG_SMP) && defined(CONFIG_RTL9607C)
				...
				struct stat_info *pstat, struct rx_frinfo *pfrinfo)
{
	...
	if (pfrinfo->physt) {
...

			{
				...
				if (priv->pshare->rf_ft_var.rssi_dump)
					memcpy(&pstat->rf_info.mimosq[0], &pfrinfo->rf_info.mimosq[0], sizeof(struct rf_misc_info) - FIELD_OFFSET(struct rf_misc_info, mimosq));
			}
