...
{
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s] Enter\n", __FUNCTION__)); 
	...
	)
{
	...
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[Beamforming]%s Start!\n", __FUNCTION__));
	PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]%s Start!\n", __FUNCTION__);

	/*ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[Beamforming]%s, RA= 0x %x%x%x%x%x%x\n",
		__FUNCTION__,
		RA[0],RA[1],RA[2],RA[3],RA[4],RA[5]));*/
	PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]%s, RA= 0x %x%x%x%x%x%x\n", __FUNCTION__,
		RA[0],RA[1],RA[2],RA[3],RA[4],RA[5]);

	/*ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[Beamforming]%s, DelBFeeEntry_Idx0= 0x %x%x%x%x%x%x\n",
	__FUNCTION__,
    ...
    if( (isEqualMACAddr(RA, pBeamformingInfo->DelEntryListByMACAddr.BFeeEntry_Idx0)) ){
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[Beamforming]@%s, (memcmp(RA, pBeamformingInfo->DelEntryListByMACAddr.BFeeEntry_Idx0, MACADDRLEN)) == 1, return NULL!!\n", __FUNCTION__));
		...
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[Beamforming]@%s, (memcmp(RA, pBeamformingInfo->DelEntryListByMACAddr.BFeeEntry_Idx1, MACADDRLEN)) == 1, return NULL!!\n", __FUNCTION__));
	...
	)
{
    ...
    if((isEqualMACAddr(RA, pBeamformingInfo->DelEntryListByMACAddr.BFerEntry_Idx0))){
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[Beamforming]@s, (memcmp(RA, pBeamformingInfo->DelEntryListByMACAddr.BFerEntry_Idx0, MACADDRLEN)) == 1, return NULL!!\n", __FUNCTION__));
		...
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[Beamforming]@s, (memcmp(RA, pBeamformingInfo->DelEntryListByMACAddr.BFerEntry_Idx1, MACADDRLEN)) == 1, return NULL!!\n", __FUNCTION__));
	...
	)
{
	...
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]Get BFee Idx = %d\n", __FUNCTION__, *Idx));
	...
	if(pEntry != NULL)
	{
		...
		if(GET_CHIP_VER(priv) == VERSION_8822B){
			...
			if ((pBeamInfo->BeamformCap & BEAMFORM_CAP_VHT_MU_BFEE) &&
				...
				pEntry->AID = AID;
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("Add BFer entry beamformer_mu_cnt=%d\n", pBeamInfo->beamformer_mu_cnt));
			...
			if ((pBeamInfo->BeamformCap & (BEAMFORMEE_CAP_VHT_SU|BEAMFORMEE_CAP_HT_EXPLICIT)) &&
				...
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("Add BFer entry beamformer_su_reg_maping=%#X, su_reg_index=%d\n", pBeamInfo->beamformer_su_reg_maping, pEntry->su_reg_index));
				PHYDM_DBG(ODMPTR, DBG_TXBF, "Add BFer entry beamformer_su_reg_maping=%#X, su_reg_index=%d\n", pBeamInfo->beamformer_su_reg_maping, pEntry->su_reg_index);
			}
		}
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, BFer Entry= %d\n", __FUNCTION__, *Idx));
...
// BIT 3 implies capable of receiving NDPA (BFEE),
		...
		if(WirelessMode == WIRELESS_MODE_AC_5G || WirelessMode == WIRELESS_MODE_AC_24G)
		{
			...
			if((GET_CHIP_VER(priv) == VERSION_8822B) && priv->pmib->dot11RFEntry.txbf_mu){
				if(pSTA->vht_cap_len && (cpu_to_le32(pSTA->vht_cap_buf.vht_cap_info) & BIT(MU_BFEE_S)) && (priv->pmib->dot11RFEntry.txbfer == 1) && get_sta_vht_mimo_mode(pSTA) == RF_1T1R)  { // AC還沒依據bfer or bfee擋
	...
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, CompSteeringNumofBFer = %d, NumofSoundingDim = %d\n", __FUNCTION__, CompSteeringNumofBFer, NumofSoundingDim));
	PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, CompSteeringNumofBFer = %d, NumofSoundingDim = %d\n", __FUNCTION__, CompSteeringNumofBFer, NumofSoundingDim);
// bfme
	if((BeamformCap & BEAMFORMEE_CAP_HT_EXPLICIT) || (BeamformCap & BEAMFORMEE_CAP_VHT_SU))
	{
		...
		if(pBeamformerEntry == NULL)
		{
			...
			if(pBeamformerEntry == NULL) {
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, Not enough BFer entry!\n", __FUNCTION__));
...
//bfer
	if(((BeamformCap & BEAMFORMER_CAP_HT_EXPLICIT) || (BeamformCap & BEAMFORMER_CAP_VHT_SU)) && (pSTA->IOTPeer != HT_IOT_PEER_INTEL))
	{
		...
		else
		{
			...
			{
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, Error State of Beamforming\n", __FUNCTION__));
	...
	*BFerBFeeIdx = (BFerIdx<<4) | BFeeIdx;
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s,  BFerIdx=0x%x, BFeeIdx=0x%x, BFerBFeeIdx=0x%x \n", __FUNCTION__, BFerIdx, BFeeIdx, *BFerBFeeIdx));
	...
	)
{
	...
	for(Idx = 0; Idx < BEAMFORMEE_ENTRY_NUM; Idx++)
	{
		...
		{
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, Reset entry idx=%d\n", __FUNCTION__, Idx));
	...
	)
{
	...
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s: Mode=%d, BeamPeriod=%d\n", __func__, pBeamTimerInfo->Mode, pBeamTimerInfo->BeamPeriod));
	...
	)
{
	...
	if(IsDelete)
	{
		if(DeleteIdx<BEAMFORMEE_ENTRY_NUM)
		{
			...
			{
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, SW DeleteIdx is wrong!\n", __FUNCTION__)); 
	...
	else
	{
		for(Idx = 0; Idx < BEAMFORMEE_ENTRY_NUM; Idx++)
		{
			...
			{
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, SW Reset entry %d\n", __FUNCTION__, Idx)); 
	...
	)
{
	...
	//not reply CSI report in Nss2 8~9
	if(not_snding_rate_interval(pEntry, VHT_RATE_ID + ntx_num*10 + 9, VHT_RATE_ID + ntx_num*10+8)){
		RTL_W8(0x718, RTL_R8(0x718) & 0xFC);	//in Nss2 8~9, set Reg 718 bit[1:0] 00 
	}else{		
		RTL_W8(0x718, RTL_R8(0x718) |0x3);	//esle, set Reg 718 bit[1:0] 11 
	}
#endif
	...
	if (is_sounding)
		...
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, Not sounding, pEntry->Sounding_En = %d\n", __FUNCTION__, pEntry->Sounding_En));
	...
	)
{
	...
	if (pEntry->bUsed == FALSE) {
		...
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, pEntry->bUsed = FALSE\n", __FUNCTION__));
...

		{
			...
			if (pstat && (pstat->state & WIFI_SLEEP_STATE)) {
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, SLEEP mode, no NDPA\n", __FUNCTION__));
...
//  debug
	if (!priv->pshare->rf_ft_var.soundingEnable) {
		...
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, %d\n", __FUNCTION__, __LINE__));
...
//	
	if(Mode == SOUNDING_SW_HT_TIMER || Mode == SOUNDING_HW_HT_TIMER || Mode == SOUNDING_AUTO_HT_TIMER)
	{
		...
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, HT NDP Rate = %d\n", __FUNCTION__, NDPTxRate)); 
		...
		if(pEntry->Sounding_En == 0)
			Beamforming_End(priv,0);
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, VHT NDP Rate = %d\n", __FUNCTION__, NDPTxRate));  
	...
	)
{
		...
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s Start!\n", __func__));
		...
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, Before notify, bSounding=%d, Idx=%d\n", __func__, bSounding, Idx));
		...
		case BEAMFORMEE_NOTIFY_ADD_MU:
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s: BEAMFORMEE_NOTIFY_ADD_SU\n", __func__));
		...
		case BEAMFORMEE_NOTIFY_DELETE_MU:
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s: BEAMFORMEE_NOTIFY_DELETE_SU\n", __func__));
			...
			if (pBeamTimerInfo->Mode == SOUNDING_FW_HT_TIMER || pBeamTimerInfo->Mode == SOUNDING_FW_VHT_TIMER) {
...
#endif
				{
					...
					//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s: No BFee left\n", __func__));
	...
	)
{
	...
	else if(pBeamInfo->BeamformState == BEAMFORMING_STATE_START_2BFee)
	{
		if(bSounding == BEAMFORMING_NOTIFY_ADD)
		{
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, should be block\n", __FUNCTION__));  
	...
	)
{
	...
	/*ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s] Status = %d, BeamformEntryState=%d, pBFeeEntry->bUsed=%d, pBFeeEntry->Sounding_En = %d\n",
	 __FUNCTION__, Status,
	...
	{
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, VHT3SS 7,8,9, do not apply V matrix.\n", __FUNCTION__));
	...
	else if (Status == 1)        // Receive CSI successful
	{
		...
		Beamforming_SetBeamFormStatus(priv, (pBeamformingInfo->BeamformeeCurIdx));
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s], Sounding Success\n", __FUNCTION__));
	...
	if (pBFeeEntry->LogStatusFailCnt > 50) {
		...
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[Beamforming]@%s, LogStatusFailCnt > 50\n", __FUNCTION__);
       	if ((GET_CHIP_VER(priv)== VERSION_8814A) || (GET_CHIP_VER(priv)== VERSION_8197F))
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, 97F and 8814A don't remove entry\n", __FUNCTION__));
	...
	)
{
	...
	if (ret == FALSE)
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, BeamformingStart_V2 fail\n", __FUNCTION__));
	...
	)
{
	...
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, Init Timer\n", __FUNCTION__));
	...
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, is_iqgen_setting_ok = %d\n", __FUNCTION__, is_iqgen_setting_ok));
	...
	)
{
	...
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, Release Timer\n", __FUNCTION__));  
...
)
{
	...
	if(Beamforming_InitEntry(priv, pstat, &BFerBFeeIdx))
	{
		...
		if(GET_CHIP_VER(priv)== VERSION_8822B) {
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, BeamformCap = 0x%x State = 0x%x \n", __FUNCTION__, pBeamInfo->BeamformCap, pBeamInfo->SoundingInfoV2.State));  
			...
			//if(pBeamInfo->BeamformCap & (BEAMFORMER_CAP_HT_EXPLICIT|BEAMFORMER_CAP_VHT_SU|BEAMFORM_CAP_VHT_MU_BFER))
			{
				if(pBeamInfo->SoundingInfoV2.State == SOUNDING_STATE_NONE)
				{
					...
					//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, Set BeamformingTimer = 2000\n", __FUNCTION__));
	...
	)
{
	...
	{
		BeamformingReset(priv);
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, Reset entry\n", __FUNCTION__));  
	...
	else
	{
		...
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s, DeInit entry %d\n", __FUNCTION__, Idx));  
	...
	)
{
	...
	TA[0] = TA[0] & 0xFE; 
        ...
        {
			/*ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[Beamforming]@%s ,ClockResetTimes=%d, NDPALogSuccess=%d, clock reset is no longer needed!!\n",
			__FUNCTION__, 
        ...
        {
			 /*ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[Beamforming]@%s ,ClockResetTimes=%d, DeInit BFer Entry !!\n",
						__FUNCTION__,
						pBeamformerEntry->ClockResetTimes,
						pBeamformerEntry->NDPALogSuccess));*/
        ...
        /*ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[Beamforming]@%s Start, Sequence=%d, NDPALogSeq=%d, NDPAPreLogSeq=%d, NDPALogRetryCnt=%d, ClockResetTimes=%d, NDPALogSuccess=%d\n",
					__FUNCTION__,
					Sequence,
	...
	else
	{
                ...
                if (pBeamformerEntry->NDPALogRetryCnt == 5)
                {
                    ...
                    /*ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[Beamforming]@%s ,Clock Reset!!! ClockResetTimes=%d\n",
	                    __FUNCTION__, 
	                    pBeamformerEntry->ClockResetTimes));*/  
#ifdef CONFIG_RTL_8812_SUPPORT	
	...
	)
{
	...
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s() ==> bBFeeLeave(%d)\n", __func__, bBFeeLeave));
	...
	if(bBFeeLeave)
	{
		...
		for (i= 0; i < BEAMFORMEE_ENTRY_NUM; i++)
		{
			if (pBeamInfo->BeamformeeEntry[i].bUsed)
			{			
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("Idx(%d) SoundPeriod=%d!\n", i, pBeamInfo->BeamformeeEntry[i].SoundPeriod));
...
)
{
	...
	for(i=0; i<BEAMFORMEE_ENTRY_NUM; i++)
	{
		if(pBeamInfo->BeamformeeEntry[i].bUsed)
		{
			...
			{		
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Invalid BFee idx(%d) Hw state=%d\n", __func__, i, pBeamInfo->BeamformeeEntry[i].HwState));
...
#if 1 //eric-mu
{
	...
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: There are %d SU and %d MU BFees in this sounding period\n", __func__, SUIdx, MUIdx));
    ...
    )
{
	...
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s] ===> pSoundingInfo->State = %d\n", __func__, pSoundingInfo->State));	
	...
	{
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Invalid State(%d) and return!\n", __func__, pSoundingInfo->State));
	...
	do
	{
		if(pSoundingInfo->State == SOUNDING_STATE_INIT)
		{
			...
			if(rtStatus == RT_STATUS_FAILURE)
			{
				WaitCnt=0;
				pSoundingInfo->State = SOUNDING_STATE_NONE;
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: No BFees found. Set to SOUNDING_STATE_NONE \n", __func__));
			...
			{			
				...
				else
				{
					WaitCnt=0;
					pSoundingInfo->State = SOUNDING_STATE_NONE;
					//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Wait changing Hw state timeout!!. Set to SOUNDING_STATE_NONE \n", __func__));
			...
			{
				WaitCnt=0;
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Unkown state! \n", __func__));
		...
		if((SUSoundingIdx < BEAMFORMEE_ENTRY_NUM))
		{
			...
			{ // SU Case
				...
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Set to SOUNDING_STATE_SU_START \n", __func__));			
		...
		else
		{
			...
			if(pSoundingInfo->CandidateMUBFeeCnt > 0)
			{
				...
				//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Set to SOUNDING_STATE_MU_START \n", __func__));
...
#endif
					//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: cnt=%d \n", __func__, cnt));
		...
		if(bSetTimeoutTimer)
		{
			odm_set_timer(ODMPTR, &pBeamInfo->BFSoundingTimeoutTimer, 5*1000); //eric-mu
			break;
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s(): Set Timeout Timer %d\n", __func__, TimeoutPeriod));
			
		}
	} while(FALSE);

	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("%s(): <===\n", __func__));
    ...
    )
{
	...
	if(pSoundingInfo->State == SOUNDING_STATE_SU_START)
	{
		if(pSoundingInfo->SUBFeeCurIdx < BEAMFORMEE_ENTRY_NUM)
			...
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Error Value of SUBFeeCurIdx = %d!\n", __func__, pSoundingInfo->SUBFeeCurIdx));
		...
		{	// SU BFee
			...
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: The BFee entry[%d] is set to bSoundingTimeout!\n", __func__, pSoundingInfo->SUBFeeCurIdx));
			...
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Set to SU SOUNDING_STATE_SOUNDING_TIMEOUT \n", __func__)); 
	...
	{
		pSoundingInfo->State = SOUNDING_STATE_SOUNDING_TIMEOUT;
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Set to MU SOUNDING_STATE_SOUNDING_TIMEOUT \n", __func__));		
		PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s]: Set to MU SOUNDING_STATE_SOUNDING_TIMEOUT \n",  __func__);
	}

	if(!beamform_IsLastSoundingPeer(priv))
	{				
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Set next sounding start time to %d ms \n", __func__, NextSoundPeriod));
		...
		{			
			...
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Set to SOUNDING_STATE_INIT \n", __func__));
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Set next sounding start time to %d ms \n", __func__, NextSoundPeriod));	
		...
		{
			pSoundingInfo->State = SOUNDING_STATE_NONE;
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_LOUD, ("[%s]: Set to SOUNDING_STATE_NONE \n", __func__));
...
{	
	...
	//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_TRACE, ("%s(): sound down, State=%x, Status=%d\n", __func__, pSoundingInfo->State, Status));
	...
	{
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_TRACE, ("%s: MU sounding done\n", __func__));
		//pBeamInfo->is_mu_sounding_in_progress = FALSE;
		pSoundingInfo->State = SOUNDING_STATE_MU_SOUNDDOWN;
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_TRACE, ("[%s]: Set to SOUNDING_STATE_MU_SOUNDDOWN \n", __func__));
	...
	if(pSoundingInfo->State == SOUNDING_STATE_SU_START)
	{
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_TRACE, ("%s(): SU entry[%d] sound down\n", __func__, pSoundingInfo->SUBFeeCurIdx));
		...
		if (!pEntry->bUsed) {
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_TRACE, ("[%s] WARNING! The entry[%d] bUsed=FALSE\n", __func__, pSoundingInfo->SUBFeeCurIdx));
		...
		if (pEntry->bSoundingTimeout) {
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_TRACE, ("[%s] WARNING! The entry[%d] is bSoundingTimeout!\n", __func__, pSoundingInfo->SUBFeeCurIdx));
		...
		else
		{
			...
			//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_TRACE, ("[%s] LogStatusFailCnt %d\n", __func__, pEntry->LogStatusFailCnt));
			PHYDM_DBG(ODMPTR, DBG_TXBF, "[%s] LogStatusFailCnt %d\n", __func__, pEntry->LogStatusFailCnt);
		}

		//pEntry->bBeamformingInProgress = FALSE;		
		//ODM_RT_TRACE(ODMPTR, DBG_TXBF, ODM_DBG_TRACE, ("[%s]: Set to SOUNDING_STATE_SU_SOUNDDOWN \n", __func__));
