...
void init_stainfo(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
    ...
    if (IS_SUPPORT_WLAN_HAL_HW_TX_SHORTCUT_HDR_CONV(priv)) {
        GET_HAL_INTERFACE(priv)->ClearHWTXShortcutBufHandler(priv,1);               
    }
...
#endif
	{
		odm_cmn_info_ptr_array_hook(ODMPTR , ODM_CMNINFO_STA_STATUS, pstat->cmn_info.aid, pstat);
		phydm_cmn_sta_info_hook(ODMPTR, pstat->cmn_info.aid, &pstat->cmn_info);
	}
#endif
...
#endif
	{
		odm_cmn_info_ptr_array_hook(ODMPTR , ODM_CMNINFO_STA_STATUS, pstat->cmn_info.aid, 0);
...
struct	stat_info *alloc_stainfo(struct rtl8192cd_priv *priv, unsigned char *hwaddr, int id)
{
	...
	if (i < NUM_STAT) {
			...
			priv->pshare->aidarray[i]->station.cmn_info.aid = i + 1; //aid 0 is reserved for AP
			priv->pshare->aidarray[i]->used = TRUE;
			pstat = &(priv->pshare->aidarray[i]->station);
#if defined(CONFIG_RTL_88E_SUPPORT) && defined(TXREPORT)
			if (GET_CHIP_VER(priv)==VERSION_8188E) {
	...
	if (act == INCREASE) {
		if (priv->assoc_num <= NUM_STAT) {
			...
			if (pstat->ht_cap_len) {
				...
				if (!pstat->is_realtek_sta && (pstat->IOTPeer != HT_IOT_PEER_BROADCOM)
						&& (pstat->IOTPeer != HT_IOT_PEER_INTEL)
						&& (pstat->IOTPeer != HT_IOT_PEER_RALINK)
						&& pstat->rssi >= priv->pshare->rf_ft_var.ner_rssi) {
					if (pstat->ht_cap_buf.support_mcs[2]) {
					} else if (pstat->ht_cap_buf.support_mcs[1]) {
						pstat->is_near_2ss_sta = 1;
						priv->pshare->ht_2ss_sta_num++;
					} else {
						pstat->is_near_1ss_sta = 1;
						priv->pshare->ht_1ss_sta_num++;
					}
				}
#endif
...
#endif
#ifdef NEAR_END_RX_REFINE
					if (pstat->is_near_2ss_sta) {
						pstat->is_near_2ss_sta = 0;
						priv->pshare->ht_2ss_sta_num--;
					} else if (pstat->is_near_1ss_sta) {
						pstat->is_near_1ss_sta = 0;
						priv->pshare->ht_1ss_sta_num--;
					}
#endif
...
#ifdef CONFIG_SPECIAL_ENV_TEST
		if (priv->pshare->in_spec_env_test_to && pstat->current_tx_pkt_size == VERIWAVE_PKT_SIZE_BIG){ 
