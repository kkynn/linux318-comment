...
int rm_fill_beacon_report_to_neighbor_report(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct dot11k_beacon_measurement_report* beacon_rep)
{
	...
	unsigned char *p, *pSSID, *bssid, *p2;
    ...
    p = beacon_rep->subelements;

    pSSID = get_ie(p + 14, _SSID_IE_, &len, beacon_rep->subelements_len);
    ...
    priv->rm_neighbor_report[i].op_class = rm_get_op_class(priv, beacon_rep->info.channel);
    ...
    if(cap_info & 0x80) { /*Immediate  Block Ack*/
        ...
        /* checking HT_CAP IE */
    p2 = get_ie(p + 14, _HT_CAP_, &len, beacon_rep->subelements_len);
    if (p2)
    {
    ...
    p2 = get_ie(p + 14, _BSS_LOAD_IE_, &len, beacon_rep->subelements_len);
    if (p2)
    {
        sta_count = le16_to_cpu(*(unsigned short *)(p2+2));
        channel_utilization = p2+4; 
    }
    ...
    if(WNM_ENABLE) {
   		...
   		if(!priv->bssTransPara.FomUser)
   			...
   			priv->rm_neighbor_report[i].subelemnt.preference = (channel_utilization) ? (255 - channel_utilization) : 0;
    }
#endif
                           ...
                           unsigned char *pframe, int len, int frame_len)
{
    ...
    if(pframe[len + 3] == 0)  /*succeed*/
    {
        ...
        if(beacon_rep)
        {
			...
			DEBUG_INFO("[ROAMING] Receive beacon_report:SUCCESS, CH = %d\n", beacon_rep->info.channel);
        }
             ...
             priv->pmib->dot11StationConfigEntry.dot11RMBeaconPassiveMeasurementActivated))
    {
        ...
        if(pstat)
        {
            ...
            if(ret == 0)   /*issue beacon measurement request succeed*/
            {
				...
				rm_insert_neighbor_report(priv,beacon_req);
            }
