		...
		if (GET_CHIP_VER(priv) == VERSION_8192D) {
			if (priv->pshare->rf_ft_var.dfs_det_print1)
				panic_printk("[%d] DFS dbg mode, Radar is detected as %x %08x (%d)!\n", priv->pshare->rf_ft_var.dfsdbgcnt,
					radar_type, phy_query_bb_reg(priv, 0xcf4, bMaskDWord), (unsigned int)RTL_JIFFIES_TO_MILISECONDS(jiffies));
		}
		else if ((GET_CHIP_VER(priv) == VERSION_8812E) || (GET_CHIP_VER(priv) == VERSION_8881A) || (GET_CHIP_VER(priv) == VERSION_8814A) || (GET_CHIP_VER(priv) == VERSION_8822B)) {
			if (priv->pshare->rf_ft_var.dfs_det_print1)
				panic_printk("[%d] DFS dbg mode, Radar is detected as %x %08x (%d)!\n", priv->pshare->rf_ft_var.dfsdbgcnt,
					radar_type, phy_query_bb_reg(priv, 0xf98, 0xffffffff), (unsigned int)RTL_JIFFIES_TO_MILISECONDS(jiffies));
	...
	if(priv->pshare->rf_ft_var.manual_dfs_regdomain == 3){	
		if((IS_METEOROLOGY_CHANNEL(priv->pmib->dot11RFEntry.dot11channel))){
			if ((GET_CHIP_VER(priv) == VERSION_8814A) || (GET_CHIP_VER(priv) == VERSION_8822B)){
				...
				phy_set_bb_reg(priv, 0x924, 0xfffffff, 0x095a8480);
			}
			else{
				phy_set_bb_reg(priv, 0x918, bMaskDWord, 0x1c17ecdf);
				phy_set_bb_reg(priv, 0x924, bMaskDWord, 0x01528480);
			}
			phy_set_bb_reg(priv, 0x91c, bMaskDWord, 0x0fa21a20);			
			phy_set_bb_reg(priv, 0x920, bMaskDWord, 0xe0f77204);
	...
	if (GET_CHIP_VER(priv) == VERSION_8192D) {
		...
		if (priv->pshare->rf_ft_var.manual_dfs_regdomain == 1) {
			if(priv->pmib->dot11nConfigEntry.dot11nUse40M == CHANNEL_WIDTH_20){
				...
				phy_set_bb_reg(priv, 0x91c, bMaskDWord, 0x68741a20);
			}
			if(priv->pshare->rf_ft_var.dfs_det_off == 1){
				if (GET_CHIP_VER(priv) == VERSION_8814A || (GET_CHIP_VER(priv) == VERSION_8822B)){
				...
				phy_set_bb_reg(priv, 0x920, bMaskDWord, 0xe0d67231);
			}
			else{
				if (GET_CHIP_VER(priv) == VERSION_8814A || (GET_CHIP_VER(priv) == VERSION_8822B)){
				...
				phy_set_bb_reg(priv, 0x920, bMaskDWord, 0xe0d6d231);
			}
		}
		// ETSI
			...
			if(priv->pshare->rf_ft_var.dfs_det_off == 1){
				if ((GET_CHIP_VER(priv) == VERSION_8814A) || (GET_CHIP_VER(priv) == VERSION_8822B)){
					phy_set_bb_reg(priv, 0x924, 0xfffffff, 0x095aa480);
				}
				else{
					phy_set_bb_reg(priv, 0x924, bMaskDWord, 0x0152a480);
				}				
				...
				if((priv->pmib->dot11RFEntry.dot11channel >= 52) &&
					(priv->pmib->dot11RFEntry.dot11channel <= 64)){
					phy_set_bb_reg(priv, 0x920, bMaskDWord, 0xe0fe7234);
					phy_set_bb_reg(priv, 0x91c, bMaskDWord, 0x0f141a20);					
					phy_set_bb_reg(priv, 0x918, bMaskDWord, 0x1c17ecdf);
					if ((GET_CHIP_VER(priv) == VERSION_8814A) || (GET_CHIP_VER(priv) == VERSION_8822B)){
						phy_set_bb_reg(priv, 0x924, 0xfffffff, 0x095a8480);
		...
		if(GET_CHIP_VER(priv) == VERSION_8822B){
			...
			phy_set_bb_reg(priv, 0x924, 0x70000, 1);
		}
		...
		if ((GET_CHIP_VER(priv) == VERSION_8814A) || (GET_CHIP_VER(priv) == VERSION_8822B)){        // for 8814 new dfs mechanism setting
			...
			if (priv->pshare->rf_ft_var.manual_dfs_regdomain == 2) {
				if((priv->pmib->dot11RFEntry.dot11channel >= 52) && (priv->pmib->dot11RFEntry.dot11channel <= 64)){
					...
					phy_set_bb_reg(priv, 0x19e8, bMaskDWord, 0xff080604); // set th2 = 4*0.4us, th3 = 6*0.4us, th4 = 8*0.4, th5 to max		
					// pulse repetition interval hist th setting
					phy_set_bb_reg(priv, 0x19b8, 0x00007f80, 86);  // th1=86*32us
					phy_set_bb_reg(priv, 0x19ec, bMaskDWord, 0xffffffff); // set th2, th3, th4, th5 to max	
				}
				else{
					// pulse width hist th setting
					phy_set_bb_reg(priv, 0x19e4, 0xff000000, 2);  // th1=2*04us
					phy_set_bb_reg(priv, 0x19e8, bMaskDWord, 0x1a0e0604); // set th2 = 4*0.4us, th3 = 6*0.4us, th4 = 14*0.4us, th5 = 26*0.4us					
					// pulse repetition interval hist th setting
					phy_set_bb_reg(priv, 0x19b8, 0x00007f80, 27);  // th1=27*32us
					phy_set_bb_reg(priv, 0x19ec, bMaskDWord, 0xffffffff); // set th2, th3, th4, th5 to max
				}
			}
			// ETSI
				...
				// pulse width hist th setting
				phy_set_bb_reg(priv, 0x19e4, 0xff000000, 9);  // th1=9*04us
				phy_set_bb_reg(priv, 0x19e8, bMaskDWord, 0xffff2c19); // // set th2 = 25*0.4us, th3 = 44*0.4us, th4, th5 = max
				// pulse repetition interval hist th setting
				//phy_set_bb_reg(priv, 0x19b8, 0x00007f80, 86);  // th1=86*32us
				//phy_set_bb_reg(priv, 0x19ec, bMaskDWord, 0xffffffff); // set th2, th3, th4, th5 to max	
			}
			// FCC
				...
				// pulse width hist th setting
				phy_set_bb_reg(priv, 0x19e4, 0xff000000, 2);  // th1=2*04us
				phy_set_bb_reg(priv, 0x19e8, bMaskDWord, 0x331a0e03); // // set th2 = 3*0.4us, th3 = 14*0.4us, th4 = 26*0.4us, th5 = 51*0.4us
				// pulse repetition interval hist th setting
				//phy_set_bb_reg(priv, 0x19b8, 0x00007f80, 86);  // th1=86*32us
				//phy_set_bb_reg(priv, 0x19ec, bMaskDWord, 0xffffffff); // set th2, th3, th4, th5 to max	
			}
