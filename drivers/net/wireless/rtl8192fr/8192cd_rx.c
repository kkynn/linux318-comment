...
/*static*/ __inline__ void translate_rssi_sq_outsrc(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo, char rate)
{
...
#endif
	{
		...
		for(i=0; i<NUM_STAT; i++) {
				...
				if(isEqualMACAddr(priv->pshare->aidarray[i]->station.cmn_info.mac_addr,GetAddr2Ptr(frame))) {
	...
	|| (GET_CHIP_VER(priv) == VERSION_8723B)
		...
		struct phy_status_rpt_8812 *pPhyStaRpt = (struct phy_status_rpt_8812 *)pfrinfo->driver_info;
		if ((GET_CHIP_VER(priv) == VERSION_8814A)) {			
			if ((pPhyStaRpt->sub_chnl >= 1) && (pPhyStaRpt->sub_chnl <= 8))
				pfrinfo->rx_bw= CHANNEL_WIDTH_20;
			else if ((pPhyStaRpt->sub_chnl >= 9) && (pPhyStaRpt->sub_chnl <= 12))
			 	pfrinfo->rx_bw = CHANNEL_WIDTH_40;
			else if (pPhyStaRpt->sub_chnl >= 13)
			 	pfrinfo->rx_bw = CHANNEL_WIDTH_80;
			else
	...
	if (GET_CHIP_VER(priv) == VERSION_8192E) {
		struct phy_status_rpt_8192cd *pPhyStaRpt = (struct phy_status_rpt_8192cd *)pfrinfo->driver_info;
		pfrinfo->rx_bw	  = (pPhyStaRpt->rxsc == 3) ? 1 : 0;
	}
#endif

#if defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8192FE)
...
__inline__ void translate_CRC32_outsrc(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo, BOOLEAN CRC32,u2Byte PKT_LEN)
{
	...
	struct phydm_fat_struct	*p_dm_fat_table = &p_dm_odm->dm_fat_table;
	struct phydm_phyinfo_struct *pPhyInfo = &pfrinfo->phy_info;
	...
	#else
	odm_antsel_statistics(p_dm_odm, 1, 1 , weighting , CRC32_METHOD, 0);
	#endif
...
static int rtl_wlan_frame_use_hw_nat(uint8 *data)
{
	...
	#define __LINUX_3_14_0__
#endif
#endif

#if defined(CLIENT_MODE) && defined(CONFIG_RTL_CUSTOM_PASSTHRU)
#if defined(__ECOS) || defined(RTK_129X_PLATFORM) || defined(__OSK__) || defined(__LINUX_3_14_0__)
...
void rtl_netif_rx(struct rtl8192cd_priv *priv, struct sk_buff *pskb, struct stat_info *pstat)
{
	...
	if(pskb->len>1300)
		priv->ext_stats.netif_rx_pkt_1518++;
#endif
...
//if(IS_ICMPV4_PROTO(pskb->data)) {
//	if (pskb->data[34] == 0x08) 
//		GDEBUG(" ECHO REQ  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>> seq=0x%02X%02X\n", pskb->data[40],pskb->data[41]);
//	else if (pskb->data[34] == 0) 
//		GDEBUG(" ECHO REPLY  >>>>>>>>>>>>>>>>>>>>>>>>>>>> seq=0x%02X%02X\n", pskb->data[40],pskb->data[41]);
//}
...
int rx_shortcut(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
        ...
        (GetFragNum(pframe) == 0) && (GetMFrag(pframe) == 0))
    {
		...
		priv->pshare->ner_rx_bytes += pfrinfo->pktlen;
#endif
	...
	)
{
	...
	//ODM_RT_TRACE(p_dm_odm, DBG_TXBF, ODM_DBG_TRACE, ("[%s] EnableSUBfer = %d, isVHT = %d\n", __func__, EnableSUBfer, isVHT));
	//ODM_RT_TRACE(p_dm_odm, DBG_TXBF, ODM_DBG_TRACE, ("[%s] Nc = %d, Nr = %d, Ng = %d, CB = %d, BW = %d\n", __func__, Nc, Nr, Ng, CB, BW));
...
#if	0 /*eric-8822 check csi*/
{
	...
	if (IS_SUPPORT_HW_FILL_MACID(priv)) {
			...
			------------------------------------------------------*/
#ifndef __ECOS
#if (defined(CONFIG_RTL_QOS_PATCH) || defined(CONFIG_RTK_VOIP_QOS) || defined(CONFIG_RTK_VLAN_WAN_TAG_SUPPORT) || defined(CONFIG_RTL_MULTI_LAN_DEV) || defined(CONFIG_RTL_EN_UNKNOWNUC_ONLY_TO_CPU) || defined(CONFIG_RTL_NIC_QUEUE) || defined(CONFIG_RTL_CUSTOM_PASSTHRU)) && (!(defined(CONFIG_RTL8672) && defined(__LINUX_3_18__))) && (!defined(NOT_RTK_BSP))
...
#ifndef __ECOS
#if (defined(CONFIG_RTK_VOIP_QOS)|| defined(CONFIG_RTK_VLAN_WAN_TAG_SUPPORT) || defined(CONFIG_RTL_MULTI_LAN_DEV) || defined(CONFIG_RTL_EN_UNKNOWNUC_ONLY_TO_CPU) || defined(CONFIG_RTL_NIC_QUEUE) || defined(CONFIG_RTL_CUSTOM_PASSTHRU)) && (!(defined(CONFIG_RTL8672) && defined(__LINUX_3_18__))) && (!defined(NOT_RTK_BSP))
#ifdef MBSSID
