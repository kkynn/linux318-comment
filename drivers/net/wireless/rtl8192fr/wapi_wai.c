...
#if !defined(NOT_RTK_BSP) && !defined(__LINUX_3_10__)
...
void wapiInit(struct rtl8192cd_priv *priv)
{
...

	{
		...
		priv->waiMCastKeyUpdateTimer.data = (unsigned long) &priv->waiMCastKeyUpdateTimerEvent;
		priv->waiMCastKeyUpdateTimer.function = timer_event_timer_fn;
		INIT_TIMER_EVENT_ENTRY(&priv->waiMCastKeyUpdateTimerEvent, wapiMCastUpdateKeyTimeout, (unsigned long)priv);
#endif
...
void wai_timer_event_fn(unsigned long __data)
{
	struct timer_event_entry *entry = (struct timer_event_entry *)__data;
	struct stat_info *pstat = (struct stat_info *)entry->data;
	struct rtl8192cd_priv *priv;

	if (!(pstat->state & WIFI_ASOC_STATE))
		return;

	priv = pstat->wapiInfo->priv;
...
#elif defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	wapiInfo->waiResendTimer.data = (unsigned long) &wapiInfo->waiResendTimerEvent;
	wapiInfo->waiResendTimer.function = wai_timer_event_fn;
	INIT_TIMER_EVENT_ENTRY(&wapiInfo->waiResendTimerEvent, wapiResendTimeout, (unsigned long)pstat);
#endif
...
#elif defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	wapiInfo->waiUCastKeyUpdateTimer.data = (unsigned long) &wapiInfo->waiUCastKeyUpdateTimerEvent;
	wapiInfo->waiUCastKeyUpdateTimer.function = wai_timer_event_fn;
	INIT_TIMER_EVENT_ENTRY(&wapiInfo->waiUCastKeyUpdateTimerEvent, wapiUCastUpdateKeyTimeout, (unsigned long)pstat);
#endif
...
static void wapiDerivedUCastKey(struct stat_info *pstat, uint8 *derivedKeys)
{
	...
	*/
	if (OPMODE & (WIFI_STATION_STATE))
	{
		...
		memcpy(&text[textLen], pstat->cmn_info.mac_addr, ETH_ALEN);
	}
	else if (OPMODE & WIFI_AP_STATE)
#endif
	{
		...
		memcpy(&text[textLen+ETH_ALEN], pstat->cmn_info.mac_addr, ETH_ALEN);
	}
	textLen = ETH_ALEN<<1;
...
mem_dump("UCastKey.dataKey", wapiInfo->wapiUCastKey[keyIdx].dataKey, WAPI_KEY_LEN);
	...
	{		
		...
		retVal = CamDeleteOneEntry(priv, pstat->cmn_info.mac_addr, delKeyId, 0);
		if (retVal) {
			priv->pshare->CamEntryOccupied--;
...
static int32 wapiFragementSend(struct sk_buff *pskb, struct rtl8192cd_priv *priv)
{
	...
	for (fragIdx=0; fragIdx<fragnum; fragIdx++)
	{
		...
		wapiAssert(fragSkb->data == skb_tail_pointer(fragSkb));
...
static int WapiRecvUnicastKeyAggrementConfirm(wapiWaiHeader *waihdr, struct stat_info *pstat, int *status)
{
	...
	if (memcmp(waiUCastAck->bkId, wapiInfo->wapiBK.micKey, WAPI_KEY_LEN) ||
		...
		memcmp(waiUCastAck->mac1, pstat->cmn_info.mac_addr, ETH_ALEN) ||
...
static int wapiRecvUnicastKeyAgreementResponse(wapiWaiHeader *waihdr, struct stat_info *pstat, int *status)
{
	...
	if (memcmp(waiCastRsp->bkId, wapiInfo->wapiBK.micKey,WAPI_KEY_LEN)||
		...
		memcmp(waiCastRsp->mac2, pstat->cmn_info.mac_addr, ETH_ALEN)
		)
	{
		WAI_DBGPRINT("[%s] Discard packet - some fields mismatch\n", __FUNCTION__);
		return WAPI_RETURN_FAILED;
...
static int wapiRecvMulticastKeyResponse(wapiWaiHeader *waihdr, struct stat_info *pstat, int *status)
{
	...
	if ( (waiMCastRsp->uskId!=wapiInfo->wapiUCastKeyId)||
		...
		(memcmp(waiMCastRsp->mac2, pstat->cmn_info.mac_addr, ETH_ALEN)))
	{
		WAI_DBGPRINT("[%s] Discard packet - some fields mismatch\n", __FUNCTION__);
		return WAPI_RETURN_FAILED;
	}
...
#define	REQ_ACTIVE_CA_LEN	64
	...
	memcpy(&para->data[ETH_ALEN], pstat->cmn_info.mac_addr, ETH_ALEN);
...
void	wapiSetBK(struct stat_info *pstat)
{
	...
	*/
	if (OPMODE & (WIFI_STATION_STATE))
	{
		...
		memcpy(addrID, pstat->cmn_info.mac_addr, ETH_ALEN);
	}
	else if (OPMODE & WIFI_AP_STATE)
#endif
	{
		...
		memcpy(&addrID[ETH_ALEN], pstat->cmn_info.mac_addr, ETH_ALEN);
...
static int	WapiSendUnicastKeyAggrementResponse(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	memcpy(pskb->data, pstat->cmn_info.mac_addr, ETH_ALEN);
	...
	*((uint16*)&pskb->data[ETH_ALEN<<1]) = __constant_htons(ETH_P_WAPI);
	...
	memcpy(wai_ucast_rsp->mac1, pstat->cmn_info.mac_addr, ETH_ALEN);
...
static int	WapiSendMulticastKeyResponse(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	*((uint16*)&pskb->data[ETH_ALEN<<1]) = __constant_htons(ETH_P_WAPI);
	...
	memcpy(wai_mcast_rsp->mac1, pstat->cmn_info.mac_addr, ETH_ALEN);
...
int	wapiSendUnicastKeyAgrementRequeset(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	*((uint16*)&pskb->data[ETH_ALEN<<1]) = __constant_htons(ETH_P_WAPI);
	...
	memcpy(wai_ucast_req->mac2, pstat->cmn_info.mac_addr, ETH_ALEN);
...
int	wapiSendUnicastKeyAgrementConfirm(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	*((uint16*)&pskb->data[ETH_ALEN<<1]) = __constant_htons(ETH_P_WAPI);
	...
	memcpy(wai_ucast_ack->mac2, pstat->cmn_info.mac_addr, ETH_ALEN);
...
int	wapiSendMulticastKeyNotification(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	*((uint16*)&pskb->data[ETH_ALEN<<1]) = __constant_htons(ETH_P_WAPI);
	...
	memcpy(wai_mcast_noti->mac2, pstat->cmn_info.mac_addr, ETH_ALEN);
