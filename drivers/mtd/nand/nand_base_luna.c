...
void process_trace(struct task_struct *curr);
int checkAddressValid(int block);
...
int rtk_PIO_read_ff_check(int page, int offset, int length);
...
static int nand_block_markbad (struct mtd_info *mtd, loff_t ofs)
{
	...
	phy_block = chunk_id/ppb;
	if(this->bbt[phy_block]>=0xff){
		// return OK if block is a bad block already
		return 0;
	}
	logical_block=phy_block+this->bbt[phy_block];
	...
	// mark block as bad in memory
	this->bbt[phy_block] = 0xFF;
	return 0;
#endif
...
//		printk("MTD_OOB_AUTO :  ops.ooblen %d, ops.len %d, ops.datbuf 0x%p, ops.oobbuf 0x%p\n",ops->ooblen,ops->len,ops->datbuf,ops->oobbuf );	
		...
		if ((ops->len==ops->ooblen) &&(ops->datbuf==NULL) ) {
			...
			goto NAND_READ_OOB_END;
		}

		//case 2 , check ooblen

		if((ops->ooblen==0) && (ops->len!=0) && (ops->datbuf!=NULL)){
			//read data only
	//		printk("case 2,only read data \n");
			
		//	printk("[%s, line %d] from=%llxh,len=%d\n"	,__FUNCTION__,__LINE__,from, ops->len);

			ret = nand_read_ecc(mtd, from, ops->len, &ops->retlen,ops->datbuf,NULL, NULL);
			goto NAND_READ_OOB_END;
		}
...
//	dbg_printf("%s:%s(%d)\n",__FILE__,__FUNCTION__,__LINE__);
	...
	while(oob_len < ops->ooblen) {
		...
		if(rc!=0){
			if (rc == -1){
					rc=rtk_PIO_read_ff_check(chunk_id,0,2112);
				...
				rc=-EBADMSG;
						...
						//some bit zero , check sum all 0xff, must recovery the data.
							memset(this->g_databuf, 0xff, chunk_size);
							memset(this->g_oobbuf, 0xff, oob_size);
...
//case 3 ,only write data
		if((ops->len!=0)&&(ops->ooblen==0)&&(ops->datbuf!=NULL)){
			ret = nand_write_ecc (mtd, to, ops->len, &ops->retlen,(u_char *)ops->datbuf,NULL , NULL);
			goto NAND_WRITE_OOB_END;
		}

		else{
			printk("1 NAND need check at nand_write_oob !!!! \n");
		}
...
///		printk("MTD_OOB_PLACE\n");
		if((ops->datbuf==NULL)&&(ops->ooblen!=0)&&(ops->oobbuf!=NULL)){
			...
			printk("2 NAND need check at nand_write_oob !!!! \n");
	}
...
static int nand_write_oob_auto  (struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops)
{
	...
	if (checkAddressValid(logical_block)) {
		return -EINVAL;
	}
...
/**	
**  rtk_PIO_read_ff_check()
**	descriptions: when read blank page ,if some bit is zero (< 4 bit) , this page still can use.
                  in this case , ecc check sum is all 0xff, ecc contoller can't fix this case, 
                  must use software driver check it.
**	parameters:  
**	return:  
...
int rtk_PIO_read_ff_check(int page, int offset, int length)
{
	...
	real_page = page;
	pio_length = length;
	dbg_printf("rtk_PIO_read_ff_check , page %d offset is %d,length %d \n",page,offset,length);
...
//			printk("[%d] data_out = 0x%08X\n", i, data_out);
			ff_count = ff_count+ NumberOfSetBits(data_out);
			frag_count=frag_count+4;

			//if(i<100)
//				printk("NumberOfSetBits(data_out) is %d\n",NumberOfSetBits(data_out));

	
			if ( ((frag_count % 528)==0) && (frag_count != 0 ) ){
				//check 528 1 bit count.
				printk("ff_count is %d\n",ff_count);
				//more than 4 bit fail.
				if(ff_count<4220){ 
					rc=-1;
				}
				ff_count=0;
	...
	while (data_len < len) {
		...
		if (rc !=0) {
			if (rc == -1){
					...
					rc=rtk_PIO_read_ff_check(chunk_id,0,2112);
					if(rc==0){
						memset(&buf[data_len], 0xff, chunk_size); //must reset data buff and ecc buffer
						memset(this->g_oobbuf, 0xff, oob_size);
					}
			}
		}
...
//	printk("nand_write_ecc :  data buf 0x%p \n",buf );			 
...
//			printk("only write data buffer to chunk_id 0x%x\n",chunk_id);
				if(memcmp(this->g_databuf,&buf[data_len],chunk_size)!=0){	
					rc=parameters._nand_write_chunk(&buf[data_len],this->g_oobbuf,chunk_id);
				}else{
					rc=0;
				}			
			}
...
void
rtk_nand_default_set(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd->priv;
	if (!chip->controller) {
...
//	mtd->writesize = page_size; 		
	...
	/*
	 * Currently, all NANDs have writebufsize equivalent to writesize (NAND page size).
	 * ref. luna_pro/linux-3.18.x/include/linux/mtd/mtd.h
	 * ref. luna_pro/linux-3.18.x/drivers/mtd/nand/nand_base.c -> nand_scan_tail
	 */
