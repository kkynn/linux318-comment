...
#include "luna_nand_protect_proc.h"
...
int
checkAddressValid(int block)
{
	int logical_block = block;
	if (logical_block < luna_nand_get_blk_protection_top()) {
		printk("[NANDINFO] Operation on wrong blocks %d (Blocks number < %d are protected)\n", logical_block, luna_nand_get_blk_protection_top());
		dump_stack();
		process_trace(current);
		return -1;
	}
#if 0
	if ( (logical_block > 1024 && (logical_block - 1024) <= 3) ||
			(logical_block > 2048 && (logical_block - 2048) <= 3) ||
			(logical_block > 4096 && (logical_block - 4096) <= 3) ) {
		printk("[NANDINFO] May Erase preloader %d\n", logical_block);
		dump_stack();
		process_trace(current);
	}
#endif
...
static void display_version (void)
{
	...
	printk("Blocks number under %d is protected!\n", luna_nand_get_blk_protection_top());
}
...
static int __init rtk_nand_init (void)
{
	...
	proc_create(LUNA_NAND_PROTECTED_NAME, 0, NULL, get_nand_protect_proc());
...
EXIT:
	if (rc < 0) {
		...
		remove_proc_entry(LUNA_NAND_PROTECTED_NAME, NULL);
	}else
...
static int
rtk_nand_check_blank(uint32_t *start_pos, uint32_t *end_pos)
{
  ...
  /* DMA start read */
	...
	local_irq_restore(flags);
		...
		if (-1 == return_value) {
			...
			return_value = rtk_nand_check_blank((uint32_t*)data_area, (uint32_t*)(data_area+(512+16)));
			if (!return_value) {
				memset(data_area, 0xFF, (512+16));
			}
#else
			return_value = -1;
#endif
...
//	if(data_area0!=NULL)
	...
	if (-1 != return_value) {
		if (0 != return_value) {
			printk( KERN_NOTICE "[MTD] ECC fix %d bits in page [%x]", return_value, chunk_id);
		}
		return_value = 0; // return success if error is correctable 
	}
  return return_value;
...
_nand_erase_block(struct mtd_info *mtd, u16 chipnr,int chunk_id) {
	...
	if (checkAddressValid(real_page / (nandflash_info.num_page_per_block) )) {
		return -1;
	}
