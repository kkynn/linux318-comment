...
int yaffs_count_bad_block(struct yaffs_dev *dev, int bad_block_info[3])
{
	int n_free = 0;
	int b;
	...
	return bad_block_info[0]+bad_block_info[1]+bad_block_info[2];
}

static char *yaffs_dump_dev_part1(char *buf, struct yaffs_dev *dev)
{
	int bad_block_info[3];
	...
	yaffs_count_bad_block(dev, bad_block_info);
	buf += sprintf(buf, "n_blocks_dead........ %u\n", bad_block_info[0]);
	buf += sprintf(buf, "n_blocks_unknown..... %u\n", bad_block_info[1]);
	buf += sprintf(buf, "n_blocks_need_scan... %u\n", bad_block_info[2]);
	return buf;
}
...
static int yaffs_proc_show(struct seq_file *m, void *v)
{
	...
	seq_puts(m, buffer);
	len = yaffs_proc_read(buffer, &start, 1, sizeof(buffer), NULL, NULL);
	seq_puts(m, buffer);
	len = yaffs_proc_read(buffer, &start, 2, sizeof(buffer), NULL, NULL);
	seq_puts(m, buffer);
	len = yaffs_proc_read(buffer, &start, 3, sizeof(buffer), NULL, NULL);
	seq_puts(m, buffer);
	return 0;
}
