...
#define QOS_SWQID_MASK		(0x7<<QOS_SWQID_OFFSET)

/* FIXME: reuse bits of QOS_SWQID_MASK */
#define FLOWID_MARK_OFFSET  4
#define FLOWID_MARK_MASK	(0x7<<FLOWID_MARK_OFFSET)
...
#define PKT_TYPE_OFFSET()	offsetof(struct sk_buff, __pkt_type_offset)
	...
	enum src_port_t         src_port;               /* for dev alloc skb number control */
	struct net_device       *from_dev;              /* Device we arrived on */
	char                    *switch_port;           /* Switch Port Name for QOS */
	__u32                   vlan_member;            /* port-based vlan membership for this packet */
...
static inline void skb_awifi_set(struct sk_buff *skb, unsigned char flag)
{
	skb->cb[44] = flag;
}

static inline bool skb_is_awifi(const struct sk_buff *skb)
{
	return ((unsigned char)(skb->cb[44]));
}
#endif
